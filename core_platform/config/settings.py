...
class SecuritySettings(BaseSettings):
    ...
    jwt_refresh_token_expires: int = Field(
        default=86400, description="Refresh token expiration in seconds"
    )
...
class StorageSettings(BaseSettings):
    ...
    model_config = ConfigDict(env_prefix="STORAGE_", case_sensitive=False)
...
class AppSettings(BaseSettings):
    ...
    def validate_debug(cls, v, info):
        ...
        if data.get("environment") == Environment.PRODUCTION:
            return False
        return v
