...
# Platform imports
from ..config.settings import get_platform_config, get_database_settings, get_messaging_settings
from ..persistence.database import DatabaseManager, get_db_session, get_async_db_session
from ..persistence.models import OutboxEvent
from ..messaging.kafka_client import get_kafka_client
from ..messaging.event_bus import get_event_bus, DomainEvent, EventPriority, initialize_event_bus
from ..messaging.outbox import get_outbox_publisher, initialize_outbox
from ..security.authentication import get_auth_service, User, Role, Permission
...
class PlatformValidator:
    ...
    async def _validate_configuration(self) -> None:
        ...
        component = ComponentStatus(name="Configuration", status="healthy", tests_passed=0, tests_failed=0)
        ...
        result = await self._run_test(
            ...
            self._test_environment_settings
        )
        component.results.append(result)
        if result.passed:
            component.tests_passed += 1
        ...
        logger.info(f"Configuration validation: {component.tests_passed} passed, {component.tests_failed} failed")
    
    async def _validate_database(self) -> None:
        """Validate database connectivity and operations."""
        logger.info("\n🗄️  VALIDATING DATABASE PERSISTENCE...")
        component = ComponentStatus(name="Database", status="healthy", tests_passed=0, tests_failed=0)
        ...
        logger.info(f"Database validation: {component.tests_passed} passed, {component.tests_failed} failed")
    
    async def _validate_messaging(self) -> None:
        """Validate messaging system connectivity."""
        logger.info("\n📨 VALIDATING MESSAGING SYSTEM...")
        component = ComponentStatus(name="Messaging", status="healthy", tests_passed=0, tests_failed=0)
        
        # Test 1: Kafka connectivity
        result = await self._run_test(
            "Kafka Connectivity",
            self._test_kafka_connectivity
        )
        component.results.append(result)
        if result.passed:
            component.tests_passed += 1
        ...
        logger.info(f"Messaging validation: {component.tests_passed} passed, {component.tests_failed} failed")
    
    async def _validate_security(self) -> None:
        """Validate security framework."""
        logger.info("\n🔒 VALIDATING SECURITY FRAMEWORK...")
        component = ComponentStatus(name="Security", status="healthy", tests_passed=0, tests_failed=0)
        
        # Test 1: Authentication service
        result = await self._run_test(
            "Authentication Service",
            self._test_authentication_service
        )
        component.results.append(result)
        if result.passed:
            component.tests_passed += 1
        ...
        result = await self._run_test(
            ...
            self._test_authorization_service
        )
        component.results.append(result)
        if result.passed:
            component.tests_passed += 1
        ...
        logger.info(f"Security validation: {component.tests_passed} passed, {component.tests_failed} failed")
    
    async def _validate_event_sourcing(self) -> None:
        """Validate event sourcing functionality."""
        logger.info("\n📝 VALIDATING EVENT SOURCING...")
        component = ComponentStatus(name="Event Sourcing", status="healthy", tests_passed=0, tests_failed=0)
        
        # Test 1: Domain events
        result = await self._run_test(
            "Domain Events",
            self._test_domain_events
        )
        component.results.append(result)
        if result.passed:
            component.tests_passed += 1
        ...
        logger.info(f"Event Sourcing validation: {component.tests_passed} passed, {component.tests_failed} failed")
    
    async def _validate_outbox_pattern(self) -> None:
        """Validate outbox pattern implementation."""
        logger.info("\n📤 VALIDATING OUTBOX PATTERN...")
        component = ComponentStatus(name="Outbox Pattern", status="healthy", tests_passed=0, tests_failed=0)
        
        # Test 1: Outbox publisher
        result = await self._run_test(
            "Outbox Publisher",
            self._test_outbox_publisher
        )
        component.results.append(result)
        if result.passed:
            component.tests_passed += 1
        ...
        result = await self._run_test(
            ...
            self._test_transactional_publishing
        )
        component.results.append(result)
        if result.passed:
            component.tests_passed += 1
        ...
        logger.info(f"Outbox Pattern validation: {component.tests_passed} passed, {component.tests_failed} failed")
    ...
    async def _validate_cross_service_integration(self) -> None:
        ...
        component = ComponentStatus(name="Integration", status="healthy", tests_passed=0, tests_failed=0)
        ...
        logger.info(f"Integration validation: {component.tests_passed} passed, {component.tests_failed} failed")
    ...
    async def _test_database_operations(self) -> Dict[str, Any]:
        ...
        async with get_async_db_session() as session:
            ...
            await session.execute(text(
                "DELETE FROM outbox_events WHERE aggregate_id = :aid"
            ), {"aid": "test-aggregate"})
            await session.commit()
    ...
    async def _test_e2e_workflow(self) -> Dict[str, Any]:
        ...
        async with get_async_db_session() as session:
            ...
            # Clean up
            await session.execute(text(
                "DELETE FROM outbox_events WHERE correlation_id = :cid"
            ), {"cid": workflow_id})
            await session.commit()
    ...
    def _generate_final_report(self) -> Dict[str, Any]:
        ...
        total_tests = sum(comp.tests_passed + comp.tests_failed for comp in self.component_status.values())
        ...
        for name, component in self.component_status.items():
            component_summary[name] = {
                ...
                "success_rate": (component.tests_passed / (component.tests_passed + component.tests_failed)) * 100 if (component.tests_passed + component.tests_failed) > 0 else 0
            }
