...
from ..persistence.models import BaseModel as DBModel, TimestampMixin
...
class RolePermission(DBModel, TimestampMixin):
    ...
    role_id = Column(PGUUID(as_uuid=True), ForeignKey('roles.id'), nullable=False, index=True)
    permission_id = Column(PGUUID(as_uuid=True), ForeignKey('permissions.id'), nullable=False, index=True)
    granted_by = Column(PGUUID(as_uuid=True), ForeignKey('users.id'), nullable=True)
    granted_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
        ...
        try:
            with get_db_session() as session:
                ...
                session_record = UserSession(
                    ...
                    expires_at=datetime.now(timezone.utc) + timedelta(seconds=self._access_token_expires),
    ...
    async def refresh_token(self, refresh_token: str) -> TokenResponse:
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                session_record.expires_at = datetime.now(timezone.utc) + timedelta(seconds=self._access_token_expires)
