...
from ..persistence.database import get_async_db_session, get_db_session
from ..persistence.models import BaseModel, OutboxEvent
...
class OutboxPublisher:
    ...
    async def publish_with_transaction(
        ...
        try:
            ...
            logger.debug(
                f"Stored event {event.event_type} in outbox for aggregate {event.aggregate_id}"
            )
    ...
    async def _get_unpublished_events(self, session: AsyncSession) -> List[OutboxEvent]:
        ...
        retry_cutoff = datetime.now(UTC) - timedelta(seconds=self.retry_delay)
                ...
                and_(
                    ...
                    or_(
                        ...
                        and_(
                            ...
                            < retry_cutoff,  # Enough time has passed for retry
                        ),
                    ),
                )
            )
    ...
    async def _publish_topic_events(self, events: List[OutboxEvent]) -> None:
        ...
        await kafka_client.publish_batch(
            topic=topic, messages=messages, keys=keys, headers=headers_list
        )
    ...
    async def _mark_events_published(
        ...
        stmt = (
            ...
            .values(processed=True, processed_at=datetime.now(UTC), error_message=None)
        )
    ...
    async def _cleanup_processed_events(self) -> None:
        ...
        try:
            cutoff_time = datetime.now(UTC) - timedelta(hours=self.cleanup_age)

            async with get_async_db_session() as session:
                stmt = delete(OutboxEvent).where(
                    and_(
                        OutboxEvent.processed is True,
...
async def publish_event_transactionally(
    session: Session, event: DomainEvent, aggregate: Optional[BaseModel] = None
) -> None:
    """Publish an event within a database transaction."""
    publisher = get_outbox_publisher()
