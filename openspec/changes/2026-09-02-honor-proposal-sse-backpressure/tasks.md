...
- [ ] 1.1 Add failing tests proving `write(true)` pulls once and
      `write(false)` stops before the next generator pull.
- [ ] 1.2 Add failing drain tests proving one resume per drain and no temporary
      listener accumulation across repeated pressure cycles.
- [ ] 1.3 Add failing fake-clock coverage proving a drain wait expires after
      30 seconds, aborts unfinished generation, and cannot be revived by a
      late drain.
- [ ] 1.4 Extract a focused response monitor/writer that preserves frame order,
...
- [ ] 2.1 Add failing tests for premature response close, socket close, exact
      response error, and request cancellation winning while a drain is pending.
- [ ] 2.2 Prove a late drain never resumes pulling, the raw response is not
      ended after disconnect, and generator return runs exactly once.
- [ ] 2.3 Prove normal `finish` then `close` does not cancel generation or
      overwrite a first cause.
- [ ] 2.4 Preserve primary-first aggregation when response and generator
      cleanup both fail, without promoting a normal disconnect into a server
      error.

## 3. Unknown-outcome audit refresh

- [ ] 3.1 Update frontend stream and whole-book contracts so cancellation no
      longer promises that the server persisted nothing after a terminal
      outcome may have landed.
- [ ] 3.2 Represent terminal-frame loss as `outcome unknown`; start and await a
...
- [ ] 3.3 After a successful audit refresh, warn that the previous attempt may
...
- [ ] 3.4 Add manual and whole-book hook/UI tests for Stop, transport loss,
...
- [ ] 4.1 Wire the proposal route through the focused writer without changing
      its pre-stream validation, SSE schema, frame format, or landing behavior.
- [ ] 4.2 Add route/application integration coverage for drain timeout both
...
- [ ] 4.3 Re-run disconnect persistence and repeated-listener lifecycle tests,
      frontend proposal/whole-book tests, and the owning package validations.
- [ ] 4.4 Run strict OpenSpec and independent review, then record fixed-SHA
