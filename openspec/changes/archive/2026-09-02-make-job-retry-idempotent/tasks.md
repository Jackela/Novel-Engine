...
- [x] 1.1 Add HTTP failures for missing, empty, short, over-128, whitespace,
...
- [x] 1.2 Add store/API red tests proving same owner/project/source/key creates
...
- [x] 1.3 Race concurrent requests at the reservation seam and prove the unique
...
- [x] 2.1 Add nullable retry-idempotency persistence and the partial unique
...
- [x] 2.2 Add a narrow store command that, in one immediate scoped transaction,
...
- [x] 2.3 Refactor shared retry orchestration so only the reservation creator
...
- [x] 2.4 Preserve startup recovery for keyed running Jobs and prove a restart
...
- [x] 3.1 Cover fresh and prior-retry source Jobs for proposal, review, and
...
- [x] 3.2 For completed and failed terminal proposal replays, assert provider
...
- [x] 3.3 For completed and failed review/export replays, assert evaluation,
...
- [x] 4.1 Add the required bounded header schema, CORS allow-list entry, stable
...
- [x] 4.2 Regenerate the OpenAPI snapshot and frontend generated types; update
      the shared API client to send `Idempotency-Key` through the existing
      credential, CSRF, abort, timeout, and parsing path.
- [x] 4.3 Add a `sessionStorage` retry-attempt registry scoped by owner/project/source
...
- [x] 4.4 Prove double activation uses one key, late old-project settlement
...
- [x] 5.1 Run focused retry, proposal, review, export, usage, restart,
...
- [x] 5.2 Run server gates, type-check, lint, architecture and full tests;
      ...
      against a fixed SHA.
- [x] 5.3 Keep this breaking change active until required CI and compatibility
