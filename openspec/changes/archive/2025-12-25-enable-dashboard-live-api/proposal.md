...
- The dashboard-data-routing-hygiene change established `/api/characters` as the canonical endpoint pattern, but events and metrics don't follow this convention.
...
## What Changes
1. **Backend SSE Endpoint** – Implement `/api/events/stream` as a Server-Sent Events endpoint returning real-time event streams with payload: `{id, type, title, description, timestamp, characterName?, severity}`. Follow the canonical `/api/*` product API surface.
...
- Code touched:
  - Backend: Create `/api/events/stream` endpoint in main_api_server.py with SSE event generator
...
- Dependencies: Builds on `dashboard-data-routing-hygiene` (canonical `/api/*` pattern). Requires authentication/role system for Performance widget RBAC.
...
- Multiple API versions (no path-based versioning)
