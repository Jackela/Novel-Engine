...
- [x] 1.1 Add failing store/API tests proving list items contain exactly the
...
- [x] 1.2 Add sentinel tests proving list SQL, including lookahead, does not
...
- [x] 1.3 Add failing detail tests for full JSON/event fidelity, oldest-first
...
- [x] 2.1 Introduce distinct `JobSummaryRecord`/page ports and an explicit
...
- [x] 2.2 Map summaries without parsing large JSON and expose one application
...
- [x] 2.3 Add strict summary and detail HTTP schemas plus the bounded scoped GET;
      keep complete terminal Job response schemas and retry execution unchanged.
- [x] 2.4 Replace the deliberate OpenAPI snapshot, regenerate frontend types,
      and prove no schema/migration drift or architecture violation.

## 3. Atomic frontend migration

- [x] 3.1 Introduce strict `StudioJobSummary` parsing/types for list pages while
      preserving strict complete `StudioJob` parsing for all terminal responses.
- [x] 3.2 Migrate jobs pagination, inspector models, panel, retry refresh, tests,
      and factories to summaries without optional large fields or detail fetches.
- [x] 3.3 Prove refresh/older/audit/project-switch ownership and focus behavior
...
- [x] 3.4 Migrate server tests that legitimately inspect request/result/events to
      the explicit detail resource; keep scalar-only assertions on summaries.

## 4. Evidence and ordered release

- [x] 4.1 Re-run pagination, retry, proposal/review/export bridges, provider
      diagnostics, unknown-outcome audit, whole-book, authorization, OpenAPI,
      type drift, and production query-plan regressions.
- [x] 4.2 Run owning full server/frontend gates, browser workflows, strict
      OpenSpec, and independent fixed-SHA code/UX review; record every skip.
- [x] 4.3 Keep this change active until required CI is green; archive pagination
