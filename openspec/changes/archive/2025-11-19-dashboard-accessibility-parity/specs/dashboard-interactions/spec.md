...
### Requirement: Entity selection SHALL reveal context panel
...
### Requirement: Character network SHALL expose actionable nodes
...
### Requirement: Timeline nodes SHALL be keyboard navigable
- **ADDITION**: Each timeline node SHALL participate in a roving tablist so users can inspect `current`, `completed`, and `upcoming` entries with consistent aria metadata.
...
### Requirement: Live telemetry indicator MUST refresh on a timer
