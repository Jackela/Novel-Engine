...
- [x] 1.1 Add store/API failures for default 50, limits 1 and 100, rejection of
      ...
      not enter the store.
- [x] 1.2 Add stable-order traversal tests for equal timestamps, no duplicate or
      missing ids, nullable terminal cursor, a newer insert between pages, and
      deletion of the prior boundary row.
- [x] 1.3 Seed at least 32,767 jobs directly in one transaction and prove a
...
- [x] 2.1 Introduce typed job-page input/output ports with `createdAtMs`, enforce
...
- [x] 2.2 Replace the single-column project index with the
...
- [x] 2.3 Add bounded query schemas (`cursor` max 1024) and strict project-bound cursor codec,
      return stable 422 validation envelopes for every invalid token, and emit
      required `{ jobs, next_cursor }` responses.
- [x] 2.4 Preserve oldest-first single terminal Job response documentation and
...
- [x] 3.1 Add API parser/query tests for required string-or-null
      `next_cursor`, limit/cursor encoding, and retained transport semantics.
- [x] 3.2 Extend jobs state with first-page replacement and explicit older-page
...
- [x] 3.3 Prove refresh, accepted-proposal refresh, retry completion, project
      ...
      old-project state.
- [x] 3.4 Add the accessible `Load older jobs` control with distinct busy/end
...
- [x] 4.1 Re-run synchronous jobs, event order, retry, proposal lifecycle,
      unknown-outcome audit, whole-book, authorization, OpenAPI, generated-type,
      migration-channel, and architecture regressions.
- [x] 4.2 Run owning server and frontend full validation, relevant Playwright
...
- [x] 4.3 Keep this change active until required CI is green; record that compact
