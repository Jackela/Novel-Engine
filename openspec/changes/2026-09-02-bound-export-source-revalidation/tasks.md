...
- [ ] 1.1 Add a failing production-store regression with complete captured
      ...
      entry.
- [ ] 1.2 Add failing empty and duplicate collection cases. Preserve the
      ...
      Jobs.
- [ ] 1.3 Delete or wrongly scope a captured revision in a later query group and
...
- [ ] 2.1 Extract or adapt one narrow production revision-query helper with an
      explicit parameter budget that includes every non-collection binding;
      keep query construction parameterized.
- [ ] 2.2 Execute every bounded group through the same existing immediate
...
- [ ] 2.3 Validate duplicate and conflicting captured identities before query
...
- [ ] 3.1 Re-run fresh and retry source-invalidation tests and prove their
      current public outcomes remain stable at a group boundary.
- [ ] 3.2 Inject failure after complete revalidation and at snapshot, artifact,
      Job, and event writes; prove one immediate transaction leaves zero partial
      database evidence.
- [ ] 3.3 Exercise the real filesystem publication seam and prove a post-render
...
- [ ] 4.1 Run focused export store, outcome, retry, artifact publication,
