...
The object has `additionalProperties: false` and `minProperties: 1`. Fastify
therefore rejects `{}`, arrays, null, unknown top-level keys, and wrong field
types with the existing 422 `VALIDATION_ERROR` envelope. The application trims
an included title and description exactly like project creation. A title that
...
The route uses the existing principal guard and PATCH CSRF double-submit. No
application/store mutation runs until request validation, authentication, and
CSRF validation have all succeeded. Observable failures use the existing
envelopes:
...
Schema validation may occur before the pre-handler under Fastify's normal
lifecycle; the security invariant is that neither authentication failure nor
CSRF failure can reach the mutation seam. After the guards, project lookup and
update stay Owner-scoped so identifiers never disclose cross-Owner existence.
...
the selected scalar assignments plus `updated_at`, and returns the updated row.
SQLite statement atomicity is the commit boundary; no read-modify-write sequence
or multi-statement transaction is needed to preserve omitted columns.

An accepted PATCH advances `updated_at` once even when an included normalized
value equals its stored value. This gives every successful settings command an
observable mutation timestamp and preserves the project library's existing
updated-descending order. A zero-row scoped update maps to the uniform Project
404. Serialization or validation happens before the UPDATE, and an injected
store failure leaves all project fields and timestamp unchanged.

## Frontend merge and lifecycle

The API client parses the response with the strict scalar Project parser rather
than the Project-shell parser. On success, the Settings action validates that
the captured route project id, current shell id, and settings intent epoch still
match. It then merges only the returned scalar fields into the current shell.
The shell's `documents` and `volumes`, the active accepted Document, Draft,
History, Review, and Export state remain untouched.
...
404 gap. This change restores the behavior contract-first without editing
ADR-0003's executed cutover record or claiming that the route existed in the
released TypeScript baseline.
...
- Treating the frontend mock tests as sufficient would leave the real API 404
  and persistence gap untested.
