...
- The project Settings panel gains an "Export diagnostics" action. One
  ...
  endpoint and saves it locally as a downloadable JSON file with a
  client-derived filename (`novel-engine-diagnostics-<date>.json`),
...
- The diagnostics summary contains: product identity and version (the
  ...
  configuration keys), a recent error summary when failed Jobs exist (error
  codes and messages of the most recent failed Jobs), and a database
  health summary with the same field family as `doctor` (integrity check,
  journal mode, foreign-key enforcement, owner status).
- Redaction is absolute and structural: secret values — the session secret
  and any provider API key — MUST never appear in the export, not masked,
  not truncated; configuration state is reported as configured/unconfigured
  or set/unset booleans only. Provider error reporting stays inside the
  provider failure diagnostics boundary: no provider response body reaches
  the export.
...
- The summary is assembled server-side by one new owner-guarded read-only
  endpoint so the redaction rules and health fields have one
...
- **Structural redaction over masking.** The summary schema contains no
  field capable of carrying a secret value; provider configuration is a
  provider id plus label plus configured-state boolean. Tests prove
  absence by seeding real-looking secrets and asserting they do not occur
  anywhere in the serialized export.
- **Doctor field parity, not doctor invocation.** The database health
  summary reproduces `doctor`'s field family (quick check, journal mode,
  foreign keys, owner configured) through the app's own database handle —
  the API never shells out to the CLI.
- **Error summary respects existing boundaries.** Recent errors come from
  failed Job records through the existing error envelope; the provider
  diagnostics boundary that keeps provider bodies private applies to the
  export unchanged.
- **Client-side save, server-side assembly.** The endpoint returns JSON;
  the filename and the download are client-derived, matching the export
  download precedent (no new file-writing server surface).
...
- Frontend: one Settings panel action, privacy statement, download flow in
  ...
  one diagnostics assembly service and one read-only route.
