...
- The dashboard data hook currently cycles through `/characters` (legacy), `/v1/characters`, and localhost fallbacks. This duplicates network traffic, leaks “demo” source chips, and contradicts the backend contract that only `/api/v1/characters` will be exposed going forward. MCP audit case §9 (API validation) now requires a single canonical endpoint.
...
2. **Canonical `/v1/characters` feed** – Introduce an API-base resolver that always targets `/api/v1/characters`, strip `/characters` fallbacks, and align the Vite proxy so `/v1/*` → `/api/v1/*`. Chips should show “API feed” unless the unified endpoint fails.
3. **TDD coverage** – Extend `dashboard-accessibility.test.tsx` to assert only `/v1/characters` URLs are attempted and update the router-warning test to mount via `createRoot` (no deprecated act). This keeps the console clean and enforces the new API contract.
...
- Tooling: rerun `npm run lint`, `npm run type-check`, full `npx vitest run`. Update UAT log to note the `/api/v1` unification.
