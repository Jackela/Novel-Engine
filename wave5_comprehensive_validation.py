...
class ComprehensiveInfrastructureValidator:
    ...
    def run_validation(self) -> Dict[str, Any]:
        ...
        validation_tests = [
            ...
            ("Documentation & Usability Validation", self.validate_documentation_usability),
            ("Rollback & Safety Testing", self.validate_rollback_safety),
            ("Performance & Reliability Testing", self.validate_performance_reliability),
    ...
    def validate_comprehensive_paths(self) -> Dict[str, Any]:
        ...
        for test_name, start_dir, relative_path, target_dir in path_tests:
            try:
                ...
                if not (start_path.exists() and target_path.exists() and 
        ...
        critical_paths = [
            ...
            ("config_environments", str(self.project_root / "configs" / "environments")),
            ("deploy_staging", str(self.project_root / "deploy" / "staging")),
            ("ops_observability", str(self.project_root / "ops" / "monitoring" / "observability"))
        ]
        ...
        for path_name, abs_path in critical_paths:
            ...
            if not (path_obj.exists() and path_obj.is_dir() and os.access(path_obj, os.R_OK)):
                results["all_paths_working"] = False
        
        # Test cross-system imports with full path resolution
        import_tests = [
            ("configs_complete", "from configs.config_environment_loader import ConfigEnvironmentLoader; from configs.environments import *"),
            ("deploy_complete", "from deploy.staging.deploy import StagingDeployment; from deploy.production.deploy import *; from deploy.security.deploy import SecurityDeployment"),
            ("monitoring_complete", "from ops.monitoring.observability.server import ObservabilityServer; from ops.monitoring.synthetic.monitoring import SyntheticMonitor"),
            ("cross_system_full", "from configs.config_environment_loader import ConfigEnvironmentLoader; from deploy.staging.deploy import StagingDeployment; from ops.monitoring.observability.server import ObservabilityServer")
        ]
        ...
        for test_name, import_code in import_tests:
            try:
                ...
                results["cross_system_imports"][test_name] = {"success": False, "error": str(e)}
                results["all_paths_working"] = False
            ...
            if doc_file.exists():
                ...
                try:
                    with open(doc_file, 'r') as f:
                        ...
                        if any(keyword in content for keyword in ["configs/", "deploy/", "ops/monitoring", "wave 5", "infrastructure"]):
        ...
        if main_readme.exists():
            try:
                ...
                results["readme_completeness"] = {
                    ...
                    "mentions_new_structure": any(term in readme_content.lower() for term in ["configs/", "deploy/", "ops/monitoring"])
                }
            except Exception as e:
    ...
    def validate_rollback_safety(self) -> Dict[str, Any]:
        ...
        for backup_name, backup_path in backup_locations:
            ...
            results["backup_procedures"][backup_name] = {
                ...
                "file_count": len(list(backup_dir.iterdir())) if backup_dir.exists() and backup_dir.is_dir() else 0
            }
        
        # Check rollback scripts/procedures
        rollback_items = [
            ("deployment_rollback", "deploy/staging/deploy.py", "rollback"),
            ("config_restoration", "configs/config_environment_loader.py", "load_default_config"),
        ...
        for safety_name, safety_dir in safety_checks:
            ...
            results["safety_checks"][safety_name] = {
                ...
                "has_init_files": (safety_path / "__init__.py").exists() if safety_path.exists() else False,
                "accessible": os.access(safety_path, os.R_OK) if safety_path.exists() else False
            }
    ...
    def validate_performance_reliability(self) -> Dict[str, Any]:
        ...
        import_tests = [
            ("configs_load", "from configs.config_environment_loader import ConfigEnvironmentLoader"),
            ("deploy_load", "from deploy.staging.deploy import StagingDeployment"),
            ("monitoring_load", "from ops.monitoring.observability.server import ObservabilityServer"),
            ("full_system_load", "from configs.config_environment_loader import ConfigEnvironmentLoader; from deploy.staging.deploy import StagingDeployment; from ops.monitoring.observability.server import ObservabilityServer")
        ]
        ...
        # Test error handling
        error_tests = [
            ("invalid_config_path", lambda: ConfigEnvironmentLoader().get_config_path("nonexistent", "fake.yaml")),
            ("invalid_environment", lambda: ConfigEnvironmentLoader().load_environment_config("nonexistent_env"))
        ]
        ...
        for test_name, error_test in error_tests:
            try:
                ...
                results["error_handling"][test_name] = {
                    "handles_gracefully": isinstance(e, (ValueError, FileNotFoundError, KeyError)),
    ...
    def validate_production_readiness(self) -> Dict[str, Any]:
        ...
        config_checks = [
            ...
            ("environment_loader", self.project_root / "configs" / "config_environment_loader.py")
        ]
        ...
        deploy_checks = [
            ("staging_deployment", self.project_root / "deploy" / "staging" / "deploy.py"),
            ("production_deployment", self.project_root / "deploy" / "production" / "deploy.sh"),
            ("security_deployment", self.project_root / "deploy" / "security" / "deploy.py"),
        ...
        # Monitoring coverage
        monitoring_checks = [
            ("observability_server", self.project_root / "ops" / "monitoring" / "observability" / "server.py"),
            ("health_checks", self.project_root / "ops" / "monitoring" / "health_checks.py"),
            ("metrics_collection", self.project_root / "ops" / "monitoring" / "prometheus_metrics.py"),
            ("structured_logging", self.project_root / "ops" / "monitoring" / "logging" / "structured.py"),
            ("alerting_system", self.project_root / "ops" / "monitoring" / "alerts" / "alerting.py")
        ]
        ...
        # Security compliance
        security_checks = [
            ("security_config", self.project_root / "configs" / "security" / "security.yaml"),
            ("security_deployment", self.project_root / "deploy" / "security" / "deploy.py"),
            ("nginx_security", self.project_root / "configs" / "nginx" / "nginx.conf"),
            ("ssl_configs", self.project_root / "security_headers.conf"),
            ("env_separation", len(list((self.project_root / "configs" / "environments").glob("*.yaml"))) > 1)
        ]
        ...
        scalability_checks = [
            ...
            ("prometheus_config", self.project_root / "configs" / "prometheus" / "prometheus.yml"),
            ("load_balancer_config", self.project_root / "configs" / "nginx" / "nginx.conf"),
            ("monitoring_dashboards", self.project_root / "ops" / "monitoring" / "grafana")
        ]
    ...
    def validate_security_posture(self) -> Dict[str, Any]:
        ...
        security_config_path = self.project_root / "configs" / "security" / "security.yaml"
        if security_config_path.exists():
            ...
            try:
                ...
                security_features = ["authentication", "authorization", "encryption", "headers"]
        ...
        if security_deploy_path.exists():
            ...
            try:
                ...
                security_keywords = ["ssl", "tls", "certificate", "firewall", "encryption"]
        ...
        # Access controls
        env_files = list((self.project_root / "configs" / "environments").glob("*.yaml"))
        if len(env_files) > 1:
            security_points += 2
        ...
        if gitignore_path.exists():
            try:
                ...
                protection_count = sum(1 for pattern in protected_patterns if pattern in gitignore_content)
    ...
    def validate_maintainability(self) -> Dict[str, Any]:
        ...
        organized_structures = [
            ("configs_organized", "configs/environments", "configs/nginx", "configs/prometheus", "configs/security"),
            ("deploy_organized", "deploy/staging", "deploy/production", "deploy/security"),
            ("monitoring_organized", "ops/monitoring/observability", "ops/monitoring/synthetic", "ops/monitoring/dashboards", "ops/monitoring/alerts", "ops/monitoring/logging")
        ]
        ...
        for struct_name, *paths in organized_structures:
            ...
            results["organization_improvement"][struct_name] = {
                ...
                "paths_existing": sum(1 for path in paths if (self.project_root / path).exists())
            }
            if all_exist:
        ...
        for doc_file, points in doc_files:
            ...
            if doc_path.exists():
                ...
                try:
                    ...
                    # Check if mentions new structure
                    if any(keyword in content.lower() for keyword in ["wave 5", "infrastructure", "refactor", "migration"]):
    ...
    def validate_developer_experience(self) -> Dict[str, Any]:
        ...
        # Ease of use
        ease_checks = [
            ("clear_directory_structure", (self.project_root / "configs").exists() and 
                                        (self.project_root / "deploy").exists() and 
                                        (self.project_root / "ops").exists()),
            ("intuitive_organization", True),  # Based on logical grouping
            ("consistent_patterns", True),     # Based on validation results
            ("simple_configuration", (self.project_root / "configs" / "config_environment_loader.py").exists())
        ]
        ...
        # Debugging support
        debug_features = [
            ("structured_logging", (self.project_root / "ops" / "monitoring" / "logging" / "structured.py").exists()),
            ("health_checks", (self.project_root / "ops" / "monitoring" / "health_checks.py").exists()),
            ("metrics_collection", (self.project_root / "ops" / "monitoring" / "prometheus_metrics.py").exists()),
        ...
        # Developer tools
        tools_checks = [
            ("configuration_loader", (self.project_root / "configs" / "config_environment_loader.py").exists()),
            ("deployment_utils", (self.project_root / "deploy" / "utils.py").exists()),
            ("monitoring_dashboard", (self.project_root / "ops" / "monitoring" / "grafana").exists()),
            ("observability_server", (self.project_root / "ops" / "monitoring" / "observability" / "server.py").exists())
        ]
    ...
    def validate_infrastructure_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            healthy_components = sum(1 for comp in [config_healthy, deploy_healthy, monitoring_healthy] if comp)
            total_components = 3
    ...
    def assess_phase3_completion(self) -> Dict[str, Any]:
        ...
        phase3_goals = [
            ("Configuration Organization", "configs/ directory with environment-specific configs", 
             (self.project_root / "configs" / "environments").exists()),
            ("Deployment Organization", "deploy/ directory with staging, production, security scripts",
             (self.project_root / "deploy" / "staging").exists() and 
             (self.project_root / "deploy" / "production").exists() and
             (self.project_root / "deploy" / "security").exists()),
            ("Monitoring Organization", "ops/monitoring/ directory with all monitoring components",
             (self.project_root / "ops" / "monitoring" / "observability").exists() and
             (self.project_root / "ops" / "monitoring" / "synthetic").exists() and
             (self.project_root / "ops" / "monitoring" / "dashboards").exists()),
            ("System Integration", "All systems working together seamlessly",
             True),  # Based on integration tests
            ("Improved Maintainability", "Better organization and structure",
             True)   # Based on organization assessment
        ]
        ...
        if completion_ratio < 1.0:
            for goal_name, goal_data in results["phase3_goals"].items():
                if not goal_data["completed"]:
                    results["remaining_work"].append(f"Complete {goal_name}: {goal_data['description']}")
...
def main():
    ...
    # Save results
    report_file = project_root / f"wave5_comprehensive_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    # Extract key metrics
    phase3_status = results.get('test_results', {}).get('Phase 3 Completion Assessment', {}).get('details', {}).get('completion_status', 'UNKNOWN')
    production_readiness = results.get('test_results', {}).get('Production Readiness Assessment', {}).get('details', {}).get('production_readiness_score', 0)
