...
class IntegrationSystemValidator:
    ...
    def run_validation(self) -> Dict[str, Any]:
        ...
        validation_tests = [
            ("Configuration → Deployment Integration", self.validate_config_deploy_integration),
            ("Configuration → Monitoring Integration", self.validate_config_monitoring_integration),
            ("Deployment → Monitoring Integration", self.validate_deploy_monitoring_integration),
    ...
    def validate_config_deploy_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if hasattr(security_deploy, 'apply_security_from_config') and security_config:
                    try:
    ...
    def validate_config_monitoring_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            prometheus_config_path = self.project_root / "configs" / "prometheus" / "prometheus.yml"
            ...
            if prometheus_config_path.exists():
                with open(prometheus_config_path, 'r') as f:
                    ...
                    results["prometheus_config_keys"] = list(prom_config.keys()) if prom_config else []
        ...
        try:
            ...
            if hasattr(staging_deploy, 'validate_deployment'):
                try:
                    ...
                    validation_result = staging_deploy.validate_deployment()
        ...
        try:
            ...
            if config:
                ...
                if dev_config and staging_config:
                    common_keys = set(dev_config.keys()).intersection(set(staging_config.keys()))
    ...
    def validate_cross_module_imports(self) -> Dict[str, Any]:
        ...
        import_tests = [
            ("configs_from_deploy", "from configs.config_environment_loader import ConfigEnvironmentLoader"),
            ("deploy_from_configs", "import deploy.staging.deploy"),
            ("monitoring_from_configs", "from ops.monitoring.observability.server import ObservabilityServer"),
            ("monitoring_from_deploy", "from ops.monitoring.health_checks import health_manager"),
            ("cross_system_import", "from configs.config_environment_loader import ConfigEnvironmentLoader; from deploy.staging.deploy import StagingDeployment; from ops.monitoring.observability.server import ObservabilityServer")
        ]
    ...
    def validate_logging_integration(self) -> Dict[str, Any]:
        ...
        try:
            from ops.monitoring.logging.structured import LoggingConfig, setup_structured_logging
    ...
    def validate_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            integration_tests = [
                ...
                ("config_monitoring", lambda: self.validate_config_monitoring_integration()),
                ("deploy_monitoring", lambda: self.validate_deploy_monitoring_integration())
            ]
            ...
            for integration_name, integration_test in integration_tests:
                try:
                    ...
                    is_healthy = isinstance(integration_result, dict) and integration_result.get("all_integrations_working", False)
    ...
    def _test_monitoring_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "healthy": all([obs is not None, health_manager is not None, metrics_collector is not None]),
...
def main():
    ...
    # Save results
    report_file = project_root / f"wave5_integration_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
