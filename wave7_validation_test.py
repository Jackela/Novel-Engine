...
def test_character_domain_layer():
    ...
    try:
        ...
        print(f"✅ Character created: {character.profile.name} (Level {character.profile.level})")
        ...
        print(f"✅ Damage system working: {original_health} -> {character.stats.vital_stats.current_health}")
        
        character.heal(3)
        print(f"✅ Healing system working: -> {character.stats.vital_stats.current_health}")
        
        original_level = character.profile.level
        character.level_up()
        assert character.profile.level > original_level
        print(f"✅ Level up system working: {original_level} -> {character.profile.level}")
...
def test_character_application_layer():
    ...
    try:
        ...
        from contexts.character.application.commands.character_commands import (
            CreateCharacterCommand, UpdateCharacterStatsCommand, LevelUpCharacterCommand
        )
        from contexts.character.application.commands.character_command_handlers import (
            CreateCharacterCommandHandler, CharacterCommandHandlerRegistry
        )
        from contexts.character.application.services.character_application_service import (
        ...
        try:
            invalid_command = CreateCharacterCommand(
...
def test_character_infrastructure_design():
    ...
    try:
        ...
        expected_classes = ["CharacterORM", "CharacterProfileORM", "CharacterStatsORM", "CharacterSkillsORM"]
        ...
        expected_methods = ["get_by_id", "save", "delete", "find_by_name", "find_by_class"]
...
def run_wave7_validation():
    ...
    results.append(("M4.3: Character Infrastructure", test_character_infrastructure_design()))
