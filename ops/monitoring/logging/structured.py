...
class LoggingConfig:
    ...
    remote_endpoint: Optional[str] = None  # e.g., "http://localhost:3100/loki/api/v1/push"
...
class StructuredLogger:
    ...
    def _setup_file_handlers(self, formatter):
        ...
        main_log_path = os.path.join(self.config.log_directory, self.config.log_filename)
        ...
        if self.config.enable_security_logging:
            security_log_path = os.path.join(self.config.log_directory, self.config.security_log_file)
        ...
        if self.config.enable_performance_logging:
            performance_log_path = os.path.join(self.config.log_directory, self.config.performance_log_file)
        ...
        if self.config.enable_audit_logging:
            audit_log_path = os.path.join(self.config.log_directory, self.config.audit_log_file)
