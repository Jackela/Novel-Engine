...
async def get_world_summary(
    ...
    include_entity_details: bool = Query(False, description="Include detailed entity type information"),
...
async def get_entities_in_area(
    ...
    """
    logger.info(f"Getting entities in area for world {world_id} at ({center_x}, {center_y}) radius {radius}")
...
async def search_worlds(
    search_term: str = Query(..., min_length=1, description="Search term for world names/descriptions"),
