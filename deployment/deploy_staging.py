...
Usage:
    python deployment/deploy_staging.py [--validate-only] [--rollback]
"""
...
'''
    ...
    def deploy_configuration(self) -> bool:
        ...
        try:
            ...
            active_settings = self.project_root / "settings.yaml"
            
            # Backup current settings
            if active_settings.exists():
                shutil.copy2(active_settings, self.backup_dir / "settings_original.yaml")
            ...
            with open(active_settings) as f:
                ...
                if env != 'staging':
                    raise ValueError(f"Configuration deployment failed - environment: {env}")
    ...
    def run_health_checks(self) -> bool:
        ...
        while attempt < max_attempts:
            try:
                ...
                logger.warning(f"⚠️ Health check attempt {attempt + 1} failed - Status: {response.status_code}")
                
            except requests.exceptions.ConnectionError:
                logger.warning(f"⚠️ Health check attempt {attempt + 1} - Connection refused")
    ...
    def _extended_health_checks(self) -> bool:
        ...
        try:
            ...
            for endpoint in endpoints:
                ...
                if response.status_code not in [200, 404]:  # 404 acceptable for empty lists
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"⚠️ System status endpoint returned {response.status_code}")
            except Exception as e:
