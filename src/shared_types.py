...
class Area(BaseModel):
    ...
    properties: Dict[str, Any] = Field(
        default_factory=dict, description="Area-specific properties"
    )
...
class Equipment(BaseModel):
    ...
    properties: Dict[str, Any] = Field(
        default_factory=dict, description="Equipment-specific properties"
    )
...
class CharacterData(BaseModel):
    ...
    equipment: List[Equipment] = Field(
        default_factory=list, description="Equipped items"
    )
    state: CharacterState = Field(
        default_factory=CharacterState, description="Current status"
    )
    ai_personality: Dict[str, Any] = Field(
        default_factory=dict, description="AI behavior parameters"
    )
...
class ActionTarget(BaseModel):
    ...
    position: Optional[Position] = Field(
        default=None, description="Target position if applicable"
    )
    properties: Dict[str, Any] = Field(
        default_factory=dict, description="Target-specific properties"
    )
...
class ActionParameters(BaseModel):
    ...
    intensity: Optional[Union[ActionIntensity, float]] = Field(
        default=ActionIntensity.NORMAL,
        description="Action intensity level or numeric value",
    )
    duration: Optional[float] = Field(
        default=1.0, ge=0.0, description="Duration in time units"
    )
    range: Optional[float] = Field(
        default=1.0, ge=0.0, description="Action range in units"
    )
    modifiers: Dict[str, float] = Field(
        default_factory=dict, description="Action modifiers"
    )
    resources_consumed: Dict[str, float] = Field(
        default_factory=dict, description="Resources required"
    )
    conditions: List[str] = Field(
        default_factory=list, description="Required conditions"
    )
...
class ValidatedAction(BaseModel):
    ...
    repairs_applied: List[str] = Field(
        default_factory=list, description="Repairs made during validation"
    )
    estimated_effects: Dict[str, Any] = Field(
        default_factory=dict, description="Predicted action effects"
    )
...
class IronLawsViolation(BaseModel):
    ...
    affected_entities: List[str] = Field(
        default_factory=list, description="Entities affected by violation"
    )
    suggested_repair: Optional[str] = Field(
        default=None, description="Suggested fix for violation"
    )
...
class IronLawsReport(BaseModel):
    ...
    final_action: Optional[ValidatedAction] = Field(
        default=None, description="Final validated action"
    )
...
class WorldEntity(BaseModel):
    ...
    visibility: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Base visibility factor"
    )
...
class EnvironmentalCondition(BaseModel):
    ...
    duration_remaining: Optional[float] = Field(
        default=None, ge=0.0, description="Remaining duration"
    )
    effects: Dict[str, float] = Field(
        default_factory=dict, description="Condition effects on entities"
    )
...
class WorldState(BaseModel):
    ...
    global_properties: Dict[str, Any] = Field(
...
class InformationSource(BaseModel):
    ...
    reliability: float = Field(
        ..., ge=0.0, le=1.0, description="Source reliability factor"
    )
    access_channels: List[FogOfWarChannel] = Field(
        ..., description="Available information channels"
    )
    range_modifiers: Dict[str, float] = Field(
        default_factory=dict, description="Channel-specific range modifiers"
    )
...
class InformationFragment(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Information timestamp"
    )
    accuracy: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Information accuracy"
    )
    freshness: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Information recency factor"
    )
...
class FogOfWarFilter(BaseModel):
    ...
    visual_range: float = Field(
        default=10.0, ge=0.0, description="Visual observation range"
    )
    radio_range: float = Field(
        default=50.0, ge=0.0, description="Radio communication range"
    )
    intel_range: float = Field(
        default=100.0, ge=0.0, description="Intelligence network range"
    )
...
class FilteredWorldView(BaseModel):
    ...
    known_information: List[InformationFragment] = Field(
        default_factory=list, description="Information available"
    )
    uncertainty_markers: List[str] = Field(
        default_factory=list, description="Areas of uncertainty"
    )
    last_updated: datetime = Field(
...
class KnowledgeFragment(BaseModel):
    ...
    knowledge_type: str = Field(
        ..., description="Type of knowledge (rule, fact, strategy, etc.)"
    )
    tags: List[str] = Field(
        default_factory=list, description="Content tags for categorization"
    )
...
class ContextualPrompt(BaseModel):
    ...
    fog_of_war_mask: Optional[str] = Field(
        default=None, description="Information filtering mask"
    )
    prompt_tokens: Optional[int] = Field(
        default=None, ge=0, description="Estimated token count"
    )
    ...
    def compile_prompt(self) -> str:
        ...
        if self.injected_knowledge:
            knowledge_text = "\n".join(
                f"- {frag.content}" for frag in self.injected_knowledge
            )
            components.append(f"\n## Relevant Knowledge\n{knowledge_text}")
...
class TurnBrief(BaseModel):
    ...
    filtered_world_view: FilteredWorldView = Field(
        ..., description="Character's view of world state"
    )
    available_actions: List[ActionType] = Field(
        ..., description="Actions available to character"
    )
    contextual_prompt: ContextualPrompt = Field(
        ..., description="AI prompt with context"
    )
    tactical_situation: str = Field(
        default="", description="Summary of tactical situation"
    )
    objectives: List[str] = Field(
        default_factory=list, description="Current character objectives"
    )
    constraints: List[str] = Field(
        default_factory=list, description="Action constraints"
    )
    token_budget: Optional[int] = Field(
        default=None, ge=0, description="Token budget for AI processing"
    )
...
class SimulationConfig(BaseModel):
    ...
    max_turns: int = Field(
        default=10, ge=1, le=100, description="Maximum simulation turns"
    )
    turn_timeout: float = Field(
        default=30.0, gt=0.0, description="Per-turn timeout in seconds"
    )
    max_agents: int = Field(
        default=20, ge=1, le=100, description="Maximum number of agents"
    )
    iron_laws_enabled: bool = Field(
        default=True, description="Enable Iron Laws validation"
    )
    fog_of_war_enabled: bool = Field(
        default=True, description="Enable Fog of War filtering"
    )
    rag_injection_enabled: bool = Field(
        default=True, description="Enable RAG knowledge injection"
    )
...
class SimulationState(BaseModel):
    ...
    metrics: Dict[str, Any] = Field(
        default_factory=dict, description="Simulation metrics"
    )
...
class TurnResult(BaseModel):
    ...
    executed_actions: List[ValidatedAction] = Field(
        default_factory=list, description="Actions executed this turn"
    )
    world_state_changes: Dict[str, Any] = Field(
        default_factory=dict, description="Changes to world state"
    )
    character_updates: Dict[str, CharacterData] = Field(
        default_factory=dict, description="Updated character data"
    )
    events_generated: List[str] = Field(
        default_factory=list, description="Events generated this turn"
    )
    performance_metrics: Dict[str, float] = Field(
        default_factory=dict, description="Turn performance data"
    )
    errors: List[str] = Field(
        default_factory=list, description="Errors encountered during turn"
    )
...
class PerformanceMetrics(BaseModel):
    ...
    memory_delta_mb: Optional[float] = Field(
        default=None, description="Memory usage change in MB"
    )
...
class StateHash(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Hash computation time"
    )
    fields_included: List[str] = Field(
        ..., description="Fields included in hash computation"
    )
    salt: Optional[str] = Field(
        default=None, description="Salt used for hash computation"
    )
