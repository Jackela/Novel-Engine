...
from .protocols import ErrorHandlerProtocol
...
class SystemErrorHandler:
    ...
    async def handle_error(self, error: Exception, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.critical(f"Error handler failed while handling error: {handler_error}")
        ...
        try:
            async with self._error_lock:
                ...
                if self._error_statistics['recovery_attempts'] > 0:
                    recovery_rate = self._error_statistics['successful_recoveries'] / self._error_statistics['recovery_attempts']
                ...
                return {
                    ...
                    'errors_by_category': self._error_statistics['errors_by_category'].copy(),
                    'errors_by_severity': self._error_statistics['errors_by_severity'].copy(),
    ...
    async def register_recovery_function(self, error_category: str, recovery_func: Callable) -> None:
    ...
    async def register_fallback_function(self, context_key: str, fallback_func: Callable) -> None:
    ...
    async def _classify_error(self, error: Exception, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _retry_operation(self, error_record: ErrorRecord) -> Dict[str, Any]:
        ...
        if error_record.recovery_attempts <= max_retries:
            ...
            self.logger.info(f"Retrying operation for error {error_record.error_id} (attempt {error_record.recovery_attempts})")
            ...
            self.logger.warning(f"Max retries exceeded for error {error_record.error_id}")
            return {
                'attempted': True,
                'successful': False,
                'strategy': 'retry',
    ...
    async def _fallback_operation(self, error_record: ErrorRecord) -> Dict[str, Any]:
        ...
        if context_key in self._fallback_functions:
            try:
                ...
                self.logger.info(f"Fallback operation successful for error {error_record.error_id}")
                return {
                    'attempted': True,
                    'successful': True,
                    'strategy': 'fallback',
            ...
            self.logger.warning(f"No fallback function registered for context: {context_key}")
            return {
                'attempted': False,
                'successful': False,
                'strategy': 'fallback',
    ...
    async def _restart_component(self, error_record: ErrorRecord) -> Dict[str, Any]:
        ...
        self.logger.warning(f"Restarting component '{component}' due to error {error_record.error_id}")
    ...
    async def _escalate_error(self, error_record: ErrorRecord) -> Dict[str, Any]:
        ...
        self.logger.critical(f"Escalating error {error_record.error_id}: {error_record.exception}")
    ...
    async def _shutdown_gracefully(self, error_record: ErrorRecord) -> Dict[str, Any]:
        ...
        self.logger.critical(f"Initiating graceful shutdown due to error {error_record.error_id}")
    ...
    async def _is_error_suppressed(self, error_id: str) -> bool:
        ...
        if error_signature in self._suppressed_errors:
            ...
            if datetime.now() - suppression_time < timedelta(seconds=self._suppression_duration):
                return True
            else:
    ...
    async def _update_system_health(self, error_record: ErrorRecord, recovery_result: Dict[str, Any]) -> None:
    ...
    def _assess_system_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_critical_error': self._system_health['last_critical_error'].isoformat() if self._system_health['last_critical_error'] else None,
        ...
        try:
            while True:
                ...
                expired_suppressions = [
                    signature for signature, suppression_time in self._suppressed_errors.items()
                    if current_time - suppression_time > timedelta(seconds=self._suppression_duration * 2)
                ]
    ...
    async def _cleanup_old_errors(self) -> None:
        ...
        if len(self._error_records) > self._max_error_history:
            ...
            self.logger.debug(f"Cleaned up old error records, kept {len(keep_errors)} most recent")
