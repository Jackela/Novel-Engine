...
if WATCHDOG_AVAILABLE:
    class ConfigFileHandler(FileSystemEventHandler):
        ...
        def on_modified(self, event):
            if not event.is_directory and event.src_path in self.config_service._watched_files:
...
class ConfigurationService:
    ...
    def __init__(self, 
        ...
        self._hot_reload_enabled = WATCHDOG_AVAILABLE  # Only enable if watchdog available
                ...
                if self._validation_enabled:
                    ...
                    if not validation_result['valid']:
                        if self._strict_mode:
                            ...
                            self.logger.warning(f"Configuration validation warnings: {validation_result['errors']}")
            ...
            async with self._config_lock:
                ...
                if self._validation_enabled:
                    ...
                    if not validation_result['valid'] and self._strict_mode:
                        ...
                        self._config_data = old_config
                        raise ValueError(f"Configuration reload validation failed: {validation_result['errors']}")
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                if file_path.suffix.lower() in ['.yml', '.yaml']:
                    if YAML_AVAILABLE:
                        ...
                        self.logger.warning(f"YAML file {file_path} skipped - 'pyyaml' package not available")
                        return {}
                else:  # Assume JSON
            ...
            for schema_key, schema in self._config_schema.items():
                ...
                if schema.required and current_value is None:
                    if schema.default_value is not None:
                        # Set default value
                        self._set_nested_value(self._config_data, schema_key, schema.default_value)
                        validation_result['warnings'].append(f"Using default value for required key '{schema_key}'")
                    else:
                        validation_result['valid'] = False
                        validation_result['errors'].append(f"Required configuration key missing: {schema_key}")
                        continue
                    ...
                    if not isinstance(current_value, schema.data_type):
                        try:
                            ...
                            if schema.data_type == int:
                                ...
                                converted_value = bool(current_value) if not isinstance(current_value, str) else current_value.lower() in ['true', '1', 'yes']
                            else:
                                converted_value = schema.data_type(current_value)
                            
                            self._set_nested_value(self._config_data, schema_key, converted_value)
                            validation_result['warnings'].append(f"Converted '{schema_key}' from {type(current_value).__name__} to {schema.data_type.__name__}")
                            current_value = converted_value
                        except (ValueError, TypeError):
                            validation_result['valid'] = False
                            validation_result['errors'].append(f"Invalid type for '{schema_key}': expected {schema.data_type.__name__}, got {type(current_value).__name__}")
                            continue
                    
                    # Validation rules
                    for rule in schema.validation_rules:
                        if not await self._apply_validation_rule(schema_key, current_value, rule):
                            validation_result['valid'] = False
                            validation_result['errors'].append(f"Validation rule '{rule}' failed for key '{schema_key}' with value '{current_value}'")
        ...
        try:
            ...
            for file_path in self._watched_files:
                ...
                if dir_path not in watched_dirs:
                    self._file_observer.schedule(event_handler, dir_path, recursive=False)
                    watched_dirs.add(dir_path)
            
            self._file_observer.start()
            self.logger.info(f"File watching started for {len(watched_dirs)} directories")
