...
class AgentLifecycleManager:
        ...
        try:
            async with self._agent_lock:
                ...
                if not validation_result['valid']:
                    self.logger.error(f"Agent {agent_id} failed interface validation: {validation_result['errors']}")
                    return False
    ...
    async def update_agent_activity(self, agent_id: str, response_time: float = 0.0):
        ...
        if metrics:
            ...
            if response_time > 0:
                ...
                metrics.avg_response_time = ((old_avg * (count - 1)) + response_time) / count
    ...
    async def _validate_agent_interface(self, agent: Any) -> Dict[str, Any]:
        ...
        for method in required_methods:
            if not hasattr(agent, method) or not callable(getattr(agent, method)):
                ...
                validation_result['errors'].append(f"Missing or invalid method: {method}")
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'healthy_agents': len([m for m in self._agent_metrics.values() if m.health_status == 'healthy']),
            'unhealthy_agents': len([m for m in self._agent_metrics.values() if m.health_status != 'healthy']),
            'avg_response_time': sum(m.avg_response_time for m in self._agent_metrics.values()) / max(1, len(self._agent_metrics)),
