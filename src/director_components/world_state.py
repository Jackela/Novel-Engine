...
from .protocols import WorldStateProtocol
...
class WorldStateManager:
    ...
    def __init__(self, state_file: Optional[str] = None, logger: Optional[logging.Logger] = None):
    ...
    async def _apply_single_update(self, path: str, new_value: Any) -> Optional[StateChange]:
            ...
            try:
                ...
                if 'metadata' in data and 'checksum' in data['metadata']:
                    ...
                    if expected_checksum != actual_checksum:
                        self.logger.warning("State checksum mismatch - data may be corrupted")
    ...
    async def get_change_history(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        async with self._state_lock:
            recent_changes = self._change_history[-limit:] if self._change_history else []
    ...
    def _calculate_state_checksum(self) -> str:
        ...
        try:
            state_json = json.dumps(self._current_state, sort_keys=True, ensure_ascii=True)
    ...
    async def export_state(self, export_path: str, include_history: bool = False) -> bool:
        ...
        try:
            ...
            if include_history:
                export_data['change_history'] = await self.get_change_history(limit=1000)
