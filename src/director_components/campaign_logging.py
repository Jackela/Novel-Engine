...
class CampaignLoggingService:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Campaign logging service initialized for session {self.session_id}")
            return True
        ...
        try:
            ...
            await self.log_event({
                ...
                'metadata': {
                    ...
                    'agent_count': summary.get('metrics', {}).get('total_agents', 0),
                    'successful_decisions': summary.get('metrics', {}).get('successful_decisions', 0),
                    'world_state_changes': len(summary.get('world_state_changes', {})),
                    'narrative_events': len(summary.get('narrative_events', [])),
                    'conflicts_resolved': summary.get('metrics', {}).get('conflicts_resolved', 0)
                },
                'turn_number': turn_number
            })
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            session_duration = (current_time - self._log_statistics['session_start']).total_seconds()
            
            # Calculate error rate in last hour
            one_hour_ago = current_time - timedelta(hours=1)
            recent_errors = sum(1 for entry in self._log_entries 
                              if entry.timestamp > one_hour_ago and entry.level in [LogLevel.ERROR, LogLevel.CRITICAL])
            ...
            return {
                ...
                'entries_by_category': self._log_statistics['entries_by_category'].copy(),
    ...
    async def _create_log_entry(self, event: Dict[str, Any]) -> LogEntry:
        ...
        category = EventCategory(category_str) if category_str in [c.value for c in EventCategory] else EventCategory.SYSTEM
    ...
    def _update_statistics(self, entry: LogEntry) -> None:
        ...
        if entry.level in [LogLevel.ERROR, LogLevel.CRITICAL]:
            ...
            # Count recent errors
            recent_error_count = sum(1 for e in self._log_entries[-100:] 
                                   if e.timestamp > one_hour_ago and 
    ...
    async def _analyze_entry(self, entry: LogEntry) -> None:
        ...
        try:
            ...
            if entry.category == EventCategory.PERFORMANCE:
                ...
                self._performance_metrics[metric_name] = [
                    m for m in self._performance_metrics[metric_name] 
    ...
    async def _archive_entries(self, entries: List[LogEntry]) -> None:
        ...
        try:
            ...
            for date_key, date_entries in entries_by_date.items():
                archive_file = self.log_dir / f"{self.session_id}_archive_{date_key}.jsonl.gz"
                ...
                self.logger.debug(f"Archived {len(date_entries)} entries to {archive_file}")
    ...
    async def _check_log_rotation(self, file_path: Path) -> None:
        ...
        try:
            if file_path.exists() and file_path.stat().st_size > self._log_rotation_size:
                # Rotate the file
    ...
    def _get_top_error_patterns(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        sorted_patterns = sorted(self._error_patterns.items(), 
    ...
    async def export_logs(self, 
        ...
        try:
            ...
            self.logger.info(f"Exported {len(entries_to_export)} log entries to {export_path}")
            return True
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            await self.log_event({
                ...
                'metadata': {
                    'total_entries': self._log_statistics['total_entries'],
                    'session_duration': (datetime.now() - self._log_statistics['session_start']).total_seconds()
                }
            })
            ...
            self.logger.info(f"Campaign logging service cleanup completed for session {self.session_id}")
