...
from .protocols import NarrativeProtocol
...
class NarrativeOrchestrator:
    ...
    async def generate_narrative_context(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            context = {
                ...
                'narrative_opportunities': await self._identify_narrative_opportunities(agent_id),
            ...
            self.logger.error(f"Failed to generate narrative context for {agent_id}: {e}")
            return {'error': str(e), 'agent_id': agent_id}
    
    async def process_narrative_events(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for event_data in events:
                try:
                    ...
                    if narrative_event:
                        ...
                        failed_events.append({'data': event_data, 'error': 'Failed to create event'})
            ...
            self.logger.info(f"Processed {len(processed_events)} narrative events, {len(failed_events)} failed")
            return result
        ...
        try:
            for key, value in updates.items():
                if key in self._story_state:
                    self._story_state[key].update(value) if isinstance(self._story_state[key], dict) else None
                    self._story_state[key] = value if not isinstance(self._story_state[key], dict) else self._story_state[key]
                else:
    ...
    async def get_story_summary(self) -> Dict[str, Any]:
        ...
        try:
            recent_events = self._narrative_events[-20:] if self._narrative_events else []
    ...
    async def _get_relevant_events(self, agent_id: str, limit: int = 10) -> List[NarrativeEvent]:
        """Get events relevant to specific agent."""
        relevant_events = []
        
        for event in reversed(self._narrative_events[-50:]):  # Check last 50 events
            if (agent_id in event.participants or
                event.event_type in [EventType.WORLD_CHANGE, EventType.TRANSITION]):
    ...
    async def _identify_narrative_opportunities(self, agent_id: str) -> List[Dict[str, Any]]:
        """Identify narrative opportunities for agent."""
        opportunities = []
        
        # Check for unresolved conflicts
        for event in self._narrative_events[-20:]:
            if (event.event_type == EventType.CONFLICT and
                agent_id in event.participants and
                event.metadata.get('resolved', False) == False):
    ...
    async def _get_world_narrative_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'major_events_today': len([e for e in self._narrative_events[-50:] 
                                      if e.impact_score > 0.7]),
            'story_phase': self._determine_story_phase()
        }
    
    async def _create_narrative_event(self, event_data: Dict[str, Any]) -> Optional[NarrativeEvent]:
        ...
        try:
            ...
            event = NarrativeEvent(
                event_id=event_data.get('id', f"event_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"),
    ...
    async def _update_character_context(self, character_id: str, event: NarrativeEvent) -> None:
        ...
        if event.event_type == EventType.CONFLICT:
            context.tension_level = min(1.0, context.tension_level + event.impact_score * 0.3)
            context.mood = "tense"
        elif event.event_type == EventType.RESOLUTION:
            context.tension_level = max(0.0, context.tension_level - event.impact_score * 0.4)
    ...
    async def _update_narrative_threads(self, event: NarrativeEvent) -> None:
        ...
        async with self._thread_lock:
            ...
            # Close threads for resolutions
                ...
                threads_to_close = []
                for thread_id, thread_data in self._active_threads.items():
                    if any(p in event.participants for p in thread_data.get('participants', [])):
    ...
    async def _manage_event_history(self) -> None:
        ...
        async with self._thread_lock:
            if len(self._active_threads) > self._max_threads:
                resolved_threads = {k: v for k, v in self._active_threads.items() 
                                  if v.get('status') == 'resolved'}
                
                if resolved_threads:
                    # Remove oldest resolved threads
                    sorted_resolved = sorted(resolved_threads.items(), 
    ...
    async def _update_character_contexts(self, character_updates: Dict[str, Any]) -> None:
    ...
    async def _calculate_narrative_health(self) -> Dict[str, Any]:
        ...
        participant_diversity = len(set(p for e in recent_events for p in e.participants))
        
        health_score = (event_diversity * 0.4 + avg_impact * 0.4 + 
                       min(1.0, participant_diversity / 5.0) * 0.2)
        
        return {
            'status': 'healthy' if health_score > 0.6 else 'degraded' if health_score > 0.3 else 'poor',
            'score': health_score,
    ...
    def _determine_story_phase(self) -> str:
        ...
        total_events = len(self._narrative_events)
        recent_conflicts = sum(1 for e in self._narrative_events[-20:] 
                              if e.event_type == EventType.CONFLICT)
        recent_resolutions = sum(1 for e in self._narrative_events[-20:] 
    ...
    async def export_narrative_data(self, export_path: str) -> bool:
        ...
        try:
            export_data = {
                ...
                'statistics': {
                    **self._story_statistics,
                    'active_characters': list(self._story_statistics['active_characters'])
                },
                'export_metadata': {
