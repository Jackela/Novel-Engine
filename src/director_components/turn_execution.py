...
from .protocols import TurnEngineProtocol, AgentManagerProtocol
...
class TurnExecutionEngine:
    ...
    def __init__(self, agent_manager: AgentManagerProtocol, logger: Optional[logging.Logger] = None):
    ...
    async def execute_turn(self, turn_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not decision_result['success']:
                turn_result['error'] = f"Decision phase failed: {decision_result['error']}"
                return turn_result
            ...
            if self._config['enable_conflict_resolution']:
                ...
                if not conflict_result['success']:
                    self.logger.warning(f"Conflict resolution issues: {conflict_result['error']}")
            ...
            metrics = TurnMetrics(
                ...
                agent_response_times=context.execution_metrics.get('agent_response_times', {}),
                success_rate=1.0,
                error_count=0
            )
            self._turn_metrics.append(metrics)
            
            self.logger.info(f"Turn {self._turn_counter} completed successfully in {turn_duration:.2f}s")
            
        except Exception as e:
            turn_duration = time.time() - turn_start_time
            self.logger.error(f"Turn {self._turn_counter} failed after {turn_duration:.2f}s: {e}")
    ...
    async def _execute_preparation_phase(self, context: TurnContext, turn_data: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            context.world_state_updates = turn_data.get('world_state', {}) if turn_data else {}
    ...
    async def _execute_decision_phase(self, context: TurnContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._config['enable_parallel_execution'] and len(agents) > 1:
                decision_results = await self._execute_parallel_decisions(context, agents)
            else:
                decision_results = await self._execute_sequential_decisions(context, agents)
            ...
            for agent_id, result in decision_results.items():
                if result.get('success', False):
                    ...
                    self.logger.warning(f"Agent {agent_id} decision failed: {result.get('error', 'Unknown')}")
    ...
    async def _execute_parallel_decisions(self, context: TurnContext, agents: List[Any]) -> Dict[str, Dict[str, Any]]:
    ...
    async def _execute_sequential_decisions(self, context: TurnContext, agents: List[Any]) -> Dict[str, Dict[str, Any]]:
    ...
    async def _execute_single_agent_decision(self, agent: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            response_time = time.time() - decision_start
            self.logger.warning(f"Agent {agent_id} decision timeout after {response_time:.2f}s")
            return {'success': False, 'error': 'Decision timeout', 'response_time': response_time}
            
        except Exception as e:
            response_time = time.time() - decision_start
            self.logger.error(f"Agent {agent_id} decision error after {response_time:.2f}s: {e}")
            return {'success': False, 'error': str(e), 'response_time': response_time}
    
    async def _execute_conflict_resolution_phase(self, context: TurnContext) -> Dict[str, Any]:
            ...
            if conflicts:
                ...
                for conflict in conflicts:
                    ...
                    if resolution.get('success'):
                        ...
                        self.logger.warning(f"Failed to resolve conflict: {resolution.get('error')}")
            
            phase_duration = time.time() - phase_start
            context.execution_metrics['phase_durations']['conflict_resolution'] = phase_duration
    ...
    async def _execute_state_update_phase(self, context: TurnContext) -> Dict[str, Any]:
        ...
        try:
            ...
            phase_duration = time.time() - phase_start
            context.execution_metrics['phase_durations']['state_update'] = phase_duration
    ...
    async def _execute_finalization_phase(self, context: TurnContext) -> Dict[str, Any]:
        ...
        try:
            ...
            phase_duration = time.time() - phase_start
            context.execution_metrics['phase_durations']['finalization'] = phase_duration
    ...
    async def _identify_conflicts(self, agent_decisions: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Identify conflicts between agent decisions."""
        conflicts = []
    ...
    async def _resolve_conflict(self, conflict: Dict[str, Any], context: TurnContext) -> Dict[str, Any]:
    ...
    def _apply_conflict_resolution(self, resolution: Dict[str, Any], context: TurnContext) -> None:
    ...
    async def _generate_narrative_events(self, context: TurnContext) -> List[Dict[str, Any]]:
        ...
        for agent_id, decision in context.agent_decisions.items():
            if isinstance(decision, dict) and not decision.get('overridden', False):
                event = {
                    ...
                    'description': decision.get('description', f'Agent {agent_id} performed an action')
                }
                events.append(event)
    ...
    async def get_turn_metrics(self, last_n: int = 10) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_turn_duration': recent_metrics[-1].total_duration if recent_metrics else 0,
