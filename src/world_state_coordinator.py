...
class WorldStateCoordinator:
    ...
    def _load_world_state(self) -> None:
        ...
        try:
            ...
            if not world_state_path.exists():
                logger.info(f"World state file {self.world_state_file_path} does not exist, creating default")
                self._initialize_default_world_state()
            ...
            logger.info(f"World state loaded successfully ({len(self.world_state_data)} keys)")
    ...
    def get_world_state_for_agent(self, agent_id: str, current_turn: int, 
    ...
    def _get_agent_discoveries_feedback(self, agent_id: str) -> List[str]:
        ...
        """
        discovered_clues = self.world_state_tracker.get('discovered_clues', {}).get(agent_id, [])
    ...
    def _get_environmental_changes_feedback(self, agent_id: str) -> List[str]:
        ...
        """
        environmental_changes = self.world_state_tracker.get('environmental_changes', {})
    ...
    def _get_world_state_summary(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'major_factions_active': len(self.world_state_data.get('faction_status', {})),
                'known_locations': len(self.world_state_data.get('locations', {})),
                'recent_events_count': len(self.world_state_data.get('global_events', [])),
                'environmental_status': self.world_state_data.get('environmental_state', {}).get('weather', 'unknown')
            }
        except Exception as e:
            logger.error(f"Error generating world state summary: {str(e)}")
            return {}
    
    def record_agent_discovery(self, agent_id: str, discovery: str, turn_number: int) -> None:
        """
        ...
        try:
            ...
            self.world_state_tracker['agent_discoveries'][turn_number][agent_id].append(discovery)
            ...
            self.world_state_tracker['investigation_history'].append(investigation_entry)
    ...
    def get_world_state_summary(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'global_events_count': len(self.world_state_data.get('global_events', [])),
                'tracked_agents': len(self.world_state_tracker.get('discovered_clues', {})),
                'total_discoveries': sum(len(discoveries) for discoveries in 
                                       self.world_state_tracker.get('discovered_clues', {}).values()),
                'environmental_changes': len(self.world_state_tracker.get('environmental_changes', {})),
                'investigation_history_count': len(self.world_state_tracker.get('investigation_history', [])),
