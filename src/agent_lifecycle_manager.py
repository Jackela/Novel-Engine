...
class AgentLifecycleManager:
    ...
    def __init__(self):
        ...
        logger.info(f"AgentLifecycleManager initialized (Iron Laws: {'enabled' if self.validation_enabled else 'disabled'})")
    
    def adjudicate_agent_action(self, agent: PersonaAgent, proposed_action: ProposedAction, 
                              character_data: Optional[CharacterData] = None) -> ActionAdjudicationResult:
        """
        ...
        try:
            ...
            logger.info(f"Adjudicating action {proposed_action.action_id} from agent {agent_id}")
            
            if not self.validation_enabled:
                # Return success if Iron Laws not available
                return ActionAdjudicationResult(
                    success=True,
                    validated_action=self._create_fallback_validated_action(proposed_action),
                    violations=[],
                    repair_log=["Iron Laws validation not available - action approved by default"],
                    adjudication_notes=["Validation system disabled"]
                )
            
            # Validate action against Iron Laws
            violations = self._validate_against_iron_laws(proposed_action, character_data)
            
            if not violations:
                # Action is valid
                validated_action = self._convert_proposed_to_validated(proposed_action, ValidationResult.VALID)
                
                self._record_successful_action(agent_id, proposed_action, validated_action)
            ...
            logger.warning(f"Action {proposed_action.action_id} has {len(violations)} violations, attempting repairs")
            ...
            if validated_action and validated_action.status == ValidationResult.VALID:
                ...
                self._record_successful_action(agent_id, proposed_action, validated_action, repair_log)
                ...
                # Repairs failed
                self._record_failed_action(agent_id, proposed_action, violations, repair_log)
    ...
    def _validate_against_iron_laws(self, proposed_action: ProposedAction, 
                                  character_data: Optional[CharacterData] = None) -> List[IronLawsViolation]:
        """
        ...
        try:
            ...
            resource_violations = self._validate_resource_law(proposed_action, character_data)
            violations.extend(resource_violations)
            
            # E003 - Physics Law validation
            physics_violations = self._validate_physics_law(proposed_action, character_data)
    ...
    def _validate_causality_law(self, proposed_action: ProposedAction) -> List[IronLawsViolation]:
        ...
        try:
            ...
            if not hasattr(proposed_action, 'action_type') or not proposed_action.action_type:
                violations.append(IronLawsViolation(
                    law_code="E001", 
    ...
    def _validate_resource_law(self, proposed_action: ProposedAction, 
                             character_data: Optional[CharacterData] = None) -> List[IronLawsViolation]:
    ...
    def _validate_physics_law(self, proposed_action: ProposedAction, 
                            character_data: Optional[CharacterData] = None) -> List[IronLawsViolation]:
    ...
    def _validate_narrative_law(self, proposed_action: ProposedAction) -> List[IronLawsViolation]:
    ...
    def _validate_social_law(self, proposed_action: ProposedAction) -> List[IronLawsViolation]:
    ...
    def _repair_causality_violations(self, action: ProposedAction, 
        ...
        for violation in violations:
            if "target specification" in violation.description:
                ...
                if hasattr(modified_action, 'action_type') and not modified_action.action_type:
            ...
            if "violates physical laws" in violation.description:
                ...
                if hasattr(modified_action, 'action_type'):
                    ...
                    if old_action in impossible_to_possible:
                        ...
                        repairs_made.append(f"Changed impossible action '{old_action}' to '{modified_action.action_type}'")
    ...
    def _repair_narrative_violations(self, action: ProposedAction, 
        ...
        for violation in violations:
            if "lacks sufficient narrative reasoning" in violation.description:
                ...
                if hasattr(modified_action, 'reasoning') and (not modified_action.reasoning or len(modified_action.reasoning.strip()) < 10):
    ...
    def _convert_proposed_to_validated(self, proposed_action: ProposedAction, 
                                     validation_result: str) -> ValidatedAction:
    ...
    def _create_fallback_validated_action(self, proposed_action: ProposedAction) -> ValidatedAction:
    ...
    def _record_successful_action(self, agent_id: str, proposed_action: ProposedAction, 
                                validated_action: ValidatedAction, repair_log: List[str] = None) -> None:
    ...
    def _record_failed_action(self, agent_id: str, proposed_action: ProposedAction, 
                            violations: List[IronLawsViolation], repair_log: List[str] = None) -> None:
    ...
    def _record_violations(self, proposed_action: ProposedAction, violations: List[IronLawsViolation]) -> None:
    ...
    def get_lifecycle_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'repair_success_rate': (self.successful_repairs_count / self.repair_attempts_count) if self.repair_attempts_count > 0 else 0.0,
    ...
    def get_violation_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'most_violated_law': max(law_violations.items(), key=lambda x: x[1])[0] if law_violations else None
            }
        except Exception as e:
