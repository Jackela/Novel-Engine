...
class MemoryInterface:
    ...
    def update_internal_memory(self, new_log: Dict[str, Any]) -> None:
        ...
        """
        try:
            logger.info(f"Agent {self.agent_core.agent_id} updating internal memory with new experience")
            
            # Validate input format
            if not isinstance(new_log, dict):
                logger.warning(f"Invalid memory log format for agent {self.agent_core.agent_id}")
                return
            ...
            if self._should_store_in_long_term_memory(memory_entry):
                ...
                logger.info(f"Storing significant event in long-term memory for {self.agent_core.agent_id}")
            ...
            logger.info(f"Memory updated for {self.agent_core.agent_id}: {event_type} - {description[:50]}...")
            
        except Exception as e:
            logger.error(f"Error updating internal memory for agent {self.agent_core.agent_id}: {str(e)}")
    ...
    def update_memory(self, event_string: str) -> None:
        ...
        try:
            ...
            logger.debug(f"Persistent memory updated for {self.agent_core.agent_id}: {event_string[:50]}...")
            
        except Exception as e:
            logger.error(f"Error updating persistent memory for agent {self.agent_core.agent_id}: {str(e)}")
            ...
            if outcome:
                ...
                if 'success' in outcome.lower():
                    if 'chaos' in faction.lower():
                        interpretation += f", viewing this success as another step toward glorious chaos"
                    elif 'imperium' in faction.lower():
                        interpretation += f", seeing this success as service to the Emperor"
                    else:
                        interpretation += f", considering this a positive outcome"
                
                elif 'failure' in outcome.lower():
                    if self.agent_core.morale_level > 0.5:
                        interpretation += f", but remains determined despite the setback"
                    else:
                        interpretation += f", feeling discouraged by this failure"
            
            # Add character-specific perspective
            if description:
                if 'combat' in event_type.lower() and self.agent_core.morale_level > 0.7:
                    interpretation += f". The combat experience reinforced their resolve."
                elif 'dialogue' in event_type.lower():
                    interpretation += f". The conversation provided valuable insights."
                elif 'discovery' in event_type.lower():
                    interpretation += f". This discovery could prove useful."
    ...
    def _manage_short_term_memory_capacity(self) -> None:
        ...
        try:
            while len(self.agent_core.short_term_memory) > self.short_term_memory_limit:
                ...
                logger.debug(f"Removed old short-term memory entry: {removed_entry.get('event_type', 'unknown')}")
        except Exception as e:
    ...
    def _manage_long_term_memory_capacity(self) -> None:
        ...
        try:
            while len(self.agent_core.long_term_memory) > self.long_term_memory_limit:
                ...
                self.agent_core.long_term_memory.sort(key=lambda x: x.get('significance', 0.0), reverse=True)
                removed_entry = self.agent_core.long_term_memory.pop(-1)
                logger.debug(f"Removed low-significance long-term memory entry: {removed_entry.get('event_type', 'unknown')}")
        except Exception as e:
    ...
    def _update_relationships_from_experience(self, experience: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for participant in participants:
                ...
                current_relationship = self.agent_core.get_relationship_strength(participant)
                ...
                if relationship_change != 0.0:
                    ...
                    logger.debug(f"Relationship with {participant} changed by {relationship_change} to {new_relationship}")
        ...
        try:
            ...
            if location:
                location_info = {
                    ...
                    'threat_level': self._assess_location_threat_from_experience(experience)
                }
                
                self.agent_core.add_to_subjective_worldview('location_knowledge', location, location_info)
            ...
            if event_type.lower() in ['combat', 'ambush', 'attack']:
                ...
                self.agent_core.add_to_subjective_worldview('active_threats', threat_id, threat_info)
            ...
            for participant in participants:
                if participant != self.agent_core.agent_id:
                    ...
                    if 'chaos' in participant.lower():
                        ...
                        self.agent_core.add_to_subjective_worldview('faction_relationships', 'chaos', faction_info)
                    elif 'imperial' in participant.lower() or 'imperium' in participant.lower():
                        faction_info = {'activity': 'active', 'threat_level': 'variable'}
                        self.agent_core.add_to_subjective_worldview('faction_relationships', 'imperium', faction_info)
                        
        except Exception as e:
            logger.error(f"Error updating worldview from experience: {str(e)}")
    
    def _assess_location_threat_from_experience(self, experience: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            if event_type.lower() == 'combat':
                if 'victory' in outcome.lower():
                    ...
                    current_aggression = self.agent_core.behavioral_modifiers.get('aggression', 0.5)
                    self.agent_core.behavioral_modifiers['aggression'] = min(1.0, current_aggression + adjustment_factor)
                elif 'defeat' in outcome.lower():
                    # Increase caution slightly
                    current_caution = self.agent_core.behavioral_modifiers.get('caution', 0.5)
                    self.agent_core.behavioral_modifiers['caution'] = min(1.0, current_caution + adjustment_factor)
            
            elif event_type.lower() == 'betrayal':
                # Increase suspicion/caution
                current_suspicion = self.agent_core.behavioral_modifiers.get('suspicion', 0.5)
                self.agent_core.behavioral_modifiers['suspicion'] = min(1.0, current_suspicion + adjustment_factor * 2)
            
            elif event_type.lower() == 'cooperation':
                if emotional_impact in ['positive', 'success']:
                    # Increase trust/cooperation tendency
                    current_cooperation = self.agent_core.behavioral_modifiers.get('cooperation', 0.5)
                    self.agent_core.behavioral_modifiers['cooperation'] = min(1.0, current_cooperation + adjustment_factor)
    ...
    def get_recent_memories(self, count: int = 5) -> List[Dict[str, Any]]:
        ...
        """
        try:
            recent_memories = self.agent_core.short_term_memory[-count:] if self.agent_core.short_term_memory else []
    ...
    def get_significant_memories(self, count: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            sorted_memories = sorted(self.agent_core.long_term_memory, 
    ...
    def search_memories(self, search_term: str, memory_type: str = 'all') -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for memory in memories_to_search:
                ...
                personal_interpretation = memory.get('personal_interpretation', '').lower()
                
                if (search_lower in description or 
                    search_lower in event_type or 
                    search_lower in personal_interpretation):
    ...
    def load_persistent_memories(self) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Loaded {len(memories)} persistent memories for {self.agent_core.agent_id}")
    ...
    def clear_short_term_memory(self) -> None:
        ...
        try:
            ...
            logger.info(f"Cleared {count} short-term memory entries for {self.agent_core.agent_id}")
        except Exception as e:
    ...
    def consolidate_memories(self) -> int:
        ...
        try:
            ...
            for memory in self.agent_core.short_term_memory.copy():
                if (memory.get('significance', 0.0) >= self.significance_threshold and
                    memory not in self.agent_core.long_term_memory):
            ...
            logger.info(f"Consolidated {consolidated_count} memories to long-term for {self.agent_core.agent_id}")
