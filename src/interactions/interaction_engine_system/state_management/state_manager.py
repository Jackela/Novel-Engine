...
try:
    from src.core.data_models import (
        ...
        MemoryType, EmotionalState, InteractionResult
    )
...
class StateManager:
    ...
    async def update_interaction_states(self, context: InteractionContext,
                                       interaction_data: Dict[str, Any]) -> StandardResponse:
        """
        ...
        """
        try:
            self.logger.info(f"Updating states for interaction: {context.interaction_id}")
            ...
            character_updates = await self._update_character_states(context, interaction_data)
            update_results["character_states"] = character_updates.get("data", {}).get("state_updates", [])
            
            # Generate and store memories
            if self.config.memory_integration_enabled and self.config.auto_generate_memories:
                memory_updates = await self._generate_interaction_memories(context, interaction_data)
                update_results["memory_updates"] = memory_updates.get("data", {}).get("memory_updates", [])
            
            # Update relationships
            relationship_updates = await self._update_relationships(context, interaction_data)
            update_results["relationship_changes"] = relationship_updates.get("data", {}).get("relationship_changes", [])
            
            # Process emotional changes
            emotional_updates = await self._process_emotional_changes(context, interaction_data)
            update_results["emotional_changes"] = emotional_updates.get("data", {}).get("emotional_changes", [])
            
            # Apply all updates
            application_result = await self._apply_pending_updates(context.interaction_id)
    ...
    async def generate_memory_from_interaction(self, context: InteractionContext,
                                             interaction_data: Dict[str, Any]) -> StandardResponse:
        """
        ...
        try:
            ...
            if self._calculate_interaction_significance(context, interaction_data) > self.state_thresholds["memory_significance_threshold"]:
                semantic_memory = await self._create_semantic_memory(context, interaction_data)
                if semantic_memory:
                    memories.append(semantic_memory)
            
            # Generate procedural memories for skill-based interactions
            if context.interaction_type in [InteractionType.INSTRUCTION, InteractionType.COOPERATION]:
                for participant in context.participants:
    ...
    async def update_relationship_weights(self, participants: List[str],
                                        interaction_type: InteractionType,
                                        interaction_outcome: Dict[str, Any]) -> StandardResponse:
        """
        ...
        try:
            ...
            base_change = self._calculate_base_relationship_change(interaction_type, interaction_outcome)
                ...
                for participant_b in participants[i+1:]:
                    if participant_a != participant_b:
                        ...
                        change_amount = self._calculate_specific_relationship_change(
                            participant_a, participant_b, base_change, interaction_outcome
                        )
                        
                        if abs(change_amount) >= self.state_thresholds["relationship_change_threshold"]:
                            update = {
                                "participant_a": participant_a,
                                "participant_b": participant_b,
                                "change_amount": change_amount,
                                "interaction_type": interaction_type.value,
                                "reason": self._determine_relationship_change_reason(interaction_type, interaction_outcome)
                            }
                            relationship_updates.append(update)
                            
                            # Track for batch processing
                            self.relationship_changes[(participant_a, participant_b)] = change_amount
    ...
    def get_state_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.state_update_stats,
            "pending_state_updates": sum(len(updates) for updates in self.pending_state_updates.values()),
            "pending_memory_updates": sum(len(updates) for updates in self.pending_memory_updates.values()),
    ...
    async def _update_character_states(self, context: InteractionContext,
                                     interaction_data: Dict[str, Any]) -> StandardResponse:
    ...
    async def _generate_interaction_memories(self, context: InteractionContext,
                                           interaction_data: Dict[str, Any]) -> StandardResponse:
        ...
        try:
            ...
            for participant in context.participants:
                ...
                memory_item = {
                    ...
                    "associated_agents": [p for p in context.participants if p != participant],
                    "location": context.location,
                    "timestamp": datetime.now(),
                    "emotional_weight": self._calculate_emotional_weight(participant, interaction_data),
                    "significance": self._calculate_memory_significance(participant, context, interaction_data),
                    "metadata": {
    ...
    async def _update_relationships(self, context: InteractionContext,
                                  interaction_data: Dict[str, Any]) -> StandardResponse:
    ...
    async def _process_emotional_changes(self, context: InteractionContext,
                                       interaction_data: Dict[str, Any]) -> StandardResponse:
        ...
        try:
            ...
            for participant in context.participants:
                emotional_impact = interaction_data.get("emotional_impacts", {}).get(participant, {})
                
                if emotional_impact and emotional_impact.get("mood_change", 0) > self.state_thresholds["emotion_change_threshold"]:
                    change = {
                        "agent_id": participant,
                        "emotion_type": "mood",
    ...
    async def _apply_pending_updates(self, interaction_id: str) -> StandardResponse:
        ...
        try:
            ...
            applied_updates["state_updates"] = sum(len(updates) for updates in self.pending_state_updates.values())
            applied_updates["memory_updates"] = sum(len(updates) for updates in self.pending_memory_updates.values())
    ...
    def _calculate_character_state_changes(self, agent_id: str, context: InteractionContext,
                                         interaction_data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    def _calculate_interaction_significance(self, context: InteractionContext,
    ...
    def _calculate_base_relationship_change(self, interaction_type: InteractionType,
    ...
    def _calculate_specific_relationship_change(self, participant_a: str, participant_b: str,
                                              base_change: float, outcome: Dict[str, Any]) -> float:
    ...
    def _determine_relationship_change_reason(self, interaction_type: InteractionType,
    ...
    def _calculate_emotional_weight(self, agent_id: str, interaction_data: Dict[str, Any]) -> float:
        """Calculate emotional weight for memory."""
        return interaction_data.get("emotional_impacts", {}).get(agent_id, {}).get("mood_change", 0.0)
    
    def _calculate_memory_significance(self, agent_id: str, context: InteractionContext,
                                     interaction_data: Dict[str, Any]) -> float:
        """Calculate memory significance for specific agent."""
        return self._calculate_interaction_significance(context, interaction_data)
    
    async def _create_episodic_memory(self, agent_id: str, context: InteractionContext,
                                    interaction_data: Dict[str, Any]) -> Optional[MemoryItem]:
        ...
        try:
            return {
                ...
                "location": context.location,
                "participants": context.participants,
                "significance": self._calculate_memory_significance(agent_id, context, interaction_data)
            }
        except Exception as e:
            self.logger.error(f"Episodic memory creation failed: {e}")
            return None
    
    async def _create_semantic_memory(self, context: InteractionContext,
                                    interaction_data: Dict[str, Any]) -> Optional[MemoryItem]:
        ...
        try:
            return {
                ...
                "timestamp": datetime.now(),
                "participants": context.participants,
                "significance": self._calculate_interaction_significance(context, interaction_data)
            }
        except Exception as e:
            self.logger.error(f"Semantic memory creation failed: {e}")
            return None
    
    async def _create_procedural_memory(self, agent_id: str, context: InteractionContext,
                                      interaction_data: Dict[str, Any]) -> Optional[MemoryItem]:
