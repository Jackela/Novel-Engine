...
from ..core.types import (
    DynamicEquipment, EquipmentItem, EquipmentStatus, EquipmentCategory,
...
try:
    ...
    AgentID = str
    ContextDatabase = type('ContextDatabase', (), {'save_memory_item': lambda *args: None})
...
class EquipmentRegistry:
            ...
            async with self._processing_lock:
                ...
                if self.context_db and self.config.context_db_enabled:
                    try:
                        ...
                        self.logger.warning(f"Failed to save equipment registration to database: {e}")
    ...
    async def get_agent_equipment(self, agent_id: str, 
                                ...
                                status_filter: Optional[List[EquipmentStatus]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for eq_id in agent_equipment_ids:
                ...
                if category:
                    eq_category = self._determine_equipment_category(equipment.base_equipment)
                ...
                equipment_info = {
                    ...
                    "category": self._determine_equipment_category(equipment.base_equipment).value,
            ...
            return {
                ...
                "categories_represented": list(set([eq["category"] for eq in equipment_list])),
                "total_wear_average": sum([eq["wear_accumulation"] for eq in equipment_list]) / max(1, len(equipment_list))
            }
    ...
    async def assign_equipment(self, equipment_id: str, agent_id: str) -> StandardResponse:
        """
        ...
        try:
            async with self._processing_lock:
                ...
                if equipment.current_status in [EquipmentStatus.DESTROYED, EquipmentStatus.MISSING]:
                    return StandardResponse(
                        success=False,
                        error=ErrorInfo(
                ...
                if equipment.assigned_agent:
                    await self._unassign_equipment_from_agent(equipment_id, equipment.assigned_agent)
                ...
                self.logger.info(f"Equipment '{equipment_id}' assigned to agent '{agent_id}'")
    ...
    async def update_equipment_status(self, equipment_id: str, 
                                    new_status: EquipmentStatus,
                                    reason: str = "") -> StandardResponse:
        """
        ...
        try:
            async with self._processing_lock:
                ...
                self.logger.info(f"Equipment '{equipment_id}' status changed: {old_status.value} -> {new_status.value}")
    ...
    async def _assign_equipment_to_agent(self, equipment_id: str, agent_id: str) -> None:
    ...
    async def _unassign_equipment_from_agent(self, equipment_id: str, agent_id: str) -> None:
    ...
    def _determine_equipment_category(self, equipment_item: EquipmentItem) -> EquipmentCategory:
        ...
        if any(word in name_lower for word in ['weapon', 'gun', 'rifle', 'pistol', 'sword', 'knife']):
