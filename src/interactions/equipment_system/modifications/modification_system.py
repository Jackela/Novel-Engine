...
from ..core.types import (
    DynamicEquipment, EquipmentModification, EquipmentStatus, EquipmentSystemConfig
)
...
class ModificationSystem:
    ...
    async def install_modification(self, equipment: DynamicEquipment,
        ...
        try:
            ...
            if self.config.compatibility_checking:
                compatibility_result = self._check_modification_compatibility(equipment, modification)
            ...
            installation_result = await self._install_modification(equipment, modification, installer)
            ...
            if "conflicts_with" in installation_result:
                equipment.incompatible_with.update(installation_result["conflicts_with"])
            
            # Restore operational status
            equipment.current_status = original_status
            
            self.logger.info(f"Modification '{modification.modification_id}' installed on equipment '{equipment.equipment_id}'")
    ...
    def get_compatible_modifications(self, equipment: DynamicEquipment) -> List[str]:
        ...
        for mod_id, template in self._modification_templates.items():
            ...
            if equipment_category in template.get("compatible_categories", []):
                ...
                has_conflict = any(
                    any(mod.modification_id == conflict for mod in equipment.modifications)
    ...
    def _apply_modification_effects(self, equipment: DynamicEquipment,
                                  modification: EquipmentModification) -> None:
