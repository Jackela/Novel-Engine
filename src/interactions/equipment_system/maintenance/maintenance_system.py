...
from ..core.types import (
    DynamicEquipment, EquipmentMaintenance, EquipmentStatus, EquipmentSystemConfig
)
...
class MaintenanceSystem:
    ...
    async def schedule_maintenance(self, equipment: DynamicEquipment,
                                 maintenance_type: str = "routine",
                                 scheduled_time: Optional[datetime] = None) -> StandardResponse:
        """
        ...
        try:
            async with self._maintenance_lock:
                ...
                heapq.heappush(self._maintenance_queue, (scheduled_time, equipment.equipment_id))
                
                # Update equipment's next maintenance due
                equipment.next_maintenance_due = scheduled_time
                
                self.logger.info(f"Maintenance scheduled for equipment '{equipment.equipment_id}' at {scheduled_time}")
    ...
    async def perform_maintenance(self, equipment: DynamicEquipment,
                                maintenance_type: str = "routine",
        ...
        try:
            async with self._maintenance_lock:
                ...
                condition_before = getattr(equipment.base_equipment, 'condition', EquipmentCondition.GOOD)
                ...
                wear_reduction = self._calculate_wear_reduction(maintenance_type, maintenance_effects)
                ...
                if self.config.auto_maintenance:
                    next_maintenance_time = maintenance_start + timedelta(hours=self.config.maintenance_interval_hours)
                    if maintenance_type == "overhaul":
                        next_maintenance_time += timedelta(hours=self.config.maintenance_interval_hours)  # Overhauls last longer
                ...
                equipment.current_status = EquipmentStatus.READY if new_condition not in ["broken", "damaged"] else EquipmentStatus.DAMAGED
                
                self.logger.info(f"Maintenance completed for equipment '{equipment.equipment_id}' - {maintenance_type} by {performed_by}")
    ...
    async def get_maintenance_due(self, equipment: DynamicEquipment) -> Dict[str, Any]:
        ...
        try:
            ...
            is_scheduled = equipment.next_maintenance_due and equipment.next_maintenance_due <= now
            ...
            if equipment.last_maintenance:
                hours_since_maintenance = (now - equipment.last_maintenance).total_seconds() / 3600
                time_based_due = hours_since_maintenance >= self.config.maintenance_interval_hours
            ...
            elif len(equipment.maintenance_history) > 0 and len(equipment.modifications) > 0:
            ...
            return {
                ...
                "next_scheduled": equipment.next_maintenance_due.isoformat() if equipment.next_maintenance_due else None,
                "days_overdue": max(0, (now - equipment.next_maintenance_due).days) if equipment.next_maintenance_due and equipment.next_maintenance_due < now else 0
            }
    ...
    async def get_overdue_maintenance(self, equipment_registry: Dict[str, DynamicEquipment]) -> List[Dict[str, Any]]:
        ...
        for equipment_id, equipment in equipment_registry.items():
            ...
            if maintenance_status.get("maintenance_due") and maintenance_status.get("days_overdue", 0) > 0:
        ...
        overdue_equipment.sort(key=lambda x: (urgency_priority.get(x["urgency"], 3), -x["days_overdue"]))
    ...
    async def _execute_maintenance_procedures(self, equipment: DynamicEquipment,
        ...
        try:
            ...
            procedures = self._maintenance_procedures.get(maintenance_type, self._maintenance_procedures["routine"])
            litanies = self._maintenance_litanies.get(maintenance_type, self._maintenance_litanies["routine"])
            ...
            for procedure in procedures:
                ...
                success_chance = 0.9 - (equipment.wear_accumulation * 0.3)  # More wear = higher failure chance
    ...
    def _calculate_condition_improvement(self, equipment: DynamicEquipment,
                                       maintenance_type: str,
                                       maintenance_effects: Dict[str, Any]) -> str:
    ...
    def _improve_equipment_condition(self, current_condition: str, improvement_level: str) -> str:
    ...
    def _calculate_wear_reduction(self, maintenance_type: str, maintenance_effects: Dict[str, Any]) -> float:
    ...
    def _apply_maintenance_performance_boost(self, equipment: DynamicEquipment, maintenance_type: str) -> None:
        ...
        for metric in equipment.performance_metrics:
            ...
            boosted_value = min(1.5, current_value + boost)  # Cap at 150% for exceptional maintenance
    ...
    def _appease_machine_spirit(self, equipment: DynamicEquipment, maintenance_type: str) -> Dict[str, Any]:
        ...
        mood_improvements = {
            "routine": {"pleased": "pleased", "content": "pleased", "agitated": "content", "angry": "agitated"},
            "repair": {"pleased": "pleased", "content": "pleased", "agitated": "pleased", "angry": "content"},
            "upgrade": {"pleased": "pleased", "content": "pleased", "agitated": "pleased", "angry": "pleased"},
            "overhaul": {"pleased": "pleased", "content": "pleased", "agitated": "pleased", "angry": "pleased"}
        }
        
        improvement_map = mood_improvements.get(maintenance_type, mood_improvements["routine"])
