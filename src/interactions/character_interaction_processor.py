...
class InteractionOutcome:
    ...
    equipment_changes: Dict[str, List[str]] = field(default_factory=dict)  # character -> equipment_ids
...
class CharacterInteractionProcessor:
    ...
    def __init__(self, database: ContextDatabase, interaction_engine: InteractionEngine,
        ...
        self.memory_significance_threshold = 0.6  # Minimum significance for memory creation
    ...
    async def process_character_interaction(self, interaction_context: InteractionContext,
        ...
        try:
            ...
            if len(characters) < 2:
                return StandardResponse(
                    ...
                    error=ErrorInfo(
                        ...
                        details={"provided_count": len(characters), "minimum_required": 2}
                    )
                )
            ...
            interaction_phases = await self._plan_interaction_phases(
                interaction_context, characters, social_environment, current_relationships
            )
            ...
            for phase in interaction_phases:
                ...
                await self._apply_phase_outcomes(phase_outcome, character_states, current_relationships)
            ...
            return StandardResponse(
                ...
                data={
                    ...
                    "relationship_changes": len([r for r in final_outcome.relationship_changes.values() if r])
                }
            )
            ...
            return StandardResponse(
                ...
                error=ErrorInfo(
                    ...
                    details={"exception": str(e), "interaction_id": interaction_context.interaction_id}
                )
            )
        ...
        try:
            ...
            return StandardResponse(
                ...
                error=ErrorInfo(
                    ...
                    details={"exception": str(e), "conflicted_characters": conflicted_characters}
                )
            )
    ...
    async def get_relationship_status(self, character_a: str, character_b: str) -> StandardResponse:
        ...
        try:
            ...
            if relationship_key in self.relationships:
                ...
                overall_sentiment = (relationship.trust_level + relationship.respect_level + 
                                   relationship.affection_level) / 3.0
                relationship_strength = (relationship.familiarity_level + 
                                       abs(overall_sentiment) + 
                                       min(relationship.interaction_count / 10.0, 1.0)) / 3.0
    ...
    async def _create_default_social_environment(self, interaction_context: InteractionContext,
        ...
        social_context = context_mapping.get(interaction_context.interaction_type, SocialContext.CASUAL)
    ...
    async def _load_relationships(self, characters: List[str]) -> Dict[Tuple[str, str], RelationshipData]:
        ...
        for i, char_a in enumerate(characters):
            for char_b in characters[i+1:]:
                ...
                if relationship_key in self.relationships:
                    relationships[relationship_key] = self.relationships[relationship_key]
                else:
    ...
    async def _plan_interaction_phases(self, interaction_context: InteractionContext,
        ...
        relationship_complexity = sum(r.interaction_count for r in relationships.values()) / max(len(relationships), 1)
        complexity_modifier = min(relationship_complexity / 10.0, 1.5)
        
        total_phases = max(2, int(base_phases * (1 + participant_modifier * 0.2 + complexity_modifier * 0.3)))
        ...
        phase_names = list(phase_templates.keys())
    ...
    async def _process_interaction_phase(self, phase: Dict[str, Any], character_states: Dict[str, CharacterState],
        ...
        outcome = InteractionOutcome(
            ...
            narrative_summary=content_result.data.get("content", "") if content_result.success else "Phase processed"
        )
        ...
        for char_a, char_b in relationships.keys():
            if char_a in phase["participants"] and char_b in phase["participants"]:
                ...
                relationship.trust_level = max(-1.0, min(1.0, relationship.trust_level + trust_change))
                relationship.respect_level = max(-1.0, min(1.0, relationship.respect_level + respect_change))
                relationship.familiarity_level = max(0.0, min(1.0, relationship.familiarity_level + familiarity_change))
        ...
        for participant in outcome.participants:
            if participant in character_states:
                ...
                if character_state.emotional_state:
                    ...
                    character_state.emotional_state.current_mood = max(1, min(10, 
                        character_state.emotional_state.current_mood + emotional_impact))
    ...
    async def _consolidate_interaction_outcomes(self, interaction_id: str, phase_outcomes: List[InteractionOutcome],
        ...
        overall_success = sum(outcome.success_level for outcome in phase_outcomes) / len(phase_outcomes)
        ...
        for i, outcome in enumerate(phase_outcomes):
            if outcome.narrative_summary:
                narrative_parts.append(f"Phase {i+1}: {outcome.narrative_summary[:100]}...")
                ...
                await conn.execute(
                    ...
                    (
                        ...
                        json.dumps({
                            ...
                            "relationship_changes": len(outcome.relationship_changes)
                        }),
                        outcome.timestamp
                    )
                )
        ...
        for participant in participants:
            try:
                ...
                episodic_memory = MemoryItem(
                    ...
                    content=outcome.narrative_summary[:500] + "..." if len(outcome.narrative_summary) > 500 else outcome.narrative_summary,
                    emotional_intensity=outcome.satisfaction_levels.get(participant, 0.5),
                    relevance_score=0.7 + (outcome.success_level * 0.3),
                    created_at=outcome.timestamp,
                    context_tags=["interaction", "character_social", outcome.interaction_id] + 
                ...
                participant_satisfaction = outcome.satisfaction_levels.get(participant, 0.5)
                if participant_satisfaction > self.memory_significance_threshold or participant_satisfaction < (1.0 - self.memory_significance_threshold):
    ...
    async def _process_equipment_interactions(self, outcome: InteractionOutcome):
        ...
        for participant in outcome.participants:
            try:
                ...
                equipment_response = await self.equipment_system.get_agent_equipment(participant)
                ...
                if equipment_response.success and equipment_response.data:
                    ...
                    for equipment_id in equipment_list:
                        ...
                        await self.equipment_system.use_equipment(
                            equipment_id, participant, usage_context, expected_duration=15
                        )
