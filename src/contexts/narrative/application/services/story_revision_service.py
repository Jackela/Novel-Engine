    ...
    def _default_terminal_arc_phase_plan(
        ...
        *,
        ...
        if phase == "aftermath":
            return {
                "summary": (
                    f"After the sacrifice, {primary_keeper} gets one quiet private beat alone while wet ink, cold floorboards, and the confirming knock hold the room in place, and {primary_keeper} already knows the resurrection failed before reaching for {vessel_label}, so the touch reads as a final goodbye rather than a rescue. "
                    f"A witness cuts in with one blunt question, and {primary_keeper} chooses the city's debt over private grief while {visible_witness} steadies the ledger and lets the next living order land. "
                    f"{confirmation_trigger} then proves the old life is gone for good."
                ),
                "objective": (
                    f"{primary_keeper} must move from grief to duty through one failed action, let a still beat land before command returns, keep {vessel_label} fully passive, and frame any late movement as a reflex rather than instinct or recovery. "
                ...
                "hook": f"The accepted cost points away from private mourning and toward the buried rule tied to {motif_anchor}.",
                "focus_character": primary_keeper,
        ...
        if phase == "rule_revelation":
            return {
                "summary": (
                    f"An early attempt to stabilize the aftermath fails in a room smelling of wet ink and cold stone. Then {primary_keeper} and {supporting_witness or living_anchor} uncover concrete evidence tied to {motif_anchor}, and a new echo of the lost rule cracks through the wall, forcing the living to pin it back before it can spread. "
                    f"The narration keeps {vessel_label} to one brief trained twitch and no more, then lets it go still without pretending a mind has returned. "
                    f"{supporting_witness or living_anchor} reads the twitch as memorial proof rather than a reply, and then breaks visibly at the ledger edge so the room understands the loss as grief instead of a diagram."
                ),
                "objective": "Make the world rule legible through observable evidence, sequence, and consequence instead of explanation alone, show why the first response fails, keep any vessel contact as witness-carried proof rather than interior life or restored consciousness, and let one sharp look or spoken line interrupt the motion so the moment stays human. Spell out the twitch as last trained motion or reflex, never as conscious return, and keep the witness reaction visible enough that the scene does not become purely technical. The witness should visibly break or weep when the proof lands so the technical evidence still reads as human loss. Let a new echo or anomaly force the living to act so the phase cannot read like a manual.",
                "hook": "Once the living understand the rule, the confession has to leave the room and survive public pressure.",
                "focus_character": primary_keeper,
        ...
        if phase == "public_reckoning":
            return {
                "summary": (
                    f"The first half of Chapter 19 tracks a fresh surge from the broken seal before the explanation ends, while wind lifts dust through the square and the crowd keeps shifting around the still vessel: one beat of silence opens while {visible_witness} tastes metal and moves first and {second_witness} closes the gap. The crowd feels the break as a real loss, not a procedural delay. "
                    ...
                    f"{vessel_label} gives one brief reflexive shudder from the surge and then stays still while the burned names darken on the ledger. "
                ...
                "objective": "Break the reckoning into distinct beats with explicit temporal markers: one crisis beat for the surge and line break, then one ritual beat for naming and sealing, with a clear pause between them. Give one witness a bodily reaction before the first move, leave a beat of silence between the surge and the naming so the witness action can land, and keep any written mark or knocked cadence as a brief reflexive motion rather than a deliberate choice. Make the supporting witness's grief visible so the scene feels human, not purely procedural, and keep the active choice with the living keeper rather than the vessel.",
                "hook": "A fresh black seal on the ledger gives the conflict a visible shape before the city can confess what it owes in daylight.",
                "focus_character": primary_keeper,
        ...
        return {
            "summary": (
                ...
                f"By dawn, after the walk to the ledger room and the last bell in the hall, {primary_keeper} writes the final entry once before passing the record forward, and {public_cost_clause} while {vessel_label} stays silent and offers no answering voice. "
                f"Before the page dries, {primary_keeper} sees a blank page take a new name without any witness speaking it aloud, and the room has to admit the leak before the ink dries. {closure_sequence}."
            ),
            "objective": (
                "Split the ending into three separate scenes: private closure, public confession in the square, and a final ledger or record-room interaction. Start the private scene with a quiet inward reflection on the vessel's silence, let the same scratch of ink drying recur across the three beats, and place explicit time markers such as By dusk, By night, and By dawn between the beats so they land separately. Let the private walk, the crowd shift, and the ledger-room door create movement so the time markers feel earned rather than mechanical. Then let one witness name a concrete remembered detail from the earlier life that is now gone, keep the keeper's recognition of the loss on the page, and make the silence of the vessel explicit without compressing the beats together. Give the supporting witness one visible emotional beat in the public scene so the ending carries grief as well as procedure. Split the public scene with one concrete act of renewal so the ending carries grief and a visible sign that the new order can still serve the living. Make the living keeper perform one small but real action in the ledger scene so the closing choice still belongs to the story's central agent. End on a concrete anomaly, such as a name appearing on a blank page without a witness, so the hook is visible instead of abstract."
            ),
            "hook": "At dawn, the primary_keeper sees a blank page take a name without a witness, and the room has to admit the rewritten rule still leaks into the living record.",
            "focus_character": primary_keeper,
    ...
    def _enforce_terminal_arc_phase_clarity(
        ...
        *,
        ...
        phase_clarity_sentences = {
            phases["aftermath"]: {
                "summary": (
                    f"{primary_keeper} fails once in front of the line, {public_witness or supporting_witness or primary_keeper} catches the break before it spreads, and a fresh echo from the broken rule forces the room to react while {vessel_label} stays heavy, silent, and unchanged."
                ),
                "objective": (
                    f"Make the vessel's stillness and physical weight concrete while {primary_keeper} moves from visible grief into command only after one failed wake attempt, and let the loss register as a bodily reaction before command resumes instead of as a temporary pause. Add a new disturbance that forces the line to move instead of merely observe."
                ),
            },
            phases["rule_revelation"]: {
                "summary": (
                    f"Before the seal closes, {supporting_witness or primary_keeper} tests the evidence in public, one cold touch or token at {vessel_label} stays answerless, and the room learns the rule through proof instead of explanation while a fresh echo presses back from the wall."
                ),
                "objective": (
                    f"Give the phase one tangible gesture between {supporting_witness or primary_keeper} and {vessel_label}, then show why that gesture cannot restore consciousness and why the evidence has to move outward. Let the breaking event feel immediate enough that the living have to answer it."
                ),
            },
            phases["public_reckoning"]: {
                "summary": (
                    f"The first surge hits before the keeper can finish the explanation. First, {public_witness or primary_keeper} moves. Then, {supporting_witness or primary_keeper} closes the gap. Only after that does the line name the mechanism while {vessel_label} gives a single reflexive shudder from the surge and then stays still. "
                    f"When the burned names are spoken aloud, the ledger darkens again so the rule can be read as visible evidence."
                ),
                "objective": (
                    "Keep the public reckoning sequential rather than compressed by separating threat, near-break tension, witness intervention, mechanism, and resolution into distinct beats with clear transitions. Use explicit markers such as first, then, and only after that so the cause-and-effect stays obvious. When burned names are spoken aloud, the ledger should visibly darken or re-mark so the rule is carried by evidence, and any late written mark or physical cadence must read as a brief reflexive motion rather than a restored mind or a fresh decision."
                ),
            },
            phases["closure"]: {
                "summary": (
                    f"Closure lands in three separate scenes: private closure first, public square second, and ledger interaction last. A living witness names one remembered habit aloud in the private scene, {vessel_label} gives no answering voice, and {primary_keeper} turns that silence into the public form the survivors now have to keep while {frame.public_cost_example}. A blank page then takes a new name without any witness speaking it aloud, so the ending closes on a visible leak rather than a quiet hint."
                ),
                "objective": (
                    "Contrast the visible bodily reaction with the keeper's inward recognition that no returning voice or interior answer will come back, so the ending lands as three separate scenes rather than hidden recovery and the public cost stays concrete. End on a visible anomaly that a reader can spot immediately instead of a subtle hint."
                ),
            },
        }
        for chapter in ctx.story.chapters:
        ...
        *,
        ...
        phase_scene_additions = {
            chapter_numbers["aftermath"]: (
                f"In the first private break, {frame.primary_keeper} gets one quiet beat alone while wet ink, cold floorboards, and the same confirming knock hold the room in place, then a fresh echo from the broken rule forces the room to react before {frame.primary_keeper} can move again. Only after that pause does {frame.primary_keeper} try once to wake {frame.vessel_label} and get only cold weight and silence back. A witness cuts in with one blunt question, and {frame.primary_keeper} chooses the city's debt over private grief before the room settles."
            ),
            chapter_numbers["discovery"]: (
                f"Testing the proof at {anchor_label}, {frame.supporting_witness or frame.primary_keeper} finds a visible consequence instead of an explanation, and {frame.vessel_label} stays unchanged while a fresh echo presses back from the wall so the room learns the rule through evidence. The keeper and witness exchange one sharp look before the proof is touched, and the witness's reaction keeps the scene human instead of technical."
            ),
            chapter_numbers["attempt"]: (
                f"At {civic_target}, the first surge hits before the explanation ends and the chapter splits into two beats. First, {frame.public_witness} moves while {heroic_witness or second_witness} drags the line closed and the crowd feels the break while dust and wind keep moving around the still vessel. Then, after a clear pause, {frame.primary_keeper} says the needed name out loud and {frame.supporting_witness or frame.public_witness} reacts to the chalk mark with a visible flinch before the ritual seal lands. "
                f"{frame.vessel_label} gives one brief reflexive shudder from the surge and then stays still while the burned names darken on the ledger. One living hand stays on the record long enough for the new order to look human instead of merely ritual."
            ),
            chapter_numbers["resolution"]: (
                f"As noon gives way to dusk and then to night, the ending moves through three scenes rather than headers, and the same scratch of ink drying threads through each one. In the private scene, a witness speaks {concrete_memory} into the silence with no answer from {frame.vessel_label} and pauses on the loss before moving on; in the public square, the city adopts the rule in daylight and one ordinary task resumes to show the new order can still serve the living; in the ledger scene, {frame.primary_keeper} writes the final entry once before the record passes forward, and {frame.public_cost_example}. Before the page dries, {frame.primary_keeper} sees a blank line take a new name without any witness speaking it aloud, and the room has to admit the leak instead of treating it like a whisper."
            ),
        }
