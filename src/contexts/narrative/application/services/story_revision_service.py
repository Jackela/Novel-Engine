    ...
    def _default_terminal_arc_phase_plan(
        ...
        *,
        ...
        if phase == "aftermath":
            return {
                "summary": (
                    f"After the sacrifice, {primary_keeper} gets one quiet private beat alone while rain taps the eaves, the rail stays cold in the palm, and the confirming knock holds the room in place, and {primary_keeper} already knows the return failed before reaching for {vessel_label}, so the touch reads as a final goodbye rather than a rescue. "
                    f"A remembered instruction from {protagonist} still shapes the next choice as memory-threaded residue, and {visible_witness} steadies the ledger while the line chooses the city's debt over private grief. "
                    f"{confirmation_trigger} then proves the old life is gone for good."
                ),
                "objective": (
                    f"{primary_keeper} must move from grief to duty through one failed action, let a still beat land before command returns, keep {vessel_label} fully passive, and frame any late movement as a conditioned reflex or residue rather than instinct or recovery. "
                    f"Let the keeper's next choice remain visibly shaped by a remembered instruction from {protagonist} so the earlier life stays present as influence, not as a returned voice. "
        ...
        if phase == "rule_revelation":
            return {
                "summary": (
                    f"An early attempt to stabilize the aftermath fails under lamp heat and chalk dust. Then {primary_keeper} and {supporting_witness or living_anchor} uncover concrete evidence tied to {motif_anchor}, and a new echo of the lost rule cracks through the wall, forcing the living to pin it back before it can spread. "
                    f"The narration keeps {vessel_label} to one brief conditioned reflex and no more, then lets it go still without pretending a mind has returned. "
                ...
                "objective": "Make the world rule legible through observable evidence, sequence, and consequence instead of explanation alone, show why the first response fails, keep any vessel contact as witness-carried proof rather than interior life or restored consciousness, and let one sharp look or spoken line interrupt the motion so the moment stays human. Spell out the motion as a conditioned reflex or last trained residue, never as conscious return, and keep the witness reaction visible enough that the scene does not become purely technical. The witness should visibly break or weep when the proof lands so the technical evidence still reads as human loss. Let a new echo or anomaly force the living to act so the phase cannot read like a manual.",
        ...
        if phase == "public_reckoning":
            return {
                "summary": (
                    f"The first half of Chapter 19 tracks a fresh surge from the broken seal before the explanation ends, while wind lifts chalk dust through the square and the crowd keeps shifting around the still vessel: one beat of silence opens while {visible_witness} tastes metal and moves first and {second_witness} closes the gap. The crowd feels the break as a real loss, not a procedural delay. "
                    ...
                    f"{vessel_label} gives one brief conditioned reflex from the surge and then stays inert while the burned names darken on the ledger and the lantern glass cracks once. "
                ...
                "objective": "Break the reckoning into distinct beats with explicit temporal markers: one crisis beat for the surge and line break, then one ritual beat for naming and sealing, with a clear pause between them. Give one witness a bodily reaction before the first move, leave a beat of silence between the surge and the naming so the witness action can land, and keep any written mark or knocked cadence as a conditioned reflex rather than a deliberate choice. Make the supporting witness's grief visible so the scene feels human, not purely procedural, and keep the active choice with the living keeper rather than the vessel. Preserve the protagonist's earlier instruction only as memory-threaded influence, not as a returned voice.",
        ...
        return {
            "summary": (
                f"By dusk, after the room has settled and the procession turns toward the side hall, {primary_keeper} holds the silence of {vessel_label} in mind while one living witness names an ordinary detail from the life now gone, pauses on the loss with a visible reaction, and lets that silence stand. "
                ...
                f"By dawn, after the walk to the ledger room and the last bell in the hall, {primary_keeper} writes the final entry once before passing the record forward, and {public_cost_clause} while {vessel_label} stays silent and offers no answering voice except a conditioned reflex. "
        ...
        *,
        ...
        phase_scene_additions = {
            chapter_numbers["aftermath"]: (
                f"In the first private break, {frame.primary_keeper} gets one quiet beat alone while rain taps the eaves, rough wood stays under the palm, and the same confirming knock holds the room in place, then a fresh echo from the broken rule forces the room to react before {frame.primary_keeper} can move again. Only after that pause does {frame.primary_keeper} try once to wake {frame.vessel_label} and get only cold weight and silence back. A remembered instruction from {frame.protagonist} still shapes the next choice as memory-threaded residue, a witness cuts in with one blunt question, and {frame.primary_keeper} chooses the city's debt over private grief before the room settles."
            ),
            chapter_numbers["discovery"]: (
                f"Testing the proof at {anchor_label}, {frame.supporting_witness or frame.primary_keeper} finds a visible consequence instead of an explanation, and {frame.vessel_label} stays unchanged while chalk dust and lamp heat press against the wall so the room learns the rule through evidence. The keeper and witness exchange one sharp look before the proof is touched, and the witness's reaction keeps the scene human instead of technical."
            ),
            chapter_numbers["attempt"]: (
                f"At {civic_target}, the first surge hits before the explanation ends and the chapter splits into two beats. First, {frame.public_witness} moves while {heroic_witness or second_witness} drags the line closed and the crowd feels the break while boots and wind keep moving around the still vessel. Then, after a clear pause, {frame.primary_keeper} says the needed name out loud and {frame.supporting_witness or frame.public_witness} reacts to the chalk mark with a visible flinch before the ritual seal lands. "
                f"{frame.vessel_label} gives one brief conditioned reflex from the surge and then stays still while the burned names darken on the ledger and the lantern glass cracks once. One living hand stays on the record long enough for the new order to look human instead of merely ritual."
            ),
            chapter_numbers["resolution"]: (
                f"As noon gives way to dusk and then to night, the ending moves through three scenes rather than headers, and the same scratch of ink drying threads through each one. In the private scene, a witness speaks {concrete_memory} into the silence with no answer from {frame.vessel_label} and pauses on the loss before moving on; in the public square, the city adopts the rule in daylight and one ordinary task resumes to show the new order can still serve the living; in the ledger scene, {frame.primary_keeper} writes the final entry once before the record passes forward, and {frame.public_cost_example}. Before the page dries, {frame.primary_keeper} sees a blank line take a new name without any witness speaking it aloud, the lamp flame gutters, and the room has to admit the leak instead of treating it like a whisper."
            ),
        }
