...
class StoryRevisionService:
    ...
    def _extract_terminal_arc_rewrite_plan(
        ...
        *,
        ...
        for item in raw_chapters:
            ...
            focus_character = self._normalize_terminal_actor_label(
                str(item.get("focus_character", "")).strip(),
                cast_names,
                fallback=protagonist if chapter_number == sacrifice_number else primary_keeper,
            )
            ...
            relationship_target = self._normalize_vessel_target_label(
                str(item.get("relationship_target", "")).strip(),
                cast_names,
            )
            relationship_target = self._normalize_terminal_actor_label(
                relationship_target,
                cast_names,
                fallback=planned["relationship_target"],
                extra_exclusions={focus_character} if focus_character else None,
            )
            if not relationship_target or relationship_target == focus_character:
                relationship_target = planned["relationship_target"]
    ...
    def _default_terminal_arc_phase_plan(
        ...
        *,
        ...
        if sacrifice_witness == protagonist:
            sacrifice_witness = living_anchor or continuity_anchor or supporting_witness or public_witness
        ...
        if not visible_witness:
            visible_witness = living_anchor or continuity_anchor or supporting_witness or public_witness or protagonist
        if second_witness == visible_witness:
            for candidate in (
                supporting_witness,
                public_witness,
        ...
        visible_witness = display_terminal_name(
            ...
            public_witness,
            protagonist,
        ) or visible_witness or living_anchor or protagonist
        ...
        if not handoff_recipient:
            handoff_recipient = sacrifice_witness or visible_witness or second_witness or living_anchor
        living_anchor = display_terminal_name(
            living_anchor,
            primary_keeper,
            continuity_anchor,
            ...
            public_witness,
            protagonist,
        ) or protagonist or continuity_anchor or supporting_witness or public_witness
        ...
        sacrifice_witness = display_terminal_name(
            ...
            public_witness,
            protagonist,
        ) or protagonist or living_anchor or continuity_anchor or supporting_witness or public_witness
        visible_witness = display_terminal_name(
            visible_witness,
            supporting_witness,
            continuity_anchor,
        ...
        second_witness = display_terminal_name(
            ...
            primary_keeper,
            protagonist,
        ) or protagonist or visible_witness or living_anchor or continuity_anchor or supporting_witness or public_witness
        ...
        closure_status = (
            f"{visible_witness} and {closure_partner} keep the living record after the seal lands"
            if closure_partner
        ...
        if phase == "sacrifice":
            return {
                ...
                "relationship_status": f"{protagonist} and {handoff_recipient} complete the final handoff before the irreversible cost",
            }
        if phase == "aftermath":
            return {
                "summary": (
                ...
                "relationship_status": f"{primary_keeper} and {visible_witness} move from shock to shared duty after the shell stays cold",
            }
        if phase == "rule_revelation":
            return {
                ...
                "relationship_status": f"{primary_keeper} and {supporting_witness or living_anchor} turn the seal's hard answer into proof",
            }
        if phase == "public_reckoning":
            return {
                ...
                "relationship_status": f"{primary_keeper} and {visible_witness} carry the public confession under pressure",
            }
        return {
            "summary": (
            ...
            "objective": (
                f"Private grief, public confession, and the final ledger handoff stay distinct: {visible_witness} names a remembered detail from the earlier life, the city carries the rule in daylight without collapsing into one explanation block, and {primary_keeper} completes the closing entry while the room watches a visible anomaly instead of a metaphor. The vessel remains a memorial object, stays inert with no answering movement, the living answer the cost in public, and the ending keeps its beats separate."
            ),
            "hook": "At dawn, the blank page takes a name in fresh black ink, the lamp flame gutters, and the colder room proves the rewritten rule is permanent instead of ghostly.",
            "focus_character": primary_keeper,
            "relationship_target": self._resolve_terminal_relationship_target(
                primary_keeper,
                visible_witness,
            ...
            "relationship_status": closure_status,
        }
        ...
        *,
        ...
        if "memory" in combined_rules or "name" in combined_rules:
            return (
                ...
                "the corrected count stays visible on the board as proof"
            )
        ...
        *,
        ...
        chapter_additions = {
            ...
            17: (
                ...
                f"Before dawn, {primary_keeper} wipes ash across the ledger margin, reads the next erased name aloud, and orders the clerks to answer it back until the square can keep the register open without them."
            ),
        }
        ...
        for phase, chapter_number in chapter_numbers.items():
            ...
            if outline_chapter is not None:
                ...
                if phase == "rule_revelation":
                    ...
                    if outline_chapter.promise != (
                        "The public record carries the working rule into daylight after one failed private attempt leaves a visible mark on the ledger and the vessel stays inert with no answering motion."
                    ):
                        outline_chapter.promise = (
                            "The public record carries the working rule into daylight after one failed private attempt leaves a visible mark on the ledger and the vessel stays inert with no answering motion."
                        )
                    ...
                    if outline_chapter.promise != (
                        "The ending lands as private closure, public confession, and lasting aftermath while the vessel stays inert with no answering motion."
                    ):
                        outline_chapter.promise = (
                            "The ending lands as private closure, public confession, and lasting aftermath while the vessel stays inert with no answering motion."
                        )
                        changed = True
                    if outline_chapter.promised_payoff != (
                        f"The living absorb the public cost through {frame.public_cost_example}, while the vessel stays inert with no answering movement and the new order becomes ordinary fact."
                    ):
                        outline_chapter.promised_payoff = (
                            f"The living absorb the public cost through {frame.public_cost_example}, while the vessel stays inert with no answering movement and the new order becomes ordinary fact."
                        )
        ...
        phase_clarity_sentences = {
            phases["aftermath"]: {
                "summary": (
                    ...
                    f"{public_witness or supporting_witness or primary_keeper} asks whether {primary_keeper} can carry the cost, and {primary_keeper} answers before the room by taking command."
                ),
                "objective": (
                    f"{primary_keeper} moves from visible grief into command only after one failed wake attempt, and the loss registers as a bodily reaction before command resumes. A new disturbance forces the line to move instead of merely observe, and one direct question-answer exchange makes the shift from shock to shared duty explicit."
                ),
            },
            phases["rule_revelation"]: {
                "summary": (
                    ...
                    "The forged authority mark tied to the lie peels off the page in plain view so the antagonist's hold ends as evidence, not rumor."
                ),
                "objective": (
            ...
            phases["public_reckoning"]: {
                "summary": (
                    f"The first surge hits before the keeper can finish the explanation. First, {public_witness or primary_keeper} moves while rain drives across the square and a banner snaps overhead. Then, {supporting_witness or primary_keeper} closes the gap. "
                    f"Before naming the mechanism, {primary_keeper} repeats one concrete line from {protagonist}'s final choice so the square hears whose cost opened the page. {antagonist_label} stands under guard while the line names the mechanism and {vessel_label} shows only ink-stiff fingers and a fixed jawline at the edge of the square. "
                    f"When the burned names are spoken aloud, the ledger darkens again and {vessel_label} never initiates movement."
                ),
                "objective": (
                    f"The public reckoning moves in distinct beats: threat, near-break tension, witness intervention, mechanism, and resolution. Burned names darken the ledger when they are spoken aloud, {antagonist_label} faces the square, and the vessel remains a memorial object that never acts independently while the living answer the cost."
                ),
            },
            phases["closure"]: {
                "summary": (
                ...
                "objective": (
                    "Three distinct scenes carry the ending: private closure, public confession, and a ledger handoff. The living witness names a remembered detail, the vessel stays inert with no answering movement as a memorial object, the public square keeps the cost concrete, and the final ledger scene ends on a visible anomaly that triggers a small physical reaction."
                ),
            },
        }
        for chapter in ctx.story.chapters:
    ...
    def _normalize_terminal_actor_label(
        self,
        candidate: str,
        cast_names: set[str],
        *,
        ...
        exclusions = {
            ...
            for item in (extra_exclusions or set())
        ...
        if (
            ...
            or self._is_generic_terminal_role_title(normalized)
            or self._is_symbolic_late_arc_candidate(normalized)
            or normalized in exclusions
        ):
            normalized = ""
        if normalized:
            return normalized

        fallback_normalized = self._canonicalize_character_name(
            " ".join(str(fallback).split()).strip(),
            cast_names,
        )
        if (
            fallback_normalized
            and not self._is_generic_terminal_placeholder(fallback_normalized)
            and not self._is_generic_terminal_role_title(fallback_normalized)
            and not self._is_symbolic_late_arc_candidate(fallback_normalized)
            and fallback_normalized not in exclusions
        ):
            return fallback_normalized
        return ""
    ...
    def _late_arc_sequence(
        ...
        *,
        ...
        sacrifice_partner = self._resolve_terminal_relationship_target(
            frame.protagonist,
            frame.primary_keeper,
            frame.supporting_witness,
            frame.public_witness,
            frame.continuity_anchor,
        ) or frame.primary_keeper
        aftermath_partner = self._resolve_terminal_relationship_target(
            frame.primary_keeper,
            frame.supporting_witness,
            frame.public_witness,
            frame.continuity_anchor,
        ) or frame.public_witness or frame.supporting_witness
        rule_partner = self._resolve_terminal_relationship_target(
            frame.primary_keeper,
            frame.supporting_witness,
            frame.public_witness,
            frame.continuity_anchor,
        ) or frame.supporting_witness or frame.public_witness
        public_partner = self._resolve_terminal_relationship_target(
            frame.primary_keeper,
            frame.public_witness,
            frame.supporting_witness,
            frame.continuity_anchor,
        ) or frame.public_witness or frame.supporting_witness
        closure_lead = frame.public_witness or frame.supporting_witness or frame.primary_keeper
        closure_partner = self._resolve_terminal_relationship_target(
            closure_lead,
            frame.supporting_witness,
            frame.continuity_anchor,
        ...
        relationship_status_by_phase = {
            "sacrifice": (
                f"{frame.protagonist} and {sacrifice_partner} complete the final handoff before the irreversible cost"
                if sacrifice_partner
                else f"{frame.protagonist} completes the final handoff before the irreversible cost"
            ),
            "aftermath": (
                f"{frame.primary_keeper} and {aftermath_partner} move from shock to shared duty after the shell stays cold"
                if aftermath_partner
                else f"{frame.primary_keeper} moves from shock to duty after the shell stays cold"
            ),
            "rule_revelation": (
                f"{frame.primary_keeper} and {rule_partner} turn the seal's hard answer into proof"
                if rule_partner
                else f"{frame.primary_keeper} turns the seal's hard answer into proof"
            ),
            "public_reckoning": (
                f"{frame.primary_keeper} and {public_partner} carry the public confession under pressure"
                if public_partner
                else f"{frame.primary_keeper} carries the public confession under pressure"
            ),
            "closure": (
                f"{closure_lead} and {closure_partner} keep the living record after the seal lands"
                if closure_partner
        ...
        *,
        ...
        phase_scene_additions = {
            chapter_numbers["aftermath"]: (
                f"In the first private break, the marked token reaches {frame.primary_keeper} and the first wake attempt fails at once when the seal answers and drives the hand back. Only after that pause does {frame.primary_keeper} touch {frame.vessel_label} once, get silence, and stop. "
                f"{frame.public_witness} asks a blunt readiness question out loud, {frame.primary_keeper} answers before the room by choosing duty over private grief, and then sets a marked token down beside the ledger."
            ),
            ...
            chapter_numbers["attempt"]: (
                f"At {civic_target}, the first surge hits before the explanation ends and the chapter splits into two beats. A pulse of fresh black ink marks the next choice without waking the vessel. {frame.public_witness} moves while {heroic_witness or second_witness} takes the still hand of {frame.vessel_label} and presses it to the ledger table, and {antagonist_label} stands under guard while boots and wind keep moving around the still vessel and a loose banner snaps in the gale. Before the confession phrase lands, {frame.primary_keeper} repeats one concrete memory from {frame.protagonist}'s final choice and then says the needed name out loud, and {frame.supporting_witness or frame.public_witness} reacts to the seal scar with a visible flinch before the ritual seal lands. "
                f"{frame.vessel_label} shows only ink-stiff fingers and a fixed jawline, never moving unless living hands place it, while the burned names darken on the ledger table and the lantern glass cracks once."
            ),
            chapter_numbers["resolution"]: (
                f"As noon gives way to dusk and then to night, the ending passes through private grief, public confession, and a final ledger handoff. In the private scene, {frame.public_witness} speaks {concrete_memory} into silence with no answer from {frame.vessel_label} and pauses on the loss before moving on; in the public square, the city adopts the rule in daylight and one ordinary task resumes; in the ledger scene, {frame.primary_keeper} writes the final entry once before the record passes forward, and {frame.public_cost_example}. Before the page dries, {frame.primary_keeper} sees a blank line take a new name in fresh black ink, the lamp gutters, the room chills around the ledger, and the window shudders in the draft."
            ),
        }
