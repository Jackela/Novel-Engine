    ...
    def __post_init__(self):
        ...
        if not has_circle and not has_bounds:
            raise QueryValidationException(
                "Either circular (center_x, center_y, radius) or rectangular bounds must be specified"
            )

        if has_circle and has_bounds:
            raise QueryValidationException(
                "Cannot specify both circular and rectangular query parameters"
            )
...
class GetWorldSliceQueryHandler:
    ...
    async def handle(self, query: GetWorldSlice) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as session:
                ...
                self.logger.debug(
                    f"GetWorldSlice query completed in {response['query_time_ms']:.1f}ms"
                )
                return response
    ...
    def _is_query_within_world_bounds(
        self, query: GetWorldSlice, read_model: WorldSliceReadModel
    ) -> bool:
        """
        ...
        if query.is_circular_query():
            ...
            return not (
                circle_max_x < read_model.min_x
                or circle_min_x > read_model.max_x
                or circle_max_y < read_model.min_y
                or circle_min_y > read_model.max_y
            )
        else:
            ...
            return not (
                ...
                or query_min_x > read_model.max_x
                or query_max_y < read_model.min_y
                or query_min_y > read_model.max_y
            )
...
class SearchWorldsQueryHandler:
    ...
    async def handle(self, query: SearchWorlds) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as session:
                # Build search query
                search_filter = WorldSliceReadModel.searchable_content.ilike(
                    f"%{query.search_term}%"
                )
