...
class PostgreSQLEventRepository(EventRepository):
    ...
    async def save(self, event: HistoryEvent) -> HistoryEvent:
        ...
        async with self._pool.acquire() as conn:
            await conn.execute(
                ...
                event.causes,
                event.consequences,
                [UUID(eid) for eid in event.preceding_event_ids] if event.preceding_event_ids else [],
                [UUID(eid) for eid in event.following_event_ids] if event.following_event_ids else [],
                [UUID(eid) for eid in event.related_event_ids] if event.related_event_ids else [],
                event.is_secret,
                event.sources,
                event.narrative_importance,
                event.impact_scope.value if event.impact_scope else None,
                [UUID(fid) for fid in event.affected_faction_ids] if event.affected_faction_ids else [],
                [UUID(lid) for lid in event.affected_location_ids] if event.affected_location_ids else [],
    ...
    async def delete(self, event_id: str) -> bool:
        ...
        async with self._pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM history_events WHERE id = $1",
                UUID(event_id)
            )
    ...
    def _row_to_entity(self, row: asyncpg.Record) -> HistoryEvent:
        ...
        location_ids = [str(lid) for lid in row["location_ids"]] if row["location_ids"] else []
        faction_ids = [str(fid) for fid in row["faction_ids"]] if row["faction_ids"] else []
        preceding_event_ids = [str(eid) for eid in row["preceding_event_ids"]] if row["preceding_event_ids"] else []
        following_event_ids = [str(eid) for eid in row["following_event_ids"]] if row["following_event_ids"] else []
        related_event_ids = [str(eid) for eid in row["related_event_ids"]] if row["related_event_ids"] else []
        affected_faction_ids = [str(fid) for fid in row["affected_faction_ids"]] if row["affected_faction_ids"] else []
        affected_location_ids = [str(lid) for lid in row["affected_location_ids"]] if row["affected_location_ids"] else []
        ...
        return HistoryEvent(
            ...
            affected_location_ids=affected_location_ids if affected_location_ids else None,
