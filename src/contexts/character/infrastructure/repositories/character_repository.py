...
class SQLAlchemyCharacterRepository(ICharacterRepository):
        ...
        try:
            with self.session_factory() as session:
                ...
                if existing_orm:
                    ...
                    existing_orm.updated_at = datetime.now(UTC)
                else:
    ...
    async def exists(self, character_id: CharacterID) -> bool:
        ...
        try:
            ...
            self.logger.error(
                "character_existence_check_failed",
                character_id=str(character_id),
                error=str(e),
            )
    ...
    async def find_by_race(self, race: CharacterRace) -> List[Character]:
        ...
        try:
            ...
            self.logger.error(
                "find_characters_by_race_failed", race=str(race), error=str(e)
            )
