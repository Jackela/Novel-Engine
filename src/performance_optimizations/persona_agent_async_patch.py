...
from .async_llm_integration import get_async_llm_client, call_llm_async_wrapper
...
class PersonaAgentAsyncPatch:
    ...
    def patch_persona_agent_llm_calls(persona_agent_instance) -> None:
        ...
        """
        logger.info(f"Applying async LLM patch to agent {persona_agent_instance.agent_id}")
        
        # Store original method for fallback
        original_call_llm = getattr(persona_agent_instance, '_call_llm', None)
        if not original_call_llm:
            logger.warning(f"Agent {persona_agent_instance.agent_id} does not have _call_llm method")
            return
        ...
        def async_optimized_call_llm(prompt: str) -> str:
            ...
            try:
                ...
                character_context = {
                    'personality_traits': getattr(persona_agent_instance, 'personality_traits', {}),
                    'decision_weights': getattr(persona_agent_instance, 'decision_weights', {}),
                    'recent_events': getattr(persona_agent_instance, 'subjective_worldview', {}).get('recent_events', []),
                    'faction': getattr(persona_agent_instance, 'character_data', {}).get('faction', '')
                }
                
                # Use async wrapper to get optimized response
                return call_llm_async_wrapper(
                    persona_agent_instance.agent_id, 
                    prompt, 
                    character_context
                )
                
            except Exception as e:
                logger.error(f"Agent {persona_agent_instance.agent_id} async LLM patch error: {e}")
                # Fallback to original method
                try:
                    return original_call_llm(prompt)
                except Exception as fallback_error:
                    logger.error(f"Agent {persona_agent_instance.agent_id} fallback also failed: {fallback_error}")
        ...
        logger.info(f"Agent {persona_agent_instance.agent_id} async LLM patch applied successfully")
    ...
    def patch_enhanced_decision_making(persona_agent_instance) -> None:
        ...
        """
        logger.info(f"Applying enhanced decision making patch to agent {persona_agent_instance.agent_id}")
        
        original_method = getattr(persona_agent_instance, '_llm_enhanced_decision_making', None)
        if not original_method:
            logger.warning(f"Agent {persona_agent_instance.agent_id} does not have _llm_enhanced_decision_making method")
            return
        
        def optimized_enhanced_decision_making(world_state_update, situation_assessment, available_actions):
            """
            ...
            try:
                ...
                if not available_actions:
                    logger.debug(f"Agent {persona_agent_instance.agent_id} no available actions - skipping LLM call")
                    return None
                ...
                if situation_key in persona_agent_instance._decision_cache:
                    logger.debug(f"Agent {persona_agent_instance.agent_id} using cached decision for similar situation")
                    cached_decision = persona_agent_instance._decision_cache[situation_key]
                    # Validate cached decision is still applicable
                    if any(action.get('action_type') == cached_decision.action_type for action in available_actions):
                ...
                if start_time > 0:
                    ...
                    logger.debug(f"Agent {persona_agent_instance.agent_id} decision making took {duration:.3f}s")
                ...
                logger.error(f"Agent {persona_agent_instance.agent_id} enhanced decision making error: {e}")
                # Fallback to basic algorithmic decision
                return persona_agent_instance._make_algorithmic_decision(available_actions)
        
        # Replace the method
        setattr(persona_agent_instance, '_llm_enhanced_decision_making', optimized_enhanced_decision_making)
        
        logger.info(f"Agent {persona_agent_instance.agent_id} enhanced decision making patch applied")
    ...
    def add_performance_monitoring(persona_agent_instance) -> None:
        ...
        def get_performance_stats():
            ...
            if stats['decision_count'] > 0:
                stats['average_decision_time'] = stats['total_decision_time'] / stats['decision_count']
            return stats
        
        setattr(persona_agent_instance, 'get_performance_stats', get_performance_stats)
        
        logger.debug(f"Agent {persona_agent_instance.agent_id} performance monitoring added")
    ...
    def apply_full_performance_patch(persona_agent_instance) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Full performance patch applied to agent {persona_agent_instance.agent_id}")
            
        except Exception as e:
            logger.error(f"Error applying performance patch to agent {persona_agent_instance.agent_id}: {e}")
...
def apply_async_optimization_to_agent_collection(agents: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    success_rate = (results['successfully_patched'] / max(1, results['total_agents'])) * 100
    logger.info(f"Agent optimization complete: {success_rate:.1f}% success rate ({results['successfully_patched']}/{results['total_agents']})")
