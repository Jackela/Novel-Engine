...
class OptimizedWorldStateTracker:
    ...
    def __init__(self, max_history_turns: int = 50, cache_size: int = 1000):
        ...
        logger.info(f"OptimizedWorldStateTracker initialized with {max_history_turns} turn history")
    ...
    def get_recent_discoveries_for_agent(self, agent_id: str, turns_back: int = 3) -> List[str]:
        ...
        discoveries = []
        current_turn = max(self.agent_discoveries.keys()) if self.agent_discoveries else 0
        
        for turn in range(max(1, current_turn - turns_back), current_turn + 1):
            if turn in self.agent_discoveries and agent_id in self.agent_discoveries[turn]:
    ...
    def get_world_state_feedback(self, requesting_agent_id: str, current_turn: int) -> Dict[str, Any]:
        """
            ...
            if turn in self.agent_discoveries:
                for agent_id, discoveries in self.agent_discoveries[turn].items():
                    if agent_id != requesting_agent_id:
                        ...
                        feedback['other_agent_discoveries'][agent_id].extend(discoveries)
        
        # Use clue index for O(1) shared clue detection
        requesting_agent_discoveries = self.get_recent_discoveries_for_agent(requesting_agent_id, 3)
    ...
    def _invalidate_agent_cache(self, agent_id: str) -> None:
        ...
        keys_to_remove = [
            key for key in self.agent_activity_cache.keys() 
    ...
    def _cleanup_old_data(self) -> None:
        ...
        if archived_count > 0:
            logger.debug(f"Archived {archived_count} old turns to prevent memory growth")
...
class AsyncCampaignLogger:
    ...
    def __init__(self, log_path: str, batch_size: int = 10, flush_interval: float = 5.0):
    ...
    async def stop(self) -> None:
        ...
        logger.info(f"Async campaign logger stopped - {self.total_events_logged} events logged")
    ...
    async def _log_worker(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                while len(events) < self.batch_size and (time.time() - start_time) < batch_timeout:
                    try:
...
class DirectorAgentPerformanceOptimizer:
    ...
    def optimize_director_agent(director_instance) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_results['before_metrics'] = DirectorAgentPerformanceOptimizer._capture_metrics(director_instance)
            ...
            optimization_results['optimizations_applied'].append('optimized_agent_registry')
            ...
            if hasattr(director_instance, 'world_state_tracker'):
                ...
                director_instance.get_world_state_feedback = optimized_tracker.get_world_state_feedback
                
                optimization_results['optimizations_applied'].append('optimized_world_state_tracker')
            ...
            if hasattr(director_instance, 'campaign_log_path'):
                ...
                def optimized_log_event(event_description: str):
                    ...
                    try:
                        ...
                        if loop.is_running():
                            asyncio.create_task(async_logger.log_event_async(event_description))
                        else:
                ...
                optimization_results['optimizations_applied'].append('async_campaign_logger')
            ...
            optimization_results['after_metrics'] = DirectorAgentPerformanceOptimizer._capture_metrics(director_instance)
            
            # Calculate performance improvements
            optimization_results['performance_improvements'] = DirectorAgentPerformanceOptimizer._calculate_improvements(
                optimization_results['before_metrics'],
                optimization_results['after_metrics']
            )
            
            logger.info(f"DirectorAgent optimization completed - {len(optimization_results['optimizations_applied'])} optimizations applied")
    ...
    def _capture_metrics(director_instance) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'world_state_size': len(getattr(director_instance, 'world_state_tracker', {}).get('agent_discoveries', {}))
            }
        except Exception as e:
    ...
    @staticmethod
    def _calculate_improvements(before_metrics: Dict, after_metrics: Dict) -> Dict[str, str]:
        """Calculate performance improvements."""
        improvements = {}
        ...
        try:
            ...
            improvements['optimizations_active'] = "Hash-based lookups, async I/O, intelligent caching"
    ...
    @staticmethod
    def _migrate_world_state_data(old_tracker: Any, new_tracker: OptimizedWorldStateTracker) -> None:
        ...
        try:
            ...
            elif hasattr(old_tracker, '__dict__') and hasattr(old_tracker, 'agent_discoveries'):
...
class PerformanceMonitor:
    ...
    def record_operation(self, operation_name: str, duration: float, **kwargs) -> None:
        ...
        if len(self.operation_times[operation_name]) > 10:
            ...
            historical_avg = sum(self.operation_times[operation_name][:-10]) / max(1, len(self.operation_times[operation_name]) - 10)
            
            if recent_avg > historical_avg * 2:  # 100% performance regression
                logger.warning(f"Performance regression detected in {operation_name}: {recent_avg:.3f}s vs {historical_avg:.3f}s")
...
def quick_optimize_director_agent(director_instance) -> bool:
    ...
    """
    try:
        result = DirectorAgentPerformanceOptimizer.optimize_director_agent(director_instance)
