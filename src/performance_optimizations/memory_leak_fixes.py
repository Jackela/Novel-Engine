...
class SlidingWindowMemoryManager:
    ...
    def __init__(self, max_size: int = 1000, archive_threshold: float = 0.8, 
                 enable_persistence: bool = True, archive_path: str = "memory_archive"):
        ...
        logger.debug(f"SlidingWindowMemoryManager initialized: max_size={max_size}, "
    ...
    def manage_list(self, data_list: List[Any], name: str = "unknown") -> int:
        ...
        logger.debug(f"Sliding window management for {name}: archived {archived_count} items, "
    ...
    def manage_deque(self, data_deque: Deque[Any], name: str = "unknown") -> int:
        ...
        logger.debug(f"Deque sliding window for {name}: archived {len(archived_items)} items")
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    serializable_items.append(f"<non-serializable: {type(item).__name__}>")
    ...
    def fix_persona_agent_memory_leaks(persona_agent_instance) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(persona_agent_instance, 'decision_history'):
                ...
                logger.info(f"PersonaAgent {persona_agent_instance.agent_id}: "
            ...
            context_attributes = ['context_history', 'interaction_history', 'narrative_context']
            ...
            persona_agent_instance._memory_monitor = PersonaAgentMemoryMonitor(persona_agent_instance)
            ...
            memory_saved = fix_results['memory_before_mb'] - fix_results['memory_after_mb']
            logger.info(f"PersonaAgent memory fixes completed: {memory_saved:.1f}MB saved")
...
class PersonaAgentMemoryMonitor:
    ...
    def __init__(self, persona_agent_instance):
        self.agent_instance = weakref.ref(persona_agent_instance)  # Weak reference to prevent circular refs
    ...
    def _check_cleanup_needs(self, agent):
        ...
        try:
            ...
            if hasattr(agent, 'decision_history') and len(agent.decision_history) > 750:
                cleanup_needed = True
                logger.warning(f"Agent {self.agent_id} decision_history at {len(agent.decision_history)} items")
            
            # Check other memory-intensive attributes
            for attr_name in ['context_history', 'interaction_history', 'narrative_context']:
                if hasattr(agent, attr_name):
                    attr_value = getattr(agent, attr_name)
                    if isinstance(attr_value, list) and len(attr_value) > 150:
                        cleanup_needed = True
                        logger.warning(f"Agent {self.agent_id} {attr_name} at {len(attr_value)} items")
            
            if cleanup_needed:
                logger.info(f"Performing automatic cleanup for PersonaAgent {self.agent_id}")
    ...
    def _check_memory_alerts(self, stats: MemoryStats, agent):
        ...
        if stats.rss_mb > self.memory_threshold_mb:
            logger.warning(f"High memory usage for PersonaAgent {self.agent_id}: {stats.rss_mb:.1f}MB")
        ...
        if len(self.memory_history) >= 10:
            ...
            if recent_growth > 1.0:  # Growing by > 1MB per measurement period
                logger.warning(f"Rapid memory growth detected for {self.agent_id}: "
...
class SystemWideMemoryManager:
    ...
    def __init__(self, memory_limit_mb: int = 1024, check_interval: int = 60):
        ...
        logger.info(f"SystemWideMemoryManager initialized: limit={memory_limit_mb}MB, "
    ...
    def register_cleanup_callback(self, callback: Callable[[], None], component_name: str):
        ...
        while self.monitoring_active:
            try:
                ...
                if len(self.memory_history) % 10 == 0:  # Every 10 measurements
                    logger.info(f"System Memory: {stats.rss_mb:.1f}MB RSS, "
    ...
    def _handle_memory_limit_exceeded(self, stats: MemoryStats):
        ...
        logger.warning(f"Memory limit exceeded: {stats.rss_mb:.1f}MB > {self.memory_limit_mb}MB")
    ...
    def _check_for_memory_leaks(self):
        ...
        if time_diff > 0:
            ...
            if growth_rate > 2.0:  # More than 2MB/min growth
                logger.warning(f"Potential memory leak detected: {growth_rate:.2f}MB/min growth")
...
def apply_memory_fixes_to_persona_agent(persona_agent_instance) -> bool:
    ...
    """
    try:
        result = PersonaAgentMemoryFixer.fix_persona_agent_memory_leaks(persona_agent_instance)
...
def monitor_persona_agent_memory(persona_agent_instance):
    ...
    if hasattr(persona_agent_instance, '_memory_monitor'):
        logger.warning(f"Memory monitor already exists for {persona_agent_instance.agent_id}")
...
def get_comprehensive_memory_report() -> Dict[str, Any]:
    ...
    return {
        ...
        'status': 'healthy' if not system_report.get('limit_exceeded', False) else 'warning'
    }
