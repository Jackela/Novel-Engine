    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'reuse_rate_percent': (self.reused_count / max(1, self.created_count + self.reused_count)) * 100
            }
...
class MemoryOptimizer:
    ...
    def _tune_gc_for_pressure(self, aggressive: bool = False):
        ...
        if aggressive:
            ...
            new_thresholds = tuple(int(t / self.gc_threshold_multiplier) for t in self.original_gc_thresholds)
        else:
            # Moderate GC tuning
            multiplier = (self.gc_threshold_multiplier + 1.0) / 2.0
            new_thresholds = tuple(int(t / multiplier) for t in self.original_gc_thresholds)
        
        gc.set_threshold(*new_thresholds)
        self.optimization_stats['gc_optimizations'] += 1
        
        logger.debug(f"GC thresholds adjusted: {self.original_gc_thresholds} -> {new_thresholds}")
    ...
    def create_object_pool(self, name: str, factory: Callable[[], Any], max_size: int = 100) -> ObjectPool:
    ...
    def get_memory_report(self) -> Dict[str, Any]:
        ...
        if len(self.memory_history) >= 2:
            ...
            memory_trend = latest_metrics.memory_percent - previous_metrics.memory_percent
        else:
            memory_trend = 0.0
    ...
    def force_memory_optimization(self) -> Dict[str, Any]:
        ...
        if metrics.memory_pressure in [MemoryPressureLevel.CRITICAL, MemoryPressureLevel.HIGH]:
    ...
    def optimize_persona_agent(self, agent: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(agent, 'decision_history') and len(agent.decision_history) > 100:
                ...
                optimization_results['optimizations_applied'].append('decision_history_pooling')
                optimization_results['memory_saved_estimate'] += len(old_decisions) * 1024  # Rough estimate
            ...
            if hasattr(agent, 'context_history') and len(agent.context_history) > 50:
                ...
                optimization_results['optimizations_applied'].append('context_history_pooling')
            ...
            self.memory_optimizer.register_weak_reference(
                agent_key,
                agent,
                lambda: logger.debug(f"PersonaAgent {optimization_results['agent_id']} garbage collected")
            )
            optimization_results['optimizations_applied'].append('weak_reference_registration')
...
class DirectorAgentMemoryOptimizer:
    ...
    def __init__(self, memory_optimizer: MemoryOptimizer):
        ...
        self.coordination_pool = memory_optimizer.create_object_pool(
            'coordination_data',
            lambda: {'requesting_agent': '', 'target_agents': [], 'coordination_type': '', 'data': {}},
            max_size=100
        )
        ...
        try:
            ...
            if hasattr(director, 'world_state_history'):
                ...
                if isinstance(world_states, dict) and len(world_states) > 20:
                    ...
                    optimization_results['optimizations_applied'].append('world_state_pooling')
                    optimization_results['memory_saved_estimate'] += len(old_turns) * 2048
            ...
            if hasattr(director, 'coordination_cache'):
                ...
                if isinstance(coord_cache, dict) and len(coord_cache) > 50:
                    ...
                    optimization_results['optimizations_applied'].append('coordination_cache_pooling')
                    optimization_results['memory_saved_estimate'] += len(items_to_remove) * 1024
...
# Context manager for easy memory optimization usage
