...
class AsyncLLMClient:
    ...
    async def call_llm_async(
        ...
        if cached_response:
            ...
            logger.debug(
                f"Agent {agent_id} cache hit - response time: {response_time:.3f}s"
            )
        ...
        try:
            ...
            if response:
                ...
                self._cache_response(
                    cache_key, response, agent_id, prompt, character_context
                )
                ...
                logger.debug(
                    f"Agent {agent_id} API success - response time: {response_time:.3f}s"
                )
                return response
            else:
            ...
            try:
                async with self._session.post(
                    ...
                    if response.status == 401:
                        ...
                        logger.error(
                            f"Agent {agent_id} API request failed: {response.status}"
                        )
                        return None
                ...
                logger.error(
                    f"Agent {agent_id} API request timeout ({self.request_timeout}s)"
                )
                return None
            except Exception as e:
...
def call_llm_async_wrapper(
    agent_id: str, prompt: str, character_context: Dict[str, Any]
) -> str:
    """
...
async def _call_async_from_sync(
    agent_id: str, prompt: str, character_context: Dict[str, Any]
) -> str:
