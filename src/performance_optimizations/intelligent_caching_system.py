...
class IntelligentCache:
    ...
    def __init__(self, 
        ...
        logger.info(f"IntelligentCache initialized: L1={l1_max_size}, L2={l2_max_size}, L3={l3_max_size}")
    ...
    def _clean_expired_entries(self):
        ...
        # Clean L1
        expired_l1 = [key for key, entry in self.l1_cache.items() 
                      if entry.is_expired()]
        ...
        # Clean L2
        expired_l2 = [key for key, entry in self.l2_cache.items() 
                      if entry.is_expired()]
        for key in expired_l2:
            self.l2_cache.pop(key)
        
        # Clean L3
        expired_l3 = [key for key, entry in self.l3_cache.items() 
                      if entry.is_expired()]
        ...
        if expired_l1 or expired_l2 or expired_l3:
            logger.debug(f"Cleaned expired entries: L1={len(expired_l1)}, "
...
class LLMResponseCache(IntelligentCache):
    ...
    def generate_cache_key(self, agent_id: str, prompt: str, 
    ...
    async def get_similar_response(self, key: str, similarity_threshold: float = 0.8) -> Optional[Any]:
        """
        ...
        for similar_key in similar_keys:
            ...
            if response is not None:
                logger.debug(f"Cache hit via similarity: {key[:16]}... -> {similar_key[:16]}...")
                return response
...
class WorldStatePrefetcher:
    ...
    async def analyze_agent_pattern(self, agent_id: str, world_state_request: Dict[str, Any]):
    ...
    async def _prefetch_predicted_requests(self, agent_id: str, pattern_data: Dict[str, Any]):
            ...
            for request_data in recent_requests:
                ...
                if 'current_turn' in request:
                    ...
                    if cached_value is None:
                        ...
                        asyncio.create_task(self._background_prefetch(prefetch_key, request))
                
                # Predict related agent data
                if 'requesting_agent' in request:
                    related_agents = self._predict_related_agents(agent_id, pattern_data)
    ...
    def _predict_related_agents(self, agent_id: str, pattern_data: Dict[str, Any]) -> List[str]:
    ...
    async def _background_prefetch(self, prefetch_key: str, sample_request: Dict[str, Any]):
...
async def get_comprehensive_caching_report() -> Dict[str, Any]:
    ...
    if _global_world_state_cache:
        report['world_state_cache'] = _global_world_state_cache.get_comprehensive_stats()
