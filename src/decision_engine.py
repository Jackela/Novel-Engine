...
class DecisionEngine:
    ...
    def _setup_llm_session(self) -> None:
        ...
        if not self.llm_available:
            logger.info("LLM integration not available - using algorithmic decision-making only")
            return
        ...
        try:
            ...
            if self.llm_available:
                try:
                    ...
                    logger.warning(f"LLM decision-making failed for {agent_id}: {str(llm_error)}")
            ...
            for action in available_actions:
                evaluation_score = self._evaluate_action_option(action, situation_assessment)
            ...
            logger.error(f"Error in decision-making for agent {self.agent_core.agent_id}: {str(e)}")
            return None
    ...
    def _process_world_state_update(self, world_state_update: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if location_info:
                self.agent_core.add_to_subjective_worldview('location_knowledge', 'current_area', location_info)
            
            if faction_info:
                for faction, status in faction_info.items():
                    self.agent_core.add_to_subjective_worldview('faction_relationships', faction, status)
            ...
            logger.debug(f"Agent {self.agent_core.agent_id} processed world state update")
    ...
    def _assess_current_situation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Agent {self.agent_core.agent_id} situation assessment: threat={situation['threat_level']}, opportunity={situation['opportunity_level']}")
    ...
    def _assess_threat_level(self) -> str:
        ...
        try:
            active_threats = self.agent_core.subjective_worldview.get('active_threats', {})
    ...
    def _assess_opportunity_level(self) -> str:
        ...
        try:
            current_goals = self.agent_core.subjective_worldview.get('current_goals', [])
            known_entities = self.agent_core.subjective_worldview.get('known_entities', {})
    ...
    def _assess_resource_status(self) -> str:
        ...
        if self.agent_core.morale_level > 0.5 and self.agent_core.current_status == 'active':
            return 'good'
        elif self.agent_core.morale_level > 0 and self.agent_core.current_status == 'active':
            return 'adequate'
        else:
            return 'poor'
    ...
    def _assess_social_context(self) -> Dict[str, Any]:
        ...
        try:
            positive_relationships = sum(1 for strength in self.agent_core.relationships.values() if strength > 0)
            negative_relationships = sum(1 for strength in self.agent_core.relationships.values() if strength < 0)
            ...
            return {'allies_present': 0, 'enemies_present': 0, 'social_standing': 'unknown'}
    
    def _assess_narrative_urgency(self) -> str:
        """Assess narrative urgency based on recent events."""
        try:
            recent_events = self.agent_core.subjective_worldview.get('recent_events', [])
    ...
    def _identify_available_actions(self, situation_assessment: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if threat_level in ['moderate', 'high']:
                actions.extend([
                    {'action_type': 'defensive_action', 'priority': 'high', 'risk': 'medium'},
            ...
            if opportunity_level in ['moderate', 'high']:
                actions.extend([
                    {'action_type': 'advance', 'priority': 'medium', 'risk': 'medium'},
                    {'action_type': 'exploit_opportunity', 'priority': 'high', 'risk': 'high'}
                ])
            ...
            if social_context.get('allies_present', 0) > 0:
                actions.append({'action_type': 'coordinate', 'priority': 'medium', 'risk': 'low'})
    ...
    def _evaluate_action_option(self, action: Dict[str, Any], situation: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if action_type in ['retreat', 'defensive_action']:
                ...
                score *= self.agent_core.decision_weights.get('personal_relationships', 0.6)
            ...
            if threat_level == 'high' and action_type in ['retreat', 'defensive_action']:
    ...
    def _select_best_action(self, action_evaluations: List[Tuple[Dict[str, Any], float]]) -> Optional[CharacterAction]:
        ...
        try:
            ...
            best_actions = [action for action, score in action_evaluations if score >= action_evaluations[0][1] * 0.8]
    ...
    def _generate_action_reasoning(self, action: Dict[str, Any]) -> str:
        ...
        try:
            ...
            base_reasoning = reasoning_templates.get(action_type, 
            ...
            if self.agent_core.morale_level > 0.7:
                ...
                morale_context = " Their low morale makes them cautious in their approach."
            
            return base_reasoning + morale_context
            
        except Exception as e:
            logger.error(f"Error generating action reasoning: {str(e)}")
            return f"Character decides to act based on current assessment."
    ...
    def _process_narrative_situation_update(self, narrative_context: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if story_phase and story_phase != 'unknown':
                self.agent_core.add_to_subjective_worldview('current_goals', 'narrative_phase', story_phase)
                
            # Process narrative events
            for event in events:
                self.agent_core.add_to_subjective_worldview('recent_events', event, datetime.now().isoformat())
            ...
            if event_type in ['cooperation', 'alliance']:
                for participant in participants:
                    if participant != self.agent_core.agent_id:
                        current_strength = self.agent_core.get_relationship_strength(participant)
                        self.agent_core.add_relationship(participant, min(1.0, current_strength + 0.1))
            elif event_type in ['conflict', 'betrayal']:
                for participant in participants:
                    if participant != self.agent_core.agent_id:
                        current_strength = self.agent_core.get_relationship_strength(participant)
                        self.agent_core.add_relationship(participant, max(-1.0, current_strength - 0.2))
                        
        except Exception as e:
            logger.error(f"Error processing event for relationships: {str(e)}")
    
    def _record_decision(self, action: Optional[CharacterAction], decision_type: str) -> None:
        ...
        try:
            decision_record = {
                ...
                'reasoning': action.get('reasoning') if action else 'Chose to wait and observe',
