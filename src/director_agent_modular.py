...
try:
    from shared_types import ProposedAction as CharacterAction  # Use ProposedAction as CharacterAction
except ImportError:
...
class DirectorAgent:
    ...
    def __init__(self, config_file: str = "config.yaml"):
        ...
        try:
            ...
            self.config = app_config.__dict__ if hasattr(app_config, '__dict__') else {
                'environment': 'development',
                'logging': {'level': 'INFO'},
                'world_state_file': 'world_state.json',
                'log_directory': 'logs'
    ...
    async def execute_turn(self, turn_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if turn_result.get('narrative_events'):
                narrative_result = await self.narrative_orchestrator.process_narrative_events(
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.error_handler, 'get_error_statistics'):
                status['error_handling'] = await self.error_handler.get_error_statistics()
            
            if hasattr(self.world_state_manager, 'get_state_statistics'):
                status['world_state'] = await self.world_state_manager.get_state_statistics()
            
            if hasattr(self.narrative_orchestrator, 'get_story_summary'):
                status['narrative'] = await self.narrative_orchestrator.get_story_summary()
    ...
    async def _handle_initialization_error(self, component_name: str, error: Exception) -> None:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            cleanup_components = [
                ("Narrative Orchestrator", getattr(self, 'narrative_orchestrator', None)),
