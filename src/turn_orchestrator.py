...
class TurnOrchestrator:
    ...
    def run_turn(self, registered_agents: List[PersonaAgent], world_state_data: Dict[str, Any], 
        ...
        2. Preparing world state updates for agents
        3. Emitting turn start event to all agents
        4. Managing turn execution flow
    ...
    def _prepare_world_state_for_turn(self, world_state_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'turn_metadata': {
                'turn_number': self.current_turn_number,
                'turn_start_time': self.current_turn_state.start_time.isoformat() if self.current_turn_state else None
            }
        }
    
    def _prepare_world_state_for_agent(self, agent: PersonaAgent, world_state_data: Dict[str, Any], 
                                     registered_agents: List[PersonaAgent]) -> Dict[str, Any]:
        """
    ...
    def handle_agent_action(self, agent: PersonaAgent, action: Optional[CharacterAction], 
                          log_event_callback: callable) -> bool:
        """
        ...
        try:
            ...
            if action:
                ...
                # Log the action
                character_name = getattr(agent, 'character_data', {}).get('name', agent.agent_id)
    ...
    def _store_turn_in_history(self, turn_summary: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if len(self.turn_history) > self.max_turn_history:
                ...
                logger.debug(f"Removed turn {removed_turn.turn_number} from history (limit: {self.max_turn_history})")
            
            # Update performance metrics
            if self.current_turn_state.start_time:
                turn_duration = (datetime.now() - self.current_turn_state.start_time).total_seconds()
                if self.average_turn_duration == 0:
                    self.average_turn_duration = turn_duration
                else:
                    # Simple exponential moving average
                    self.average_turn_duration = 0.9 * self.average_turn_duration + 0.1 * turn_duration
            
            logger.info(f"Turn {self.current_turn_number} stored in history ({len(self.turn_history)} total)")
