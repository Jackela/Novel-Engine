...
class RedisStateStore(StateStore):
    ...
    async def set(self, key: StateKey, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            result = await self.redis.setex(key.to_string(), ttl_seconds, serialized_value)
    ...
    async def list_keys(self, pattern: str) -> List[StateKey]:
        ...
        try:
            ...
            return [StateKey.from_string(key.decode() if isinstance(key, bytes) else key) for key in keys]
...
class PostgreSQLStateStore(StateStore):
        ...
        try:
            async with self.pool.acquire() as conn:
                await conn.execute(
                    ...
                    """,
                    key_hash, key.namespace, key.entity_type, key.entity_id, key.version, value, expires_at
                )
                return True
    ...
    async def query_by_namespace(self, namespace: str, limit: int = 100) -> List[Dict[str, Any]]:
...
class UnifiedStateManager:
    ...
    async def get(self, key: StateKey, use_cache: bool = True) -> Optional[Any]:
        ...
        if value is not None and use_cache and key.entity_type != 'cache' and primary_store != self.redis_store:
            cache_key = StateKey(
                namespace=f"cache:{key.namespace}",
                entity_type='cache',
                entity_id=f"{key.entity_type}:{key.entity_id}",
    ...
    async def list_keys(self, pattern: str, store_type: Optional[str] = None) -> List[StateKey]:
    ...
    def _apply_env_overrides(self):
        ...
        for env_var, config_path in env_mappings.items():
            ...
            if env_value:
                ...
                if final_key in ['port', 'workers', 'cache_ttl', 'max_retries', 'connection_timeout']:
...
# Factory functions
def create_unified_state_manager(config: Optional[StateStoreConfig] = None) -> UnifiedStateManager:
...
def create_configuration_manager(config_paths: List[str] = None) -> ConfigurationManager:
...
if __name__ == "__main__":
    ...
    async def example_usage():
        ...
        await state_manager.set(session_key, {"player_id": "123", "current_location": "forest"})
        
        # Agent state (PostgreSQL)
        agent_key = StateKey("simulation", "agent", "alice")
        await state_manager.set(agent_key, {"name": "Alice", "personality": {"curiosity": 0.8}})
        
        # Narrative document (S3)
        narrative_key = StateKey("story", "narrative", "chapter_1")
        await state_manager.set(narrative_key, "In the beginning, there was a mysterious forest...")
