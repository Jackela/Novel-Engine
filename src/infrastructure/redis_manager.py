...
class RedisConnectionPool:
    ...
    def _serialize_value(self, value: Any, strategy: RedisStorageStrategy = RedisStorageStrategy.JSON) -> str:
    ...
    def _deserialize_value(self, value: str, strategy: RedisStorageStrategy = RedisStorageStrategy.JSON) -> Any:
    ...
    async def cache_character(self, character_id: str, character_data: Dict[str, Any], ttl: int = 3600) -> bool:
    ...
    async def cache_interaction_result(
        ...
        cache_key = CacheKey("novel_engine", "interaction", f"{session_id}:{interaction_id}").build()
        return await self.set(cache_key, result_data, ttl)
    
    async def store_session(self, session_id: str, session_data: Dict[str, Any], ttl: int = 7200) -> bool:
        """Store session data."""
        ...
        for field, value in session_data.items():
            await pipe.hset(session_key, field, self._serialize_value(value))
