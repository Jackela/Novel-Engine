...
class PostgreSQLConfig:
    ...
    enable_extensions: List[str] = field(default_factory=lambda: ["uuid-ossp", "pg_trgm", "btree_gin"])
...
class PostgreSQLConnectionPool:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(f"PostgreSQL connection pool initialized: {self.config.host}:{self.config.port}")
    ...
    async def search_characters(self, search_query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [
            {
                'id': str(row['id']),
                'name': row['name'],
                'data': json.loads(row['character_data']) if isinstance(row['character_data'], str) else row['character_data'],
    ...
    async def search_memories(
        ...
        where_clauses = ["agent_id = $1", "search_vector @@ plainto_tsquery('english', $2)"]
        params = [agent_id, search_query]
        
        if memory_types:
            where_clauses.append(f"memory_type = ANY($3)")
...
async def create_postgresql_manager(config: Optional[PostgreSQLConfig] = None) -> PostgreSQLManager:
