...
class MetricsCollector:
    ...
    def record_http_request(self, method: str, endpoint: str, status: int, duration: float):
        ...
        self.observe_histogram('http_request_duration', duration, {
            'method': method,
            'endpoint': endpoint
        })
    ...
    def update_system_metrics(self):
        ...
        try:
            ...
            self.set_gauge('memory_usage_bytes', memory.available, {'type': 'available'})
...
class TracingManager:
    ...
    def start_span(self, operation_name: str, parent_span_id: str = None, **labels) -> str:
...
class PerformanceProfiler:
    ...
    def finish_operation(self, operation_id: str, operation_name: str, success: bool = True):
        ...
        for operation_name, times in self.operation_times.items():
            if times:
                ...
                stats[operation_name] = {
                    ...
                    'error_rate': self.error_counts[operation_name] / max(1, self.operation_counts[operation_name]),
                    'avg_duration_ms': sum(times_list) * 1000 / len(times_list),
                    'min_duration_ms': min(times_list) * 1000,
                    'max_duration_ms': max(times_list) * 1000,
                    'p95_duration_ms': sorted(times_list)[int(len(times_list) * 0.95)] * 1000 if times_list else 0,
                    'p99_duration_ms': sorted(times_list)[int(len(times_list) * 0.99)] * 1000 if times_list else 0
                }
...
class SecurityAuditor:
    ...
    def log_security_threat(self, threat_type: str, severity: str, details: Dict[str, Any]):
        """Log security threat"""
        event_data = {
        ...
        if severity in ['HIGH', 'CRITICAL']:
            self.logger.critical(f"Security threat detected: {threat_type}", **event_data)
        else:
            self.logger.warning(f"Security threat detected: {threat_type}", **event_data)
        ...
        for name, check_func in self.health_checks.items():
            try:
                ...
                if isinstance(result, bool):
                    result = {'healthy': result, 'message': 'OK' if result else 'Failed'}
...
class ObservabilityManager:
    ...
    def __init__(self, service_name: str = "novel-engine", config: Dict[str, Any] = None):
        self.service_name = service_name
        self.config = config or {}
        
        # Initialize components
        self.metrics = MetricsCollector(namespace=service_name.replace('-', '_'))
        self.logger = StructuredLogger(service_name, level=self.config.get('log_level', 'INFO'))
    ...
    def create_monitoring_routes(self):
        ...
        async def get_metrics():
            ...
            return PlainTextResponse(content=content, media_type=CONTENT_TYPE_LATEST if PROMETHEUS_AVAILABLE else "text/plain")
                ...
                with self.tracer.trace_operation(func_name):
                    with self.profiler.profile_operation(func_name):
                        try:
                            ...
                            self.logger.debug(f"Function {func_name} completed successfully")
                            return result
                        except Exception as e:
                            self.logger.error(f"Function {func_name} failed", error=str(e))
                            raise
...
# Factory function
def create_observability_manager(service_name: str = "novel-engine", 
                                config: Dict[str, Any] = None) -> ObservabilityManager:
