...
class S3StorageManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(f"S3 storage manager initialized: bucket={self.config.bucket_name}")
    ...
    async def _ensure_bucket_exists(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to create S3 bucket {self.config.bucket_name}: {e}")
                raise
    ...
    async def upload_file(
        ...
        try:
            ...
            extra_args = {
                ...
                'StorageClass': (storage_class or self.config.default_storage_class).value
            }
            ...
            def progress_handler(bytes_transferred: int):
                ...
                progress.percentage = (bytes_transferred / file_size) * 100 if file_size > 0 else 0
            ...
            if self._metrics['upload_times']:
                self._metrics['avg_upload_time'] = sum(self._metrics['upload_times']) / len(self._metrics['upload_times'])
            
            logger.info(f"Uploaded file to S3: {s3_key} ({file_size} bytes in {upload_time:.2f}s)")
        ...
        try:
            ...
            async with aiofiles.open(local_path, 'wb') as f:
                ...
                async for chunk in response['Body'].iter_chunks(8192):
                    ...
                    progress.percentage = (progress.bytes_transferred / file_size) * 100 if file_size > 0 else 0
            ...
            if self._metrics['download_times']:
                self._metrics['avg_download_time'] = sum(self._metrics['download_times']) / len(self._metrics['download_times'])
            
            logger.info(f"Downloaded file from S3: {s3_key} to {local_path} ({file_size} bytes in {download_time:.2f}s)")
            return True
    ...
    async def delete_object(self, s3_key: str, version_id: Optional[str] = None) -> bool:
    ...
    async def store_narrative_export(
        ...
        await self.upload_bytes(
            content=content,
            s3_key=s3_key,
            content_type=S3ContentType.TEXT.value if export_format in ['txt', 'md'] else S3ContentType.JSON.value,
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            list_response = await self.list_objects(max_keys=1)
