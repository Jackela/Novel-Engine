...
from .postgresql_manager import PostgreSQLManager, PostgreSQLConfig, create_postgresql_config_from_env
from .redis_manager import RedisManager, RedisConfig, create_redis_config_from_env
from .s3_manager import S3StorageManager, S3Config, create_s3_config_from_env
...
class EnterpriseStorageManager:
            ...
            try:
                ...
                if self.postgresql:
                    try:
                        ...
                        self._metrics['health']['postgresql'] = health.get('healthy', False)
                    except Exception as e:
    ...
    def _get_cache_backend_for_category(self, category: DataCategory) -> Optional[StorageBackend]:
    ...
    async def _record_operation_metrics(self, category: DataCategory, backend: StorageBackend, response_time: float):
        ...
        if len(self._metrics['performance']['response_times']) > 1000:
            self._metrics['performance']['response_times'] = self._metrics['performance']['response_times'][-1000:]
    ...
    async def store_character(self, character_id: str, character_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.postgresql:
                await self.postgresql.connection_pool.store_character(character_id, character_data)
            
            # Cache in Redis if configured
            cache_backend = self._get_cache_backend_for_category(DataCategory.CHARACTERS)
            if cache_backend == StorageBackend.REDIS and self.redis:
                await self.redis.connection_pool.cache_character(character_id, character_data)
            ...
            await self._record_operation_metrics(DataCategory.CHARACTERS, StorageBackend.POSTGRESQL, response_time)
    ...
    async def get_character(self, character_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cache_backend = self._get_cache_backend_for_category(DataCategory.CHARACTERS)
            if cache_backend == StorageBackend.REDIS and self.redis:
                cached_data = await self.redis.connection_pool.get_cached_character(character_id)
                if cached_data:
                    response_time = asyncio.get_event_loop().time() - start_time
                    await self._record_operation_metrics(DataCategory.CHARACTERS, StorageBackend.REDIS, response_time)
                    return cached_data
            
            # Fetch from primary backend
            if self.postgresql:
                results = await self.postgresql.connection_pool.search_characters(
                    search_query=character_id,
                    limit=1
                )
    ...
    async def store_memory(
        ...
        try:
            ...
            # Record metrics
            response_time = asyncio.get_event_loop().time() - start_time
            await self._record_operation_metrics(DataCategory.MEMORY, StorageBackend.POSTGRESQL, response_time)
    ...
    async def store_session(self, session_id: str, session_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Record metrics
            response_time = asyncio.get_event_loop().time() - start_time
            await self._record_operation_metrics(DataCategory.SESSIONS, StorageBackend.REDIS, response_time)
        ...
        try:
            ...
            if self.redis:
                ...
                if cached_value is not None:
                    response_time = asyncio.get_event_loop().time() - start_time
                    await self._record_operation_metrics(DataCategory.SYSTEM_STATE, StorageBackend.REDIS, response_time)
                    return cached_value
            ...
            if self.postgresql:
                ...
                response_time = asyncio.get_event_loop().time() - start_time
                await self._record_operation_metrics(DataCategory.SYSTEM_STATE, StorageBackend.POSTGRESQL, response_time)
    ...
    async def set_system_state(self, key: str, value: Any, updated_by: str = "system") -> bool:
        ...
        try:
            # Store in PostgreSQL
            if self.postgresql:
                await self.postgresql.connection_pool.set_system_state(key, value, updated_by)
    ...
    async def store_file(
        ...
        try:
            ...
            # Record metrics
            response_time = asyncio.get_event_loop().time() - start_time
            await self._record_operation_metrics(DataCategory.FILES, StorageBackend.S3, response_time)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self.postgresql:
            try:
                ...
                health_status['backends']['postgresql'] = {'healthy': False, 'error': str(e)}
                health_status['overall_healthy'] = False
...
def create_enterprise_config_from_env() -> EnterpriseStorageConfig:
    ...
    # Migration settings
    config.enable_migration_mode = os.getenv("ENABLE_MIGRATION", "false").lower() == "true"
    config.sqlite_database_path = os.getenv("SQLITE_DATABASE_PATH")
    
    return config


async def create_enterprise_storage_manager(config: Optional[EnterpriseStorageConfig] = None) -> EnterpriseStorageManager:
