...
class ServiceRegistry:
    ...
    async def register_service(self, service: ServiceInstance) -> bool:
        ...
        async with self._lock:
            ...
            if existing:
                ...
                logger.info(f"Updated service instance: {service.service_name}:{service.instance_id}")
            else:
                # Add new instance
                self._services[service.service_name].append(service)
                logger.info(f"Registered new service instance: {service.service_name}:{service.instance_id}")
    ...
    def get_service_instance(self, service_name: str, load_balancing: str = "round_robin") -> Optional[ServiceInstance]:
...
class EventBus:
    ...
    async def publish(self, event_type: str, data: Any, metadata: Optional[Dict] = None):
...
class APIGateway:
    ...
    async def _make_request(self, instance: ServiceInstance, path: str, 
                          method: str, data: Any, headers: Optional[Dict]) -> Dict:
...
class CharacterService:
    ...
    async def create_character(self, character_data: Dict) -> Dict:
        ...
        # Publish event
        await self.event_bus.publish("character.created", {
...
class StoryService:
    ...
    async def generate_story(self, characters: List[str], scenario: str) -> Dict:
        ...
        # Publish event
        await self.event_bus.publish("story.generation_started", {
            "story_id": story_id,
            "characters": characters,
            "scenario": scenario
        })
...
async def main():
    ...
    async def on_character_created(event):
        logger.info(f"Character created event received: {event['data']['character_id']}")
