...
from .response_models import HealthCheckData, SuccessResponse, APIMetadata
from .error_handlers import ServiceUnavailableException
...
class HealthMonitor:
    ...
    def register_system_resource_checks(self):
        ...
        self.register_checker(HealthChecker(
            name="memory",
    ...
    async def run_health_checks(self, include_non_critical: bool = True) -> SystemHealth:
        ...
        critical_failures = [
            ...
            any(c.critical for c in checkers_to_run if c.name == r.component)
        ]
    ...
    def get_cached_health(self, max_age_seconds: int = 30) -> Optional[SystemHealth]:
        ...
        if (self.last_check_result and 
            ...
            (datetime.now() - self.last_check_time).total_seconds() < max_age_seconds):
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        healthy_count = len([h for h in recent_checks if h.status == HealthStatus.HEALTHY])
        
        return {
            "total_checks_run": len(self.check_history),
            "recent_healthy_percentage": (healthy_count / len(recent_checks)) * 100,
            "last_check": self.last_check_time.isoformat() if self.last_check_time else None,
            "uptime_hours": (datetime.now() - self.app_start_time).total_seconds() / 3600
        }
...
def create_health_data_response(system_health: SystemHealth) -> HealthCheckData:
    ...
    return HealthCheckData(
        ...
        orchestrator_status=next(
            (c.status.value for c in system_health.checks if c.component == "orchestrator"),
            "unknown"
        ),
        active_agents=next(
            (c.details.get("active_agents", 0) for c in system_health.checks 
...
__all__ = [
    'HealthStatus', 'ComponentType', 'HealthCheckResult', 'SystemHealth',
    'HealthChecker', 'HealthMonitor', 'create_health_data_response'
]