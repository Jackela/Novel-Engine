...
class APIVersionRegistry:
    ...
    def mark_deprecated(self, version: APIVersion, sunset_date: Optional[datetime] = None):
...
class VersionExtractor:
    ...
    def extract_from_request(self, request: Request) -> APIVersion:
        ...
        if "application/vnd.novel-engine.v" in accept_header:
            try:
                version_part = accept_header.split("application/vnd.novel-engine.v")[1].split("+")[0]
...
class CompatibilityLayer:
    ...
    def transform_response(self, response_data: Dict[str, Any], target_version: APIVersion) -> Dict[str, Any]:
...
class VersionedRoute(APIRoute):
    ...
    def __init__(self, *args, version_handlers: Optional[Dict[APIVersion, Callable]] = None, **kwargs):
...
def setup_versioning(app):
    ...
    app.add_api_route("/api/versions", version_info_handler, methods=["GET"], tags=["System"])
    
    logger.info("API versioning system initialized")
    return version_middleware

__all__ = [
    'APIVersion', 'VersionStatus', 'VersionInfo', 'APIVersionRegistry',
    'VersionExtractor', 'CompatibilityLayer', 'VersionMiddleware',
    'VersionedRoute', 'setup_versioning'
]