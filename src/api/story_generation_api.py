...
class ConnectionPool:
    ...
    async def cleanup_stale_connections(self):
        ...
        stale_generations = [
            gen_id for gen_id, last_time in self.last_activity.items()
...
class StoryGenerationAPI:
        ...
        while True:
            try:
                ...
                completed_generations = [
                    gen_id for gen_id, state in self.active_generations.items()
                    if state.get('status') in ['completed', 'failed'] and 
                    current_time - state.get('completion_time', current_time) > 3600
                ]
    ...
    def setup_routes(self, app: FastAPI):
        ...
        async def websocket_progress(websocket: WebSocket, generation_id: str):
            ...
            try:
                ...
                logger.info(f"WebSocket connection closed for generation {generation_id}")
            except Exception as e:
    ...
    async def _generate_story_async(self, generation_id: str):
        ...
        async with self.generation_semaphore:  # Limit concurrent generations
            try:
                ...
                await self._update_progress(generation_id, 5, "initializing", "Setting up generation parameters")
                ...
                await self._update_progress(generation_id, 20, "initialized", "Generation context prepared")
                
                # Stage 2: Character Analysis (20-40%)
                await self._update_progress(generation_id, 25, "analyzing", "Analyzing character personalities")
                
                # Parallel character analysis
                character_tasks = [
                    self._analyze_character(char) for char in state['request'].characters
                ]
                character_results = await asyncio.gather(*character_tasks, return_exceptions=True)
                await self._update_progress(generation_id, 40, "analyzed", "Character analysis complete")
                
                # Stage 3: Narrative Planning (40-60%)
                await self._update_progress(generation_id, 45, "planning", "Creating story structure")
                
                # Use orchestrator for actual narrative planning
                planning_result = await self._create_narrative_plan(generation_id, character_results)
                await self._update_progress(generation_id, 60, "planned", "Story structure complete")
                
                # Stage 4: Story Generation (60-90%)
                await self._update_progress(generation_id, 65, "generating", "AI agents creating content")
                ...
                for turn in range(turn_count):
                    ...
                    await self._update_progress(
                        generation_id, progress, "generating", 
                        f"Turn {turn + 1}/{turn_count} in progress"
                    )
                    
                    # Generate turn content
                    await self._generate_story_turn(generation_id, turn, planning_result)
                
                # Stage 5: Finalizing (90-100%)
                await self._update_progress(generation_id, 95, "finalizing", "Quality checks and formatting")
                ...
                await self._update_progress(generation_id, 100, "completed", 
                                           f"Story generation completed in {state['generation_duration']:.1f}s")
                logger.info(f"Story generation {generation_id} completed in {state['generation_duration']:.1f}s")
                
            except Exception as e:
                logger.error(f"Error generating story {generation_id}: {e}")
                if generation_id in self.active_generations:
                    self.active_generations[generation_id]["status"] = "failed"
                    self.active_generations[generation_id]["completion_time"] = time.time()
                    await self._update_progress(generation_id, 0, "failed", f"Generation failed: {str(e)}")
    
    async def _update_progress(self, generation_id: str, progress: float, stage: str, stage_detail: str):
    ...
    def _create_progress_message(self, generation_id: str) -> str:
        ...
        update = ProgressUpdate(
            ...
            estimated_time_remaining=self._calculate_time_remaining(state),
            active_agents=["DirectorAgent", "PersonaAgent", "ChroniclerAgent"]  # Could be dynamic
        )
    ...
    async def _send_progress_update(self, generation_id: str, websocket: WebSocket):
        ...
        update = ProgressUpdate(
            ...
            stage_detail=state.get("stage_detail", ""),
            estimated_time_remaining=self._calculate_time_remaining(state),
            active_agents=["DirectorAgent", "PersonaAgent", "ChroniclerAgent"]  # Mock data
        )
    ...
    async def _generate_story_turn(self, generation_id: str, turn_number: int, planning_result):
        ...
        try:
            ...
            logger.error(f"Turn generation failed for {generation_id}, turn {turn_number}: {e}")
            return None
