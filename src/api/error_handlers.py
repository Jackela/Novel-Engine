...
from .response_models import (
    ErrorResponse, APIError, APIErrorType, ValidationError, 
    APIMetadata, ResponseStatus
)
...
class RateLimitException(NovelEngineException):
    ...
    def __init__(self, retry_after: Optional[int] = None):
        super().__init__(
            ...
            detail=f"Retry after {retry_after} seconds" if retry_after else "Please slow down your requests"
        )
    ...
    def create_error_response(
        ...
        if isinstance(error, NovelEngineException):
            ...
            api_error = APIError(
                ...
                message="An unexpected error occurred" if not self.debug else str(error),
...
def setup_error_handlers(app, debug: bool = False):
    ...
    async def novel_engine_exception_handler(request: Request, exc: NovelEngineException):
...
__all__ = [
    'NovelEngineException', 'ValidationException', 'ResourceNotFoundException',
    'ServiceUnavailableException', 'ConflictException', 'RateLimitException',
    'ErrorHandler', 'setup_error_handlers'
]