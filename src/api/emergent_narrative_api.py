...
class EmergentNarrativeRequest(BaseModel):
    ...
    agents: List[str] = Field(..., min_items=1, description="Participating agent IDs")
    time_range: Optional[Tuple[datetime, datetime]] = Field(None, description="Time range for narrative")
    narrative_perspective: str = Field("omniscient", description="Narrative perspective (omniscient, agent_specific, observer)")
    focus_agent: Optional[str] = Field(None, description="Primary focus agent for agent_specific perspective")
    include_predictions: bool = Field(True, description="Include predicted future events")
    coherence_threshold: float = Field(0.7, ge=0.0, le=1.0, description="Minimum coherence required")
    max_events: int = Field(50, ge=5, le=200, description="Maximum events to include")
    narrative_style: str = Field("dramatic", description="Narrative style (dramatic, documentary, poetic)")
...
class NarrativeEventResponse(BaseModel):
    ...
    causal_links: List[CausalLinkResponse] = Field(default_factory=list, description="Causal relationships")
...
class EmergentNarrativeData(BaseModel):
    ...
    events: List[NarrativeEventResponse] = Field(..., description="Sequence of narrative events")
    character_arcs: Dict[str, Dict[str, Any]] = Field(..., description="Character development arcs")
    plot_threads: Dict[str, Dict[str, Any]] = Field(..., description="Active plot threads")
    causal_graph_stats: Dict[str, Any] = Field(..., description="Causal graph statistics")
    coherence_report: Dict[str, Any] = Field(..., description="Narrative coherence analysis")
    narrative_patterns: Dict[str, List[str]] = Field(..., description="Detected narrative patterns")
    predicted_events: List[Dict[str, Any]] = Field(default_factory=list, description="Predicted future events")
    generation_metadata: Dict[str, Any] = Field(..., description="Generation process metadata")
    created_at: datetime = Field(default_factory=datetime.now, description="Narrative creation timestamp")

class NarrativeBuildRequest(BaseModel):
    """Request to build narrative from story elements"""
    story_elements: List[Dict[str, Any]] = Field(..., description="Story elements to incorporate")
    build_strategy: str = Field("chronological", description="Build strategy (chronological, thematic, character_focused)")
    ...
    target_length: Optional[int] = Field(None, description="Target narrative length in events")
...
class CausalGraphRequest(BaseModel):
    ...
    time_window: Optional[Tuple[datetime, datetime]] = Field(None, description="Time window filter")
    agent_filter: Optional[List[str]] = Field(None, description="Filter by agents")
    include_predictions: bool = Field(False, description="Include predicted relationships")
...
class CausalGraphData(BaseModel):
    ...
    edges: List[CausalLinkResponse] = Field(..., description="Graph edges (causal relationships)")
    ...
    narrative_patterns: Dict[str, List[str]] = Field(..., description="Detected narrative patterns")
    predictions: List[Dict[str, Any]] = Field(default_factory=list, description="Event predictions")

class NegotiationRequest(BaseModel):
    """Request to start negotiation"""
    initiator_id: str = Field(..., description="Initiating agent ID")
    target_agents: List[str] = Field(..., min_items=1, description="Target agents for negotiation")
...
class NegotiationResponse(BaseModel):
    ...
    resolution: Optional[Dict[str, Any]] = Field(None, description="Final resolution if completed")
...
class EmergentNarrativeAPI:
    ...
    def setup_routes(self, app: FastAPI):
        ...
        @app.post("/api/v1/narratives/emergent/generate",
        ...
        async def generate_emergent_narrative(
            ...
            try:
                if not self.orchestrator or not self.orchestrator.emergent_narrative_engine:
                    raise HTTPException(status_code=503, detail="Emergent narrative engine not available")
                ...
                if not narrative_result.get('success', False):
                    error_msg = narrative_result.get('error', 'Failed to generate narrative')
                    raise HTTPException(status_code=500, detail=error_msg)
                
                # Transform to response format
                narrative_data = EmergentNarrativeData(
                    narrative_id=narrative_result.get('narrative_id', f"narrative_{int(datetime.now().timestamp())}"),
        ...
        @app.post("/api/v1/narratives/build",
                 response_model=StandardResponse[EmergentNarrativeData],
        ...
        async def build_narrative(
            ...
            try:
                ...
                if not narrative_result.get('success', False):
                    error_msg = narrative_result.get('error', 'Failed to build narrative')
                    raise HTTPException(status_code=500, detail=error_msg)
                
                # Transform to response format
                built_narrative = narrative_result.get('narrative', {})
                narrative_data = EmergentNarrativeData(
                    narrative_id=built_narrative.get('narrative_id', f"built_narrative_{int(datetime.now().timestamp())}"),
        ...
        @app.get("/api/v1/causality/graph",
        ...
        @app.get("/api/v1/narratives/emergent/{narrative_id}",
                response_model=StandardResponse[EmergentNarrativeData],
        ...
        async def get_emergent_narrative(
            ...
            include_events: bool = Query(True, description="Include detailed event list"),
            include_causal_graph: bool = Query(False, description="Include causal relationship data"),
            format: str = Query("structured", description="Response format (structured, text, timeline)"),
            current_user: Dict = Depends(get_current_user)
        ):
            try:
                if not self.orchestrator or not self.orchestrator.emergent_narrative_engine:
