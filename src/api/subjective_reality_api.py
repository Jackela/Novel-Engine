...
class TurnBriefRequest(BaseModel):
    ...
    confidence_threshold: float = Field(0.3, ge=0.0, le=1.0, description="Minimum information confidence")
...
class InformationFragmentResponse(BaseModel):
    ...
    reliability: float = Field(..., ge=0.0, le=1.0, description="Information reliability")
...
class TurnBriefData(BaseModel):
    ...
    subjective_world_state: Dict[str, Any] = Field(..., description="Agent's filtered world view")
    available_information: List[InformationFragmentResponse] = Field(..., description="Available information fragments")
    recommended_actions: List[str] = Field(..., description="Recommended actions based on agent personality")
    narrative_context: str = Field(..., description="Narrative description of current situation")
    confidence_levels: Dict[str, float] = Field(..., description="Confidence in different information categories")
    visible_locations: List[str] = Field(..., description="Currently visible locations")
    known_agents: List[Dict[str, Any]] = Field(..., description="Known agents and their visibility")
    personality_factors: Dict[str, float] = Field(..., description="Agent's personality influences")
    fog_of_war_status: Dict[str, Any] = Field(..., description="Information access limitations")
    created_at: datetime = Field(default_factory=datetime.now, description="Brief generation timestamp")

class MultiBriefRequest(BaseModel):
    """Request for multiple agent briefs"""
    agent_filter: Optional[List[str]] = Field(None, description="Filter to specific agents")
    include_context: bool = Field(True, description="Include contextual information")
    confidence_threshold: float = Field(0.3, ge=0.0, le=1.0, description="Minimum information confidence")
...
class MultiBriefData(BaseModel):
    ...
    turn_number: int = Field(..., description="Turn sequence number")
    agent_briefs: Dict[str, TurnBriefData] = Field(..., description="Agent ID to brief mapping")
    ...
    conflicts_detected: List[Dict[str, Any]] = Field(default_factory=list, description="Detected agent conflicts")

class TurnBriefGenerationRequest(BaseModel):
    """Request to generate turn briefs"""
    global_world_state: Dict[str, Any] = Field(..., description="Current global world state")
    recent_events: List[Dict[str, Any]] = Field(default_factory=list, description="Recent events to process")
    agent_filter: Optional[List[str]] = Field(None, description="Generate briefs for specific agents only")
    force_refresh: bool = Field(False, description="Force regeneration of cached information")
...
class BeliefModelRequest(BaseModel):
    ...
    category_filter: Optional[List[str]] = Field(None, description="Filter by knowledge categories")
    reliability_threshold: float = Field(0.0, description="Minimum reliability threshold")

class BeliefModelData(BaseModel):
    """Agent belief model data"""
    agent_id: str = Field(..., description="Agent identifier")
    personality_bias: Dict[str, float] = Field(..., description="Personality biases affecting perception")
    trust_network: Dict[str, float] = Field(..., description="Trust levels for other agents")
    information_fragments: List[InformationFragmentResponse] = Field(..., description="Known information fragments")
    active_hypotheses: Dict[str, float] = Field(..., description="Active hypotheses and confidence levels")
    cognitive_filters: Dict[str, float] = Field(..., description="Cognitive filtering preferences")
    total_fragments: int = Field(..., description="Total information fragments")
    average_reliability: float = Field(..., description="Average information reliability")
...
class BeliefUpdateRequest(BaseModel):
    ...
    new_information: List[Dict[str, Any]] = Field(..., description="New information to add")
    trust_updates: Optional[Dict[str, float]] = Field(None, description="Trust level updates")
    personality_adjustments: Optional[Dict[str, float]] = Field(None, description="Personality bias adjustments")
...
class SubjectiveRealityAPI:
    ...
    def setup_routes(self, app: FastAPI):
        ...
        @app.get("/api/v1/turns/{turn_id}/briefs/{agent_id}",
        ...
        async def get_turn_brief(
            ...
            try:
                if not self.orchestrator or not hasattr(self.orchestrator, 'director'):
                    raise HTTPException(status_code=503, detail="Turn execution engine not available")
                
                # Get turn engine from orchestrator
                turn_engine = getattr(self.orchestrator.director, 'turn_engine', None)
                if not turn_engine:
                    raise HTTPException(status_code=503, detail="Turn execution engine not initialized")
        ...
        @app.get("/api/v1/turns/{turn_id}/briefs",
        ...
        @app.get("/api/v1/agents/{agent_id}/beliefs",
        ...
        async def get_agent_beliefs(
            ...
            try:
                if not self.orchestrator or not self.orchestrator.subjective_reality_engine:
                    raise HTTPException(status_code=503, detail="Subjective reality engine not available")
                
                # Get belief model from subjective reality engine
                belief_model = await self.orchestrator.subjective_reality_engine.get_agent_belief_model(agent_id)
                
                if not belief_model:
                    raise HTTPException(status_code=404, detail=f"Belief model not found for agent {agent_id}")
