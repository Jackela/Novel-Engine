...
async def create_or_resume_guest_session(
    ...
    # CodeQL: py/cookie-injection - Safe: workspace.id is generated by store.create() and validated by _validate_workspace_id
    # which enforces strict regex pattern (32 hex chars). This is not user-controlled input.
    validated_id = _validate_workspace_id(workspace.id)
    response.set_cookie(
        manager.cookie_name,
        manager.encode(validated_id),
        httponly=True,
