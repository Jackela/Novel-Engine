...
from .response_models import ResponseStatus, APIErrorType
...
class IntegrationTestFramework:
    ...
    async def _test_health_check(self) -> TestResult:
        ...
        async with httpx.AsyncClient() as client:
            ...
            detailed_response = await client.get(f"{self.base_url}/api/v1/health/detailed")
    ...
    async def _test_character_crud(self) -> TestResult:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Read character
            read_response = await client.get(f"{self.base_url}/api/v1/characters/{character_id}")
    ...
    async def _test_error_handling(self) -> TestResult:
        ...
        async with httpx.AsyncClient() as client:
            # Test 404 error
            response_404 = await client.get(f"{self.base_url}/api/v1/characters/nonexistent")
    ...
    async def _test_concurrent_requests(self) -> TestResult:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for i in range(5):
                ...
                task = client.post(f"{self.base_url}/api/v1/characters", json=character_data)
                tasks.append(task)
            ...
            successful_responses = [r for r in responses if hasattr(r, 'status_code') and r.status_code in [200, 201]]
    ...
    async def _test_security_validation(self) -> TestResult:
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.get(f"{self.base_url}/api/v1/characters/{malicious_id}")
...
async def test_integration_suite():
    ...
    assert report["failed_tests"] == 0, f"Integration tests failed: {report['failed_tests']} failures"
    assert report["success_rate"] >= 90, f"Success rate too low: {report['success_rate']}%"
...
__all__ = [
    'TestScenario', 'PerformanceExpectation', 'TestResult',
    'IntegrationTestFramework', 'test_integration_suite'
]