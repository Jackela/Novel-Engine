...
class TurnExecutionEngine:
    ...
    def __init__(
        ...
        self.turn_brief_factory = (
            self.subjective_reality_engine.turn_brief_factory
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.logger.info(
                "Initializing TurnExecutionEngine with core engines"
            )
            ...
            self.logger.info(
                "SubjectiveRealityEngine and EmergentNarrativeEngine initialized"
            )
            self._initialized = True
            return True
        except Exception as e:
            self.logger.error(
                f"TurnExecutionEngine initialization failed: {e}"
            )
            return False
    ...
    async def execute_turn(
        ...
        try:
            self.logger.debug(
                f"Executing simulation turn #{self.current_turn_number}"
            )
    ...
    def _initialize_enhanced_turn_result(
    ...
    async def _generate_turn_briefs(
        self, turn_result: Dict[str, Any], turn_data: Optional[Dict[str, Any]]
    ) -> None:
        ...
        try:
            ...
            for agent_id, agent in self.agent_manager.agents.items():
                try:
                    ...
                    self.logger.debug(
                        f"Generated turn brief for agent {agent_id}"
                    )

                except Exception as e:
                    self.logger.error(
                        f"Failed to generate brief for agent {agent_id}: {e}"
                    )
    ...
    async def _execute_all_agents_with_briefs(
        self, turn_result: Dict[str, Any], turn_data: Optional[Dict[str, Any]]
    ) -> None:
        """Execute turn for all agents with their personalized briefs."""
        for agent_id, agent in self.agent_manager.agents.items():
            await self._execute_single_agent_with_brief(
                turn_result, agent_id, agent, turn_data
            )

    async def _execute_all_agents(
        self, turn_result: Dict[str, Any], turn_data: Optional[Dict[str, Any]]
    ) -> None:
        """Execute turn for all agents (legacy method)."""
        await self._execute_all_agents_with_briefs(turn_result, turn_data)

    async def _execute_single_agent_with_brief(
        self,
        turn_result: Dict[str, Any],
        agent_id: str,
        agent: Any,
        turn_data: Optional[Dict[str, Any]],
    ) -> None:
        ...
        try:
            ...
            agent_result = await self._execute_agent_turn_with_brief(
                agent, agent_brief, turn_data
            )
            turn_result["agent_results"][agent_id] = agent_result

            # Record causal events for narrative engine
            await self._record_causal_events(
                agent_id, agent_result, turn_result
            )
    ...
    def _collect_agent_changes(
        self, turn_result: Dict[str, Any], agent_result: Dict[str, Any]
    ) -> None:
    ...
    async def _record_causal_events(
        self,
        agent_id: str,
        ...
        try:
            if agent_result.get("success", False):
                ...
                event_data = {
                    ...
                    "world_state_changes": agent_result.get(
                        "world_state_changes", {}
                    ),
            ...
            self.logger.error(
                f"Failed to record causal event for {agent_id}: {e}"
            )

    async def _process_narrative_coherence(
        ...
        try:
            ...
            coherence_result = (
                await self.emergent_narrative_engine.analyze_turn_coherence(
                    turn_number=self.current_turn_number,
            ...
            predictions = (
                await self.emergent_narrative_engine.predict_next_events(
    ...
    def _finalize_enhanced_turn_result(
        self, turn_result: Dict[str, Any], start_time: datetime
    ) -> None:
        """Finalize turn result with enhanced metrics including subjective reality data."""
        execution_time = (datetime.now() - start_time).total_seconds()
        successful_agents = sum(
            1
            for r in turn_result["agent_results"].values()
            if r.get("success", False)
        )
        successful_briefs = sum(
            1 for b in turn_result["agent_briefs"].values() if "error" not in b
        )
        ...
        turn_result["subjective_metrics"] = {
            ...
            "brief_success_rate": (
                successful_briefs / len(self.agent_manager.agents)
                if self.agent_manager.agents
                else 0
            ),
            "causal_relationships_tracked": len(turn_result["causal_events"]),
            "narrative_coherence_score": turn_result.get(
                "narrative_coherence", {}
        ...
        self.logger.info(
            f"Turn #{self.current_turn_number}executed in {execution_time:.3f}s: "
    ...
    async def _execute_agent_turn_with_brief(
        self,
        agent: Any,
        ...
        try:
            ...
            if hasattr(agent, "make_decision_with_brief"):
                ...
                result = await agent.make_decision_with_brief(
                    agent_brief, turn_data
                )
                ...
                result = {
                    ...
                    "confidence": agent_brief.get("confidence_levels", {}).get(
                        "overall", 0.7
                    ),
                }
            ...
            return {
                ...
                "brief_used": agent_brief.get(
                    "brief_id", f"brief_{agent_id}_{self.current_turn_number}"
                ),
            }
    ...
    async def _execute_agent_turn(
        self, agent: Any, turn_data: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        return await self._execute_agent_turn_with_brief(
            agent, minimal_brief, turn_data
        )

    async def get_agent_turn_brief(
        self,
        agent_id: str,
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get turn brief for agent {agent_id}: {e}"
            )
            return {
                "success": False,
                "error": str(e),
    ...
    async def get_all_agent_briefs(
        self,
        turn_number: Optional[int] = None,
        world_state: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
    ...
    async def get_causal_graph_data(self) -> Dict[str, Any]:
        ...
        try:
            return (
                await self.emergent_narrative_engine.get_causal_graph_summary()
            )
        except Exception as e:
