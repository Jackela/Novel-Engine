...
class TurnExecutionEngine:
    ...
    async def _execute_single_agent_with_brief(
        ...
        try:
            ...
            turn_result["agent_results"][agent_id] = {"success": False, "error": str(e)}
    ...
    def _collect_agent_changes(
        ...
        if "narrative_events" in agent_result:
            turn_result["narrative_events"].extend(agent_result["narrative_events"])

    async def _record_causal_events(
        self, agent_id: str, agent_result: Dict[str, Any], turn_result: Dict[str, Any]
    ) -> None:
    ...
    def _finalize_enhanced_turn_result(
        ...
        successful_agents = sum(
            1 for r in turn_result["agent_results"].values() if r.get("success", False)
        )
        ...
        turn_result["subjective_metrics"] = {
            ...
            "narrative_coherence_score": turn_result.get("narrative_coherence", {}).get(
                "coherence_score", 0.0
            ),
        }
