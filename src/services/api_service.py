...
class ApiOrchestrationService:
    ...
    def __init__(self, orchestrator: SystemOrchestrator, event_bus: EventBus, character_factory: Optional[CharacterFactory] = None):
    ...
    async def _run_orchestration_loop(self, character_names: List[str], total_turns: int):
        """
        ...
        try:
            ...
            for name in character_names:
                ...
                agents.append(agent)
                await self._broadcast_sse("character", f"Agent Created: {name}", f"Initialized {name}", "low")
            ...
            await self._broadcast_sse("system", "Orchestration Started", "Beginning story generation", "medium")
            ...
            for turn_num in range(1, total_turns + 1):
                ...
                await self._broadcast_sse("story", f"Turn {turn_num}/{total_turns}", f"Processing turn {turn_num}", "low")
                ...
                self._state["average_processing_time"] = sum(turn_times) / len(turn_times)
                self._narrative["turns_completed"] = turn_num
                
                await self._broadcast_sse("story", f"Turn {turn_num} Completed", f"Finished in {duration:.2f}s", "low")

            # 5. Finalize
            self._state["status"] = "completed" if not self._stop_flag else "stopped"
            
            await self._broadcast_sse("story", "Generating Narrative", "Finalizing story...", "medium")
            
            # Generate Narrative
            chronicler = ChroniclerAgent(self.event_bus, character_names=character_names)
            ...
            await self._broadcast_sse("story", "Narrative Complete", "Story generation finished", "high")
    ...
    async def _broadcast_sse(self, event_type: str, title: str, description: str, severity: str):
