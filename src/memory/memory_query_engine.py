...
class MemoryQueryEngine:
    ...
    async def execute_query(self, query_text: str, 
        ...
        try:
            ...
            logger.info(f"Query executed: '{query_text[:50]}...' in {query_duration_ms:.2f}ms")
            
            return StandardResponse(
                success=True,
                data={
                    "query_result": enhanced_result, "query_metrics": metrics, "query_type": query_type.value
                }
            )
            ...
            return StandardResponse(
                success=False,
                error=ErrorInfo(code="QUERY_FAILED", message=f"Query execution failed: {e}")
            )
    ...
    async def execute_associative_query(self, seed_memory_id: str,
        ...
        try:
            ...
            if not seed_memories:
                return StandardResponse(success=False, error=ErrorInfo(code="SEED_NOT_FOUND", message="Seed memory not found"))
            ...
            for depth in range(association_depth):
                ...
                for memory in current_level:
                    ...
                    associations = await self._find_contextual_associations(memory, max_results)
            ...
            logger.info(f"Associative query found {len(result.memories)} associations for {seed_memory_id}")
            
            return StandardResponse(success=True, data={"query_result": result})
            
        except Exception as e:
            logger.error(f"Associative query failed: {e}", exc_info=True)
            return StandardResponse(success=False, error=ErrorInfo(code="ASSOCIATIVE_QUERY_FAILED", message=str(e)))
    ...
    async def execute_temporal_analysis(self, timeframe_days: int = 30,
        ...
        try:
            ...
            temporal_query = MemoryQueryRequest(
                query_text="", temporal_range=(start_time, end_time), max_results=1000, relevance_threshold=0.1
            )
            ...
            analysis_func = analysis_map.get(analysis_type, self._analyze_general_patterns)
            analysis_result = analysis_func(memories)
            
            logger.info(f"Temporal analysis '{analysis_type}' completed for the last {timeframe_days} days.")
            
            return StandardResponse(
                success=True,
                data={
                    "analysis_result": analysis_result, "timeframe_days": timeframe_days,
                    "analysis_type": analysis_type, "memories_analyzed": len(memories)
                }
            )
            
        except Exception as e:
            logger.error(f"Temporal analysis failed: {e}", exc_info=True)
            return StandardResponse(success=False, error=ErrorInfo(code="TEMPORAL_ANALYSIS_FAILED", message=str(e)))
    ...
    async def _process_temporal_query(self, query_text: str, context: QueryContext) -> StandardResponse:
        ...
        temporal_range = self._extract_temporal_range(query_text) or self._resolve_temporal_focus(context.temporal_focus)
        
        query_request = MemoryQueryRequest(
            query_text=query_text, temporal_range=temporal_range, max_results=25,
            relevance_threshold=context.min_relevance, participants=context.active_participants
        )
        return await self.layered_memory.query_memories(query_request)
    ...
    async def _process_emotional_query(self, query_text: str, context: QueryContext) -> StandardResponse:
        ...
        query_request = MemoryQueryRequest(
            query_text=query_text, memory_types=[MemoryType.EMOTIONAL, MemoryType.EPISODIC],
            emotional_filters=emotional_filters, max_results=20, relevance_threshold=context.min_relevance
        )
        return await self.layered_memory.query_memories(query_request)
    ...
    async def _process_semantic_query(self, query_text: str, context: QueryContext) -> StandardResponse:
        ...
        query_request = MemoryQueryRequest(
            query_text=query_text, memory_types=[MemoryType.SEMANTIC], max_results=15,
            relevance_threshold=max(0.5, context.min_relevance), include_working_memory=False
        )
        return await self.layered_memory.query_memories(query_request)
    ...
    async def _process_contextual_query(self, query_text: str, context: QueryContext) -> StandardResponse:
        ...
        query_request = MemoryQueryRequest(
            query_text=enhanced_query, participants=context.active_participants, max_results=20,
            relevance_threshold=context.min_relevance
        )
        return await self.layered_memory.query_memories(query_request)
    ...
    def _apply_contextual_ranking(self, result: MemoryQueryResult, 
        ...
        for i, memory in enumerate(result.memories):
            ...
            if context.active_participants and set(memory.participants) & set(context.active_participants):
                score += 0.1
        ...
        sorted_results = sorted(zip(result.memories, enhanced_scores, result.memory_sources), key=lambda x: x[1], reverse=True)
        
        result.memories, result.relevance_scores, result.memory_sources = zip(*sorted_results)
        return result
    ...
    async def _find_contextual_associations(self, memory: MemoryItem,
        ...
        if memory.participants:
            query = MemoryQueryRequest(query_text="", participants=memory.participants, max_results=max_results)
    ...
    def _resolve_temporal_focus(self, temporal_focus: Optional[str]) -> Optional[Tuple[datetime, datetime]]:
    ...
    async def _get_memory_by_id(self, memory_id: str) -> List[MemoryItem]:
        ...
        result = await self.layered_memory.query_memories(query)
        if result.success:
            return [mem for mem in result.data['query_result'].memories if mem.memory_id == memory_id]
        return []
    ...
    def _analyze_emotional_patterns(self, memories: List[MemoryItem]) -> Dict[str, Any]:
        ...
        emotions = [mem.emotional_weight for mem in memories if mem.emotional_weight != 0]
    ...
    def _cleanup_expired_cache(self):
        ...
        expired_keys = [k for k, (ts, _) in self._query_cache.items() if ts < expiry_limit]
    ...
    def _update_query_statistics(self, query_type: QueryType, metrics: QueryMetrics):
        ...
        self._query_statistics['average_query_time'] = total_duration / total_queries if total_queries > 0 else 0
...
async def test_memory_query_engine():
    ...
    test_memory = MemoryItem(
        agent_id="test_agent", content="A test event happened yesterday.",
        memory_type=MemoryType.EPISODIC, emotional_weight=2.0
    )
