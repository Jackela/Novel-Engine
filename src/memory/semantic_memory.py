...
class KnowledgeFact:
    ...
    def __post_init__(self):
        ...
        if not self.fact_id:
            fact_hash = hashlib.sha1(f"{self.subject}{self.predicate}{self.object_value}".encode()).hexdigest()[:8]
...
class SemanticMemory:
    ...
    async def extract_and_store_knowledge(self, memory: MemoryItem) -> StandardResponse:
        ...
        """
        try:
            extracted_facts = self._extract_facts_from_content(memory.content, memory.memory_id, memory.relevance_score)
            ...
            logger.info(f"Extracted {stored_count} facts from memory {memory.memory_id}")
            ...
            return StandardResponse(success=False, error=ErrorInfo(code="KNOWLEDGE_EXTRACTION_FAILED", message=str(e)))
    
    def _extract_facts_from_content(self, content: str, memory_id: str, relevance: float) -> List[KnowledgeFact]:
        ...
        for pattern, predicate in self._fact_patterns:
            for match in re.findall(pattern, content, re.IGNORECASE):
                ...
                facts.append(KnowledgeFact(
                    fact_id="", subject=subject.strip(), predicate=predicate,
                    object_value=object_value.strip(), source_memory_id=memory_id,
    ...
    async def query_facts_by_subject(self, subject: str, 
        ...
        try:
            ...
            matching_facts = [self._facts[fid] for fid in fact_ids if self._facts[fid].confidence >= threshold]
            ...
            return StandardResponse(success=False, error=ErrorInfo(code="SUBJECT_FACT_RETRIEVAL_FAILED", message=str(e)))
    ...
    async def query_facts_by_predicate(self, predicate: str,
        ...
        try:
            ...
            matching_facts = [self._facts[fid] for fid in fact_ids if self._facts[fid].confidence >= self.confidence_threshold]
            ...
            return StandardResponse(success=False, error=ErrorInfo(code="PREDICATE_FACT_RETRIEVAL_FAILED", message=str(e)))
    ...
    async def get_concept_knowledge(self, concept_name: str) -> StandardResponse:
        ...
        try:
            ...
            if not concept:
                return StandardResponse(success=False, error=ErrorInfo(code="CONCEPT_NOT_FOUND", message=f"Concept '{concept_name}' not found"))
            
            associated_facts = [self._facts[fid].to_natural_language() for fid in concept.associated_facts if fid in self._facts]
            ...
            return StandardResponse(success=False, error=ErrorInfo(code="CONCEPT_KNOWLEDGE_RETRIEVAL_FAILED", message=str(e)))
    ...
    async def _ensure_concept_exists(self, entity_name: str) -> ConceptNode:
        ...
        if entity_lower not in self._concepts:
            self._concepts[entity_lower] = ConceptNode(concept_id=entity_lower, concept_name=entity_name)
    ...
    def _extract_entities(self, content: str) -> List[str]:
        ...
        return [word for word in re.findall(r'\b[A-Z][a-z]+\b', content) if len(word) > 2]
...
async def test_semantic_memory():
    ...
    print(f"Knowledge extraction successful: {result.success}, Facts extracted: {result.data.get('facts_extracted')}")
    
    facts_result = await semantic_memory.query_facts_by_subject("sky")
    print(f"Query for 'sky' facts successful: {facts_result.success}, Facts: {facts_result.data.get('facts')}")
