...
from src.core.data_models import MemoryItem, StandardResponse, ErrorInfo
...
class WorkingMemory:
    ...
    def __init__(self, agent_id: AgentID, 
        ...
        logger.info(f"WorkingMemory initialized for {agent_id} with capacity {self.capacity}")
    ...
    def add_memory(self, memory: MemoryItem, attention_weight: float = 1.0) -> StandardResponse:
        ...
        try:
            ...
            working_item = WorkingMemoryItem(
                memory_item=memory, attention_weight=attention_weight,
            ...
            if len(self._items) >= self.capacity:
                ...
                if evicted_item:
                    logger.info(f"Evicted item from working memory: {evicted_item.memory_item.memory_id}")
            
            self._items.append(working_item)
            self._priority_index[memory.memory_id] = working_item
            self.total_activations += 1
            
            logger.info(f"Added to working memory: {memory.memory_id} (size: {len(self._items)})")
            
            return StandardResponse(success=True, data={"added": True, "current_size": len(self._items)})
            
        except Exception as e:
            logger.error(f"Failed to add to working memory: {e}", exc_info=True)
            return StandardResponse(success=False, error=ErrorInfo(code="ADD_FAILED", message=str(e)))
    ...
    def get_active_memories(self, limit: Optional[int] = None) -> List[MemoryItem]:
        ...
        sorted_items = sorted(self._items, key=lambda item: item.effective_priority, reverse=True)
    ...
    def focus_on_memory(self, memory_id: str, attention_boost: float = 0.3) -> StandardResponse:
        ...
        try:
            ...
            if not working_item:
                return StandardResponse(success=False, error=ErrorInfo(code="NOT_FOUND", message="Memory not in working memory"))
            ...
            return StandardResponse(success=True, data={"focused": True, "new_priority": working_item.effective_priority})
            
        except Exception as e:
            logger.error(f"Failed to focus on memory: {e}", exc_info=True)
            return StandardResponse(success=False, error=ErrorInfo(code="FOCUS_FAILED", message=str(e)))
    ...
    def perform_maintenance(self) -> StandardResponse:
        ...
        try:
            ...
            for item in items_to_remove:
                ...
                logger.info(f"Item decayed from working memory: {item.memory_item.memory_id}")
            ...
            return StandardResponse(
                success=True,
                data={"items_removed": len(items_to_remove), "current_size": len(self._items)}
            )
            
        except Exception as e:
            logger.error(f"Working memory maintenance failed: {e}", exc_info=True)
            return StandardResponse(success=False, error=ErrorInfo(code="MAINTENANCE_FAILED", message=str(e)))
