...
from ..events.event_bus import Event, EventBus, EventPriority
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "command_type": (
                ...
                if isinstance(self.command_type, CommandType)
...
class CommandHandler(ABC, Generic[T]):
    ...
    async def compensate(
        self, command: T, original_result: CommandResult
    ) -> CommandResult:
        """
...
class IdempotencyManager:
    ...
    async def cleanup_old_records(self, max_age_hours: int = 24):
        ...
        expired_keys = [
            key
            for key, timestamp in self._command_timestamps.items()
...
class CommandBus:
    ...
    def unregister_handler(self, handler: CommandHandler):
        ...
        for command_type in handler.handled_command_types:
            ...
            if (
                command_type_str in self._handlers
                and self._handlers[command_type_str] == handler
            ):
        ...
        async with self._processing_semaphore:
            ...
            try:
                ...
                if (
                    ...
                    if existing_result:
                        logger.info(
                            f"Returning cached result for idempotent command {command.command_id}"
                        )
                ...
                if not handler:
                    raise CommandExecutionError(
                        f"No handler registered for command type: {command_type_str}"
                    )
                ...
                if not await handler.authorize(command):
                    raise CommandAuthorizationError(
                        f"Command authorization failed: {command.command_id}"
                    )

                # Execute command
                command.status = CommandStatus.EXECUTING
                result = await asyncio.wait_for(
                    handler.handle(command), timeout=command.timeout_seconds
                )
    ...
    async def execute_saga(
        self, saga_id: str, commands: List[Command]
    ) -> List[CommandResult]:
        """
        ...
        try:
            for command in commands:
                ...
                if not result.success:
                    logger.warning(
                        f"Saga {saga_id} failed at command {command.command_id}, starting compensation"
                    )
    ...
    def _update_metrics(self, result: CommandResult):
        ...
        self._metrics["average_execution_time"] = (
            current_avg * (total_commands - 1) + result.execution_time_ms
        ) / total_commands
