...
class DirectorAgentBase:
    ...
    def __init__(self, event_bus: EventBus, world_state_file_path: Optional[str] = None, 
        ...
        self._setup_file_paths(world_state_file_path, campaign_log_path, campaign_brief_path)
    ...
    def _setup_file_paths(self, world_state_file_path: Optional[str], 
                         campaign_log_path: Optional[str], campaign_brief_path: Optional[str]) -> None:
    ...
    def register_agent(self, agent: PersonaAgent) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully registered agent '{agent.agent_id}' (total: {len(self.registered_agents)})")
    ...
    def remove_agent(self, agent_id: str) -> bool:
        ...
        try:
            ...
            self.registered_agents = [agent for agent in self.registered_agents if agent.agent_id != agent_id]
            
            if len(self.registered_agents) < initial_count:
                logger.info(f"Successfully removed agent '{agent_id}' (remaining: {len(self.registered_agents)})")
                return True
            else:
    ...
    def get_simulation_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                'simulation_status': 'running' if self.registered_agents else 'initialized',
    ...
    def _handle_error(self, error_message: str, exception: Optional[Exception] = None) -> None:
        """
        ...
        if exception:
            logger.error(f"DirectorAgent error: {error_message} - Exception: {str(exception)}")
        else:
    ...
    def is_initialized(self) -> bool:
        ...
        try:
            return (
                ...
                self.registered_agents is not None and
                self.simulation_start_time is not None and
                self.narrative_resolver is not None
            )
        except:
            return False