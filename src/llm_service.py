...
class UnifiedLLMService:
    ...
    def __init__(self, cost_control: Optional[CostControl] = None):
        ...
        logger.info("Unified LLM Service initialized with Gemini 2.0 Flash primary provider")
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            if request.provider == LLMProvider.GEMINI:
                ...
                raise Exception(f"Provider {request.provider.value} not implemented yet")
            
            # Validate response format
            format_validated = self._validate_response_format(content, request.response_format)
            ...
            logger.info(f"LLM request {request_id} completed: {response_time_ms}ms, ${cost_estimate:.4f}")
            return response
    ...
    async def _call_gemini(self, request: LLMRequest, provider_config: Dict[str, Any]) -> str:
        """
        ...
        request_body = {
            ...
            "generationConfig": {
                ...
                "maxOutputTokens": min(request.max_tokens, provider_config.get('max_tokens', 8192))
            }
        }
    ...
    def _validate_response_format(self, content: str, format_type: ResponseFormat) -> bool:
        """
        ...
        try:
            if format_type == ResponseFormat.ACTION_FORMAT:
                ...
                has_reasoning = bool(re.search(r'REASONING:\s*(.+)', content, re.IGNORECASE))
            ...
            elif format_type in [ResponseFormat.NARRATIVE_FORMAT, ResponseFormat.CLUE_TEXT, ResponseFormat.DIALOGUE_TEXT]:
    ...
    def _get_cached_response(self, request: LLMRequest) -> Optional[LLMResponse]:
        ...
        if cached_data:
            ...
            if (datetime.now() - cache_time).total_seconds() < self.cost_control.cache_ttl:
    ...
    def _cache_response(self, request: LLMRequest, response: LLMResponse) -> None:
        ...
        if len(self._request_cache) > 1000:
            ...
            sorted_keys = sorted(self._request_cache.keys(), 
    ...
    def _update_metrics(self, response: LLMResponse, success: bool) -> None:
        ...
        if success:
            ...
            # Update daily costs
            today = datetime.now().date().isoformat()
            self._daily_costs[today] = self._daily_costs.get(today, 0.0) + response.cost_estimate
            
            # Update average response time
            total_time = self.metrics.average_response_time * (self.metrics.successful_requests - 1)
            self.metrics.average_response_time = (total_time + response.response_time_ms) / self.metrics.successful_requests
    ...
    async def generate_action(self, prompt: str, requester: str = "agent") -> LLMResponse:
    ...
    async def generate_narrative(self, prompt: str, style: str = "dramatic", requester: str = "chronicler") -> LLMResponse:
    ...
    async def generate_clue(self, target: str, action_type: str, requester: str = "director") -> LLMResponse:
    ...
    async def generate_dialogue(self, character_name: str, personality: Dict[str, float], 
                               emotion: str, context: Dict[str, Any], 
                               requester: str = "character") -> LLMResponse:
...
Output only the dialogue line:"""
    ...
    async def generate_event(self, event_type: str, characters: List[str], 
                           story_context: Dict[str, Any], plot_stage: str,
                           requester: str = "orchestrator") -> LLMResponse:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cost": {
                ...
                "budget_remaining": max(0, self.cost_control.daily_budget - self.metrics.daily_spend)
            },
            "providers": {
                "available": [p.value for p, config in self.providers.items() if config['available']],
...
def create_llm_service_for_testing(mock_responses: Optional[Dict[str, str]] = None) -> UnifiedLLMService:
