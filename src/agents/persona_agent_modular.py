...
import logging
import asyncio
from typing import Dict, List, Optional, Any  # Removed unused Union import
from datetime import datetime
from dataclasses import asdict

# Import all modular components
from .persona_agent.protocols import (
    CharacterDataManagerProtocol, AgentStateManagerProtocol, DecisionEngineProtocol,
    WorldInterpretationProtocol as WorldInterpreterProtocol,
    MemoryManagerProtocol, LLMIntegrationProtocol
)

from .persona_agent.core import CharacterDataManager, AgentStateManager
from .persona_agent.decision_engine import DecisionProcessor, ThreatAssessor, GoalManager
from .persona_agent.world_interpretation import WorldInterpreter, MemoryManager
from .persona_agent.llm_integration import LLMClient, ResponseProcessor
from .persona_agent.utilities import Validator, ResponseGenerator

# Import shared types with fallbacks
try:
    from shared_types import ProposedAction as CharacterAction, ActionPriority
except ImportError:
...
class PersonaAgent:
    ...
    def __init__(self, character_id: str, character_directory_path: str = None, 
                 character_data: Dict[str, Any] = None, logger: Optional[logging.Logger] = None):
        """
        ...
        self.logger.info(f"PersonaAgent {character_id} initialized with modular architecture")
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            self.llm_client = LLMClient(
                self.character_id,
                logger=self.logger.getChild("llm")
            )
            
            self.response_processor = ResponseProcessor(
                self.character_id,
                logger=self.logger.getChild("response")
            )
            ...
            self.response_generator = ResponseGenerator(
                self.character_id,
                logger=self.logger.getChild("generator")
            )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self.character_directory_path and not self._character_data:
                self._character_data = await self.character_data_manager.load_character_data(
                    self.character_directory_path
                )
            
            # Validate character data
            if self._character_data:
                validation_result = await self.validator.validate_character_data(self._character_data)
                if not validation_result.is_valid:
                    self.logger.warning(f"Character data validation issues: {validation_result.issues}")
            ...
            self._integration_stats["initialization_time"] = (datetime.now() - start_time).total_seconds()
            
            self.logger.info(f"PersonaAgent {self.character_id} initialization completed successfully")
            return True
        ...
        try:
            ...
            if recent_events:
                for event_data in recent_events:
                    ...
                    interpretation = await self.world_interpreter.interpret_event(event, character_context)
            ...
            if recent_events:
                ...
                try:
                    ...
                    threat_assessment = await self.threat_assessor.assess_threat(event, character_context)
            ...
            decision = await self.decision_processor.make_decision(interpreted_state, character_context)
            
            # Ensure decision has required fields
            if isinstance(decision, dict):
                if 'description' not in decision:
                    decision['description'] = f"Character action: {decision.get('action_type', 'unknown')}"
                ...
                decision_dict = asdict(decision) if hasattr(decision, '__dataclass_fields__') else {'action_type': 'wait'}
                if 'description' not in decision_dict:
                    decision_dict['description'] = f"Character action: {decision_dict.get('action_type', 'wait')}"
            ...
            decision_dict = asdict(decision) if hasattr(decision, '__dataclass_fields__') else decision
            validation_result = await self.validator.validate_action(decision_dict, interpreted_state)
            if not validation_result.is_valid:
                self.logger.warning(f"Decision validation issues: {validation_result.issues}")
            ...
            # Update statistics
            self._integration_stats["last_decision_time"] = (datetime.now() - decision_start).total_seconds()
            self._integration_stats["total_decisions"] += 1
            
            self.logger.debug(f"Decision made in {self._integration_stats['last_decision_time']:.3f}s: {decision.get('action_type')}")
    ...
    async def generate_response(self, prompt: str, context: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            try:
                llm_response = await self.llm_client.generate_character_response(prompt, character_context)
            ...
            fallback_response = await self.response_generator.generate_response(character_context)
    ...
    async def process_world_events(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    async def get_active_goals(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [asdict(goal) if hasattr(goal, '__dict__') else goal for goal in goals]
        except Exception as e:
        ...
        try:
            ...
            for component_name, status in stats["component_status"].items():
                ...
                if component and hasattr(component, 'get_statistics'):
                    try:
                        component_stats[component_name] = await component.get_statistics()
                    except:
            ...
            if hasattr(self.validator, 'get_validation_statistics'):
                stats["validation_statistics"] = await self.validator.get_validation_statistics()
            
            # Add LLM usage statistics
            if hasattr(self.llm_client, 'get_usage_statistics'):
                stats["llm_usage_statistics"] = await self.llm_client.get_usage_statistics()
    ...
    async def _build_character_context(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add recent memories
            try:
                recent_memories = await self.memory_manager.retrieve_memories({'type': 'recent'}, limit=5)
...
async def create_persona_agent(character_id: str, character_directory_path: str = None, 
                              character_data: Dict[str, Any] = None, 
                              logger: Optional[logging.Logger] = None) -> PersonaAgent:
    """
