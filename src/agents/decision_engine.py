...
class DecisionEngine:
    ...
    def _assess_current_situation(
        ...
        current_location = (
            world_state_update.get("current_location")
            or self.core.state.current_location
        )
    ...
    def _evaluate_action(
        ...
        modified_score = self._apply_situational_modifiers(
            modified_score, action, situation
        )
    ...
    def _apply_personality_modifiers(
        self, base_score: float, action: Dict[str, Any]
    ) -> float:
    ...
    def _apply_situational_modifiers(
        ...
        if "combat" in action_type:
            combat_resources = situation.available_resources.get(
        ...
        if situation.current_location:
            location_modifiers = self._get_location_modifiers(
                situation.current_location, action_type
            )
    ...
    def _select_best_action(
        ...
        top_actions = [
            ...
            if e.modified_score >= evaluations[0].modified_score * 0.8
        ]
    ...
    def _get_goal_actions(
        self, goal: Dict[str, Any], situation: SituationAssessment
    ) -> List[Dict[str, Any]]:
