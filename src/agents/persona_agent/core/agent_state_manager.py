...
from ..protocols import AgentStateManagerProtocol
...
class AgentStateManager:
    ...
    def __init__(self, agent_id: str, logger: Optional[logging.Logger] = None):
        ...
        self._valid_statuses = {
            "active", "injured", "unconscious", "dead", "resting", 
            "stunned", "incapacitated", "fleeing", "hiding"
        }
        
        self._valid_health_statuses = {
            "healthy", "injured", "critical", "dead", "recovering"
        }
    ...
    async def update_state(self, state_updates: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for key, value in state_updates.items():
                if hasattr(self._state, key):
                    ...
                    if key == "current_status" and value not in self._valid_statuses:
                        self.logger.warning(f"Invalid status '{value}', using current status")
                        continue
                    
                    if key == "health_status" and value not in self._valid_health_statuses:
                        self.logger.warning(f"Invalid health status '{value}', using current status")
                        continue
            ...
            await self._record_state_change(old_state, asdict(self._state), state_updates)
    ...
    async def load_state(self, file_path: str) -> bool:
        ...
        try:
            ...
            if state_data.get("agent_id") != self.agent_id:
                self.logger.error(f"Agent ID mismatch: expected {self.agent_id}, got {state_data.get('agent_id')}")
                return False
    ...
    def is_active(self) -> bool:
        ...
        return self._state.current_status == "active" and self._state.health_status != "dead"
    ...
    async def adjust_morale(self, adjustment: float, reason: str = "") -> None:
        ...
        self.logger.info(f"Morale adjusted by {adjustment:+.2f} ({reason}): {new_morale:.2f}")
    ...
    async def set_priority(self, priority_name: str, priority_value: float) -> None:
        ...
        new_priorities = {**self._state.current_priorities, priority_name: priority_value}
        await self.update_state({"current_priorities": new_priorities})
    
    async def add_temporary_modifier(self, modifier_name: str, modifier_data: Any, duration: Optional[int] = None) -> None:
        ...
        new_modifiers = {**self._state.temporary_modifiers, modifier_name: modifier_info}
    ...
    async def _record_state_change(self, old_state: Dict[str, Any], new_state: Dict[str, Any], updates: Dict[str, Any]) -> None:
        """Record state change in history."""
        try:
            change_record = {
                "timestamp": datetime.now().isoformat(),
