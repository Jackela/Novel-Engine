...
from ..protocols import CharacterDataManagerProtocol
...
class CharacterDataManager:
    ...
    async def load_character_data(self, character_directory_path: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not character_path.exists():
                raise FileNotFoundError(f"Character directory not found: {character_directory_path}")
            ...
            self.logger.info(f"Character data loaded successfully from {character_directory_path}")
            return character_data
            
        except Exception as e:
            self.logger.error(f"Failed to load character data from {character_directory_path}: {e}")
            raise
                ...
                for attr in numeric_attrs:
                    if attr in attributes:
                        try:
                            ...
                            self.logger.warning(f"Invalid numeric value for {attr}: {attributes[attr]}")
            ...
            for weight_name, weight_value in decision_weights.items():
                try:
                    ...
                    if not -1.0 <= weight_val <= 1.0:
                        self.logger.warning(f"Decision weight {weight_name} out of range [-1.0, 1.0]: {weight_val}")
                except (ValueError, TypeError):
                    self.logger.warning(f"Invalid decision weight value for {weight_name}: {weight_value}")
    ...
    def get_faction(self) -> str:
        ...
        return self._character_data.get("faction_info", {}).get("faction", "Independent")
    ...
    def get_decision_weights(self) -> Dict[str, float]:
        ...
        return self._character_data.get("decision_weights", self._get_default_decision_weights())
    ...
    async def _parse_markdown_structure(self, content: str, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for section, pattern in sections.items():
                ...
                if match:
                    ...
                    if section in ["goals"]:
                        ...
                        data[section] = [{"description": goal, "priority": 0.5} for goal in goals]
                    else:
                        data[section] = value
            ...
            if data:
                ...
                if "personality" in data:
                    structured_data["personality"] = {"description": data["personality"]}
                if "goals" in data:
            ...
            self.logger.warning(f"Failed to parse markdown structure for {filename}: {e}")
            return {}
    ...
    def _merge_character_data(self, base_data: Dict[str, Any], new_data: Dict[str, Any]) -> Dict[str, Any]:
