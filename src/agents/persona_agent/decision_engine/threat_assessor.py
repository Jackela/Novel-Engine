...
from ..protocols import WorldEvent, ThreatLevel, SubjectiveInterpretation
...
class ThreatAssessor:
    ...
    async def assess_threat(self, event: WorldEvent, character_data: Dict[str, Any], 
                           world_context: Optional[Dict[str, Any]] = None) -> ThreatAssessment:
        """
        ...
        try:
            ...
            threat_factors = await self._analyze_threat_factors(event, character_data, world_context)
            
            # Calculate overall threat level
            overall_threat = await self._calculate_overall_threat(threat_factors)
            
            # Determine confidence level
            confidence = await self._calculate_assessment_confidence(threat_factors, character_data)
            ...
            self.logger.debug(f"Threat assessment completed: {overall_threat.value} (confidence: {confidence:.2f})")
            return assessment
    ...
    async def assess_environmental_threats(self, location: str, world_state: Dict[str, Any], 
                                         character_data: Dict[str, Any]) -> List[ThreatFactor]:
        """
        ...
        try:
            ...
            for hazard in hazards:
                threat_factor = await self._evaluate_environmental_hazard(hazard, character_data)
                if threat_factor:
                    environmental_threats.append(threat_factor)
            
            # Check weather/climate threats
            weather_threats = await self._assess_weather_threats(location, world_state, character_data)
            environmental_threats.extend(weather_threats)
            
            # Check resource scarcity threats
            resource_threats = await self._assess_resource_threats(location, world_state, character_data)
    ...
    async def assess_political_threats(self, character_data: Dict[str, Any], world_state: Dict[str, Any]) -> List[ThreatFactor]:
        """
        ...
        try:
            ...
            character_faction = character_data.get('faction_info', {}).get('faction', 'Independent')
            ...
            for conflict in factional_conflicts:
                if character_faction in conflict.get('involved_factions', []):
                    threat_factor = await self._evaluate_factional_conflict(conflict, character_data)
                    if threat_factor:
                        political_threats.append(threat_factor)
            
            # Check reputation threats
            reputation_threats = await self._assess_reputation_threats(character_data, world_state)
            political_threats.extend(reputation_threats)
            
            # Check authority threats
            authority_threats = await self._assess_authority_threats(character_data, world_state)
    ...
    async def update_threat_patterns(self, recent_events: List[WorldEvent], outcomes: List[Dict[str, Any]]) -> None:
        """
        ...
        try:
            for event, outcome in zip(recent_events, outcomes):
                ...
                pattern['average_severity'] = (
                    (pattern['average_severity'] * (pattern['occurrences'] - 1) + event_severity) / 
                    pattern['occurrences']
                )
    ...
    async def get_threat_trend(self, time_window_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_assessments = [
                assessment for assessment in self._assessment_history
            ...
            threat_levels = [self._threat_level_to_numeric(a.overall_threat_level) for a in recent_assessments]
    ...
    async def _analyze_threat_factors(self, event: WorldEvent, character_data: Dict[str, Any], 
                                    world_context: Optional[Dict[str, Any]]) -> List[ThreatFactor]:
        ...
        try:
            ...
            indirect_threats = await self._analyze_indirect_threats(event, character_data, world_context)
            threat_factors.extend(indirect_threats)
            
            # Analyze relational threats
            relational_threats = await self._analyze_relational_threats(event, character_data)
            threat_factors.extend(relational_threats)
            
            # Consider character-specific vulnerabilities
            vulnerability_threats = await self._analyze_vulnerability_threats(event, character_data)
    ...
    async def _analyze_direct_threats(self, event: WorldEvent, character_data: Dict[str, Any]) -> List[ThreatFactor]:
        ...
        try:
            ...
            if character_id in event.affected_entities:
                severity = await self._calculate_direct_threat_severity(event, character_data)
                ...
                threat = ThreatFactor(
                    ...
                    evidence=[f"Character {character_id} directly affected by event {event.event_id}"]
                )
    ...
    async def _analyze_indirect_threats(self, event: WorldEvent, character_data: Dict[str, Any],
                                      world_context: Optional[Dict[str, Any]]) -> List[ThreatFactor]:
        ...
        try:
            ...
            if character_location == event.location:
                severity = await self._calculate_indirect_threat_severity(event, character_data)
                certainty = 0.7  # Moderate certainty for location-based threats
                immediacy = await self._calculate_threat_immediacy(event) * 0.8  # Slightly reduced immediacy
                ...
                threat = ThreatFactor(
                    ...
                    evidence=[f"Event at same location as character: {character_location}"]
                )
                threats.append(threat)
            
            # Check factional relationship threats
            character_faction = character_data.get('faction_info', {}).get('faction', 'Independent')
            event_source_faction = world_context.get('entity_factions', {}).get(event.source) if world_context else None
            
            if event_source_faction and character_faction != event_source_faction:
                # Check if factions are hostile
                faction_relations = world_context.get('faction_relations', {}) if world_context else {}
                ...
                if relation_score < -0.3:  # Hostile relations
                    ...
                    threat = ThreatFactor(
                        ...
                        description=f"Indirect political threat from hostile faction",
                        evidence=[f"Hostile relations with faction {event_source_faction}"]
                    )
                    threats.append(threat)
    ...
    async def _calculate_overall_threat(self, threat_factors: List[ThreatFactor]) -> ThreatLevel:
        ...
        try:
            ...
            weighted_scores = [factor.get_weighted_severity() for factor in threat_factors]
    ...
    async def _calculate_direct_threat_severity(self, event: WorldEvent, character_data: Dict[str, Any]) -> float:
    ...
    async def _calculate_indirect_threat_severity(self, event: WorldEvent, character_data: Dict[str, Any]) -> float:
        """Calculate severity of indirect threats."""
        # Indirect threats are typically less severe
        direct_severity = await self._calculate_direct_threat_severity(event, character_data)
    ...
    async def _analyze_relational_threats(self, event: WorldEvent, character_data: Dict[str, Any]) -> List[ThreatFactor]:
        """Analyze threats based on character relationships."""
        return []  # Placeholder
    
    async def _analyze_vulnerability_threats(self, event: WorldEvent, character_data: Dict[str, Any]) -> List[ThreatFactor]:
        """Analyze threats based on character vulnerabilities."""
        return []  # Placeholder
    
    async def _evaluate_environmental_hazard(self, hazard: Dict[str, Any], character_data: Dict[str, Any]) -> Optional[ThreatFactor]:
        """Evaluate a specific environmental hazard."""
        return None  # Placeholder
    
    async def _assess_weather_threats(self, location: str, world_state: Dict[str, Any], character_data: Dict[str, Any]) -> List[ThreatFactor]:
        """Assess weather-related threats."""
        return []  # Placeholder
    
    async def _assess_resource_threats(self, location: str, world_state: Dict[str, Any], character_data: Dict[str, Any]) -> List[ThreatFactor]:
        """Assess resource scarcity threats."""
        return []  # Placeholder
    
    async def _evaluate_factional_conflict(self, conflict: Dict[str, Any], character_data: Dict[str, Any]) -> Optional[ThreatFactor]:
        """Evaluate threat from factional conflict."""
        return None  # Placeholder
    
    async def _assess_reputation_threats(self, character_data: Dict[str, Any], world_state: Dict[str, Any]) -> List[ThreatFactor]:
        """Assess reputation-based threats."""
        return []  # Placeholder
    
    async def _assess_authority_threats(self, character_data: Dict[str, Any], world_state: Dict[str, Any]) -> List[ThreatFactor]:
        """Assess threats from authorities."""
        return []  # Placeholder
    
    async def _calculate_assessment_confidence(self, threat_factors: List[ThreatFactor], character_data: Dict[str, Any]) -> float:
        """Calculate confidence in threat assessment."""
        if not threat_factors:
            return 0.5
        
        # Average certainty of all factors
        avg_certainty = sum(factor.certainty for factor in threat_factors) / len(threat_factors)
        return avg_certainty
    
    async def _identify_primary_threat_source(self, threat_factors: List[ThreatFactor]) -> Optional[str]:
    ...
    async def _record_assessment(self, assessment: ThreatAssessment, event: WorldEvent, character_data: Dict[str, Any]) -> None:
