...
from ..protocols import WorldEvent
...
class Validator:
    ...
    async def validate_character_data(
        ...
        try:
            ...
            if "basic_info" in character_data:
                basic_info_issues = await self._validate_basic_info(
            ...
            if "personality" in character_data:
                personality_issues = await self._validate_personality(
            ...
            if "attributes" in character_data:
                attributes_issues = await self._validate_attributes(
            ...
            if "faction_info" in character_data:
                faction_issues = await self._validate_faction_info(
                    character_data["faction_info"]
                )
            ...
            if "decision_weights" in character_data:
                weights_issues = await self._validate_decision_weights(
                    character_data["decision_weights"]
                )
            ...
            result = ValidationResult(
                ...
                score=score,
                metadata={
                    "sections_validated": len(
                        [s for s in required_sections if s in character_data]
                    ),
    ...
    async def validate_action(
        ...
        try:
            self.logger.debug(
                f"Validating action: {action.get('action_type', 'unknown')}"
            )
            issues = []
            ...
            if "action_type" in action:
                action_type_issues = await self._validate_action_type(
                    action["action_type"]
                )
            ...
            # Feasibility check
            feasibility_issues = await self._validate_action_feasibility(
                action, context
            )
    ...
    async def validate_system_state(self, state: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            if "memory_system" in state:
                memory_issues = await self._validate_memory_system(
                    state["memory_system"]
                )
            ...
            if "decision_system" in state:
                decision_issues = await self._validate_decision_system(
                    state["decision_system"]
                )
            ...
            if "llm_integration" in state:
                llm_issues = await self._validate_llm_integration(
                    state["llm_integration"]
                )
            ...
            result = ValidationResult(
                ...
                score=score,
                metadata={
                    "subsystems_validated": len(
                        [k for k in state.keys() if k.endswith("_system")]
                    ),
                    "validation_timestamp": datetime.now().isoformat(),
                },
            )
    ...
    async def _validate_personality(
    ...
    async def _validate_faction_info(
        self, faction_info: Dict[str, Any]
    ) -> List[ValidationIssue]:
    ...
    async def _validate_decision_weights(
    ...
    async def _validate_against_schema(
        self, data: Dict[str, Any], schema: Dict[str, Any], context: str
    ) -> List[ValidationIssue]:
    ...
    async def _update_validation_stats(self, result: ValidationResult) -> None:
        ...
        try:
            ...
            self._stats["issues_by_type"][result.validation_type.value] += len(
                result.issues
            )
    ...
    async def _validate_action_parameters(
        self, parameters: Dict[str, Any], action_type: Optional[str]
    ) -> List[ValidationIssue]:
        """Validate action parameters."""
        return []  # Placeholder

    async def _validate_action_context(
