...
class ResponseTemplate:
    ...
    def matches_context(self, personality: Dict[str, float], faction_beliefs: Dict[str, float], 
...
class ResponseGenerator:
    ...
    async def generate_response(self, context: Dict[str, Any], category: Optional[ResponseCategory] = None, 
                              tone: Optional[ResponseTone] = None) -> str:
        """
        ...
        """
        try:
            self.logger.debug(f"Generating response for category: {category}, tone: {tone}")
            ...
            matching_templates = await self._find_matching_templates(category, tone, context)
            
            # Generate base response
            if matching_templates:
                base_response = await self._generate_from_template(matching_templates[0], context)
                self._stats["template_used"] += 1
            else:
                base_response = await self._generate_fallback_response(category, tone, context)
                self._stats["fallback_used"] += 1
            
            # Enhance response with character traits
            enhanced_response = await self._enhance_with_personality(base_response, context)
            
            # Apply emotional modifiers
            emotional_response = await self._apply_emotional_modifiers(enhanced_response, context)
    ...
    async def generate_dialogue(self, speaker_context: Dict[str, Any], conversation_context: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            dialogue = await self.generate_response(context, category, ResponseTone.CASUAL)
    ...
    async def generate_action_narration(self, action: CharacterAction, context: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            if personality.get('aggression', 0.5) > 0.7 and action_type in ['attack', 'defend']:
                modifiers = ["with fierce determination", "aggressively", "without hesitation"]
                base_narration += f" {random.choice(modifiers)}"
            elif personality.get('intelligence', 0.5) > 0.7:
                modifiers = ["strategically", "with careful consideration", "tactically"]
                base_narration += f" {random.choice(modifiers)}"
    ...
    async def generate_emotional_response(self, emotion: str, intensity: float, context: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            templates = emotion_templates.get(emotion, {}).get(intensity_level, ["experiences emotions"])
    ...
    async def get_generation_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "template_success_rate": (self._stats["template_used"] / total) if total > 0 else 0.0,
    ...
    async def _update_character_data(self, context: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._current_emotional_state = context.get('state', {}).get('emotional_state', 'neutral')
            
        except Exception as e:
            self.logger.debug(f"Character data update failed: {e}")
    
    async def _determine_response_category(self, context: Dict[str, Any]) -> ResponseCategory:
        ...
        try:
            ...
            if recent_events:
                ...
                if last_event:
                    event_type = last_event.get('event_type', '') if isinstance(last_event, dict) else getattr(last_event, 'event_type', '')
    ...
    async def _determine_response_tone(self, context: Dict[str, Any], category: ResponseCategory) -> ResponseTone:
    ...
    async def _find_matching_templates(self, category: ResponseCategory, tone: ResponseTone, 
                                     context: Dict[str, Any]) -> List[ResponseTemplate]:
        ...
        try:
            ...
            matching_templates.sort(
                key=lambda t: t.matches_context(self._personality, self._faction_beliefs, context),
                reverse=True
            )
    ...
    async def _generate_from_template(self, template: ResponseTemplate, context: Dict[str, Any]) -> str:
    ...
    async def _generate_fallback_response(self, category: ResponseCategory, tone: ResponseTone, 
    ...
    async def _enhance_with_personality(self, response: str, context: Dict[str, Any]) -> str:
    ...
    async def _apply_emotional_modifiers(self, response: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            modifiers = {
                ...
                'fearful': {
                    ...
                    'replacements': {'move': 'carefully advance', 'look': 'peer anxiously'}
                },
                'excited': {
    ...
    async def _replace_placeholders(self, template: str, context: Dict[str, Any]) -> str:
        """Replace placeholders in template with context values."""
        try:
            # Common placeholders
            placeholders = {
                '{character_name}': context.get('basic_info', {}).get('name', 'I'),
                '{faction}': context.get('faction_info', {}).get('faction', 'Independent'),
    ...
    def _initialize_vocabulary(self) -> Dict[str, List[str]]:
        ...
        return {
            'basic': ['see', 'go', 'think', 'know', 'want', 'get', 'make', 'look', 'find'],
            'advanced': ['observe', 'proceed', 'analyze', 'comprehend', 'desire', 'acquire', 
                        'construct', 'examine', 'locate'],
            'sophisticated': ['perceive', 'advance', 'contemplate', 'understand', 'aspire', 
                            'obtain', 'fabricate', 'scrutinize', 'discern']
        }
    
    def _initialize_phrase_libraries(self) -> Dict[str, List[str]]:
        """Initialize phrase libraries for different contexts."""
        return {
            'uncertainty': ['I\'m not entirely sure', 'It\'s difficult to say', 'I have my doubts'],
            'confidence': ['I\'m certain that', 'Without question', 'I have no doubt'],
            'urgency': ['Time is of the essence', 'We must act quickly', 'This cannot wait'],
            'caution': ['We should be careful', 'Prudence suggests', 'It would be wise to']
        }