...
from ..protocols import WorldEvent, ThreatLevel, SubjectiveInterpretation, WorldInterpretationProtocol
...
class WorldInterpreter:
    ...
    def __init__(self, character_id: str, logger: Optional[logging.Logger] = None):
        ...
        self._faction_perceptions: Dict[str, Dict[str, float]] = {}  # faction -> attributes
        self._entity_relationships: Dict[str, float] = {}  # entity_id -> relationship score
    ...
    async def interpret_event(self, event: WorldEvent, character_context: Dict[str, Any]) -> SubjectiveInterpretation:
        """
        ...
        """
        try:
            self.logger.debug(f"Interpreting event {event.event_id} for character {self.character_id}")
            ...
            self.logger.debug(f"Event interpretation completed with threat level: {threat_assessment}")
    ...
    async def update_worldview(self, interpretation: SubjectiveInterpretation) -> None:
        ...
        try:
            ...
            for belief, impact in interpretation.belief_impact.items():
                ...
                new_strength = current_strength + (impact * 0.1)  # Gradual belief change
            ...
            for entity_id, change in interpretation.relationship_changes.items():
                ...
                new_score = current_score + (change * 0.05)  # Gradual relationship change
            ...
            self.logger.debug(f"Worldview updated from interpretation of event {interpretation.original_event_id}")
            
        except Exception as e:
            self.logger.error(f"Worldview update failed: {e}")
    
    async def get_relevant_memories(self, context: Dict[str, Any], limit: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            def memory_relevance_score(memory):
                recency = max(0.1, 1.0 - (current_time - memory.timestamp) / (30 * 24 * 3600))  # 30 days
    ...
    async def store_memory(self, memory: Dict[str, Any], memory_type: str = "episodic") -> bool:
        """
    ...
    async def get_worldview_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            moderate_beliefs = {k: v for k, v in self._worldview.items() if 0.3 < abs(v) <= 0.7}
            ...
            # Bias summary
            dominant_biases = [bias.value for bias, strength in self._bias_tendencies.items() if strength > 0.5]
            ...
            return {
                ...
                "allies": allies,
                "enemies": enemies,
                "neutral_relationships": len(self._entity_relationships) - len(allies) - len(enemies),
    ...
    async def _build_interpretation_context(self, character_context: Dict[str, Any]) -> InterpretationContext:
        ...
        try:
            ...
            current_biases = await self._determine_active_biases(personality, current_state)
    ...
    async def _generate_base_understanding(self, event: WorldEvent, context: InterpretationContext) -> str:
        ...
        try:
            ...
            if event.location and event.location in context.knowledge_base:
                ...
                base_description += f" at {location_info.get('description', event.location)}"
            
            # Consider affected entities character knows about
            known_entities = [entity for entity in event.affected_entities 
    ...
    async def _apply_interpretation_biases(self, base_understanding: str, event: WorldEvent, 
        ...
        try:
            ...
            for bias in context.current_biases:
                if bias == InterpretationBias.OPTIMISTIC:
                    ...
                    # See hidden threats
                    if event.source not in context.relationship_context or context.relationship_context[event.source] < 0.5:
                        biased_understanding += ". There may be hidden motives behind this."
                    ...
                    if "justice" in faction_beliefs and faction_beliefs["justice"] > 0.5:
    ...
    async def _generate_emotional_response(self, event: WorldEvent, understanding: str, 
                                         context: InterpretationContext) -> str:
        ...
        try:
            ...
            possible_emotions = event_emotions.get(event.event_type, ["neutral", "curious"])
            ...
            if personality.get("aggression", 0.5) > 0.7:
                if "angry" not in possible_emotions and any(word in understanding.lower() 
                                                          for word in ["attack", "threat", "enemy"]):
                    possible_emotions.append("angry")
            
            if personality.get("loyalty", 0.5) > 0.7:
                if any(entity in event.affected_entities for entity in context.relationship_context 
            ...
            if context.stress_level > 0.7:
                ...
                primary_emotion = possible_emotions[0] if possible_emotions else "neutral"
    ...
    async def _calculate_belief_impacts(self, event: WorldEvent, understanding: str, 
                                      context: InterpretationContext) -> Dict[str, float]:
        ...
        try:
            ...
            if event.event_type == "victory":
                if event.source in context.relationship_context and context.relationship_context[event.source] > 0.5:
            ...
            for belief, impact in belief_impacts.items():
                ...
                personality_strength = sum(abs(v) for v in context.character_personality.values()) / len(context.character_personality)
    ...
    async def _assess_subjective_threat(self, event: WorldEvent, context: InterpretationContext) -> ThreatLevel:
        ...
        try:
            ...
            # Threats to allies
            allies = [entity for entity, relationship in context.relationship_context.items() 
                     if relationship > 0.5]
            ...
            # Factional threats
            if event.source in context.relationship_context and context.relationship_context[event.source] < -0.3:
    ...
    async def _calculate_relationship_changes(self, event: WorldEvent, understanding: str, 
                                            context: InterpretationContext) -> Dict[str, float]:
        ...
        try:
                ...
                if event.source != self.character_id:
                    ...
                    if self.character_id in event.affected_entities:
                        ...
                        for entity in event.affected_entities:
                            if entity in context.relationship_context and context.relationship_context[entity] > 0.5:
                                # Attacked my ally
            ...
            # Shared experiences
            if len(event.affected_entities) > 1 and self.character_id in event.affected_entities:
                # Shared experience with others
                for entity in event.affected_entities:
                    if entity != self.character_id:
                        if "victory" in event.event_type:
                            relationship_changes[entity] = 0.1  # Bonding through shared victory
                        elif "defeat" in event.event_type:
                            relationship_changes[entity] = 0.05  # Small bonding through shared hardship
    ...
    async def _calculate_memory_priority(self, event: WorldEvent, emotional_response: str, 
        ...
        try:
            ...
            high_emotion_responses = ["angry", "fearful", "elated", "shocked", "devastated"]
            ...
            similar_events = len([memory for memory in self._memory_fragments 
    ...
    async def _determine_active_biases(self, personality: Dict[str, float], current_state: Dict[str, Any]) -> List[InterpretationBias]:
    ...
    async def _store_interpretation_as_memory(self, interpretation: SubjectiveInterpretation, event: WorldEvent) -> None:
    ...
    async def _learn_interpretation_pattern(self, interpretation: SubjectiveInterpretation) -> None:
        ...
        try:
            ...
            if len(self._memory_fragments) > max_memories:
                ...
                def memory_importance(memory):
                    ...
                    return memory.emotional_weight * memory.decay_factor * recency_factor
                ...
                self.logger.debug(f"Removed {removed_count} old memories to manage capacity")
