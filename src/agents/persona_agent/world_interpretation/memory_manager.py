...
class Memory:
    ...
    emotional_weight: float = 0.0  # -1.0 to 1.0 (negative = traumatic, positive = pleasant)
    ...
    def get_current_strength(self) -> float:
        ...
        decay_factor = math.exp(-self.decay_rate * time_since_reinforcement / (24 * 3600))  # Daily decay
    ...
    def get_retrieval_probability(self, query_relevance: float = 1.0) -> float:
        ...
        emotional_boost = abs(self.emotional_weight) * 0.2  # Emotional memories more retrievable
        recency_boost = max(0, 1.0 - (datetime.now().timestamp() - self.last_accessed) / (7 * 24 * 3600)) * 0.1
        return min(1.0, current_strength * query_relevance + emotional_boost + recency_boost)
...
class MemoryManager:
    ...
    def __init__(self, character_id: str, logger: Optional[logging.Logger] = None):
        ...
        self._memory_associations: Dict[str, Dict[str, float]] = {}  # memory_id -> {related_id: strength}
    ...
    async def store_memory(self, memory: Dict[str, Any], memory_type: str = "episodic") -> bool:
        ...
        try:
            ...
            if memory_type in ["episodic", "emotional"] or abs(memory_obj.emotional_weight) > 0.3:
    ...
    async def retrieve_memories(self, query: Dict[str, Any], limit: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for memory_id in candidates:
                ...
                relevance_score = await self._calculate_relevance_score(memory, memory_query)
                retrieval_probability = memory.get_retrieval_probability(relevance_score)
            ...
            self.logger.debug(f"Retrieved {len(retrieved_memories)} memories from {len(candidates)} candidates")
    ...
    async def consolidate_memories(self) -> None:
        ...
        try:
            ...
            for memory, score in consolidation_candidates[:20]:  # Limit per consolidation cycle
    ...
    async def reinforce_memory(self, memory_id: str, reinforcement_strength: float = 0.1) -> bool:
        """
        ...
        try:
            ...
            memory.consolidation_level = min(1.0, memory.consolidation_level + reinforcement_strength * 0.5)
            
            self.logger.debug(f"Reinforced memory {memory_id}: {old_strength:.2f} -> {memory.strength:.2f}")
            return True
    ...
    async def forget_memory(self, memory_id: str, reason: str = "natural_decay") -> bool:
        """
        ...
        try:
            ...
            if memory_id in self._memory_associations:
                ...
                for related_id, strength in self._memory_associations[memory_id].items():
    ...
    async def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._memories:
                total_strength = sum(memory.get_current_strength() for memory in self._memories.values())
            ...
            consolidated_memories = sum(1 for memory in self._memories.values() 
                                      if memory.consolidation_level >= self._config["consolidation_threshold"])
        ...
        try:
            if memory_id in self._working_memory:
                ...
                if len(self._working_memory) > self._working_memory_capacity:
                    ...
                    self.logger.debug(f"Removed {removed_id} from working memory (capacity limit)")
    ...
    async def _get_candidate_memories(self, query: MemoryQuery) -> List[str]:
        ...
        try:
            ...
            if query.entities:
                ...
                for memory_id in candidates:
                    memory = self._memories[memory_id]
                    if any(entity in memory.associated_entities for entity in query.entities):
            ...
            if query.locations:
                ...
                for memory_id in candidates:
                    memory = self._memories[memory_id]
                    if any(location in memory.associated_locations for location in query.locations):
    ...
    async def _calculate_relevance_score(self, memory: Memory, query: MemoryQuery) -> float:
        ...
        try:
            ...
            if query.keywords:
                ...
                keyword_matches = sum(1 for keyword in query.keywords if keyword.lower() in content_text)
                score += (keyword_matches / len(query.keywords)) * 0.4
            
            # Entity matching
            if query.entities:
                entity_matches = len(set(query.entities) & set(memory.associated_entities))
                score += (entity_matches / len(query.entities)) * 0.3
            
            # Location matching
            if query.locations:
                location_matches = len(set(query.locations) & set(memory.associated_locations))
            ...
            if score == 0.0 and not (query.keywords or query.entities or query.locations or query.context):
    ...
    async def _calculate_consolidation_score(self, memory: Memory) -> float:
        ...
        try:
            ...
            if age_days > 0:
                ...
                score += min(0.3, access_frequency / 5.0) * 0.4  # Normalize to reasonable frequency
    ...
    async def _consolidate_memory(self, memory: Memory) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Consolidated memory {memory.memory_id}: level {old_level:.2f} -> {memory.consolidation_level:.2f}")
    ...
    async def _create_memory_associations(self, new_memory: Memory) -> None:
        ...
        try:
            ...
            for memory_id, existing_memory in self._memories.items():
                ...
                association_strength = await self._calculate_memory_association_strength(new_memory, existing_memory)
                
                if association_strength > 0.3:
                    # Create bidirectional association
                    self._memory_associations[new_memory.memory_id][memory_id] = association_strength
                    
                    if memory_id not in self._memory_associations:
                        self._memory_associations[memory_id] = {}
                    self._memory_associations[memory_id][new_memory.memory_id] = association_strength
            
        except Exception as e:
            self.logger.debug(f"Memory association creation failed: {e}")
    
    async def _calculate_memory_association_strength(self, memory1: Memory, memory2: Memory) -> float:
        """Calculate association strength between two memories."""
        try:
            strength = 0.0
            
            # Entity overlap
            entity_overlap = len(set(memory1.associated_entities) & set(memory2.associated_entities))
            if memory1.associated_entities and memory2.associated_entities:
                entity_score = entity_overlap / max(len(memory1.associated_entities), len(memory2.associated_entities))
                strength += entity_score * 0.4
            
            # Location overlap
            location_overlap = len(set(memory1.associated_locations) & set(memory2.associated_locations))
            if memory1.associated_locations and memory2.associated_locations:
                location_score = location_overlap / max(len(memory1.associated_locations), len(memory2.associated_locations))
    ...
    async def _manage_memory_capacity(self) -> None:
        ...
        try:
            ...
            if removal_count > 0:
                self.logger.debug(f"Removed {removal_count} weak memories for capacity management")
