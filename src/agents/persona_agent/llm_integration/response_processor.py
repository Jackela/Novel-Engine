...
class ResponseProcessor:
    ...
    async def process_response(self, raw_response: str, context: Dict[str, Any], 
                             validation_level: ValidationLevel = ValidationLevel.MODERATE) -> ProcessingResult:
        """
        ...
        """
        try:
            self.logger.debug(f"Processing response with {validation_level.value} validation")
            ...
            if validation_level in [ValidationLevel.MODERATE, ValidationLevel.STRICT]:
                consistency_check = await self._check_character_consistency(cleaned_response, context)
                if consistency_check.overall_score < self._config["consistency_threshold"]:
            ...
            parsed_content = await self._parse_structured_content(cleaned_response, response_type)
            ...
            result = ProcessingResult(
                ...
                metadata={
                    ...
                    "consistency_check": consistency_check.__dict__ if consistency_check else None,
            ...
            self.logger.debug(f"Response processing completed with confidence {confidence:.2f}")
            return result
    ...
    async def extract_actions(self, response: str, context: Dict[str, Any]) -> List[CharacterAction]:
        """
                ...
                for match in matches:
                    ...
                    if action_type:
                        ...
                        action = {
                            ...
                            "priority": await self._infer_action_priority(action_context),
                            "confidence": 0.7,
    ...
    async def validate_response_safety(self, response: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            violence_keywords = ["kill", "murder", "torture", "brutal", "savage", "slaughter"]
            violence_count = sum(1 for word in violence_keywords if word in response.lower())
            
            if violence_count > 3:
                issues.append("Contains excessive violent content")
            
            # Check for out-of-character behavior
            if any(phrase in response.lower() for phrase in ["as an ai", "i cannot", "i'm not able"]):
    ...
    async def get_processing_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = (self._stats["successful_processing"] / total) if total > 0 else 0.0
    ...
    async def _update_character_data(self, context: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._speaking_patterns = {
                ...
                'emotional_expression': self._personality_traits.get('emotional_expression', 0.5)
            }
    ...
    async def _classify_response_type(self, response: str, context: Dict[str, Any]) -> ResponseType:
        """Classify the type of response."""
        try:
            response_lower = response.lower()
            
            # Look for action indicators
            action_indicators = ["i will", "i am going to", "i decide to", "i choose to", "my action is"]
            ...
            if '"' in response or any(word in response_lower for word in ["says", "replies", "responds"]):
                return ResponseType.DIALOGUE
            
            # Look for internal thought indicators
            thought_indicators = ["i think", "i believe", "i wonder", "i consider", "my thoughts"]
    ...
    async def _check_character_consistency(self, response: str, context: Dict[str, Any]) -> CharacterConsistencyCheck:
        ...
        try:
            ...
            overall_score = (personality_score + faction_score + emotional_score + knowledge_score) / 4
    ...
    async def _check_personality_consistency(self, response: str) -> float:
        ...
        try:
            ...
            aggressive_words = len(re.findall(r'\b(?:fight|attack|destroy|eliminate|crush)\b', response_lower))
            peaceful_words = len(re.findall(r'\b(?:peace|calm|negotiate|discuss|diplomacy)\b', response_lower))
            ...
            loyalty_words = len(re.findall(r'\b(?:loyal|faithful|devoted|committed|allegiance)\b', response_lower))
    ...
    async def _check_emotional_consistency(self, response: str, context: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]:
                if any(word in response_lower for word in ['calm', 'relaxed', 'peaceful', 'happy']):
                    score -= 0.3  # Too calm for high threat
                elif any(word in response_lower for word in ['alert', 'ready', 'cautious', 'determined']):
            ...
            if current_emotion == 'angry' and 'anger' not in response_lower:
                if any(word in response_lower for word in ['calm', 'peaceful', 'happy']):
                    score -= 0.2
    ...
    async def _check_knowledge_consistency(self, response: str, context: Dict[str, Any]) -> float:
    ...
    async def _parse_structured_content(self, response: str, response_type: ResponseType) -> str:
    ...
    async def _calculate_confidence_score(self, response: str, consistency_check: Optional[CharacterConsistencyCheck], context: Dict[str, Any]) -> float:
    ...
    def _initialize_response_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'action_starters': [
                "I will", "I shall", "I intend to", "My plan is to", "I decide to"
            ],
            'dialogue_markers': [
                '"', "'", "says", "replies", "responds", "declares"
            ],
            'thought_indicators': [
                "I think", "I believe", "I wonder", "In my opinion", "It seems to me"
            ]
        }
    ...
    async def _infer_action_priority(self, action_text: str) -> str:
        ...
        if any(word in action_lower for word in ['urgent', 'immediate', 'critical', 'emergency']):
            return 'critical'
        elif any(word in action_lower for word in ['important', 'priority', 'must', 'need']):
            return 'high'
    ...
    async def _apply_consistency_corrections(self, response: str, consistency_check: CharacterConsistencyCheck, context: Dict[str, Any]) -> str:
    ...
    async def _add_emotional_context(self, response: str, context: Dict[str, Any]) -> str:
        """Add emotional context if missing."""
    ...
    async def _add_faction_language(self, response: str, context: Dict[str, Any]) -> str:
        """Add faction-appropriate language."""
