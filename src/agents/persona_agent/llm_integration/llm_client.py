...
from ..protocols import LLMIntegrationProtocol
...
class LLMClient:
    ...
    def __init__(self, character_id: str, logger: Optional[logging.Logger] = None):
        ...
        self._provider_priority = [LLMProvider.GEMINI, LLMProvider.OPENAI, LLMProvider.LOCAL, LLMProvider.FALLBACK]
        
        # Rate limiting
        self._rate_limits = {
            LLMProvider.GEMINI: {"requests_per_minute": 60, "last_request": 0, "request_count": 0},
            LLMProvider.OPENAI: {"requests_per_minute": 60, "last_request": 0, "request_count": 0},
            LLMProvider.LOCAL: {"requests_per_minute": 120, "last_request": 0, "request_count": 0}
        }
    ...
    async def generate_character_response(self, prompt: str, context: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            request = LLMRequest(
                ...
                max_tokens=context.get('max_tokens', self._config['default_max_tokens']),
                temperature=context.get('temperature', self._config['default_temperature'])
            )
    ...
    async def format_prompt(self, base_prompt: str, character_data: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            faction = character_data.get('faction_info', {}).get('faction', 'Independent')
            ...
            if personality:
                ...
                for trait, value in personality.items():
                    if isinstance(value, (int, float)) and value != 0.5:
                        intensity = "high" if value > 0.7 else "low" if value < 0.3 else "moderate"
            ...
            if current_state:
                ...
                if 'morale_level' in current_state:
                    ...
                    morale_desc = "high" if morale > 0.6 else "low" if morale < 0.4 else "moderate"
            ...
            if goals:
                goal_descriptions = [goal.get('title', goal.get('description', ''))[:50] for goal in goals[:3]]
            ...
            if recent_events:
                context_parts.append(f"Recent Events: {len(recent_events)} events to consider")
...
Respond as {character_name}, staying in character based on the personality, faction, and current situation described above."""
    ...
    async def get_usage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = (self._usage_stats['successful_requests'] / total_requests) if total_requests > 0 else 0.0
            ...
            for provider, count in self._usage_stats['provider_usage'].items():
                provider_stats[provider] = {
                    'requests': count,
                    'percentage': (count / total_requests * 100) if total_requests > 0 else 0.0
                }
            ...
            return {
                ...
                'available_providers': [p.value for p in self._provider_priority if await self._is_provider_available(p)]
            }
    ...
    async def _call_local(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            local_endpoint = os.getenv('LOCAL_LLM_ENDPOINT', 'http://localhost:8000/generate')
    ...
    async def _call_fallback(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            if any(word in prompt_lower for word in ['attack', 'combat', 'fight', 'battle']):
                response_category = 'combat'
            elif any(word in prompt_lower for word in ['negotiate', 'talk', 'diplomacy', 'discuss']):
                response_category = 'social'
            elif any(word in prompt_lower for word in ['explore', 'investigate', 'search', 'look']):
                response_category = 'exploration'
            elif any(word in prompt_lower for word in ['help', 'assist', 'support', 'aid']):
                response_category = 'helpful'
            else:
                response_category = 'neutral'
            
            # Get fallback response
            responses = self._fallback_responses.get(response_category, self._fallback_responses['neutral'])
            
            # Simple selection based on character context
            character_name = request.character_context.get('basic_info', {}).get('name', 'Character')
            faction = request.character_context.get('faction_info', {}).get('faction', 'Independent')
    ...
    def _setup_local_client(self) -> Optional[Any]:
        ...
        return {"configured": True, "endpoint": os.getenv('LOCAL_LLM_ENDPOINT', 'http://localhost:8000')}
    ...
    async def _is_provider_available(self, provider: LLMProvider) -> bool:
        ...
        try:
            if provider == LLMProvider.FALLBACK:
                ...
                return self._providers[provider] and os.getenv('GEMINI_API_KEY') is not None
            elif provider == LLMProvider.OPENAI:
                return self._providers[provider] and os.getenv('OPENAI_API_KEY') is not None
            elif provider == LLMProvider.LOCAL:
    ...
    async def _cache_response(self, request: LLMRequest, response: LLMResponse) -> None:
        ...
        try:
            ...
            if len(self._response_cache) > 1000:
                ...
                sorted_cache = sorted(self._response_cache.items(), key=lambda x: x[1][1])
    ...
    async def _update_usage_stats(self, response: LLMResponse, success: bool) -> None:
        ...
        try:
            ...
            if success:
                ...
                total_time = self._usage_stats['average_response_time'] * (self._usage_stats['successful_requests'] - 1)
                self._usage_stats['average_response_time'] = (total_time + response.response_time) / self._usage_stats['successful_requests']
            else:
                self._usage_stats['failed_requests'] += 1
