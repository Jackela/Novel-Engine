...
class PersonaAgent(_PersonaAgentImpl):
    ...
    def __init__(
        ...
        if is_test_runtime:
            if character_directory_path:
                resolved_path_obj = Path(character_directory_path)
                if (
                    not os.path.isdir(resolved_path_obj)
                    and event_bus is not None
                    and hasattr(event_bus, "subscribe")
                ):
                    raise FileNotFoundError(
                        f"Character directory not found: {resolved_path_obj}"
                    )
            elif character_name:
                resolved_path_obj = DEFAULT_CHARACTERS_ROOT / character_name
            else:
                resolved_path_obj = DEFAULT_CHARACTERS_ROOT
        else:
            ...
            if character_name:
                ...
                matched_name = next(
                    (item for item in available_character_dirs if item == safe_name),
                    None,
                )
                if not matched_name:
                    raise FileNotFoundError(
                        f"Character directory not found: {safe_name}"
                    )
    ...
    def get_character_state(self) -> Dict[str, Any]:
        ...
        return {
            "identity": {
                ...
                "character_directory": self.character_directory_path,
            },
            "context": {
    ...
    def process_communication(
        ...
        """Return a simple echo-style response for legacy tests."""
        return {
    ...
    def _make_decision(
        ...
        try:
            ...
            situation = self._assess_current_situation(state)
    ...
    def _assess_current_situation(
        ...
        if isinstance(result, SituationAssessment):
    ...
    def _select_best_action(
        ...
        if isinstance(action_evaluations[0], tuple):
            best_action, _score = max(action_evaluations, key=lambda item: item[1])
        ...
        if callable(selector):
            ...
            if evaluation is None:
                return None
            creator = getattr(self.decision_engine, "_create_character_action", None)
            if callable(creator):
                return creator(evaluation)
        return None
    ...
    def _call_llm(self, prompt: str) -> Optional[str]:
        ...
        if not api_key:
            if is_development and not is_test_runtime:
                # 开发环境：立即崩溃
                raise RuntimeError(
