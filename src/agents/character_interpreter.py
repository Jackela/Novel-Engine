...
class CharacterInterpreter:
        ...
        try:
            ...
            if not os.path.exists(self.character_directory_path):
                raise FileNotFoundError(
                    f"Character directory not found: {self.character_directory_path}"
                )

            if not os.path.isdir(self.character_directory_path):
                raise ValueError(
                    f"Path is not a directory: {self.character_directory_path}"
                )
            ...
            if not md_files and not yaml_files:
                raise ValueError(
                    f"No .md or .yaml files found in directory: {self.character_directory_path}"
                )
    ...
    def _extract_basic_info(self, content: str) -> Dict[str, Any]:
        ...
        try:
            # First, try to extract name from header (excluding separators)
            # Header-based names are often more reliable than field-based names
            ...
            # If no header name found, fall back to name field patterns
    ...
    def _extract_core_identity(self) -> None:
        ...
        try:
            ...
            if "rank" in self.character_data or "title" in self.character_data:
                self.character_data["archetype"] = self.character_data.get(
                    "rank", self.character_data.get("title", "Operative")
                )
        ...
        try:
            ...
            if isinstance(personality_traits, list):
                for trait in personality_traits:
                    ...
                    if any(
                        word in trait_lower
                        for word in ["brave", "courageous", "fearless"]
                    ):
                    ...
                    if any(
                        word in trait_lower
                        for word in ["aggressive", "violent", "hostile"]
                    ):
                        traits_dict["aggression"] = 0.7
                    if any(
                        word in trait_lower
                        for word in ["cautious", "careful", "prudent"]
                    ):
                        traits_dict["caution"] = 0.7
                    if any(
                        word in trait_lower
                        for word in ["intelligent", "smart", "clever"]
                    ):
        ...
        try:
            ...
            if "loyalty" in personality_scores:
                decision_weights["faction_loyalty"] = min(
                    1.0, personality_scores["loyalty"]
                )

            if "bravery" in personality_scores:
                decision_weights["self_preservation"] = max(
                    0.1, 1.0 - personality_scores["bravery"]
                )

            if "caution" in personality_scores:
                decision_weights["self_preservation"] = min(
                    1.0,
                    decision_weights["self_preservation"]
                    + personality_scores["caution"] * 0.3,
                )
        ...
        try:
            ...
            if isinstance(relationships, dict):
                for entity, relationship_desc in relationships.items():
                    ...
                    if any(
                        word in desc_lower
                        for word in ["friend", "ally", "trusted", "companion"]
                    ):
                        relationships_dict[entity] = 0.7
                    elif any(
                        word in desc_lower
                        for word in ["enemy", "rival", "hostile", "hatred"]
                    ):
        ...
        try:
            ...
            if background:
                ...
                if any(
                    ...
                    for word in ["tech", "mechanical", "engineering"]
                ):
                    knowledge_domains.append("technology")
                if any(
                    word in background.lower()
                    for word in ["combat", "tactical", "warfare"]
                ):
                    knowledge_domains.append("combat")
                if any(
                    word in background.lower()
                    for word in ["diplomacy", "negotiation", "politics"]
                ):
        ...
        try:
            ...
            if decision_weights:
                for weight_name, weight_value in decision_weights.items():
                    if not isinstance(weight_value, (int, float)) or not (
                        0.0 <= weight_value <= 1.0
                    ):
                        issues.append(
                            f"Invalid decision weight for {weight_name}: {weight_value}"
                        )
            ...
            if personality_scores:
                for trait_name, trait_value in personality_scores.items():
                    if not isinstance(trait_value, (int, float)) or not (
                        ...
                        issues.append(
                            f"Invalid personality score for {trait_name}: {trait_value}"
                        )
            ...
            if relationship_scores:
                for entity, score in relationship_scores.items():
                    if not isinstance(score, (int, float)) or not (
                        ...
                        issues.append(
                            f"Invalid relationship score for {entity}: {score}"
                        )
