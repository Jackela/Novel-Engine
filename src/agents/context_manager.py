...
class CharacterContextManager:
    ...
    def load_character_context(self) -> None:
        ...
        try:
            ...
            if not content:
                self.logger.error(f"Failed to read character sheet: {character_sheet_path}")
                return
            ...
            self.logger.info(f"Character context loaded successfully for {self.core.identity.character_name}")
            ...
            if section_content:
                ...
                if parser_method:
                    try:
                        ...
                        self.logger.warning(f"Failed to parse {section_name} section: {e}")
                        parsed_data[section_name] = {}
                else:
                    # Fallback to simple parsing
                    parsed_data[section_name] = self._parse_simple_field_format(section_content)
    ...
    def _parse_identity_section(self, section_content: str) -> Dict[str, Any]:
        ...
        backstory_match = re.search(backstory_pattern, section_content, re.IGNORECASE | re.DOTALL)
        if backstory_match:
    ...
    def _extract_bullet_points(self, content: str) -> Dict[str, str]:
        ...
        for line in lines:
            ...
            if line.startswith('* ') or line.startswith('- '):
                ...
                if current_category and isinstance(bullet_points.get(current_category), list):
    ...
    def _extract_core_identity(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Core identity extracted: {self.core.identity.character_name} ({self.core.identity.primary_faction})")
