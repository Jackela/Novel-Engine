...
class CharacterFactory:
    ...
    def __init__(self, event_bus: EventBus, base_character_path: str = "characters"):
        ...
        logger.info(
            f"CharacterFactory initialized with base path: {self.base_character_path}"
        )
    ...
    def _find_project_root(self, start_path: str) -> str:
        ...
        while current_path != os.path.dirname(current_path):  # Not at filesystem root
            for marker in markers:
                if os.path.exists(os.path.join(current_path, marker)):
                    logger.debug(
                        f"Found project root at {current_path} (marker: {marker})"
                    )
                    return current_path
        ...
        logger.warning(
            f"Could not determine project root, using fallback: {fallback_root}"
        )
