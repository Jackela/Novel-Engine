...
class ServiceContainer:
    ...
    def __init__(self, config_manager: Optional[ConfigurationManager] = None,
        ...
        self._instances: Dict[Type, Dict[str, ServiceInstance]] = {}  # Type -> {scope_key: instance}
        ...
        try:
            with self._lock:
                ...
                if service_type not in self._services:
                    raise DependencyResolutionError(f"Service not registered: {service_type.__name__}")
            ...
            raise DependencyResolutionError(f"Failed to resolve service {service_type.__name__}: {e}")
        ...
        with self._lock:
            ...
            for service_type in self._tags[tag]:
                try:
                    ...
                    logger.warning(f"Failed to resolve tagged service: {service_type.__name__}")
    ...
    async def initialize_all_services(self) -> None:
        ...
        if failed_services:
            failure_details = "; ".join([f"{svc.__name__}: {err}" for svc, err in failed_services])
            raise RuntimeError(f"Service initialization failed for {len(failed_services)} services: {failure_details}")
    ...
    async def perform_health_checks(self) -> Dict[str, Dict[str, Any]]:
        ...
        for instance in all_instances:
            ...
            try:
                ...
                instance.health_status = "healthy" if health_result["healthy"] else "unhealthy"
    ...
    def _analyze_dependencies(self, implementation: Type) -> List[Type]:
        ...
        try:
            ...
            logger.debug(f"Could not analyze dependencies for {implementation.__name__}: {e}")
    ...
    def _create_instance(self, service_type: Type, descriptor: ServiceDescriptor, scope_key: str) -> Any:
        """Create new service instance with dependency injection."""
        if service_type in self._initializing:
            raise DependencyResolutionError(f"Circular dependency detected for {service_type.__name__}")
        ...
        try:
            ...
            if descriptor.factory:
                ...
                instance = self._construct_with_dependencies(descriptor.implementation, dependencies)
    ...
    def _construct_with_dependencies(self, implementation: Type, dependencies: Dict[Type, Any]) -> Any:
            ...
            for param_name, param in init_signature.parameters.items():
                ...
                if param_name in type_hints:
                    ...
                    if param_type in dependencies:
                        ...
                        logger.warning(f"No dependency found for required parameter {param_name} of type {param_type}")
    ...
    def _apply_configuration(self, instance: Any, config_section: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to apply configuration to {type(instance).__name__}: {e}")
    ...
    def _update_initialization_order(self) -> None:
        ...
        def visit(service_type: Type):
            if service_type in temp_visited:
                raise DependencyResolutionError(f"Circular dependency involving {service_type.__name__}")
        ...
        self._initialization_order = sorted(
            order,
            key=lambda st: self._services[st].priority,
            reverse=True
        )
    ...
    async def _check_service_health(self, service_instance: ServiceInstance) -> Dict[str, Any]:
...
def register_service(interface: Type[T], implementation: Type[T], **kwargs) -> ServiceContainer:
