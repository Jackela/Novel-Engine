...
class NarrativeProcessor:
        ...
        try:
            ...
            if self.campaign_brief:
                ...
                if hasattr(self.campaign_brief, 'initial_phase'):
                    self.story_state['current_phase'] = self.campaign_brief.initial_phase
                else:
                    self.story_state['current_phase'] = 'investigation'
                    
                logger.info(f"Story initialized in '{self.story_state['current_phase']}' phase")
            else:
    ...
    def generate_narrative_context(self, agent_id: str, 
                                 registered_agents: List[PersonaAgent],
                                 current_turn_number: int) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            narrative_context = {
                ...
                'active_events': self._check_narrative_event_triggers(target_agent, current_turn_number),
                'available_actions': self._identify_available_narrative_actions(target_agent, registered_agents),
                'relationship_status': self._get_agent_relationships(target_agent),
                'story_progress': self.story_state['story_progression'][-3:],  # Last 3 progressions
            }
    ...
    def _check_narrative_event_triggers(self, agent: PersonaAgent, 
                                      current_turn_number: int) -> List[Dict[str, Any]]:
        """
    ...
    def _identify_available_narrative_actions(self, agent: PersonaAgent,
                                            registered_agents: List[PersonaAgent]) -> List[str]:
        """
    ...
    def process_narrative_action(self, action: CharacterAction, 
                               agent: PersonaAgent) -> Optional[NarrativeOutcome]:
        """
    ...
    def _update_story_state(self, narrative_outcome: NarrativeOutcome) -> None:
        ...
        try:
            ...
            if hasattr(narrative_outcome, 'phase_change') and narrative_outcome.phase_change:
