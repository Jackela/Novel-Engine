...
from src.templates.character_template_manager import CharacterTemplateManager, CharacterArchetype
from src.interactions.interaction_engine import InteractionEngine, InteractionContext, InteractionType
...
class SystemOrchestrator:
            ...
            if not startup_result.success:
                return StandardResponse(
                    ...
                    error=ErrorInfo(
                        ...
                        details={"database_error": str(startup_result.error.message if startup_result.error else "Unknown error")}
                    )
                )
            
            # Initialize memory systems
            self.memory_system = LayeredMemorySystem("system_orchestrator", self.database)
            self.memory_query_engine = MemoryQueryEngine(self.database, self.memory_system)
    ...
    async def process_dynamic_context(self, context: DynamicContext) -> StandardResponse:
        ...
        try:
            ...
            if context.agent_id not in self.active_agents:
                agent_result = await self.create_agent_context(context.agent_id, context.character_state)
            ...
            return StandardResponse(
                ...
                data={
                    ...
                    "character_updated": character_update_result.success if character_update_result else False,
                    "environmental_processed": environmental_result.success if environmental_result else False,
    ...
    async def orchestrate_multi_agent_interaction(self, participants: List[str], 
        ...
        try:
            ...
            missing_agents = [agent for agent in participants if agent not in self.active_agents]
            if missing_agents:
            ...
            interaction_result = await self.character_processor.process_character_interaction(
    ...
    async def get_system_metrics(self) -> StandardResponse:
        ...
        try:
            ...
            operations_per_minute = (self.operation_count / max(uptime / 60, 1)) if uptime > 0 else 0
            error_rate = (self.error_count / max(self.operation_count, 1)) if self.operation_count > 0 else 0
    ...
    async def _perform_memory_cleanup(self):
        ...
        try:
            ...
            inactive_agents = [
                agent_id for agent_id, last_activity in self.active_agents.items()
    ...
    async def _perform_backup(self):
        ...
        try:
            ...
            backup_path = Path("data") / "backups" / f"system_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def _update_character_state(self, agent_id: str, character_state: CharacterState) -> StandardResponse:
        ...
        try:
            ...
            update_result = await self.character_manager.update_character_state(agent_id, character_state)
