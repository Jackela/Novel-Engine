...
class NarrativeActionResolver:
    ...
    def resolve_investigate_action(
        ...
        if success:
            discovered_info = self._generate_investigation_discovery(
            ...
            outcome = NarrativeOutcome(
                ...
                character_impact={
                    character_data.get(
                        "agent_id", "unknown"
                    ): f"Your investigation of {target} yields no results."
                },
        ...
        self.logger.info(
            f"Investigation action resolved - Success: {success}, Target: {target}"
        )
        return outcome

    def resolve_dialogue_action(
        self, action: CharacterAction, character_data: Dict, world_state: Dict
    ) -> NarrativeOutcome:
        """
        ...
        if success:
            outcome = NarrativeOutcome(
                ...
                character_impact={
                    character_data.get(
                        "agent_id", "unknown"
                    ): f"You have a productive conversation with {target}."
                },
            ...
            outcome = NarrativeOutcome(
                ...
                character_impact={
                    character_data.get(
                        "agent_id", "unknown"
                    ): f"Your attempt to speak with {target} is unsuccessful."
                },
                environmental_change="",
                story_advancement=[],
                relationship_changes={},
        ...
        self.logger.info(
            f"Dialogue action resolved - Success: {success}, Target: {target}"
        )
        return outcome
    ...
    def _generate_investigation_discovery(
        self, target: str, character_data: Dict
    ) -> List[str]:
