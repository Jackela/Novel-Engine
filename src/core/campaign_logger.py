...
class CampaignLogger:
    ...
    def _initialize_campaign_log(self) -> None:
        ...
        try:
            if os.path.exists(self.campaign_log_path):
                logger.info(f"Continuing existing campaign log: {self.campaign_log_path}")
    ...
    def _create_new_campaign_log(self) -> None:
        ...
        try:
            ...
            with open(self.campaign_log_path, 'w') as f:
                ...
                f.write(f"**Campaign Started:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            ...
            fallback_path = f"campaign_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    def log_simulation_start(self, agent_count: int, simulation_config: dict = None) -> None:
        """Log simulation start event with configuration."""
        config_info = ""
        if simulation_config:
            config_info = f" with configuration: {simulation_config}"
        
        self.log_event(f"🎬 **SIMULATION STARTED** - {agent_count} agents registered{config_info}")
    ...
    def log_turn_end(self, turn_number: int, actions_processed: int) -> None:
        """Log turn end event."""
        self.log_event(f"✅ **TURN {turn_number} COMPLETE** - {actions_processed} actions processed")
    
    def log_agent_action(self, agent_name: str, agent_id: str, action_type: str, reasoning: str = None) -> None:
        """Log agent action with formatting."""
        action_description = f"🎭 **{agent_name}** ({agent_id}) decided to **{action_type}**"
        if reasoning:
    ...
    def log_simulation_end(self, total_turns: int, total_events: int) -> None:
        ...
        self.log_event(f"🏁 **SIMULATION ENDED** - {total_turns} turns completed, {total_events} events logged")
    ...
    def rotate_log(self, max_size_mb: float = 10.0) -> bool:
        ...
        if log_size_mb > max_size_mb:
            try:
                ...
                # Create new log
                self._create_new_campaign_log()
                self.log_event(f"📋 Log rotated. Previous log archived to: {archive_path}")
