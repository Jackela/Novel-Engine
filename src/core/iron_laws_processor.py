...
class IronLawsProcessor:
    ...
    def adjudicate_action(self, proposed_action: 'ProposedAction', 
                         agent: 'PersonaAgent',
                         world_context: Dict[str, Any]) -> 'IronLawsReport':
        """
        ...
        if not self.validation_enabled:
            ...
            from src.shared_types import IronLawsReport, ValidationResult, ValidationStatus
        ...
        try:
            ...
            all_violations.extend(self._validate_causality_law(proposed_action, character_data, world_context))
            all_violations.extend(self._validate_resource_law(proposed_action, character_data))
            all_violations.extend(self._validate_physics_law(proposed_action, character_data, world_context))
            all_violations.extend(self._validate_narrative_law(proposed_action, agent, world_context))
            all_violations.extend(self._validate_social_law(proposed_action, agent, world_context))
            
            # Determine overall validation result
            validation_result = self._determine_overall_validation_result(all_violations)
            ...
            return IronLawsReport(
                ...
                violations=[IronLawsViolation(
                    ...
                    affected_entities=[getattr(proposed_action, 'character_id', 'unknown')],
    ...
    def _validate_causality_law(self, action: 'ProposedAction', 
        ...
        if hasattr(action, 'contradicts_previous_action') and action.contradicts_previous_action:
            violations.append(IronLawsViolation(
                law_code="E001",
                law_name="Causality_Law",
                severity="warning",
    ...
    def _validate_resource_law(self, action: 'ProposedAction', 
        ...
        current_stamina = getattr(character_data.resources, 'stamina', ResourceValue(current=100, maximum=100))
    ...
    def _validate_physics_law(self, action: 'ProposedAction', 
                            character_data: Optional['CharacterData'],
                            world_context: Dict[str, Any]) -> List['IronLawsViolation']:
        """
        E003 Physics Law: Actions must obey basic physical constraints.
        ...
        if action.action_type in [ActionType.ATTACK, ActionType.OBSERVE] and action.target:
    ...
    def _validate_narrative_law(self, action: 'ProposedAction', 
                              agent: 'PersonaAgent',
                              world_context: Dict[str, Any]) -> List['IronLawsViolation']:
        """
        E004 Narrative Law: Actions must maintain story coherence.
    ...
    def _validate_social_law(self, action: 'ProposedAction', 
                           agent: 'PersonaAgent',
                           world_context: Dict[str, Any]) -> List['IronLawsViolation']:
        """
        E005 Social Law: Actions must respect established relationships/hierarchies.
    ...
    def _attempt_action_repairs(self, proposed_action: 'ProposedAction', 
        ...
        if modified_action != proposed_action or repair_log:
            ...
            validated_action = self._convert_proposed_to_validated(modified_action, validation_result)
    ...
    def _character_has_equipment(self, character_data: 'CharacterData', equipment: str) -> bool:
    ...
    def _repair_causality_violations(self, action, violations, character_data) -> Tuple['ProposedAction', List[str]]:
        """Repair causality law violations."""
        return action, ["Causality repair not implemented"]
    
    def _repair_resource_violations(self, action, violations, character_data) -> Tuple['ProposedAction', List[str]]:
        """Repair resource law violations."""
        return action, ["Resource repair not implemented"]
    
    def _repair_physics_violations(self, action, violations, character_data) -> Tuple['ProposedAction', List[str]]:
        """Repair physics law violations."""
        return action, ["Physics repair not implemented"]
    
    def _repair_narrative_violations(self, action, violations, character_data) -> Tuple['ProposedAction', List[str]]:
        """Repair narrative law violations."""
        return action, ["Narrative repair not implemented"]
    
    def _repair_social_violations(self, action, violations, character_data) -> Tuple['ProposedAction', List[str]]:
