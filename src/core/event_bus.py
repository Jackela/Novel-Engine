...
class EventStore:
    ...
    def store_event(self, event: Event) -> None:
        ...
        with self._lock:
            ...
            if event.metadata.correlation_id:
                self._correlation_index[event.metadata.correlation_id].append(event.metadata.event_id)
    ...
    def get_events_by_correlation(self, correlation_id: str) -> List[Event]:
        ...
        with self._lock:
            ...
            return [self._event_index[event_id] for event_id in event_ids if event_id in self._event_index]
...
class EventBus:
    ...
    def __init__(self, error_handler: Optional[CentralizedErrorHandler] = None,
        ...
        with self._lock:
            if event_type in self._handlers:
                ...
                self._handlers[event_type] = [
                    reg for reg in self._handlers[event_type] 
    ...
    async def publish(self, event: Event) -> EventProcessingResult:
        ...
        try:
            ...
            logger.debug(f"Event published: {event.event_type} ({event.metadata.event_id})")
            return result
            ...
            if self.error_handler:
                error_context = ErrorContext(
                    component="EventBus",
                    operation="publish",
                    metadata={"event_id": event.metadata.event_id, "event_type": event.event_type}
                )
    ...
    async def publish_and_wait(self, event: Event, timeout: float = 30.0) -> EventProcessingResult:
    ...
    async def _process_events(self, priority: EventPriority) -> None:
        ...
        while self._running:
            try:
                ...
                logger.error(f"Error in event processor for priority {priority.name}: {e}")
        ...
        try:
            ...
            for result in handler_results:
                if isinstance(result, Exception):
                    failed_handlers += 1
                    logger.error(f"Handler failed for event {event.metadata.event_id}: {result}")
                elif result:
            ...
            if metrics['count'] == 1:
                ...
                metrics['avg_processing_time'] = (
                    (current_avg_time * (metrics['count'] - 1) + processing_time) / metrics['count']
                )
            
            logger.debug(f"Event processed: {event.event_type} - {successful_handlers} successful, {failed_handlers} failed")
            
        except Exception as e:
            logger.error(f"Critical error processing event {event.metadata.event_id}: {e}")
    ...
    def _handler_matches_filters(self, event: Event, registration: HandlerRegistration) -> bool:
    ...
    async def _execute_handler(self, event: Event, registration: HandlerRegistration) -> bool:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'event_store_size': len(self.event_store._events) if self.event_store else 0
        }
    ...
    async def replay_events(
        ...
        if correlation_id:
            events_to_replay = self.event_store.get_events_by_correlation(correlation_id)
        elif event_type:
            events_to_replay = self.event_store.get_events_by_type(event_type, limit=1000)
        else:
...
async def publish_event(event_type: str, payload: Any, **metadata) -> EventProcessingResult:
