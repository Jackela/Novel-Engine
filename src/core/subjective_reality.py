...
class InformationFragment:
    ...
    def get_current_reliability(self) -> float:
        """计算考虑时间衰减的当前可靠性"""
        time_decay = max(0, 1.0 - self.decay_rate * 
...
class BeliefModel:
    ...
    def add_information(self, fragment: InformationFragment) -> None:
        ...
        if conflicts:
            logger.debug(f"Information conflict detected for {self.agent_id}: {conflicts}")
            # 根据可靠性和信任度处理冲突
    ...
    def _detect_conflicts(self, new_fragment: InformationFragment) -> List[InformationFragment]:
        """检测信息冲突"""
        conflicts = []
        for existing in self.information_fragments:
            if (existing.category == new_fragment.category and 
                existing.location_context == new_fragment.location_context and
                self._are_contradictory(existing.content, new_fragment.content)):
    ...
    def _resolve_conflicts(self, new_fragment: InformationFragment, conflicts: List[InformationFragment]):
        """解决信息冲突"""
    ...
    def _update_hypotheses(self, fragment: InformationFragment):
        ...
        # 更新假设可信度
        self.active_hypotheses[hypothesis_key] = min(1.0, 
            current_confidence + reliability * category_weight * 0.1)
...
class FogOfWarState:
    ...
    information_access_level: Dict[KnowledgeCategory, float] = field(default_factory=dict)
    last_update: datetime = field(default_factory=datetime.now)
    
    def can_access_information(self, category: KnowledgeCategory, location: str = None) -> float:
        """判断能否访问某类信息，返回访问程度(0-1)"""
        base_access = self.information_access_level.get(category, 0.3)
        
        # 位置影响
        location_modifier = 1.0 if (location and location in self.visible_locations) else 0.5
        
        # 时间衰减
        time_decay = max(0.1, 1.0 - (datetime.now() - self.last_update).total_seconds() / 7200)
...
class FogOfWarService:
    ...
    def __init__(self):
        ...
        self.global_visibility_map: Dict[str, Set[str]] = defaultdict(set)  # location -> visible_agents
    ...
    def update_visibility(self, agent_id: str, new_locations: List[str], lost_locations: List[str] = None):
        """更新Agent的可见区域"""
            ...
            for location in fog_state.visible_locations:
                if location in world_state.get("locations", {}):
                    filtered_state["environmental_info"][location] = {
                        "description": world_state["locations"][location].get("description", "Unknown"),
                        "visibility": env_access
                    }
        
        # 生成可用行动
        filtered_state["available_actions"] = self._generate_available_actions(fog_state, world_state)
        
        return filtered_state
    
    def _generate_available_actions(self, fog_state: FogOfWarState, world_state: Dict) -> List[str]:
        """根据迷雾战争状态生成可用行动"""
...
class TurnBriefFactory:
    ...
    def initialize_belief_model(self, agent_id: str, personality_traits: Dict[str, float] = None):
        """初始化Agent的信念模型"""
        ...
        belief_model = BeliefModel(
            agent_id=agent_id,
            personality_bias=personality_traits
        )
        
        self.belief_models[agent_id] = belief_model
        return belief_model
    
    async def create_personalized_brief(self, 
                                      agent_id: str, 
                                      turn_number: int,
                                      global_world_state: Dict,
                                      recent_events: List[Dict] = None) -> PersonalizedTurnBrief:
        """创建个性化回合简报"""
        ...
        # 提取可用信息
        available_info = self._extract_available_information(belief_model, filtered_world_state)
        ...
        # 计算信息可信度
        confidence_levels = self._calculate_confidence_levels(belief_model, available_info)
    ...
    async def _process_recent_events(self, belief_model: BeliefModel, events: List[Dict]):
        """处理最近发生的事件"""
        for event in events:
            # 将事件转换为信息片段
    ...
    def _extract_available_information(self, belief_model: BeliefModel, world_state: Dict) -> List[InformationFragment]:
        """提取可用的信息片段"""
        available_info = []
    ...
    def _generate_recommended_actions(self, 
                                    belief_model: BeliefModel, 
                                    world_state: Dict, 
                                    available_info: List[InformationFragment]) -> List[str]:
        """生成推荐行动"""
        ...
        # 社交性格偏好
        if personality_bias.get('sociability', 0.5) > 0.6 and len(world_state.get('known_agents', [])) > 0:
        ...
        # 基于可用信息的行动推荐
        tactical_info = [f for f in available_info if f.category == KnowledgeCategory.TACTICAL]
        if tactical_info and len(tactical_info) > 2:
            actions.add('plan')
        
        resource_info = [f for f in available_info if f.category == KnowledgeCategory.RESOURCE]
        if resource_info:
            actions.update(['gather', 'craft'])
        
        return list(actions)[:8]  # 限制行动数量
    
    def _generate_narrative_context(self, 
                                   agent_id: str, 
                                   world_state: Dict, 
                                   available_info: List[InformationFragment]) -> str:
        """生成叙事背景"""
        ...
        # 获取叙事模板
        template = self.narrative_templates.get(situation_type, self.narrative_templates['mystery'])
        
        # 填充模板变量
        context_vars = {
            'agent_name': agent_id.replace('_', ' ').title(),
            'location': ', '.join(world_state.get('visible_locations', ['未知区域'])),
            'other_agents': ', '.join([a.get('agent_id', 'Unknown') for a in world_state.get('known_agents', [])]),
        }
    ...
    def _determine_situation_type(self, world_state: Dict, available_info: List[InformationFragment]) -> str:
        """确定当前情境类型"""
    ...
    def _calculate_confidence_levels(self, 
                                   belief_model: BeliefModel, 
                                   available_info: List[InformationFragment]) -> Dict[str, float]:
        """计算各类信息的可信度"""
        confidence = {}
        
        # 计算各知识类别的平均可信度
        for category in KnowledgeCategory:
            relevant_info = [info for info in available_info if info.category == category]
            if relevant_info:
                avg_reliability = sum(info.get_current_reliability() for info in relevant_info) / len(relevant_info)
        ...
        # 计算整体信息可信度
        if available_info:
            overall_confidence = sum(info.get_current_reliability() for info in available_info) / len(available_info)
...
class SubjectiveRealityEngine:
    ...
    async def initialize_agent(self, agent_id: str, 
                             ...
                             initial_location: str = None) -> BeliefModel:
        """初始化Agent的主观现实系统"""
        ...
        # 初始化信念模型
        belief_model = self.turn_brief_factory.initialize_belief_model(agent_id, personality_traits)
    ...
    async def generate_turn_brief(self, 
                                agent_id: str, 
                                turn_number: int,
                                global_world_state: Dict,
                                recent_events: List[Dict] = None) -> PersonalizedTurnBrief:
        ...
        logger.debug(f"Generated personalized brief for {agent_id} (Turn {turn_number})")
        return brief
    
    async def update_agent_knowledge(self, 
                                   agent_id: str, 
                                   new_information: List[Dict]):
        """更新Agent的知识状态"""
        
        if agent_id not in self.active_agents:
            logger.warning(f"Attempted to update knowledge for uninitialized agent: {agent_id}")
            return
        ...
        for info_dict in new_information:
            fragment = InformationFragment(
                ...
                timestamp=datetime.fromisoformat(info_dict.get('timestamp', datetime.now().isoformat())),
                agent_id=agent_id,
        ...
        logger.debug(f"Updated knowledge for {agent_id}: {len(new_information)} new information fragments")
