...
class AgentCoordinator:
    ...
    def register_agent(self, agent: PersonaAgent) -> bool:
        ...
        if any(existing.agent_id == agent.agent_id for existing in self.registered_agents):
        ...
        logger.info(f"Successfully registered agent: {character_name} (ID: {agent.agent_id})")
    ...
    def execute_turn(self, world_state_callback=None, action_handler=None) -> Dict[str, Any]:
        """
        ...
        logger.info(f"🎯 Starting Turn {self.current_turn_number} with {len(self.registered_agents)} agents")
        ...
        turn_summary['duration_seconds'] = (turn_end_time - turn_start_time).total_seconds()
    ...
    def _create_action_summary(self, agent: PersonaAgent, action: Optional[CharacterAction]) -> Dict[str, Any]:
    ...
    def validate_agents(self) -> List[str]:
        ...
        for agent in self.registered_agents:
            ...
            if not hasattr(agent, 'character_data') or not agent.character_data:
                issues.append(f"Agent {getattr(agent, 'agent_id', 'unknown')} missing character_data")
