...
class MemoryManager:
    ...
    def update_internal_memory(self, new_log: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Agent {self.agent_id} updated memory with {len(self.short_term_memory)} short-term items")
    ...
    def get_relevant_memories(self, context: Dict[str, Any], limit: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def _assess_emotional_impact(self, log_entry: Dict[str, Any]) -> float:
        ...
        if any(word in description for word in ['success', 'victory', 'achievement', 'help']):
            impact += 0.3
        
        # Negative emotional triggers
        if any(word in description for word in ['failure', 'death', 'injury', 'threat', 'danger']):
            impact -= 0.3
    ...
    def _should_store_in_long_term_memory(self, memory_entry: Dict[str, Any]) -> bool:
        ...
        return (importance > importance_threshold or 
                abs(emotional_impact - 0.5) > emotional_threshold - 0.5)
