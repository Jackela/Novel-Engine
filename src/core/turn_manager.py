...
class TurnManager:
    ...
    def run_turn(self, registered_agents: List[PersonaAgent], 
                 world_state_data: Dict[str, Any],
                 log_event_callback) -> Dict[str, Any]:
        """
    ...
    def handle_agent_action(self, agent: PersonaAgent, action: Optional[CharacterAction],
        ...
        if action:
            ...
            log_event_callback(f"{agent.character_data.get('name', agent.agent_id)} is waiting and observing.")

    def prepare_world_state_for_agent(self, agent: PersonaAgent, 
                                    registered_agents: List[PersonaAgent],
                                    world_state_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def store_turn_in_history(self, turn_summary: Dict[str, Any], 
                             world_state_data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            if len(world_state_data['turn_history']) > self.max_turn_history:
                world_state_data['turn_history'] = world_state_data['turn_history'][-self.max_turn_history:]
                logger.info(f"Turn history trimmed to last {self.max_turn_history} turns")
