...
class CharacterConfig:
    ...
    default_sheets: List[str] = field(
        default_factory=lambda: ["character_krieg.md", "character_ork.md"]
    )
...
class ConfigLoader:
    ...
    def _load_yaml_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Successfully parsed YAML configuration: {len(config_data)} sections"
            )
            return config_data
    ...
    def _create_config_from_data(self, config_data: Dict[str, Any]) -> AppConfig:
        ...
        if "paths" in config_data:
            ...
            config.paths = PathsConfig(
                ...
                simulation_logs_directory=paths_data.get(
        ...
        if "director" in config_data:
            ...
            config.director = DirectorConfig(
                campaign_log_filename=director_data.get(
                    "campaign_log_filename", "campaign_log.md"
                ),
        ...
        if "chronicler" in config_data:
            ...
            config.chronicler = ChroniclerConfig(
                ...
                output_directory=chronicler_data.get(
        ...
        if "testing" in config_data:
            ...
            config.testing = TestingConfig(
                ...
                test_output_directory=testing_data.get(
        ...
        if "validation" in config_data:
            ...
            config.validation = ValidationConfig(
                require_character_sheets=validation_data.get(
                    "require_character_sheets", True
                ),
                validate_campaign_log=validation_data.get(
                    "validate_campaign_log", True
                ),
                check_file_permissions=validation_data.get(
    ...
    def _apply_env_overrides(self, config: AppConfig) -> AppConfig:
        ...
        env_mappings = {
            "W40K_SIMULATION_TURNS": lambda val: setattr(
                config.simulation, "turns", int(val)
            ),
            "W40K_LOG_FILE_PATH": lambda val: setattr(
                config.paths, "log_file_path", val
            ),
            "W40K_OUTPUT_DIRECTORY": lambda val: setattr(
                config.paths, "output_directory", val
            ),
            "W40K_CHARACTER_SHEETS_PATH": lambda val: setattr(
                config.paths, "character_sheets_path", val
            ),
            "W40K_MAX_AGENTS": lambda val: setattr(
                config.simulation, "max_agents", int(val)
            ),
            "W40K_API_TIMEOUT": lambda val: setattr(
                config.simulation, "api_timeout", int(val)
            ),
            "W40K_LOGGING_LEVEL": lambda val: setattr(
                config.simulation, "logging_level", val
            ),
            "W40K_NARRATIVE_STYLE": lambda val: setattr(
                config.chronicler, "narrative_style", val
            ),
        }
    ...
    def _validate_config(self, config: AppConfig) -> None:
        ...
        if config.chronicler.narrative_style not in valid_styles:
            raise ConfigurationError(
                f"chronicler.narrative_style must be one of: {valid_styles}"
            )
