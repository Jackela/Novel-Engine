...
class ConfigPaths:
    ...
    environments_config: str = "config/environments/environments.yaml"
    development_config: str = "config/environments/development.yaml"
    settings_config: str = "config/environments/settings.yaml"
    security_config: str = "config/security/security.yaml"
    nginx_config: str = "config/nginx/nginx.conf"
    prometheus_config: str = "config/prometheus/prometheus.yml"
    prometheus_rules: str = "config/prometheus/rules/novel-engine.yml"
...
class EnvironmentConfigLoader:
    ...
    def _detect_environment(self, env_override: Optional[str] = None) -> Environment:
        ...
        if env_override:
            try:
                ...
                logger.warning(
                    f"Invalid environment override: {env_override}, using development"
                )
        ...
        for env_var in env_vars:
            ...
            if env_value:
                try:
                    ...
                    logger.info(
                        f"Environment detected from {env_var}: {environment.value}"
                    )
                    return environment
                except ValueError:
        ...
        if any(
            ...
            for marker in [".git", "venv", ".venv", "node_modules"]
        ):
    ...
    def _get_default_configuration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "simulation": {"turns": 3, "max_agents": 10, "api_timeout": 30},
            "storage": {"kb_type": "file", "session_storage": "file"},
        }
