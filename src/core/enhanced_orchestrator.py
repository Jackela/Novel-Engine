...
from .service_container import ServiceContainer, ServiceScope, register_service, get_service
...
from .system_orchestrator import SystemOrchestrator, OrchestratorConfig, OrchestratorMode, SystemHealth
...
class SystemShutdownEvent:
    ...
    def __init__(self, orchestrator_id: str, uptime_seconds: float, timestamp: datetime):
        self.orchestrator_id = orchestrator_id
...
class SystemHealthEvent:
    ...
    def __init__(self, previous_health: str, current_health: str, details: Dict[str, Any]):
    ...
    async def handle(self, event: Event) -> bool:
        ...
        try:
            if event.event_type == "system.health.changed":
                ...
                logger.info(f"Health status changed: {payload.previous_health} -> {payload.current_health}")
...
class EnhancedSystemOrchestrator:
    ...
    async def startup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            startup_result = {
                ...
                "services_initialized": len(self.service_container._services) if self.service_container else 0,
                "database_pools": len(self.database_manager._pools) if self.database_manager else 0,
                "health_status": self.current_health.value,
                "timestamp": datetime.now().isoformat()
            }
            
            logger.info(f"Enhanced orchestrator startup completed in {startup_duration:.2f}s")
    ...
    async def shutdown(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Enhanced orchestrator shutdown completed in {shutdown_duration:.2f}s")
    ...
    async def _initialize_core_components(self) -> None:
        ...
        # Service container
        self.service_container = ServiceContainer(self.config_manager, self.error_handler)
        logger.debug("Service container initialized")
        
        # Event bus
        self.event_bus = EventBus(self.error_handler, enable_event_store=self.config.enable_event_sourcing)
    ...
    async def _register_core_services(self) -> None:
        ...
        self.service_container.register_singleton(ConfigurationManager, self.config_manager)
        self.service_container.register_singleton(CentralizedErrorHandler, self.error_handler)
        self.service_container.register_singleton(EventBus, self.event_bus)
        self.service_container.register_singleton(DatabaseManager, self.database_manager)
    ...
    async def _initialize_legacy_orchestrator(self) -> None:
        ...
        legacy_config = OrchestratorConfig(
            mode=OrchestratorMode.PRODUCTION if self.config.mode == EnhancedOrchestratorMode.PRODUCTION else OrchestratorMode.DEVELOPMENT,
        ...
        if not startup_result.success:
            raise RuntimeError(f"Legacy orchestrator startup failed: {startup_result.error}")
    ...
    async def _start_background_tasks(self) -> None:
        ...
        self.event_bus.subscribe("system.metrics.collected", performance_handler, priority=100)
    ...
    async def _publish_startup_event(self) -> None:
        ...
        startup_event = Event(
            ...
            metadata={"priority": EventPriority.SYSTEM, "source": "enhanced_orchestrator"}
        )
    ...
    async def _perform_initial_health_check(self) -> None:
        ...
        try:
            ...
            all_healthy = (
                all(pool["healthy"] for pool in db_health.values()) and
                all(result["healthy"] for result in service_health.values()) and
                legacy_metrics.success
            )
            
            self.current_health = SystemHealth.OPTIMAL if all_healthy else SystemHealth.DEGRADED
            ...
            try:
                ...
                for service_name, health_result in health_results.items():
                    if not health_result.get("healthy", False):
                        logger.warning(f"Service {service_name} is unhealthy: {health_result}")
    ...
    async def _check_system_health(self) -> None:
        ...
        try:
            ...
            # Check service health
            service_health = await self.service_container.perform_health_checks()
            services_healthy = all(result["healthy"] for result in service_health.values())
            
            # Check event bus health
            event_stats = self.event_bus.get_statistics()
            event_healthy = event_stats["processing_stats"]["error_rate"] < 0.1  # Less than 10% error rate
    ...
    async def _collect_performance_metrics(self) -> None:
        ...
        try:
            ...
            comprehensive_metrics = {
                ...
                'legacy_orchestrator': legacy_metrics.data if legacy_metrics.success else None,
                'timestamp': datetime.now().isoformat()
            }
    ...
    async def _check_performance_thresholds(self, metrics: Dict[str, Any]) -> None:
        ...
        error_rate = processing_stats.get('error_rate', 0.0) * 100  # Convert to percentage
        
        if error_rate > thresholds.get('error_rate', 5.0):
            logger.warning(f"High error rate detected: {error_rate:.2f}%")
    
    async def _handle_critical_health_event(self, health_event: SystemHealthEvent) -> None:
    ...
    async def _handle_degraded_health_event(self, health_event: SystemHealthEvent) -> None:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'service_count': len(self.service_container._services) if self.service_container else 0,
            'database_pools': len(self.database_manager._pools) if self.database_manager else 0,
            'event_processing': self.event_bus.get_statistics() if self.event_bus else {},
    ...
    async def create_agent_context(self, agent_id: str, initial_state=None):
        ...
        if self.legacy_orchestrator:
            return await self.legacy_orchestrator.create_agent_context(agent_id, initial_state)
        else:
    ...
    async def orchestrate_multi_agent_interaction(self, participants: List[str], **kwargs):
        """Orchestrate multi-agent interaction (legacy compatibility)."""
        if self.legacy_orchestrator:
            return await self.legacy_orchestrator.orchestrate_multi_agent_interaction(participants, **kwargs)
        else:
            raise RuntimeError("Legacy orchestrator not initialized")
    ...
    def subjective_reality_engine(self):
        ...
        try:
            ...
            return self.legacy_orchestrator.subjective_reality_engine if self.legacy_orchestrator else None
    ...
    def emergent_narrative_engine(self):
        ...
        try:
            ...
            return self.legacy_orchestrator.emergent_narrative_engine if self.legacy_orchestrator else None