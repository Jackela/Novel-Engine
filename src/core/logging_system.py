...
class StructuredLogger:
    ...
    def track_performance(self, operation: str, 
                         context: Optional[LogContext] = None) -> PerformanceTracker:
    ...
    def get_performance_summary(self, operation_filter: Optional[str] = None) -> Dict[str, Any]:
        ...
        for entry in self.performance_metrics:
            if (operation_filter is None or 
                operation_filter in entry.get('context', {}).get('operation', '')):
...
class AuditFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if hasattr(record, 'structured_data'):
            data = record.structured_data
            return (data.get('level') == 'AUDIT' or 
                   data.get('category') == 'audit')
        return False
