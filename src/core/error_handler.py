...
class CentralizedErrorHandler:
    ...
    async def handle_error(
        ...
        try:
            ...
            if recovery_strategy is None:
                recovery_strategy = self._select_recovery_strategy(error, severity, category)
    ...
    def _detect_error_category(self, error: Exception, context: ErrorContext) -> ErrorCategory:
    ...
    def _select_recovery_strategy(self, error: Exception, severity: ErrorSeverity, 
    ...
    def _find_similar_error(self, error_record: ErrorRecord) -> Optional[str]:
        ...
        for existing_id, existing_record in self.error_records.items():
            ...
            if error_signature == existing_signature:
                ...
                if datetime.now() - existing_record.last_occurrence < timedelta(hours=1):
                    return existing_id
    ...
    async def _attempt_recovery(self, error_record: ErrorRecord) -> None:
        ...
        try:
            ...
            recovery_handler = self.recovery_handlers.get(error_record.recovery_strategy)
            if recovery_handler:
    ...
    async def _handle_graceful_degradation(self, error_record: ErrorRecord) -> bool:
        ...
        self.logger.info(f"Activating graceful degradation for {error_record.context.component}")
        return True
    
    async def _handle_restart_component(self, error_record: ErrorRecord) -> bool:
        """Handle component restart recovery."""
        self.logger.warning(f"Component restart recommended for {error_record.context.component}")
        return False  # Manual intervention required
    
    async def _handle_user_intervention(self, error_record: ErrorRecord) -> bool:
        """Handle user intervention recovery."""
        self.logger.critical(f"User intervention required for error {error_record.error_id}")
        return False
    ...
    def _update_metrics(self, error_record: ErrorRecord) -> None:
        ...
        with self._lock:
            ...
            total_recovery_attempts = sum(1 for r in self.error_records.values() if r.recovery_attempted)
            successful_recoveries = sum(1 for r in self.error_records.values() if r.recovery_successful)
            
            if total_recovery_attempts > 0:
                self.performance_metrics['recovery_success_rate'] = successful_recoveries / total_recovery_attempts
    ...
    def _get_recovery_statistics(self) -> Dict[str, Any]:
        ...
        successful = sum(1 for r in self.error_records.values() if r.recovery_successful)
