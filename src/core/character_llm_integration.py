...
class LLMIntegration:
    ...
    def enhanced_decision_making(self, world_state_update: Dict[str, Any],
                               ...
                               relationships: Dict[str, float]) -> Optional[CharacterAction]:
        """
        ...
        try:
            ...
            prompt = self._construct_character_prompt(
                ...
                character_data, personality_traits, decision_weights,
                subjective_worldview, current_status, morale_level,
                current_location, relationships
            )
    ...
    def _construct_character_prompt(self, world_state_update: Dict[str, Any],
                                  ...
                                  morale_level: float,
                                  current_location: Optional[str],
                                  relationships: Dict[str, float]) -> str:
        ...
        character_background = self._build_character_background_section(
            character_name, primary_faction, character_data, current_status, morale_level
        )
        personality_section = self._build_personality_section(personality_traits)
        decision_weights_section = self._build_decision_weights_section(decision_weights)
        situation_section = self._build_situation_section(world_state_update, situation_assessment, current_location, subjective_worldview)
        ...
        action_history_section = self._build_action_history_section(world_state_update, morale_level)
...
{decision_request}"""
    ...
    def _build_character_background_section(self, character_name: str, primary_faction: str,
                                          character_data: Dict[str, Any], current_status: str,
                                          morale_level: float) -> str:
...
Morale Level: {morale_level:.2f} (-1.0 to 1.0 scale)"""
    ...
    def _build_decision_weights_section(self, decision_weights: Dict[str, float]) -> str:
    ...
    def _build_situation_section(self, world_state_update: Dict[str, Any],
                                situation_assessment: Dict[str, Any],
                                current_location: Optional[str],
                                subjective_worldview: Dict[str, Any]) -> str:
...
Recent Events: {len(subjective_worldview.get('recent_events', []))}"""
    ...
    def _build_action_history_section(self, world_state_update: Dict[str, Any], morale_level: float) -> str:
...
"""
    ...
    def _build_relationships_section(self, relationships: Dict[str, float]) -> str:
        ...
        important_relationships = {k: v for k, v in relationships.items() if abs(v) > 0.3}
...
REASONING: As a loyal servant of the Imperium, my duty requires me to engage threats to protect innocent civilians. My aggressive nature and high mission success priority compel me to take direct action."""
    ...
    def _parse_llm_response(self, llm_response: str, available_actions: List[Dict[str, Any]]) -> Optional[CharacterAction]:
        ...
        try:
            ...
            if not self._is_valid_llm_response_format(llm_response):
                logger.warning(f"Agent {self.agent_id} received invalid LLM response format")
                return None
            ...
            if not action_match:
                logger.warning(f"Agent {self.agent_id} LLM response missing ACTION field")
                return None
            ...
            if not action_type:
                logger.warning(f"Agent {self.agent_id} could not determine action type from: {action_text}")
                return None
    ...
    def _determine_llm_action_priority(self, action_type: str, reasoning: str) -> ActionPriority:
        ...
        if any(keyword in reasoning_lower for keyword in ['urgent', 'critical', 'emergency', 'immediate']):
            return ActionPriority.CRITICAL
        
        if any(keyword in reasoning_lower for keyword in ['important', 'must', 'duty', 'loyalty']):
