...
class StoryQualityEngine:
    ...
    def __init__(self):
        ...
        logger.info(
            "StoryQualityEngine initialized with comprehensive analysis capabilities"
        )
    ...
    async def analyze_story_quality(
        ...
        try:
            ...
            if genre and genre in self.genre_templates:
                report.genre_compliance = await self._analyze_genre_compliance(
                    story_text, genre
                )
            ...
            logger.info(
                f"Story quality analysis completed for {story_id} "
                    ...
                    improvement_priorities.append(
                        {
                            ...
                            "effort_estimate": self._estimate_improvement_effort(
    ...
    async def _analyze_plot_coherence(
        self, story_text: str, genre: Optional[StoryGenre]
    ) -> QualityScore:
    ...
    async def _analyze_character_development(
        ...
        try:
            ...
            development_count = sum(
                story_text.lower().count(word) for word in development_words
            )
            ...
            interaction_count = sum(
                story_text.lower().count(word) for word in interaction_words
            )

            # Calculate character development score
            character_presence = min(
                len(main_characters) / 3, 1.0
            )  # Ideal: 2-3 main characters
            development_density = min(
                development_count / (len(story_text.split()) * 0.02), 1.0
            )
            interaction_density = min(
                interaction_count / (len(story_text.split()) * 0.03), 1.0
            )

            overall_score = (
                character_presence * 0.4
                + development_density * 0.3
            ...
            if len(main_characters) >= 2:
                evidence.append(
                    f"Multiple well-developed characters identified: {', '.join(main_characters[:3])}"
                )
            else:
                suggestions.append(
                    "Develop more distinct character voices and personalities"
                )

            if development_density < 0.5:
                suggestions.append(
                    "Show more character growth and internal development"
                )
    ...
    async def _analyze_dialogue_quality(
        self, story_text: str, genre: Optional[StoryGenre]
    ) -> QualityScore:
        ...
        try:
            ...
            if not dialogues:
                return QualityScore(
                    ...
                    confidence=0.8,
                    suggestions=[
                        "Add dialogue to improve character interaction and story engagement"
                    ],
                )
            ...
            sum(story_text.lower().count(tag) for tag in dialogue_tags)
            tag_variety = len(
                [tag for tag in dialogue_tags if tag in story_text.lower()]
            )
            ...
            if tag_variety < 3:
                suggestions.append(
                    "Use more varied dialogue tags to enhance readability"
                )
    ...
    async def _analyze_pacing(
        ...
        try:
            ...
            length_score = 1.0 - min(
                abs(avg_sentence_length - 12) / 12, 1.0
            )  # Ideal: ~12 words
            ...
            overall_score = (
                length_score * 0.4 + variety_score * 0.3 + rhythm_score * 0.3
            )
    ...
    async def _analyze_world_building(
        self, story_text: str, genre: Optional[StoryGenre]
    ) -> QualityScore:
        ...
        try:
            ...
            sensory_count = sum(
                story_text.lower().count(word) for word in sensory_words
            )
            ...
            setting_count = sum(
                story_text.lower().count(word) for word in setting_words
            )
            ...
            if sensory_density > 0.7:
                ...
                suggestions.append(
                    "Add more sensory details to bring the world to life"
                )

            if setting_density < 0.5:
                suggestions.append(
                    "Develop the setting with more environmental details"
                )

            if descriptive_density < 0.5:
                suggestions.append(
                    "Use more descriptive language to paint vivid scenes"
                )
    ...
    async def _analyze_emotional_impact(
        ...
        try:
            ...
            positive_count = sum(
                story_text.lower().count(word) for word in positive_emotions
            )
            negative_count = sum(
                story_text.lower().count(word) for word in negative_emotions
            )
            intense_count = sum(
                story_text.lower().count(word) for word in intense_emotions
            )
    ...
    async def _analyze_originality(
        ...
        try:
            ...
            cliche_count = sum(
                story_text.lower().count(cliche) for cliche in common_cliches
            )
        ...
        if genre and genre in self.genre_templates:
            ...
            if "pacing" in template.style_guidelines:
                ...
                recommendations.append(
                    f"Consider {pacing_style} pacing typical of {genre.value}"
                )
    ...
    async def _analyze_genre_compliance(
        self, story_text: str, genre: StoryGenre
    ) -> float:
    ...
    def _estimate_improvement_effort(
        self, dimension: QualityDimension, current_score: float
    ) -> str:
