...
class AnalyticsPlatform:
    ...
    def __init__(self, data_retention_days: int = 90):
        ...
        logger.info(f"AnalyticsPlatform initialized with {data_retention_days} days retention")
    ...
    async def start_background_processing(self):
        ...
        try:
            ...
            logger.info(f"Started {len(self.background_tasks)} background analytics tasks")
    ...
    async def track_story_generation(self, story_id: str, user_id: str,
    ...
    async def track_user_engagement(self, user_id: str, session_id: str,
        ...
        try:
            ...
            await self.track_event(event)
            await self._update_user_engagement(user_id, 'session_activity', engagement_data)
            
        except Exception as e:
            logger.error(f"Failed to track user engagement: {e}")
    
    async def track_character_usage(self, character_id: str, user_id: str,
                                  usage_context: str, story_id: Optional[str] = None):
        """
        ...
        try:
            ...
            await self.track_event(event)
            await self._update_character_usage(character_id, user_id, story_id, usage_context)
    ...
    async def generate_analytics_report(self, report_type: str, time_window: TimeWindow,
        ...
        try:
            ...
            if report_type == "user_engagement":
                report = await self._generate_engagement_report(report_id, time_window, filtered_events)
            elif report_type == "story_analytics":
                report = await self._generate_story_report(report_id, time_window, filtered_events)
            elif report_type == "character_analytics":
                report = await self._generate_character_report(report_id, time_window, filtered_events)
            elif report_type == "system_performance":
                report = await self._generate_performance_report(report_id, time_window, filtered_events)
            elif report_type == "comprehensive":
                report = await self._generate_comprehensive_report(report_id, time_window, filtered_events)
            else:
        ...
        try:
            ...
            dashboard = {
                ...
                "overview": {
                    ...
                    "stories_generated": len([e for e in recent_events if e.event_type == "story_generation"]),
                    "character_usages": len([e for e in recent_events if e.event_type == "character_usage"]),
                    "total_events": len(recent_events)
                },
                "performance": {
                    "avg_generation_time": self._calculate_avg_generation_time(recent_events),
    ...
    async def get_user_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get user's stories
            user_stories = [
                story for story in self.story_analytics.values()
            ...
            insights = {
                ...
                "story_analytics": {
                    ...
                    "avg_quality_score": statistics.mean([s.quality_score for s in user_stories]) if user_stories else 0,
                    "favorite_genres": engagement.favorite_genres,
                    "word_count_total": sum(s.word_count for s in user_stories),
                    "avg_generation_time": statistics.mean([s.generation_time for s in user_stories]) if user_stories else 0
                },
    ...
    async def get_character_insights(self, character_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            if character_id:
                ...
                return {
                    ...
                    "trending_up": [c for c in all_characters if c["popularity_trend"] == "increasing"][:5],
                    "trending_down": [c for c in all_characters if c["popularity_trend"] == "decreasing"][:5],
    ...
    async def export_analytics_data(self, export_format: str, time_window: TimeWindow,
        ...
        try:
            ...
            if "user_engagement" in data_types:
                export_data["user_engagement"] = {
                    uid: asdict(engagement) for uid, engagement in self.user_engagement.items()
                }
            
            if "character_analytics" in data_types:
                export_data["character_analytics"] = {
                    cid: asdict(analytics) for cid, analytics in self.character_analytics.items()
                }
            
            if "story_analytics" in data_types:
                export_data["story_analytics"] = {
                    sid: asdict(analytics) for sid, analytics in self.story_analytics.items()
                }
            ...
            if export_format.lower() == "json":
                export_result = {
                    ...
                    "record_count": sum(len(v) if isinstance(v, (list, dict)) else 1 for v in export_data.values())
                }
            else:
            ...
            logger.info(f"Exported analytics data: {export_format}, {time_window.value}")
    ...
    async def _process_event(self, event: AnalyticsEvent):
        ...
        try:
            ...
            if event.user_id:
                await self._update_user_engagement(event.user_id, event.event_type, event.properties)
    ...
    async def _update_user_engagement(self, user_id: str, activity_type: str,
        ...
        try:
            ...
            if activity_type == "story_generated":
                ...
                if activity_data:
                    engagement.total_time_spent += activity_data.get('session_duration', 0)
    ...
    async def _update_character_usage(self, character_id: str, user_id: Optional[str],
                                    story_id: Optional[str], context: str = "story"):
        ...
        try:
            ...
            # Track unique users
            if user_id and user_id not in getattr(char_analytics, '_unique_users_set', set()):
    ...
    async def _update_real_time_metrics(self, event: AnalyticsEvent):
        ...
        if current_minute not in self.real_time_metrics:
            self.real_time_metrics[current_minute] = {
                'events': 0, 'users': set(), 'stories': 0, 'errors': 0
            }
        ...
        while True:
            try:
                ...
                self.real_time_metrics = {
                    timestamp: metrics for timestamp, metrics in self.real_time_metrics.items()
        ...
        while True:
            try:
                ...
                self.system_metrics_history = deque(
                    (metric for metric in self.system_metrics_history if metric.timestamp >= cutoff_date),
                    maxlen=self.system_metrics_history.maxlen
                )
                
                logger.info(f"Completed data cleanup for data older than {self.data_retention_days} days")
    ...
    def _filter_events_by_time(self, start_time: datetime, 
                              filters: Optional[Dict[str, Any]] = None) -> List[AnalyticsEvent]:
        ...
        if filters:
            ...
            if 'story_id' in filters:
                filtered_events = [e for e in filtered_events if e.story_id == filters['story_id']]
        
        return filtered_events
    
    def _calculate_engagement_level(self, engagement: UserEngagement) -> EngagementLevel:
    ...
    async def _generate_engagement_report(self, report_id: str, time_window: TimeWindow,
                                        events: List[AnalyticsEvent]) -> AnalyticsReport:
    ...
    async def _generate_story_report(self, report_id: str, time_window: TimeWindow,
                                   events: List[AnalyticsEvent]) -> AnalyticsReport:
    ...
    async def _generate_character_report(self, report_id: str, time_window: TimeWindow,
                                       events: List[AnalyticsEvent]) -> AnalyticsReport:
    ...
    async def _generate_performance_report(self, report_id: str, time_window: TimeWindow,
                                         events: List[AnalyticsEvent]) -> AnalyticsReport:
    ...
    async def _generate_comprehensive_report(self, report_id: str, time_window: TimeWindow,
                                           events: List[AnalyticsEvent]) -> AnalyticsReport:
        """Generate comprehensive analytics report."""
        return AnalyticsReport(
            report_id=report_id,
    ...
    def _get_active_alerts(self) -> List[Dict[str, Any]]:
        ...
        if self.system_metrics_history:
            ...
            if latest_metrics.api_response_time > self.alert_thresholds['response_time']:
                alerts.append({
                    "type": "response_time",
                    "severity": "medium",
    ...
    async def _check_system_alerts(self, metrics: SystemMetrics):
        ...
        if metrics.api_response_time > self.alert_thresholds['response_time']:
            logger.warning(f"High response time detected: {metrics.api_response_time:.2f}s")
