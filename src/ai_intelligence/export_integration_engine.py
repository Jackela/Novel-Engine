...
class ExportIntegrationEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"ExportIntegrationEngine initialized with storage at {self.storage_path}"
        )
...
""",
    ...
    async def export_story(self, request: ExportRequest) -> ExportResult:
        ...
        try:
            ...
            if request.template:
                story_data = await self._apply_export_template(
                    story_data, request.template, request.options
                )
            ...
            if request.watermark:
                export_data = await self._apply_watermark(
                    export_data, request.watermark, request.format
                )
    ...
    async def create_story_share(
        self, share_config: ShareConfiguration
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Created share for story {share_config.story_id}: {share_config.share_id}"
            )
            return result
        ...
        try:
            ...
            if share_config.password_protected:
                if not password or not self._verify_password(
                    password, share_config.password_hash
                ):
            ...
            if parent_version:
                parent_content = next(
                    (
                        ...
                        if v.version_id == parent_version
                    ),
                    None,
                )
                if parent_content:
                    changes_summary = await self._calculate_content_changes(
    ...
    async def get_story_versions(
        self, story_id: str, branch_name: Optional[str] = None, limit: int = 50
    ) -> List[StoryVersion]:
        """
    ...
    async def restore_story_version(
        self, story_id: str, version_id: str, user_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Restored story {story_id} to version {target_version.version_number}"
            )
            return result
    ...
    async def sync_integration(
        self, connection_id: str, operation: str = "export"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not handler:
                return {
                    "success": False,
                    "error": f"No handler for integration type {connection.integration_type.value}",
                }
            ...
            logger.info(
                f"Synced integration {connection_id} with operation {operation}"
            )
            return sync_result
    ...
    async def batch_export_stories(
        ...
        try:
            ...
            if options and options.get("create_archive", False):
                archive_path = await self._create_batch_archive(
                    batch_id, export_results, export_format
                )
            ...
            logger.info(
                f"Completed batch export {batch_id} for {len(story_ids)} stories"
            )
            return result
    ...
    async def _export_html(
        ...
        try:
            ...
            # Get template
            template_config = self.export_templates.get(
                template_name, self.export_templates["html_standard"]
            )
            ...
            if isinstance(content, list):
                ...
                content = "\n".join(
                    f"<p>{paragraph.replace(chr(10), ' ')}</p>"
            ...
            if "metadata" in story_data:
                ...
                for key, value in story_data["metadata"].items():
                    metadata_items.append(
                        f"<span class='meta-item'><strong>{key.title()}:</strong> {value}</span>"
                    )
    ...
    async def _export_txt(
        ...
        try:
            ...
            text_content.append("-" * 50)
            text_content.append(
                f"Generated by Novel Engine on {datetime.now().strftime('%Y-%m-%d %H:%M')}"
            )
                ...
                if isinstance(story_data["content"], str):
                    ...
                    for i, paragraph in enumerate(story_data["content"]):
                        para_elem = ET.SubElement(
                            content_elem, "paragraph", {"index": str(i)}
                        )
    ...
    def _generate_export_cache_key(
        self, request: ExportRequest, story_data: Dict[str, Any]
    ) -> str:
        """Generate cache key for export."""
        story_hash = hashlib.md5(
            json.dumps(story_data, sort_keys=True).encode()
        ).hexdigest()
    ...
    async def _apply_watermark(
        self, export_data: bytes, watermark: str, format: ExportFormat
    ) -> bytes:
    ...
    async def _save_export_file(
        self, export_data: bytes, export_id: str, format: ExportFormat
    ) -> Path:
    ...
    async def _generate_export_metadata(
        self, story_data: Dict[str, Any], request: ExportRequest
    ) -> Dict[str, Any]:
    ...
    def _increment_version_number(
        self, current_version: str, action: VersionAction
    ) -> str:
...
def create_export_integration_engine(
    storage_path: str = "exports", max_file_size: int = 100 * 1024 * 1024
