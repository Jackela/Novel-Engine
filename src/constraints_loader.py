...
def validate_character_name(name: str) -> tuple[bool, Optional[str]]:
    ...
    if len(name) < constraints['minLength'] or len(name) > constraints['maxLength']:
        return False, f"Character name must be {constraints['minLength']}-{constraints['maxLength']} characters"
...
def validate_character_description(description: str) -> tuple[bool, Optional[str]]:
    ...
    if len(description) < constraints['minLength'] or len(description) > constraints['maxLength']:
        return False, f"Description must be {constraints['minLength']}-{constraints['maxLength']} characters"
...
def validate_file_upload(file_size: int, filename: str) -> tuple[bool, Optional[str]]:
    ...
    if file_size > constraints['maxSize']:
        return False, f"File '{filename}' exceeds maximum size of {constraints['maxSizeMB']}MB"
    
    file_extension = os.path.splitext(filename)[1].lower()
    if file_extension not in constraints['allowedTypes']:
        allowed_types = ', '.join(constraints['allowedTypes'])
        return False, f"File '{filename}' has unsupported type. Allowed: {allowed_types}"
