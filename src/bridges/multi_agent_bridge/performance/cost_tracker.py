...
class CostTracker:
    ...
    def update_cost(self, request_type: str, cost: float, tokens: int) -> bool:
        ...
        try:
            ...
            if self.current_turn_cost > self.max_cost_per_turn:
                self.logger.warning(f"Turn cost exceeded: ${self.current_turn_cost:.4f} > ${self.max_cost_per_turn:.4f}")
                return False
                
            if self.total_cost > self.max_total_cost:
                self.logger.warning(f"Total cost exceeded: ${self.total_cost:.4f} > ${self.max_total_cost:.4f}")
                return False
    ...
    def is_under_budget(self, estimated_additional_cost: float = 0.0) -> bool:
        ...
        return (projected_turn_cost <= self.max_cost_per_turn and 
                projected_total_cost <= self.max_total_cost)
        ...
        try:
            ...
            if total_requests > 0:
                for request_type, cost in self.costs_by_type.items():
                    cost_percentage = (cost / self.total_cost) * 100 if self.total_cost > 0 else 0
                    count_percentage = (self.request_counts[request_type] / total_requests) * 100
