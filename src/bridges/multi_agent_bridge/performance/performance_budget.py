...
class PerformanceBudget:
    ...
    def record_batch_time(self, duration: float) -> None:
        ...
        if duration > self.max_batch_time_seconds:
            self.logger.warning(f"Batch time exceeded budget: {duration:.3f}s > {self.max_batch_time_seconds:.3f}s")
    ...
    def record_llm_time(self, duration: float) -> None:
        ...
        if duration > self.max_llm_wait_seconds:
            self.logger.warning(f"LLM time exceeded budget: {duration:.3f}s > {self.max_llm_wait_seconds:.3f}s")
    ...
    def complete_turn(self) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_data = {
                ...
                'avg_llm_time': sum(self.llm_times) / len(self.llm_times) if self.llm_times else 0.0
            }
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            turn_times = [entry['total_turn_time'] for entry in self.performance_history]
            budget_exceeds = sum(1 for entry in self.performance_history if entry['budget_exceeded'])
            utilizations = [entry['budget_utilization'] for entry in self.performance_history]
    ...
    def _calculate_performance_trend(self) -> str:
        ...
        try:
            ...
            recent_avg = sum(entry['total_turn_time'] for entry in self.performance_history[-5:]) / 5
            previous_avg = sum(entry['total_turn_time'] for entry in self.performance_history[-10:-5]) / 5
    ...
    def adjust_budgets_for_performance(self, target_utilization: float = 80.0) -> Dict[str, float]:
        ...
        try:
            ...
            adjustment_factor = target_utilization / current_utilization if current_utilization > 0 else 1.0
