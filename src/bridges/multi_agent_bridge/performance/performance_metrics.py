...
class PerformanceMetrics:
    ...
    def _get_trend_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Cost trend
            costs = [
                turn["cost_data"]["current_turn_cost"]
                for turn in recent_turns
                if "cost_data" in turn
            ]
            cost_trend = "stable"
            if len(costs) >= 2:
    ...
    def _calculate_system_health_score(self) -> float:
        ...
        try:
            ...
            remaining_budget_pct = (
                cost_stats.get("remaining_total_budget", 0)
                / max(0.01, self.cost_tracker.max_total_cost)
            ) * 100
            cost_health = min(
                1.0, remaining_budget_pct / 50
            )  # 50% remaining = full health
    ...
    def _get_combined_recommendations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            recommendations.extend(
                self.performance_budget.get_optimization_recommendations()
            )
