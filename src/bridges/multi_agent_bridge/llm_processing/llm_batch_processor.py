...
class LLMBatchProcessor:
    ...
    async def _should_process_immediately(self, batch_request: LLMBatchRequest) -> bool:
        ...
        try:
            ...
            if (
                batch_request.priority == RequestPriority.HIGH
                and len(self._request_queue) < 2
            ):
                return True
        ...
        try:
            ...
            if llm_response.success:
                ...
                parsed_results = self._parse_batch_response(
                    llm_response.content, requests
                )

                # Update tracking
                self.cost_tracker.update_cost(
                    request_type, llm_response.cost, llm_response.tokens_used
                )
    ...
    def _create_dialogue_batch_prompt(self, requests: List[LLMBatchRequest]) -> str:
        ...
        batch_parts.append(
            "Process each dialogue independently and provide responses in order."
        )
    ...
    def _parse_batch_response(
        ...
        try:
            ...
            matches = re.findall(
                response_pattern, response_content, re.DOTALL | re.IGNORECASE
            )
    ...
    def _update_batch_stats(self, batch_size: int, processing_time: float) -> None:
        ...
        try:
            ...
            self._stats["avg_batch_time"] = (
                (current_avg_time * (total_batches - 1)) + processing_time
            ) / total_batches
