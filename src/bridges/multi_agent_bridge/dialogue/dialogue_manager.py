...
from ..core.types import (
    AgentDialogue, CommunicationType, DialogueState, 
    RequestPriority, LLMBatchRequest
)
...
class DialogueManager:
    ...
    def __init__(self, llm_processor: LLMBatchProcessor, 
        ...
        self._stats = {
            ...
            'dialogues_by_type': {comm_type.value: 0 for comm_type in CommunicationType}
        }
    
    async def initiate_dialogue(self, initiator_id: str, target_id: str,
                              communication_type: CommunicationType = CommunicationType.DIALOGUE,
                              context: Optional[Dict[str, Any]] = None,
                              max_exchanges: int = 3) -> str:
        """
        ...
        try:
            ...
            self.logger.info(f"Initiated {communication_type.value} dialogue {dialogue_id} between {initiator_id} and {target_id}")
    ...
    async def execute_dialogue(self, dialogue_id: str, fast_mode: bool = False) -> Dict[str, Any]:
        """
    ...
    async def _execute_dialogue_with_llm(self, dialogue: AgentDialogue) -> Dict[str, Any]:
        ...
        try:
            ...
            # Wait for result
            llm_result = await self.llm_processor.wait_for_result(request_id, timeout_seconds=20.0)
            
            if llm_result.get('success'):
                # Process LLM response
                dialogue_result = self._process_llm_dialogue_result(dialogue, llm_result)
            ...
            return {'success': False, 'error': str(e), 'dialogue_id': dialogue.dialogue_id}
    ...
    def _determine_dialogue_priority(self, dialogue: AgentDialogue) -> RequestPriority:
        ...
        try:
            ...
            if dialogue.communication_type in [CommunicationType.NEGOTIATION, CommunicationType.STRATEGIC]:
        ...
        try:
            ...
            for line in content.split('\n'):
                ...
                if line.startswith('**Outcome:**'):
                    ...
                    relationship_impact = line.replace('**Relationship Impact:**', '').strip()
    ...
    def _generate_fast_dialogue_outcome(self, dialogue: AgentDialogue) -> Dict[str, Any]:
        ...
        try:
            ...
            outcome = outcomes.get(dialogue.communication_type, "Completed their interaction")
    ...
    def _calculate_dialogue_quality(self, dialogue_outcome: Dict[str, Any], llm_result: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if dialogue_outcome.get('outcome') and len(dialogue_outcome.get('outcome', '')) > 20:
                quality_score += 0.1
    ...
    def _update_quality_stats(self, quality: float) -> None:
        ...
        try:
            ...
            # Running average
            self._stats['avg_dialogue_quality'] = ((current_avg * (successful_count - 1)) + quality) / successful_count
    ...
    def get_dialogue_history(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        recent_history = self._dialogue_history[-limit:] if self._dialogue_history else []
    ...
    def get_dialogue_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats.update({
                ...
                'success_rate': (self._stats['successful_dialogues'] / max(1, self._stats['total_dialogues_initiated'])) * 100,
                'most_common_type': max(self._stats['dialogues_by_type'].items(), key=lambda x: x[1])[0] if self._stats['dialogues_by_type'] else 'none'
            })
    ...
    def cleanup_old_dialogues(self, max_history: int = 100) -> None:
        ...
        try:
            if len(self._dialogue_history) > max_history:
                ...
                self.logger.debug(f"Cleaned up dialogue history to {max_history} entries")
        except Exception as e:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            for dialogue_id in list(self._active_dialogues.keys()):
                result = await self._simulate_dialogue_fast(self._active_dialogues[dialogue_id])
                self.logger.info(f"Completed dialogue {dialogue_id} during shutdown: {result.get('outcome', 'unknown')}")
