...
class PerformanceMonitor:
    ...
    def get_metric_stats(self, metric: PerformanceMetric, 
                        window_seconds: float = 60.0) -> Dict[str, float]:
        """Get statistical summary of a metric over time window."""
        now = time.time()
        cutoff = now - window_seconds
        
        values = [value for timestamp, value in self.metrics[metric] 
                 if timestamp >= cutoff]
...
class BatchProcessor:
    ...
    def __init__(self, config: OptimizationConfig):
        self.config = config
        self.pending_operations: Dict[str, List[Tuple[Any, asyncio.Future]]] = defaultdict(list)
    ...
    async def _process_database_batch(self, operations: List[Tuple[Any, asyncio.Future]]):
    ...
    async def _process_cache_invalidation_batch(self, operations: List[Tuple[Any, asyncio.Future]]):
...
class PerformanceOptimizationEngine:
        ...
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self.monitor.add_metric(PerformanceMetric.RESPONSE_TIME, duration_ms)
                    ...
                    duration_ms = (time.time() - start_time) * 1000
                    logger.error(f"Operation {operation_name} failed after {duration_ms:.2f}ms: {e}")
                    raise
    ...
    async def _run_optimizations(self):
        ...
        # Performance alerts
        response_time_stats = self.monitor.get_metric_stats(PerformanceMetric.RESPONSE_TIME)
        if response_time_stats and response_time_stats.get("avg", 0) > self.targets.max_response_time_ms:
            logger.warning(f"High average response time: {response_time_stats['avg']:.2f}ms")
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if self.request_times:
            recent_requests = len([t for t in self.request_times if time.time() - t/1000 < 60])
        ...
        return {
            ...
            "performance_targets": {
                ...
                "target_met": {
                    "response_time": avg_response_time <= self.targets.max_response_time_ms,
