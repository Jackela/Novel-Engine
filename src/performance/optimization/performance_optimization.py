...
class PerformanceOptimizer:
    ...
    async def cached_operation(self, cache_key: str, operation: callable, *args, **kwargs):
...
async def optimize_character_loading(character_factory, character_name: str):
    ...
    async def load_character():
        ...
        await performance_optimizer.event_bus.emit("character_loading_start", character_name)
        
        # Load character (this would be the original synchronous operation)
        agent = character_factory.create_character(character_name)
        
        await performance_optimizer.event_bus.emit("character_loading_complete", character_name)
        return agent
...
class AsyncSimulationManager:
    ...
    async def start_simulation(self, simulation_id: str, character_names: List[str], turns: int):
        ...
        try:
            ...
            result = {
                ...
                'duration_seconds': time.time() - self.active_simulations[simulation_id]['start_time']
            }
    ...
    async def cleanup_completed_simulations(self):
        ...
        async with self._lock:
            ...
            for sim_id, sim_data in self.active_simulations.items():
                if (sim_data['status'] in ['completed', 'failed'] and 
                    current_time - sim_data['start_time'] > 3600):  # 1 hour
...
if __name__ == "__main__":
    ...
    async def test_performance():
        ...
        async with performance_optimizer.get_db_connection() as conn:
            await conn.execute("CREATE TABLE IF NOT EXISTS test (id INTEGER, data TEXT)")
            await conn.commit()
