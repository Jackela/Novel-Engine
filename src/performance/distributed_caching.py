...
class RedisCache(CacheInterface):
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379", 
                 prefix: str = "novel_engine:", default_ttl: int = 3600):
...
class DistributedCache:
    ...
    def _update_combined_metrics(self, hit_type: str, response_time: float):
        ...
        self.combined_metrics.average_response_time = (
            (self.combined_metrics.average_response_time * (total - 1) + response_time) / total
        )
...
class CharacterCache:
    ...
    async def set_character(self, character_id: str, character_data: Dict, ttl: int = 3600) -> bool:
    ...
    async def _load_character(self, key: str) -> Optional[Dict]:
        ...
        return {"id": character_id, "name": f"Character {character_id}", "loaded_from_db": True}
...
async def main():
    ...
    custom_character = {"id": "custom_1", "name": "Custom Character", "faction": "Imperial"}
    ...
    for level, metric in metrics.items():
        logger.info(f"{level} cache - Hit rate: {metric.hit_rate:.1f}%, "
