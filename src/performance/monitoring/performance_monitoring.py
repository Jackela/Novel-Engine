...
class MetricsCollector:
    ...
    def get_metric_history(self, name: str, duration_seconds: int = 3600) -> List[MetricPoint]:
...
class SystemMetricsCollector:
    ...
    def collect_system_metrics(self):
        ...
        try:
            ...
            self.collector.record_gauge("system.memory.total_mb", memory.total / 1024 / 1024)
            self.collector.record_gauge("system.memory.available_mb", memory.available / 1024 / 1024)
            self.collector.record_gauge("system.memory.used_percent", memory.percent)
            
            # Process-specific metrics
            process_memory = self.process.memory_info()
            self.collector.record_gauge("process.memory.rss_mb", process_memory.rss / 1024 / 1024)
            self.collector.record_gauge("process.memory.vms_mb", process_memory.vms / 1024 / 1024)
            ...
            if self.last_io_counters:
                read_rate = (io_counters.read_bytes - self.last_io_counters.read_bytes) / 1024 / 1024
                write_rate = (io_counters.write_bytes - self.last_io_counters.write_bytes) / 1024 / 1024
            ...
            for i, stats in enumerate(gc_stats):
                self.collector.record_gauge(f"gc.generation_{i}.collections", stats['collections'])
                self.collector.record_gauge(f"gc.generation_{i}.collected", stats['collected'])
                self.collector.record_gauge(f"gc.generation_{i}.uncollectable", stats['uncollectable'])
...
class ApplicationMetricsCollector:
    ...
    def end_request(self, request_id: str, status_code: int = 200, endpoint: str = "unknown"):
        """End tracking a request."""
        start_time = self.request_start_times.pop(request_id, None)
        if start_time:
            duration = time.time() - start_time
            self.collector.record_timer("http.request.duration", duration, {
                'endpoint': endpoint,
                'status_code': str(status_code)
            })
            self.collector.record_counter("http.requests.total", 1.0, {
                'endpoint': endpoint,
                'status_code': str(status_code)
            })
    ...
    def record_database_query(self, query_type: str, duration: float, success: bool = True):
        ...
        self.collector.record_timer("database.query.duration", duration, {
            'query_type': query_type,
            'success': str(success)
        })
        self.collector.record_counter("database.queries.total", 1.0, {
            'query_type': query_type,
            'success': str(success)
        })
    ...
    def record_cache_operation(self, operation: str, hit: bool = True):
        ...
        self.collector.record_counter("cache.operations.total", 1.0, {
            'operation': operation,
            'result': 'hit' if hit else 'miss'
        })
    
    def record_task_execution(self, task_type: str, duration: float, success: bool = True):
        """Record task execution metrics."""
        self.collector.record_timer("task.execution.duration", duration, {
            'task_type': task_type,
            'success': str(success)
        })
        self.collector.record_counter("task.executions.total", 1.0, {
            'task_type': task_type,
            'success': str(success)
        })
...
class AlertManager:
    ...
    def add_threshold(self, threshold: PerformanceThreshold):
        ...
        logger.info(f"Added performance threshold: {threshold.metric_name} {threshold.operator} {threshold.threshold_value}")
    ...
    def check_thresholds(self):
        ...
        for metric_name, threshold in self.thresholds.items():
            ...
            if violated:
                ...
                if self.violation_counts[metric_name] >= threshold.consecutive_violations:
    ...
    def _generate_alert(self, metric_name: str, current_value: float, threshold: PerformanceThreshold):
...
class PerformanceRegression:
    ...
    def establish_baseline(self, metric_name: str, duration_hours: int = 24):
        ...
        logger.info(f"Established baseline for {metric_name}: mean={baseline['mean']:.2f}, stddev={baseline['stddev']:.2f}")
    
    def check_regression(self, metric_name: str, current_value: Optional[float] = None) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'direction': direction,
            'severity': 'high' if z_score > self.sensitivity * 1.5 else 'medium' if is_regression else 'low'
        }
...
class PerformanceMonitor:
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        trends = {}
        key_metrics = ["process.memory.rss_mb", "system.cpu.usage_percent", "http.request.duration"]
    ...
    def _calculate_system_health(self) -> Dict[str, Any]:
        ...
        health_factors = {
            'memory': self.collector.get_current_value("system.memory.used_percent") or 0,
            'cpu': self.collector.get_current_value("system.cpu.usage_percent") or 0,
            'process_memory': self.collector.get_current_value("process.memory.rss_mb") or 0,
        }
        
        # Calculate health score (0-100)
        memory_score = max(0, 100 - health_factors['memory'])
        cpu_score = max(0, 100 - health_factors['cpu'])
        process_memory_score = max(0, 100 - min(health_factors['process_memory'] / 10, 100))  # Scale by 10MB
...
class TimerContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            performance_monitor.collector.record_timer(self.metric_name, duration, self.tags)
...
if __name__ == "__main__":
    ...
    async def test_performance_monitoring():
        ...
        for i in range(20):
            ...
            performance_monitor.app_collector.record_cache_operation("GET", hit=(i % 3 != 0))
