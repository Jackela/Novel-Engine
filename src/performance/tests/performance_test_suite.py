...
class ResourceMonitor:
    ...
    def start_monitoring(self, target_process_name: str = "python"):
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                if target_process_name in proc.info[
                    "name"
...
class PerformanceTestSuite:
    ...
    async def single_user_test(
        self, endpoint: str, requests_count: int = 10
    ) -> PerformanceMetrics:
        """Test a single endpoint with sequential requests."""
        logger.info(
            f"Running single user test for {endpoint} ({requests_count} requests)"
        )
    ...
    async def concurrent_user_test(
        self, endpoint: str, concurrent_users: int, duration_seconds: int = 60
    ) -> PerformanceMetrics:
    ...
    async def stress_test(
        ...
        while current_users <= max_users:
            ...
            metrics = await self.concurrent_user_test(
    ...
    async def endpoint_load_test(
        self, endpoints: List[str], concurrent_users: int = 10
    ) -> Dict[str, Any]:
    ...
    def run_simulation_load_test(
        self, concurrent_simulations: int = 5
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for endpoint in critical_endpoints:
                try:
                    ...
                    logger.info(
                        f"Stress test {endpoint}: max stable load = {max_stable} users"
                    )
    ...
    def generate_performance_report(self, results: Dict[str, Any]) -> str:
        ...
        if resource_metrics:
            ...
            if cpu_max > 80:
                ...
                report.append(
                    "   - Optimize algorithmic complexity in story generation"
                )
        ...
        total_errors = sum(
            metrics.get("error_count", 0)
            for metrics in results.get("baseline_tests", {}).values()
            if "error" not in metrics
        )
        if total_errors == 0:
            score_factors.append(100)
