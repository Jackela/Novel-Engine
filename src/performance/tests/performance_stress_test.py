...
class PerformanceStressTester:
    ...
    async def _test_api_load(self) -> StressTestResult:
        ...
        async def simulate_api_request():
            ...
            try:
                ...
                await asyncio.sleep(
                    0.01 + (0.005 * (len(response_times) % 10))
                )  # Variable delay
        ...
        async def worker_operations(worker_id: int):
            ...
            for i in range(operations_per_worker):
                ...
                try:
                    ...
                    # Memory allocation
                    [f"data_{worker_id}_{i}_{j}" for j in range(50)]
    ...
    async def _test_memory_stress(self) -> StressTestResult:
        ...
        try:
            ...
            for i in range(100):
                ...
                data = {f"key_{j}": f"value_{j}" * 100 for j in range(1000)}
    ...
    def _calculate_performance_grade(self) -> str:
        ...
        for result in self.results:
            ...
            rps_score = (
                min(result.requests_per_second / 100, 1.0) * 25
            )  # Target: 100 RPS
    ...
    def _analyze_system_resources(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_usage": {
                ...
                "average_mb": statistics.mean(
                    [m.memory_mb for m in self.system_metrics]
                ),
            },
            "io_activity": {
        ...
        for result in self.results:
            if result.error_rate > 1.0:
                recommendations.append(
                    f"High error rate in {result.test_name}: {result.error_rate:.1f}% - investigate error handling"
                )
            ...
            if result.cpu_usage_percent > 80:
                recommendations.append(
                    f"High CPU usage in {result.test_name}: {result.cpu_usage_percent:.1f}% - optimize algorithms"
                )

            if result.requests_per_second < 50:
                recommendations.append(
                    f"Low throughput in {result.test_name}: {result.requests_per_second:.1f} RPS - scale horizontally"
                )

        # General recommendations
        if not recommendations:
            recommendations.append(
                "Excellent performance! System is optimized for production workloads."
            )
