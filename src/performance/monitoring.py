...
class PerformanceMonitor:
    ...
    def record_request_duration(self, endpoint: str, duration_ms: float, 
        ...
        self.record_metric("response_time_ms", duration_ms, MetricType.TIMER, current_time)
        ...
        if status_code >= 400 or error:
            ...
            self.record_metric("error_count", 1, MetricType.COUNTER, current_time, 
        ...
        self.record_metric("error_rate_percent", error_rate, MetricType.GAUGE, current_time)
    ...
    def record_database_query(self, query_type: str, duration_ms: float, 
        ...
        self.record_metric("database_query_time_ms", duration_ms, MetricType.TIMER, 
                          current_time, tags)
        
        if error:
            self.record_metric("database_error_count", 1, MetricType.COUNTER, 
                              current_time, tags)
    ...
    def record_cache_operation(self, operation: str, hit: bool, duration_ms: float):
        ...
        self.record_metric("cache_operation_time_ms", duration_ms, MetricType.TIMER, 
                          current_time, tags)
        
        if hit:
            self.record_metric("cache_hit_count", 1, MetricType.COUNTER, current_time, tags)
        else:
            self.record_metric("cache_miss_count", 1, MetricType.COUNTER, current_time, tags)
        
        # Calculate hit rate
        hit_metrics = [m for m in self.metrics["cache_hit_count"] if m.timestamp > current_time - 300]
        miss_metrics = [m for m in self.metrics["cache_miss_count"] if m.timestamp > current_time - 300]
    ...
    async def _check_alerts(self):
        ...
        for metric_name, thresholds in self.config.alert_thresholds.items():
            ...
            for severity_name, threshold in thresholds.items():
                severity = AlertSeverity.CRITICAL if severity_name == "critical" else AlertSeverity.WARNING
    ...
    async def _export_metrics(self):
        ...
        try:
            ...
            export_files = sorted([
                f for f in os.listdir(self.config.export_path) 
    ...
    def get_metric_stats(self, metric_name: str, 
        ...
        return {
            ...
            "percentile_95": statistics.quantiles(recent_metrics, n=20)[18] if len(recent_metrics) >= 20 else max(recent_metrics),
            "percentile_99": statistics.quantiles(recent_metrics, n=100)[98] if len(recent_metrics) >= 100 else max(recent_metrics)
        }
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "timestamp": current_time,
            "uptime_seconds": current_time - (self.metrics["cpu_usage_percent"][0].timestamp if self.metrics["cpu_usage_percent"] else current_time),
            "active_alerts": len(self.active_alerts),
            "total_requests": sum(stats["count"] for stats in self.endpoint_metrics.values()),
            "total_errors": sum(stats["errors"] for stats in self.endpoint_metrics.values()),
            "avg_response_time_ms": statistics.mean(recent_response_times) if recent_response_times else 0,
    ...
    def decorator(func: Callable) -> Callable:
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                monitor.record_metric(f"{name}_duration_ms", duration_ms, MetricType.TIMER)
...
__all__ = [
    ...
    'MonitoringConfig', 'SystemResourceMonitor', 'PerformanceMonitor',
    'measure_performance', 'get_performance_monitor', 'initialize_performance_monitor'
]