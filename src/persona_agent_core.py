...
class PersonaAgentCore:
    ...
    def __init__(self, character_directory_path: str, event_bus: EventBus, agent_id: Optional[str] = None):
        ...
        """
        logger.info(f"Initializing PersonaAgent core infrastructure for path: {character_directory_path}")
    ...
    def _setup_event_handling(self) -> None:
        ...
        try:
            ...
            logger.info(f"PersonaAgent '{self.agent_id}' subscribed to TURN_START events")
        except Exception as e:
    ...
    def handle_turn_start(self, world_state_update: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"Error handling TURN_START for agent {self.agent_id}: {str(e)}")
        ...
        try:
            ...
            if isinstance(world_state_update, dict):
                ...
                if len(self.subjective_worldview['recent_events']) > 10:
                    self.subjective_worldview['recent_events'] = self.subjective_worldview['recent_events'][-10:]
                
                logger.debug(f"Agent {self.agent_id} processed world state update for turn {event_summary['turn_number']}")
                
        except Exception as e:
            logger.error(f"Error processing world state update for agent {self.agent_id}: {str(e)}")
    
    def _create_basic_action(self, world_state_update: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(f"Error creating basic action for agent {self.agent_id}: {str(e)}")
            return None
    ...
    def get_agent_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'known_entities_count': len(self.subjective_worldview.get('known_entities', {})),
                'recent_events_count': len(self.subjective_worldview.get('recent_events', [])),
    ...
    def update_character_state(self, status: Optional[str] = None, location: Optional[str] = None, 
        ...
        try:
            ...
            if location is not None:
                ...
                logger.info(f"Agent {self.agent_id} moved from {old_location} to {location}")
            
            if morale is not None:
                # Clamp morale to valid range
                self.morale_level = max(-1.0, min(1.0, morale))
                logger.info(f"Agent {self.agent_id} morale updated to: {self.morale_level}")
    ...
    def add_relationship(self, entity_id: str, relationship_strength: float) -> None:
        ...
        try:
            ...
            logger.info(f"Agent {self.agent_id} relationship with {entity_id} set to {strength}")
    ...
    def add_to_subjective_worldview(self, category: str, key: str, value: Any) -> None:
        ...
        try:
            if category in self.subjective_worldview:
                ...
                logger.debug(f"Agent {self.agent_id} updated subjective worldview: {category}[{key}]")
            else:
                logger.warning(f"Unknown worldview category '{category}' for agent {self.agent_id}")
                
        except Exception as e:
            logger.error(f"Error updating subjective worldview for {self.agent_id}: {e}")
    ...
    def get_core_metrics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'worldview_size': {
                    'known_entities': len(self.subjective_worldview.get('known_entities', {})),
                    'location_knowledge': len(self.subjective_worldview.get('location_knowledge', {})),
                    'recent_events': len(self.subjective_worldview.get('recent_events', []))
                },
                'relationships_count': len(self.relationships),
