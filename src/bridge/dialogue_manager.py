...
from .types import AgentDialogue, CommunicationType, DialogueState, RequestPriority
...
class DialogueManager:
    ...
    async def initiate_dialogue(
        ...
        try:
            ...
            if not await self._are_agents_available([initiator_id, target_id]):
                self.logger.warning(f"One or more agents unavailable for dialogue: {initiator_id}, {target_id}")
                return None
            ...
            self.logger.info(f"Dialogue {dialogue_id} initiated: {initiator_id} -> {target_id} ({communication_type.value})")
    ...
    async def add_message(
        ...
        try:
            ...
            if sender_id not in dialogue.participants:
                self.logger.error(f"Agent {sender_id} not a participant in dialogue {dialogue_id}")
                return False
            
            # Check if dialogue is in valid state for messages
            if dialogue.state not in [DialogueState.ACTIVE, DialogueState.INITIATING]:
                self.logger.warning(f"Cannot add message to dialogue {dialogue_id} in state {dialogue.state}")
                return False
    ...
    async def conclude_dialogue(
        ...
        try:
            ...
            dialogue.state = DialogueState.INTERRUPTED if forced else DialogueState.CONCLUDED
            ...
            self.logger.info(f"Dialogue {dialogue_id} concluded ({'forced' if forced else 'natural'})")
    ...
    async def get_agent_dialogues(self, agent_id: str) -> List[AgentDialogue]:
        ...
        try:
            ...
            return [self.active_dialogues[did] for did in dialogue_ids if did in self.active_dialogues]
        except Exception as e:
        ...
        try:
            ...
            if dialogue.current_exchange >= dialogue.max_exchanges:
                await self.conclude_dialogue(
                    ...
                    {'reason': 'max_exchanges_reached', 'total_exchanges': dialogue.current_exchange}
                )
                return
            
            # Auto-conclude if inactive for too long
            if dialogue.messages:
                last_message_time = datetime.fromisoformat(dialogue.messages[-1]['timestamp'])
                if datetime.now() - last_message_time > timedelta(minutes=10):
                    await self.conclude_dialogue(
                        dialogue.dialogue_id,
                        {'reason': 'timeout', 'last_activity': last_message_time.isoformat()}
                    )
    ...
    def _get_average_exchanges(self) -> float:
        ...
        total_exchanges = sum(d.current_exchange for d in self.active_dialogues.values())
    ...
    async def cleanup_expired_dialogues(self) -> int:
        ...
        try:
            ...
            for dialogue_id in expired_dialogues:
                if await self.conclude_dialogue(dialogue_id, {'reason': 'expired'}, forced=True):
                    cleanup_count += 1
