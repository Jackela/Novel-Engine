...
class EnterpriseSecurityManager:
    ...
    async def evaluate_request_security(self, request: Request, user_id: Optional[str] = None) -> Tuple[bool, List[SecurityAction], ThreatLevel]:
        """
        ...
        try:
            ...
            if ip_reputation["threat_level"] != ThreatLevel.LOW:
                threat_indicators.append(f"ip_reputation_{ip_reputation['threat_level']}")
            ...
            pattern_analysis = await self._analyze_request_patterns(client_ip, request_path, user_agent)
            ...
            is_allowed = max_threat_level not in [ThreatLevel.CRITICAL, ThreatLevel.BLOCKED]
    ...
    async def _check_rate_limits(self, ip_address: str, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _analyze_geographic_risk(self, ip_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            is_high_risk = (
                country_code in GEO_BLOCK_HIGH_RISK_COUNTRIES or
                response.traits.is_anonymous_proxy or
    ...
    async def _analyze_behavioral_patterns(self, user_id: str, ip_address: str, 
                                         user_agent: str, request_path: str) -> Dict[str, Any]:
        """Behavioral analytics and anomaly detection"""
        ...
        if geo_info["country_code"] and geo_info["country_code"] not in profile.typical_countries:
            anomaly_score += 0.3
        ...
        await self._update_behavioral_profile(user_id, current_hour, 
                                            geo_info.get("country_code"), 
                                            user_agent, request_category)
        
        return {
            "anomaly_score": min(anomaly_score, 1.0),
            "anomalies": anomalies,
            "profile_age_days": (datetime.now(timezone.utc) - profile.last_updated).days
        }
    
    async def _analyze_request_patterns(self, ip_address: str, request_path: str, 
        ...
        sql_patterns = [
            r"union.*select", r"or\s+1=1", r"drop\s+table", 
            r"insert\s+into", r"delete\s+from", r"update.*set"
        ]
            ...
            async with aiosqlite.connect(self.database_path) as conn:
                ...
                if row:
                    return BehavioralProfile(
                        ...
                        last_updated=datetime.fromisoformat(row[6]) if row[6] else datetime.now(timezone.utc)
                    )
        except Exception as e:
    ...
    async def _update_behavioral_profile(self, user_id: str, current_hour: int,
                                       country_code: Optional[str], user_agent: str,
            ...
            if not profile:
                ...
                if current_hour not in profile.typical_access_hours:
                    ...
                    if len(profile.typical_access_hours) > 24:
                        profile.typical_access_hours = profile.typical_access_hours[-24:]
    ...
    async def _log_security_event(self, event_type: str, severity: ThreatLevel, 
                                source_ip: str, user_id: Optional[str], user_agent: str,
                                request_path: str, threat_indicators: List[str],
                                automated_response: List[SecurityAction], 
                                evidence: Dict[str, Any]):
        ...
        try:
            async with aiosqlite.connect(self.database_path) as conn:
                await conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    event.id, event.timestamp, event.event_type, event.severity.value,
                    event.source_ip, event.user_id, event.user_agent, event.request_path,
                    json.dumps(event.threat_indicators), json.dumps([a.value for a in event.automated_response]),
                    json.dumps(event.evidence, default=str), json.dumps([f.value for f in event.compliance_tags])
                ))
                await conn.commit()
    ...
    async def _send_security_alert(self, event: SecurityEvent):
        ...
        await self.redis_client.publish("security_alerts", json.dumps(alert_data, default=str))
        
        logger.warning(f"🚨 SECURITY ALERT: {event.severity.value.upper()} - {event.event_type} from {event.source_ip}")
    ...
    async def add_ip_to_reputation_list(self, ip_address: str, list_type: str, 
                                      reason: str, severity: ThreatLevel, 
        ...
        logger.info(f"🛡️ Added IP {ip_address} to {list_type} (severity: {severity.value})")
...
__all__ = [
    ...
    'ThreatIntelligence', 'BehavioralProfile', 'SecurityEvent', 'SecurityMetrics',
