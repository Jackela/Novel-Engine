...
class RateLimiter:
    ...
    def _detect_suspicious_behavior(self, client: ClientState, request: Request) -> ThreatLevel:
        ...
        if client.last_request and now - client.last_request < 0.1:  # Less than 100ms
            client.suspicious_patterns["rapid_requests"] = client.suspicious_patterns.get("rapid_requests", 0) + 1
        ...
        if not user_agent or len(user_agent) < 10:
            client.suspicious_patterns["no_user_agent"] = client.suspicious_patterns.get("no_user_agent", 0) + 1
            threat_level = max(threat_level, ThreatLevel.MEDIUM)
        
        # Check for missing common headers
        if not request.headers.get("accept"):
            client.suspicious_patterns["missing_headers"] = client.suspicious_patterns.get("missing_headers", 0) + 1
            threat_level = max(threat_level, ThreatLevel.MEDIUM)
        ...
        if any(path in request.url.path for path in suspicious_paths):
            client.suspicious_patterns["suspicious_paths"] = client.suspicious_patterns.get("suspicious_paths", 0) + 1
            threat_level = max(threat_level, ThreatLevel.HIGH)
    ...
    async def check_rate_limit(self, request: Request) -> bool:
        ...
        if endpoint_config:
            endpoint_limit = endpoint_config.get("requests_per_minute", self.config.requests_per_minute)
    ...
    def get_client_info(self, request: Request) -> Dict[str, Any]:
        ...
        return {
            ...
            "remaining_tokens": {
                "minute": int(client.minute_bucket.tokens) if client.minute_bucket else 0,
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if "remaining_tokens" in client_info:
                ...
                response.headers["X-RateLimit-Remaining"] = str(client_info["remaining_tokens"]["minute"])
