...
class EncryptionService:
    ...
    def __init__(self, master_key: Optional[str] = None):
        if master_key:
            ...
            self.master_key = os.getenv("ENCRYPTION_MASTER_KEY", secrets.token_urlsafe(32)).encode()
...
class DataProtectionService:
    ...
    async def decrypt_personal_data(self, encrypted_data: Dict[str, Any], 
        ...
        if element.encryption_required:
            ...
            # Add additional sensitive fields
            sensitive_fields = ['email', 'password_hash', 'personal_info', 'content']
            fields_to_decrypt.extend([f for f in sensitive_fields if f in encrypted_data])
            
            return self.encryption_service.decrypt_dict(encrypted_data, fields_to_decrypt)
...
__all__ = [
    'DataClassification', 'PrivacyLevel', 'DataRetentionPeriod', 'ConsentStatus',
    'ProcessingLawfulBasis', 'DataElement', 'ConsentRecord', 'DataProcessingRecord',
    'EncryptionService', 'PseudonymizationService', 'DataProtectionService',
    'get_data_protection_service', 'initialize_data_protection_service'
]