...
class EnhancedSecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Security checks
        security_result = await self._perform_security_checks(request, client_ip, user_agent, endpoint)
        ...
        if processing_time > 1.0:  # Slow request - potential attack
            await self._log_security_event(
                "slow_request", client_ip, user_agent, endpoint,
                "medium", {"processing_time": processing_time}
            )
    ...
    async def _log_security_event(self, event_type: str, source_ip: str, 
                                user_agent: str, endpoint: str, severity: str, 
                                details: Dict[str, Any]):
