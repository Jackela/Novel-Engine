        ...
        try:
            ...
            async with aiosqlite.connect(self.database_path) as conn:
                ...
                """, (
                    ...
                    event.timestamp, event.source_ip, event.user_id, event.username,
                    event.user_agent, event.endpoint, event.method, event.status_code,
        ...
        try:
            async with aiosqlite.connect(self.database_path) as conn:
                ...
                """, (
                    ...
                    audit_log.resource, audit_log.timestamp, audit_log.ip_address,
    ...
    async def _analyze_threat(self, event: SecurityEvent):
        ...
        try:
            ...
            if event.event_type == SecurityEventType.LOGIN_FAILURE:
                ...
                recent_failures = [
                    t for t in self.failed_attempts[event.source_ip]
                    if t > time.time() - 3600  # Last hour
                ]
                
                if len(recent_failures) > 10:
                    risk_score += 0.5
                    await self._trigger_security_alert(event, "Brute force attack detected", risk_score)
        ...
        try:
            async with aiosqlite.connect(self.database_path) as conn:
                ...
                if row:
                    ...
                    new_risk = min(current_risk + 0.1, 1.0) if event.severity in [SecurityEventSeverity.HIGH, SecurityEventSeverity.CRITICAL] else current_risk
                    ...
                    """, (
                        ...
                        event.user_agent, event.timestamp, event.timestamp,
                        "active", 1
                    ))
            ...
            async with aiosqlite.connect(self.database_path) as conn:
                ...
                for row in rows:
                    ...
                    if event_dict['geolocation']:
                        event_dict['geolocation'] = json.loads(event_dict['geolocation'])
                    if event_dict['tags']:
...
__all__ = [
    'SecurityEventType', 'SecurityEventSeverity', 'ThreatLevel', 'SecurityEvent',
    'SecurityAuditLog', 'ThreatIntelligence', 'SecurityLogger',
    'get_security_logger', 'initialize_security_logger'
]