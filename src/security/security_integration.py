...
from .auth_system import (
    SecurityService, initialize_security_service, get_security_service,
    UserRole, Permission, User, TokenPair, UserRegistration, UserLogin
)
from .enterprise_security_manager import (
    EnterpriseSecurityManager, initialize_enterprise_security_manager,
    get_enterprise_security_manager, ThreatLevel, SecurityAction, ComplianceFramework
)
...
from .security_dashboard import (
    SecurityDashboard, initialize_security_dashboard, get_security_dashboard,
    AlertSeverity, IncidentStatus
)
...
class EnterpriseSecuritySuite:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"🎉 ENTERPRISE SECURITY SUITE INITIALIZED SUCCESSFULLY")
            logger.info(f"⏱️ Initialization completed in {init_duration:.2f}s")
            logger.info(f"🛡️ Security Features Active:")
            logger.info(f"   • JWT Authentication & RBAC")
            logger.info(f"   • Real-time Threat Detection")
            logger.info(f"   • Behavioral Analytics: {'✅' if self.enable_behavioral_analytics else '❌'}")
            logger.info(f"   • Geo-blocking: {'✅' if self.enable_geo_blocking else '❌'}")
            logger.info(f"   • Compliance Frameworks: {', '.join([f.value.upper() for f in self.compliance_frameworks])}")
            logger.info(f"   • Security Dashboard & Monitoring")
            logger.info(f"   • Input Validation & Sanitization")
    ...
    def configure_fastapi_security(self, app: FastAPI) -> FastAPI:
        ...
        if not self.is_initialized:
            raise RuntimeError("Security suite must be initialized before configuring FastAPI")
        ...
        async def security_evaluation_middleware(request: Request, call_next):
            ...
            try:
                ...
                is_allowed, security_actions, threat_level = await self.security_manager.evaluate_request_security(
                    request, user_id
                )
                ...
                if security_actions:
                    response.headers["X-Security-Actions"] = ",".join([a.value for a in security_actions])
    ...
    def _add_security_api_endpoints(self, app: FastAPI):
        ...
        async def get_security_metrics(
            current_user: User = Depends(self.auth_service.require_permission(Permission.SYSTEM_ADMIN))
        ):
    ...
    def _add_dashboard_endpoints(self, app: FastAPI):
        ...
        async def get_dashboard_overview(
            current_user: User = Depends(self.auth_service.require_permission(Permission.SYSTEM_HEALTH))
        ):
    ...
    async def get_security_status(self) -> Dict[str, Any]:
        ...
        if not self.is_initialized:
            return {"status": "not_initialized", "message": "Security suite not initialized"}
...
def create_secure_app(
    ...
    app = FastAPI(
        ...
        openapi_url="/openapi.json" if security_config.get("enable_docs", False) else None
    )
...
__all__ = [
    ...
    'create_secure_app', 'SecurityService', 'EnterpriseSecurityManager',
    'SecurityDashboard', 'SecurityMiddleware', 'UserRole', 'Permission',
    'ThreatLevel', 'SecurityAction', 'ComplianceFramework'
]