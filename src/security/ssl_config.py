...
class SSLConfig:
    ...
    protocol: ssl.Protocol = ssl.PROTOCOL_TLS_SERVER
...
class SSLCertificateManager:
    ...
    def generate_self_signed_cert(self, 
        ...
        subject = issuer = x509.Name([
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Dynamic Context Engineering"),
        ...
        cert = x509.CertificateBuilder().subject_name(
            ...
            x509.SubjectAlternativeName([
                x509.DNSName(name) for name in alt_names if not name.replace(".", "").replace(":", "").isdigit()
            ] + [
                x509.IPAddress(name) for name in alt_names if name.replace(".", "").replace(":", "").isdigit()
            ]),
            critical=False,
        ).add_extension(
            x509.KeyUsage(
...
class SSLContextBuilder:
    ...
    @staticmethod
    def create_client_context(ca_file: Optional[str] = None,
...
def setup_development_ssl(domain: str = "localhost") -> SSLConfig:
    ...
    cert_file, key_file = cert_manager.generate_self_signed_cert(
        domain=domain,
        alt_names=["localhost", "127.0.0.1", "::1", domain]
    )
...
__all__ = [
    'SSLConfig', 'SSLCertificateManager', 'SSLContextBuilder', 
    'HTTPSConfigGenerator', 'setup_development_ssl', 'setup_production_ssl'
]