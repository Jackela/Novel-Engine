...
try:
    ...
    from src.shared_types import (
        ...
        ActionTarget, EntityType, ValidationStatus, IronLawsReport
    )
...
class NovelEngineRunner:
    ...
    def setup_simulation(self) -> bool:
        ...
        try:
            ...
            self.director = DirectorAgent(
                ...
                campaign_log_path=temp_log_path
            )
            ...
            for char_config in self.config.characters:
                ...
                if agent:
                    ...
                    if success:
                        ...
                        logger.error(f"❌ Failed to register agent: {char_config.get('character_id')}")
                        return False
            ...
            logger.info(f"✅ Simulation setup complete: {len(self.agents)} agents ready")
            return True
    ...
    def _create_agent_from_config(self, char_config: Dict[str, Any]) -> Optional[PersonaAgent]:
        """
    ...
    def _build_character_data(self, char_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        character_data['stats'] = CharacterStats(
            strength=ResourceValue(**stats_config.get('strength', {'current': 10, 'maximum': 15, 'minimum': 0})),
            dexterity=ResourceValue(**stats_config.get('dexterity', {'current': 10, 'maximum': 15, 'minimum': 0})),
            intelligence=ResourceValue(**stats_config.get('intelligence', {'current': 10, 'maximum': 15, 'minimum': 0})),
            constitution=ResourceValue(**stats_config.get('constitution', {'current': 10, 'maximum': 15, 'minimum': 0})),
            wisdom=ResourceValue(**stats_config.get('wisdom', {'current': 10, 'maximum': 15, 'minimum': 0})),
            charisma=ResourceValue(**stats_config.get('charisma', {'current': 10, 'maximum': 15, 'minimum': 0}))
        )
        
        # Add resources
        resources_config = char_config.get('resources', {})
        character_data['resources'] = CharacterResources(
            health=ResourceValue(**resources_config.get('health', {'current': 100, 'maximum': 100, 'minimum': 0})),
            stamina=ResourceValue(**resources_config.get('stamina', {'current': 100, 'maximum': 100, 'minimum': 0})),
            morale=ResourceValue(**resources_config.get('morale', {'current': 100, 'maximum': 100, 'minimum': 0}))
        )
    ...
    def _apply_world_state_config(self):
        ...
        if hasattr(self.director, 'world_state_tracker'):
            world_config = self.config.world_state
    ...
    def run_evaluation(self) -> EvaluationMetrics:
        ...
        try:
            ...
            for turn_num in range(1, self.config.estimated_turns + 1):
                ...
                logger.info(f"⚡ Executing turn {turn_num}/{self.config.estimated_turns}")
            ...
            logger.info(f"🎯 Evaluation completed: Score {metrics.overall_score:.2f} "
    ...
    def _calculate_metrics(self, execution_time: float, turn_times: List[float], 
                          iron_laws_reports: List[Dict[str, Any]]) -> EvaluationMetrics:
        """Calculate comprehensive evaluation metrics."""
        
        # Basic execution metrics
        total_turns = len(self.turn_results)
        total_actions = sum(result.get('total_actions', 0) for result in self.turn_results)
        ...
        for report in iron_laws_reports:
            ...
            repair_successes += sum(1 for repair in repairs if repair.get('success', False))
        
        # Calculate scores (simplified scoring for baseline)
        violation_rate = total_violations / max(total_actions, 1)
        repair_success_rate = repair_successes / max(repair_attempts, 1) if repair_attempts > 0 else 1.0
        
        # Decision quality scores (placeholder - would need more sophisticated analysis)
        logical_consistency_score = max(0.0, 1.0 - violation_rate * 2)  # Penalize violations
        ...
        completion_rate = (primary_completed + secondary_completed) / max(primary_total + secondary_total, 1)
        ...
        evaluation_passed = (overall_score >= pass_threshold and 
                           total_violations <= self.config.pass_thresholds.get('max_violations', 5))
        ...
        return EvaluationMetrics(
            ...
            errors_encountered=sum(len(result.get('errors', [])) for result in self.turn_results),
...
class EvaluationReporter:
    ...
    def generate_individual_report(self, config: SeedConfiguration, metrics: EvaluationMetrics,
                                 execution_log: List[str]) -> Path:
        """
        ...
        report_data = {
            ...
            "detailed_log": execution_log[:100] if execution_log else []  # Limit log size
        }
        ...
        summary_data = {
            "evaluation_summary": {
                ...
                "overall_score_statistics": {
                    ...
                    "std_dev": stdev(overall_scores) if len(overall_scores) > 1 else 0.0,
            ...
            "iron_laws_analysis": {
                ...
                "average_violations_per_evaluation": sum(all_violations.values()) / total_evaluations
            },
            "individual_results": [
                {
                    "seed_id": m.seed_id,
...
class BaselineEvaluator:
    ...
    def __init__(self, output_dir: Optional[Path] = None):
        ...
        if not NOVEL_ENGINE_AVAILABLE:
            raise RuntimeError("Novel Engine components not available - cannot run evaluation")
    ...
    def batch_evaluation(self, seed_path: Path, iterations: int = 5) -> List[EvaluationMetrics]:
        """
        ...
        """
        logger.info(f"🔄 Running batch evaluation: {iterations} iterations of {seed_path}")
...
def main():
    ...
    # Options
    parser.add_argument("--iterations", type=int, default=5, help="Iterations for batch mode")
    parser.add_argument("--output", type=Path, help="Output directory for reports")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument("--save-detailed", action="store_true", help="Save detailed execution logs")
    ...
    try:
        ...
        if args.seed:
            ...
            if all_metrics:
                ...
                print(f"   Pass Rate: {pass_count}/{len(all_metrics)} ({pass_count/len(all_metrics)*100:.1f}%)")
            ...
            if batch_metrics:
                ...
                print(f"   Pass Rate: {pass_count}/{len(batch_metrics)} ({pass_count/len(batch_metrics)*100:.1f}%)")
