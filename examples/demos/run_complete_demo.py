...
class CompleteDemoRunner:
    ...
    async def _demonstrate_story_1_character_creation(self):
        ...
        print(
            f"Creating {len(characters_data)} diverse characters with different archetypes..."
        )
        ...
        for char_data in characters_data:
            ...
            result = await self.orchestrator.create_agent_context(
                char_data["id"], character_state
            )

            if result.success:
                print(
                    f"✅ Created: {char_data['name']} ({char_data['archetype'].value})"
                )
    ...
    async def _demonstrate_story_2_real_time_interactions(self):
        ...
        print(
            f"Running {len(interaction_scenarios)} different interaction scenarios..."
        )
        ...
        for i, scenario in enumerate(interaction_scenarios, 1):
            ...
            if result.success:
                ...
                print(
                    f"   💝 Relationship changes: {result.data.get('relationship_changes', 0)}"
                )
    ...
    async def _demonstrate_story_5_story_generation(self):
        ...
        print(
            f"Generating {len(story_configurations)} stories with different configurations..."
        )
