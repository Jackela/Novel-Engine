...
def demo_llm_enhanced_decision_making():
    ...
    print(
        f"   Primary Faction: {agent.subjective_worldview.get('primary_faction', 'Unknown')}"
    )
...
def demo_llm_fallback_mechanism():
    ...
    print(
        "   ✓ Agent gracefully handled LLM failure and used algorithmic decision-making"
    )
...
def demo_character_specific_prompts():
    ...
    try:
        prompt = agent._construct_character_prompt(
