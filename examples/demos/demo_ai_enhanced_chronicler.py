...
def analyze_campaign_log_quality(log_path: str) -> dict:
    ...
    try:
        ...
        avg_reasoning_length = sum(len(reason) for reason in reasoning_matches) / len(reasoning_matches) if reasoning_matches else 0
...
def demonstrate_ai_enhanced_transcription():
    ...
    wh40k_terms = [
        ...
        'warrior', 'conflict', 'fate', 'destiny', 'darkness', 'light'
    ]
    ...
    if narrative_files:
        latest_file = max(narrative_files, key=lambda f: os.path.getctime(os.path.join(output_directory, f)))
