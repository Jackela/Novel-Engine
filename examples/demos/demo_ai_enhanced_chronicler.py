...
def analyze_campaign_log_quality(log_path: str) -> dict:
    ...
    try:
        ...
        print(
            f"   • Character actions: {analysis['ai_integration_metrics']['character_actions']}"
        )
        print(
        ...
        print("\n👥 CHARACTER DIVERSITY:")
        print(
            f"   • Unique characters: {analysis['character_diversity']['character_count']}"
        )
        print(
            f"   • Characters: {', '.join(analysis['character_diversity']['unique_characters'])}"
        )
        print(
            f"   • Unique factions: {analysis['character_diversity']['faction_count']}"
        )
        print(
            f"   • Factions: {', '.join(analysis['character_diversity']['unique_factions'])}"
        )
...
def demonstrate_ai_enhanced_transcription():
    ...
    avg_sentence_length = (
        narrative_words / narrative_sentences if narrative_sentences > 0 else 0
    )
    ...
    term_count = sum(
        complete_narrative.lower().count(term.lower()) for term in wh40k_terms
    )
    ...
    characters_in_narrative = sum(
        1
        for char in log_analysis.get("character_diversity", {}).get(
            "unique_characters", []
        )
    ...
    if characters_in_narrative > 0:
        print(
            f"   ✅ Character integration: {characters_in_narrative} characters featured"
        )
        quality_score += 1
    else:
    ...
    if narrative_files:
        latest_file = max(
            narrative_files,
            key=lambda f: os.path.getctime(os.path.join(output_directory, f)),
        )
        print(f"   • Latest narrative: {latest_file}")
        print(
            f"   • Full path: {os.path.abspath(os.path.join(output_directory, latest_file))}"
        )
