...
Include their emotional journey from confusion to acceptance to rebellion.""",
                expected_ai_indicators=["character", "story", "author", "realize", "narrative"],
                template_killer_aspects=[
...
Include specific examples of loving and hateful actions toward the same person in the same moment.""",
                expected_ai_indicators=["love", "hate", "emotion", "contradiction", "justify"],
                template_killer_aspects=[
...
the mystery. Include specific navigation instructions that work within this impossible geometry.""",
                expected_ai_indicators=["building", "interior", "impossible", "geometry", "mystery"],
                template_killer_aspects=[
...
Include conversations between their different quantum states.""",
                expected_ai_indicators=["quantum", "consciousness", "reality", "superposition", "contradictory"],
                template_killer_aspects=[
    ...
    async def submit_creative_prompt(self, page: Page, scenario: AIValidationScenario) -> Tuple[bool, str, int]:
        ...
        try:
            ...
            for attempt in range(12):  # 60 seconds total
                    ...
                    try:
                        ...
                        if response_element:
                            ...
                            if content and len(content.strip()) > 50:
                                ...
                                response_found = True
                                print(f"   ✅ Response found in {selector}: {len(response_content)} chars")
                                break
                    except:
                        continue
                ...
                if page_content and len(page_content) > 1000:
                    ...
                    story_indicators = sum(1 for pattern in story_patterns 
                                         if re.search(pattern, page_content))
                    
                    if story_indicators >= 2:
                        response_content = page_content[-2000:]  # Last 2000 chars
                        response_found = True
                        print(f"   ✅ Story content detected in page: {story_indicators} indicators")
                        break
            ...
            if response_found:
                print(f"   🎉 Response received: {len(response_content)} chars in {response_time_ms}ms")
    ...
    def analyze_content_for_ai(self, content: str, scenario: AIValidationScenario) -> ContentAnalysis:
        ...
        ai_matches = sum(1 for pattern in ai_indicators 
                        if re.search(pattern, content))
        
        # Scenario-specific indicator matching
        scenario_matches = sum(1 for indicator in scenario.expected_ai_indicators 
                             if indicator.lower() in content.lower())
        
        # Complexity indicators
        complexity_indicators = {
            "has_dialogue": '"' in content or "'" in content,
            "has_narrative_flow": bool(re.search(r'(?i)(then|next|after|while)', content)),
            "has_character_development": bool(re.search(r'(?i)(realized|understood|felt|thought)', content)),
            "has_descriptive_language": bool(re.search(r'(?i)(beautiful|dark|bright|mysterious|ancient)', content)),
            "has_varied_sentence_structure": sentence_count > 5 and word_count / sentence_count > 8,
            "addresses_scenario_complexity": scenario_matches >= len(scenario.expected_ai_indicators) // 2
        }
        ...
        template_probability = min(1.0, max(0.0, template_matches / max(1, word_count // 50)))
        ...
        complexity_bonus = sum(complexity_indicators.values()) / len(complexity_indicators)
        
        ai_probability = min(1.0, (ai_base_score + scenario_bonus + complexity_bonus) / 3)
        
        # Quality metrics
        quality_metrics = {
            "word_count": word_count,
            "sentence_variety": sentence_count / max(1, word_count // 10),
            "scenario_relevance": scenario_matches / len(scenario.expected_ai_indicators),
        ...
        try:
            ...
            async with async_playwright() as playwright:
                ...
                for i, scenario in enumerate(self.test_scenarios, 1):
                    ...
                    success, response_content, response_time = await self.submit_creative_prompt(page, scenario)
                    
                    # Take screenshot of result
                    await page.screenshot(path=self.evidence_dir / f"{i:02d}_{scenario.name.replace(' ', '_')}_result.png")
                    
                    # Analyze content
                    if success and response_content:
                        content_analysis = self.analyze_content_for_ai(response_content, scenario)
                        ...
                        print(f"   Creativity Score: {content_analysis.creativity_score:.2f}")
                        print(f"   AI Probability: {content_analysis.ai_probability:.2f}")
                        print(f"   Template Probability: {content_analysis.template_probability:.2f}")
                        print(f"   🎯 AI Validation Score: {ai_score:.2f}")
                        
                        # Show complexity indicators
                        print(f"   🔧 Complexity Indicators:")
                        for indicator, present in content_analysis.complexity_indicators.items():
                            status = "✅" if present else "❌"
                            print(f"      {status} {indicator.replace('_', ' ').title()}")
                    else:
                        content_analysis = None
                        ai_score = 0.0
                        print(f"❌ Test failed - no response received")
                    ...
                    result = TestResult(
                        ...
                        evidence_paths=[
                            ...
                            str(self.evidence_dir / f"{i:02d}_{scenario.name.replace(' ', '_')}_result.png")
                        ],
                        raw_content=response_content[:1000] if response_content else ""  # First 1000 chars
                    )
                ...
                await context.tracing.stop(path=self.evidence_dir / "playwright_trace.zip")
    ...
    def generate_final_assessment(self) -> Dict[str, Any]:
        ...
        for result in successful_tests:
            if result.content_analysis:
                template_probabilities.append(result.content_analysis.template_probability)
        ...
        avg_template_prob = sum(template_probabilities) / len(template_probabilities) if template_probabilities else 1.0
        avg_ai_prob = sum(ai_probabilities) / len(ai_probabilities) if ai_probabilities else 0.0
...
async def main():
    ...
    try:
        ...
        report_path = f"ai_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        if "metrics" in assessment:
            ...
            print(f"   Average Content Length: {metrics.get('average_content_length', 0):.0f} chars")
