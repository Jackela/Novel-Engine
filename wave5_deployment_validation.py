...
class DeploymentSystemValidator:
    ...
    def validate_staging_deployment(self) -> Dict[str, Any]:
        ...
        if staging_script.exists():
            results["script_readable"] = staging_script.is_file() and os.access(staging_script, os.R_OK)
            ...
            try:
                ...
                for method_name in expected_methods:
                    results["methods_available"][method_name] = hasattr(deployment, method_name)
    ...
    def validate_production_deployment(self) -> Dict[str, Any]:
        ...
        if prod_script.exists():
            results["script_readable"] = prod_script.is_file() and os.access(prod_script, os.R_OK)
    ...
    def validate_security_deployment(self) -> Dict[str, Any]:
        ...
        if security_script.exists():
            results["script_readable"] = security_script.is_file() and os.access(security_script, os.R_OK)
            ...
            try:
                ...
                # Test method availability
                expected_methods = ["deploy", "validate_security", "apply_security_policies", "audit"]
                for method_name in expected_methods:
                    results["methods_available"][method_name] = hasattr(deployment, method_name)
    ...
    def validate_deployment_utils(self) -> Dict[str, Any]:
        ...
        if utils_script.exists():
            results["utils_readable"] = utils_script.is_file() and os.access(utils_script, os.R_OK)
            ...
            try:
                ...
                # Test method availability
                expected_methods = ["backup_current_deployment", "restore_deployment", "verify_deployment", "get_deployment_status"]
                for method_name in expected_methods:
                    results["methods_available"][method_name] = hasattr(utils, method_name)
        ...
        for script_name, script_path, should_be_executable in scripts_to_check:
            ...
            if full_path.exists():
                ...
                results["script_permissions"][script_name] = {
                    ...
                    "permissions_correct": is_readable and (is_executable == should_be_executable)
                }
    ...
    def validate_deployment_dependencies(self) -> Dict[str, Any]:
        ...
        for dep_name, dep_command in system_deps:
            try:
                ...
                results["system_dependencies"][dep_name] = {
                    ...
                    "version_info": result.stdout.strip()[:100] if result.stdout else None
                }
        ...
        for config_path in config_deps:
            ...
            if not (full_path.exists() and full_path.is_file() and os.access(full_path, os.R_OK)):
                results["all_dependencies_met"] = False
...
def main():
    ...
    # Save results
    report_file = project_root / f"wave5_deployment_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
