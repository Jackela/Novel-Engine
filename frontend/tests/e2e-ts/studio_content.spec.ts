...
test.describe.serial('#318 whole-book generation loop', () => {
  test.setTimeout(120_000);
  ...
  test('auto-accepts chapters in reading order and preserves them after a stop', async () => {
    ...
    await studio.getByRole('button', { name: 'Add Manuscript' }).click();
    await expect(studio.getByRole('textbox', { name: 'Document title' })).toHaveValue('Chapter 3');
    ...
    const stopButton = studio.getByRole('button', { name: 'Stop generating' });
    try {
