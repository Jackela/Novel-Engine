...
test.describe
  .serial("#276 content acceptance", () => {
    ...
    test("Inspector tabs stay contained and its disclosure recovers across breakpoints", async () => {
      ...
      // Eight tabs fill the fixed three-column grid as three rows (#653 stats
      // first, #614 lore second — the eighth tab joins the third row).
    ...
    test("deleting a project removes its export directory and every store row", async () => {
      ...
      const artifact = catalog.exports[0] ?? {
        id: "",
        download_url: "",
        size_bytes: 0,
      };
