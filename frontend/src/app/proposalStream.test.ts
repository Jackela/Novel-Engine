...
describe('ProposalStreamParser', () => {
  ...
  it('ignores frames with an unknown type', () => {
    const parser = new ProposalStreamParser();
    const frames = parser.append('data: {"type":"tick","at":1}\n\ndata: {"type":"delta","text":"D"}\n\n');
  ...
  it.each([
    ...
    ['error frame missing message', '{"type":"error","error":{"code":"PROVIDER_FAILED"}}', /error\.message/],
...
describe('streamProposal', () => {
  it('relays deltas, keeps credentials/CSRF semantics, and resolves the done-frame job', async () => {
    ...
    const job = await streamProposal({ ...baseRequest(), onDelta: (text) => deltas.push(text) });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        body: JSON.stringify({ operation: 'continue', instruction: 'Polish', provider: 'mock' }),
      }),
    );
  });
  ...
  it('reads the envelope error returned before the stream starts', async () => {
    stubFetch(
      new Response(JSON.stringify({ error: { code: 'INVALID_OPERATION', message: 'nope' } }), {
        status: 422,
  ...
  it('forwards the caller signal to fetch', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    const failure = streamProposal({ ...baseRequest(), signal: controller.signal }).catch(
