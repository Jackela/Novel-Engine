 ...
 */
type NavStyle = 'buttons' | 'breadcrumb' | 'tabs' | 'minimal';
type NavVariant = 'horizontal' | 'vertical';
type NavSize = 'small' | 'medium' | 'large';

export interface WizardStep { id?: string | number; title?: string; description?: string; estimatedTime?: string }

interface WizardNavigationProps {
  steps?: WizardStep[];
  currentStep?: number;
  completedSteps?: number[];
  onStepChange?: (stepIndex: number, reason: string) => void;
  onComplete?: (info: { completedSteps: number[]; navigationHistory: number[]; totalSteps: number; timestamp: string }) => void;
  onCancel?: (info: { currentStep: number; completedSteps: number[]; navigationHistory: number[]; timestamp: string }) => void;
  validateStep?: (stepIndex: number, step: WizardStep) => Promise<{ isValid: boolean; errors?: string[] }>;
  allowSkip?: boolean;
 ...
 * StepList Component
 */
function StepList({ steps, currentStep, completedSteps, onStepClick, allowBacktrack }: { steps: WizardStep[]; currentStep: number; completedSteps: number[]; onStepClick: (idx: number) => void; allowBacktrack: boolean }) {
  return (
 ...
 */
function BreadcrumbNavigation({ steps, currentStep, completedSteps, onStepClick, variant, size, className }: { steps: WizardStep[]; currentStep: number; completedSteps: number[]; onStepClick: (idx: number) => void; variant: NavVariant; size: NavSize; className: string }) {
  return (
 ...
 */
function TabNavigation({ steps, currentStep, completedSteps, onStepClick, variant, size, className }: { steps: WizardStep[]; currentStep: number; completedSteps: number[]; onStepClick: (idx: number) => void; variant: NavVariant; size: NavSize; className: string }) {
  return (
 ...
 */
function MinimalNavigation({ currentStep, totalSteps, onNext, onPrevious, canProceed, isFirstStep, isLastStep, size, className }: { currentStep: number; totalSteps: number; onNext: () => void; onPrevious: () => void; canProceed: boolean; isFirstStep: boolean; isLastStep: boolean; size: NavSize; className: string }) {
  return (
