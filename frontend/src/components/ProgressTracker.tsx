 ...
 */
type Variant = 'horizontal' | 'vertical' | 'circular';
type Size = 'small' | 'medium' | 'large';
type ThemeKind = 'gradient' | 'solid' | 'minimal';

export interface ProgressStep { id?: string | number; title?: string; description?: string; estimatedTime?: string }
...
function CircularProgressTracker({ 
  ...
  theme, 
  className 
}: { steps: ProgressStep[]; currentStep: number; completedSteps: number[]; progressPercentage: number; size: Size; theme: ThemeKind; className: string }) {
...
function VerticalProgressTracker({ 
  ...
  theme, 
  className 
}: { steps: ProgressStep[]; currentStep: number; completedSteps: number[]; showStepLabels: boolean; size: Size; theme: ThemeKind; className: string }) {
  return (
...
export function ProgressMonitor({ 
  ...
  autoUpdate = true, 
  updateInterval = 1000 
}: { onProgress?: (value: number) => void; autoUpdate?: boolean; updateInterval?: number }) {
 ...
 */
export function ProgressAnalytics({ steps, currentStep, startTime }: { steps: ProgressStep[]; currentStep: number; startTime?: number }) {
