...
const CharacterSelection = () => {
  ...
  const [charactersList, setCharactersList] = useState([]); // 英灵清单 - Registry of available heroes
  const [selectedCharacters, setSelectedCharacters] = useState([]); // 被选中的英灵 - Chosen champions
  const [isLoading, setIsLoading] = useState(true); // 数据载入圣礼状态 - Loading ritual state
  const [error, setError] = useState(null); // 错误信息记录 - Error communion logs
  const [validationError, setValidationError] = useState(''); // 选择验证错误 - Selection validation errors
  const [manualValidationError, setManualValidationError] = useState(false); // Flag for manual validation errors
  ...
  }, [fetchCharacters, location.state, t]);
  ...
  const fetchCharacters = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`${apiConfig.baseUrl}/characters`, {
        timeout: apiConfig.timeout,
      });
      
      console.log('API Response:', response.data);
      
      if (response.data && Array.isArray(response.data.characters) && response.data.characters.length > 0) {
        setCharactersList(response.data.characters);
      } else if (response.data && Array.isArray(response.data) && response.data.length > 0) {
        // Handle case where characters array is returned directly
        setCharactersList(response.data);
      } else {
        console.warn('No characters found in API response');
        setCharactersList([]);
      }
      ...
      // Clear characters list on error to prevent showing stale data
      setCharactersList([]);
      
      if (err.code === 'ECONNABORTED') {
        setError(t('characterSelection.errors.timeout'));
      } else if (err.response) {
        if (err.response.status === 404) {
          setError(t('characterSelection.errors.notFound'));
        } else {
          setError(t('characterSelection.errors.serverError'));
        }
      } else if (err.request) {
        setError(t('characterSelection.errors.networkError'));
      } else {
        setError(err.message || t('characterSelection.errors.serverError'));
      }
    } finally {
      setIsLoading(false);
    }
  }, [apiConfig.baseUrl, apiConfig.timeout, t]);
  ...
  const getCounterColor = () => {
    const count = selectedCharacters.length;
    if (count < selectionConstraints.minSelection) return '#CC0000'; // Red
    if (count > selectionConstraints.maxSelection) return '#CC0000'; // Red
    return '#228B22'; // Green
  };
