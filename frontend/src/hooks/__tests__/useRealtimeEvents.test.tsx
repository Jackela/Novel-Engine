...
const installEventSourceMock = (impl: typeof EventSource) => {
  const globalWithEventSource = globalThis as typeof globalThis & {
    EventSource?: typeof EventSource;
  };
  globalWithEventSource.EventSource = impl;
};

const setupDefaultMockEventSource = () => {
  resetMockState();
  installEventSourceMock(MockEventSourceImpl as unknown as typeof EventSource);
};

const setupCapturingMockEventSource = () => {
  resetMockState();
  ...
  installEventSourceMock(CapturingMockEventSource as unknown as typeof EventSource);
  return { getOnMessage: () => onMessageFn };
};
...
describe('useRealtimeEvents - connection', () => {
  beforeEach(setupDefaultMockEventSource);
  ...
  it('cleans up EventSource on unmount', async () => {
    ...
    expect(mockReadyState).toBe(2);
  });
});

describe('useRealtimeEvents - event parsing', () => {
  beforeEach(setupDefaultMockEventSource);
...
describe('useRealtimeEvents - event buffering', () => {
  beforeEach(setupDefaultMockEventSource);
...
describe('useRealtimeEvents - invalid payloads', () => {
  beforeEach(setupDefaultMockEventSource);

  it('handles invalid JSON in event data gracefully', async () => {
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const { getOnMessage } = setupCapturingMockEventSource();
    let helpers: { triggerOpen: () => void } | null = null;
    ...
    await act(async () => {
      const onMessageFn = getOnMessage();
    ...
    const errorCall = consoleSpy.mock.calls.find(
      (call) => typeof call[0] === 'string' && call[0].includes('Failed to parse SSE event data')
    );
...
describe('useRealtimeEvents - options', () => {
  beforeEach(setupDefaultMockEventSource);
