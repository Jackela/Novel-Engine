...
class SyntheticMonitor:
    ...
    async def _validate_http_response(
        ...
        if config.expected_json_schema:
            try:
                ...
                schema_valid = self._validate_json_schema(response_json, config.expected_json_schema)
    ...
    async def _run_api_check(self, check: SyntheticCheck) -> CheckResult:
        ...
        try:
            ...
            if not all_passed:
                ...
                result.error_message = f"API endpoints failed: {', '.join(failed_endpoints)}"
    ...
    async def _test_api_endpoint(self, config: ApiCheckConfig, endpoint_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _run_user_journey_check(self, check: SyntheticCheck) -> CheckResult:
        ...
        try:
            journey_session = aiohttp.ClientSession() if config.maintain_session else self.http_session
            ...
            try:
                for step in config.steps:
                    step_result = await self._execute_journey_step(step, journey_session, journey_context)
                ...
                if not journey_success:
                    failed_steps = [r["step_name"] for r in step_results if not r["success"]]
                ...
                result.validations_failed = sum(1 for r in step_results if not r["success"])
    ...
    async def _execute_validation_step(self, step: UserJourneyStep, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_extraction_step(self, step: UserJourneyStep, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_check_statistics(self, check_name: str, hours: int = 24) -> Dict[str, Any]:
        ...
        successful_checks = sum(1 for r in recent_results if r.status == CheckStatus.SUCCESS)
        failed_checks = sum(1 for r in recent_results if r.status == CheckStatus.FAILURE)
        error_checks = sum(1 for r in recent_results if r.status == CheckStatus.ERROR)
        timeout_checks = sum(1 for r in recent_results if r.status == CheckStatus.TIMEOUT)
    ...
    def get_overall_statistics(self) -> Dict[str, Any]:
        ...
        if all_results:
            success_rate = (sum(1 for r in all_results if r.status == CheckStatus.SUCCESS) / len(all_results)) * 100
            avg_response_time = sum(r.duration_ms for r in all_results if r.duration_ms) / len(all_results)
        else:
            success_rate = 0
            avg_response_time = 0
        ...
        return {
            ...
            "checks_with_failures": sum(1 for count in self.consecutive_failures.values() if count > 0)
        }
