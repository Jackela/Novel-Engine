...
class AlertManager:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_alerts_rule_name ON alerts(rule_name)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_alerts_fired_at ON alerts(fired_at)")
    ...
    async def _evaluate_rule(self, rule: AlertRule):
        ...
        try:
            ...
            threshold_violated = self._check_threshold(current_value, rule.condition, rule.threshold)
            
            # Check anomaly detection if enabled
            anomaly_detected = False
            if rule.use_anomaly_detection:
                anomaly_detected = buffer.is_anomaly(current_value, rule.anomaly_sensitivity)
    ...
    async def _fire_alert(self, rule: AlertRule, current_value: float, is_anomaly: bool = False):
        """Fire a new alert"""
        alert_id = f"{rule.name}_{rule.metric_name}"
        ...
        try:
            ...
            if success:
                ...
                if notification["attempt"] < 3:
                    notification["scheduled_at"] = time.time() + 300  # Retry in 5 minutes
                    self.notification_queue.append(notification)
                
        except Exception as e:
            logger.error(f"Error sending notification: {e}")
    
    async def _send_email_notification(self, alert: Alert, notification: Dict[str, Any]) -> bool:
    ...
    async def _send_slack_notification(self, alert: Alert, notification: Dict[str, Any]) -> bool:
    ...
    async def _send_webhook_notification(self, alert: Alert, notification: Dict[str, Any]) -> bool:
    ...
    async def _send_pagerduty_notification(self, alert: Alert, notification: Dict[str, Any]) -> bool:
    ...
    async def _send_sms_notification(self, alert: Alert, notification: Dict[str, Any]) -> bool:
    ...
    async def _persist_alert(self, alert: Alert):
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                """, (
                    alert.id, alert.rule_name, alert.metric_name, alert.severity.value,
                    alert.status.value, alert.message,
                    ...
                    alert.last_notification.timestamp() if alert.last_notification else None,
