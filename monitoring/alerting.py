...
class AlertManager:
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        resolved_alerts = [
            alert for alert in self.alert_history[-100:] if alert.resolved_at and alert.fired_at
        ]

        if resolved_alerts:
            resolution_times = [
                (alert.resolved_at - alert.fired_at).total_seconds() for alert in resolved_alerts
            ]
...
def create_default_alert_rules() -> List[AlertRule]:
    ...
    return [
        ...
        AlertRule(
            ...
            notification_channels=[NotificationChannel.EMAIL, NotificationChannel.SLACK],
            escalation_rules=[
                EscalationRule(delay_minutes=15, channels=[NotificationChannel.PAGERDUTY])
            ],
        ),
