...
| `NOT_FOUND` | 404 | The addressed resource does not exist in the addressed scope, or it exists in a different project — the two are indistinguishable by design. Messages name the resource type and the addressed id (`<Resource> not found: <id>.`); prefer `details` on paths that carry it. | Re-check the id and the project path from your records. Cross-project probing is pointless: other scopes are invisible, not an error. Also used for unknown routes (`Route <method> <url> is not known...`). |
