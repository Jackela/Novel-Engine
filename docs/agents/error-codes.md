...
| `OPERATION_IN_FLIGHT` | 409 | An identical pipeline operation (generation, review, export, retry) is already running for the document. | Do not resubmit. Poll `GET /api/projects/{projectId}/jobs`; `details` carries `project_id`, `document_id`, `operation`. |
