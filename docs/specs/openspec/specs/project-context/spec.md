...
The repository MUST maintain `docs/specs/openspec/project.md` as the authoritative project context (purpose, tech stack, conventions, and validation gates) and it MUST NOT remain a placeholder template.

#### Scenario: Project context is updated with meaningful content
- **WHEN** a contributor opens `docs/specs/openspec/project.md`
...
Material changes to architecture patterns, API conventions, or validation gates MUST update `docs/specs/openspec/project.md` in the same change that introduces the new convention.
...
- **THEN** `docs/specs/openspec/project.md` is updated to reflect the convention so future contributors and AI assistants have a single source of truth.
