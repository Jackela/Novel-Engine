# User and Developer Guides

**Last Updated**: 2024-11-04  
**Status**: Current

**Navigation**: [Home](../../README.md) > [Docs](../index.md) > Guides

---

## Overview

Comprehensive guides for Novel Engine development, usage, and best practices.

---

## 📚 Available Guides

### [Coding Standards](../coding-standards.md)
Repository coding standards and CI parity expectations

### [Context Integration Design](./CONTEXT_INTEGRATION_DESIGN.md)
Dynamic context engineering framework design and integration patterns
- **Topics**: Context supply chain, fog-of-war filtering, token budgeting, provenance tracking
- **Audience**: Advanced Developers, Architects
- **Last Updated**: 2024-08-28

### [Core Systems Integration Guide](./CORE_SYSTEMS_INTEGRATION_GUIDE.md)
Guide to integrating with core Novel Engine systems
- **Topics**: System orchestrator, multi-agent coordination, platform integration
- **Audience**: Integration Teams
- **Last Updated**: 2024-08-26

---

## 📖 Related Documentation

### Architecture
- [System Architecture](../architecture/SYSTEM_ARCHITECTURE.md)
- [Bounded Contexts](../architecture/bounded-contexts.md)
- [Context Mapping](../architecture/context-mapping.md)

### API Documentation
- [API Reference](../api/API_REFERENCE.md)
- [Data Schemas](../SCHEMAS.md)

### Deployment
- [Deployment Guide](../deployment/DEPLOYMENT_GUIDE.md)
- [Operations Runbook](../operations/OPERATIONS_RUNBOOK.md)

### Quick References
- [Quick Start](../QUICK_START.md)
- [Foundations](../FOUNDATIONS.md)
- [Developer Guide](../DEVELOPER_GUIDE.md)

---

## 🤝 Contributing to Guides

We welcome contributions to our documentation! To contribute:

1. Follow the [Coding Standards](../coding-standards.md)
2. Use clear, concise language
3. Include code examples where appropriate
4. Add navigation breadcrumbs
5. Update this INDEX when adding new guides

---

**Maintained by**: Novel Engine Documentation Team  
**License**: MIT
