...
class TestPhaseType:
    ...
    def test_get_display_name(self):
        ...
        assert PhaseType.SUBJECTIVE_BRIEF.get_display_name() == "Subjective Brief Generation"
        assert PhaseType.INTERACTION_ORCHESTRATION.get_display_name() == "Interaction Orchestration"
        assert PhaseType.EVENT_INTEGRATION.get_display_name() == "Event Integration"
        assert PhaseType.NARRATIVE_INTEGRATION.get_display_name() == "Narrative Integration"
    ...
    def test_get_next_phase(self):
        ...
        assert PhaseType.SUBJECTIVE_BRIEF.get_next_phase() == PhaseType.INTERACTION_ORCHESTRATION
        assert PhaseType.INTERACTION_ORCHESTRATION.get_next_phase() == PhaseType.EVENT_INTEGRATION
        assert PhaseType.EVENT_INTEGRATION.get_next_phase() == PhaseType.NARRATIVE_INTEGRATION
    ...
    def test_get_previous_phase(self):
        ...
        assert PhaseType.INTERACTION_ORCHESTRATION.get_previous_phase() == PhaseType.SUBJECTIVE_BRIEF
        assert PhaseType.EVENT_INTEGRATION.get_previous_phase() == PhaseType.INTERACTION_ORCHESTRATION
        assert PhaseType.NARRATIVE_INTEGRATION.get_previous_phase() == PhaseType.EVENT_INTEGRATION
...
class TestPhaseStatusEnum:
    ...
    def test_can_transition_to_valid(self):
        ...
        assert PhaseStatusEnum.PENDING.can_transition_to(PhaseStatusEnum.RUNNING) is True
        assert PhaseStatusEnum.PENDING.can_transition_to(PhaseStatusEnum.SKIPPED) is True
        
        # RUNNING can go to COMPLETED or FAILED
        assert PhaseStatusEnum.RUNNING.can_transition_to(PhaseStatusEnum.COMPLETED) is True
        assert PhaseStatusEnum.RUNNING.can_transition_to(PhaseStatusEnum.FAILED) is True
        
        # COMPLETED can go to COMPENSATING
        assert PhaseStatusEnum.COMPLETED.can_transition_to(PhaseStatusEnum.COMPENSATING) is True
        
        # FAILED can go to COMPENSATING or COMPENSATED
        assert PhaseStatusEnum.FAILED.can_transition_to(PhaseStatusEnum.COMPENSATING) is True
        assert PhaseStatusEnum.FAILED.can_transition_to(PhaseStatusEnum.COMPENSATED) is True

    def test_can_transition_to_invalid(self):
        """Test invalid status transitions."""
        # PENDING cannot go directly to COMPLETED
        assert PhaseStatusEnum.PENDING.can_transition_to(PhaseStatusEnum.COMPLETED) is False
        
        # COMPLETED cannot go back to RUNNING
        assert PhaseStatusEnum.COMPLETED.can_transition_to(PhaseStatusEnum.RUNNING) is False
        
        # TERMINAL states cannot transition
        assert PhaseStatusEnum.COMPENSATED.can_transition_to(PhaseStatusEnum.PENDING) is False
        assert PhaseStatusEnum.SKIPPED.can_transition_to(PhaseStatusEnum.RUNNING) is False
...
class TestPhaseStatusValidation:
    ...
    def test_validation_completed_without_timestamp(self):
        ...
        with pytest.raises(ValueError, match="Completed phases must have completion timestamp"):
            PhaseStatus(
                phase_type=PhaseType.WORLD_UPDATE,
    ...
    def test_validation_completed_without_full_progress(self):
        ...
        with pytest.raises(ValueError, match="Completed phases must have 100% progress"):
            PhaseStatus(
                phase_type=PhaseType.WORLD_UPDATE,
                status=PhaseStatusEnum.COMPLETED,
