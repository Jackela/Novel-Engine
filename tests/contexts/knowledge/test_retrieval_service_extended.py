...
class TestRetrievalServiceExtended:
    ...
    async def test_retrieve_relevant_embedding_error(self, retrieval_service, mock_embedding_service):
        """Test retrieve with embedding error."""
        mock_embedding_service.embed = AsyncMock(side_effect=EmbeddingError("Embedding failed"))
    ...
    async def test_retrieve_relevant_vector_store_error(self, retrieval_service, mock_vector_store):
        """Test retrieve with vector store error."""
        mock_vector_store.query = AsyncMock(side_effect=VectorStoreError("Query failed", "query"))
    ...
    async def test_retrieve_relevant_with_results(self, retrieval_service, mock_vector_store):
        ...
        mock_results = [
            QueryResult(
                ...
                score=0.9,
                metadata={"source_id": "source_1", "source_type": "CHARACTER", "chunk_index": 1, "total_chunks": 2},
            ),
            QueryResult(
                id="chunk_2",
                text="Test content 2 - completely different second chunk with unique information",
                score=0.8,
                metadata={"source_id": "source_1", "source_type": "CHARACTER", "chunk_index": 2, "total_chunks": 2},
            ),
        ]
    ...
    async def test_retrieve_relevant_score_filtering(self, retrieval_service, mock_vector_store):
    ...
    async def test_retrieve_relevant_with_reranking(self, retrieval_service, mock_vector_store):
        """Test retrieve with reranking enabled."""
        mock_rerank_service = AsyncMock()
        mock_rerank_service.rerank = AsyncMock(return_value=Mock(
            reranked=True,
            chunks=[
                RetrievedChunk(
                    chunk_id="chunk_1",
    ...
    def test_format_context_with_max_tokens(self, retrieval_service):
        ...
        chunks = [
            RetrievedChunk(
                ...
                content="This is a test content that is reasonably long for testing purposes. " * 10,
                score=0.9,
    ...
    def test_content_similarity_different(self, retrieval_service):
        ...
        similarity = retrieval_service._content_similarity("hello world", "goodbye world")
    ...
    async def test_retrieve_relevant_without_reranker(self, retrieval_service, mock_vector_store):
    ...
    async def test_retrieve_relevant_reranker_exception(self, retrieval_service, mock_vector_store):
