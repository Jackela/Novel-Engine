...
class TestCoherenceScoreGetLevel:
    ...
    def test_get_level_excellent(self) -> None:
        ...
        score = CoherenceScore(score=0.85, internal_coherence=0.8, boundary_quality=0.8, size_score=0.8)
        
        assert score.get_level() == "excellent"

    def test_get_level_good(self) -> None:
        """Test get_level returns 'good' for scores >= 0.6 and < 0.8."""
        score = CoherenceScore(score=0.75, internal_coherence=0.8, boundary_quality=0.8, size_score=0.8)
        
        assert score.get_level() == "good"
        
        score = CoherenceScore(score=0.6, internal_coherence=0.8, boundary_quality=0.8, size_score=0.8)
        assert score.get_level() == "good"

    def test_get_level_fair(self) -> None:
        """Test get_level returns 'fair' for scores >= 0.4 and < 0.6."""
        score = CoherenceScore(score=0.55, internal_coherence=0.8, boundary_quality=0.8, size_score=0.8)
        
        assert score.get_level() == "fair"
        
        score = CoherenceScore(score=0.4, internal_coherence=0.8, boundary_quality=0.8, size_score=0.8)
        assert score.get_level() == "fair"

    def test_get_level_poor(self) -> None:
        """Test get_level returns 'poor' for scores < 0.4."""
        score = CoherenceScore(score=0.35, internal_coherence=0.8, boundary_quality=0.8, size_score=0.8)
        
        assert score.get_level() == "poor"
        
        score = CoherenceScore(score=0.0, internal_coherence=0.8, boundary_quality=0.8, size_score=0.8)
        assert score.get_level() == "poor"
