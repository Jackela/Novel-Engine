...
try:
    from scripts.term_guard import (
        TermGuard, ViolationType, FilterAction, FilterRule, 
        IPViolation, CleaningReport
    )
...
class TestTermGuard:
    ...
    def test_term_guard_default_initialization(self):
        ...
        assert any(rule.violation_type == ViolationType.CHARACTER_NAME 
    ...
    def test_compliance_report_generation(self, temp_config):
        ...
        assert compliance_report["summary"]["safe_content_percentage"] == 50.0  # 1 of 2 safe
...
class TestTermGuardCLI:
    ...
    def test_cli_analyze_only(self, temp_input_file, capsys):
        ...
        with patch('sys.argv', ['term_guard.py', '--input', str(temp_input_file), '--analyze-only']):
            result = main()
