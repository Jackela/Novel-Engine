...
from src.security.auth_system import (
    SecurityService, UserRole, Permission, User, UserRegistration, UserLogin,
    AuthenticationError, AuthorizationError, ROLE_PERMISSIONS
)
from src.security.input_validation import (
    InputValidator, ValidationRule, ValidationError, ValidationSeverity, InputType
)
from src.security.rate_limiting import (
    RateLimiter, RateLimitConfig, RateLimitExceeded, ThreatLevel
)
from src.security.security_headers import (
    SecurityHeaders, SecurityHeadersConfig, CSPDirective
)
from src.security.data_protection import (
    DataProtectionService, EncryptionService, ConsentStatus, DataClassification
)
...
class TestRateLimiter:
    ...
    @pytest.mark.asyncio
    async def test_rate_limiting_allows_normal_requests(self, rate_limiter, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_rate_limiting_blocks_excessive_requests(self, rate_limiter, mock_request):
...
class TestSecurityHeaders:
    ...
    def test_security_headers_application(self, security_headers, mock_request, mock_response):
    ...
    async def test_security_statistics(self, security_logger):
        ...
        events = [
            SecurityEvent(
                ...
                severity=SecurityEventSeverity.INFO if i % 2 == 0 else SecurityEventSeverity.WARNING,
                timestamp=datetime.now(timezone.utc),
...
class TestIntegratedSecurity:
    ...
    async def test_security_middleware_integration(self):
        ...
        validated_query = validator.validate_input(
            mock_request.query_params["search"], 
            InputType.TEXT
        )
