    ...
    def simulate_memory_usage(self, turns: int = 50, agents: int = 20):
        ...
        for turn in range(turns):
            world_state = {
                "turn": turn,
                "agents": {
                    f"agent_{i}": {"location": f"loc_{i}", "status": "active"}
        ...
        for i in range(100):
            coord_data = {
                ...
                "data": {
                    "prompt": f"Coordination prompt {i} " * 20,
                    "context": list(range(15)),
                },
            }
...
class MemoryOptimizationTest:
    ...
    async def _test_memory_optimizer_core(self) -> Dict[str, Any]:
        ...
        try:
            ...
            optimizer = MemoryOptimizer(
                target_memory_percent=75.0, monitoring_interval=1.0
            )
            ...
            success = (
                "current_memory" in report
                and "memory_pressure" in report
                and "optimization_stats" in report
            ...
            memory_improvement = min(
                20.0, gc_improvements * 0.1
            )  # Estimate improvement

            logger.info(
                f"Memory optimizer: {report['memory_pressure']} pressure, {gc_improvements} optimizations"
            )
    ...
    async def _test_object_pool_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_improvement = (
                ((no_pool_time - pooled_time) / no_pool_time) * 100
                if no_pool_time > 0
                else 0
            )
            ...
            success = (
                pool_stats["reused_count"] > 0
                and pool_stats["reuse_rate_percent"] > 50
                and pooled_time
                <= no_pool_time
                + 0.001  # Should be faster or equal (with small tolerance)
            )
    ...
    async def _test_weak_reference_management(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(20):
                obj = {
                    "id": i,
                    "data": f"test_object_{i}",
                    "heavy_data": list(range(100)),
                }
                test_objects.append(obj)

                # Register with cleanup callback
                weak_manager.register(
                    f"test_obj_{i}", obj, lambda oid=i: cleanup_callback(oid)
                )
            ...
            return {
                ...
                "memory_improvement_percent": (cleaned_refs / 20)
                * 100,  # Percentage of references cleaned
            }
    ...
    async def _test_persona_agent_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"PersonaAgent optimization: {decisions_before}/{contexts_before} -> {decisions_after}/{contexts_after}"
            )
            logger.info(
                f"Memory: {memory_before:.1f}MB -> {memory_after:.1f}MB ({memory_improvement:.1f}MB saved)"
            )

            # Verify optimization effectiveness
            success = (
                decisions_after <= 50  # Should be capped at 50
                and contexts_after <= 25  # Should be capped at 25
                and len(optimization_result["optimizations_applied"])
                >= 2  # Multiple optimizations applied
                and decisions_reduced > 0
                and contexts_reduced > 0  # Should have reduced data
            )
    ...
    async def _test_director_agent_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"DirectorAgent optimization: {world_states_before}/{coordination_before} -> {world_states_after}/{coordination_after}"
            )
            logger.info(
                f"Memory: {memory_before:.1f}MB -> {memory_after:.1f}MB ({memory_improvement:.1f}MB saved)"
            )
            ...
            # Verify optimization effectiveness
            success = (
                world_states_after <= 10  # Should be capped at 10
                and coordination_after <= 25  # Should be capped at 25
                and len(optimization_result["optimizations_applied"])
                >= 1  # Optimizations applied
                and world_states_reduced > 0
                or coordination_reduced > 0  # Should have reduced data
            )
    ...
    async def _test_memory_pressure_response(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with memory_optimization_context() as setup:
                ...
                try:
                    ...
                    for i in range(20):
                        ...
                        large_obj = {
                            "id": i,
                            "data": [
                                f"memory_hog_{j}" for j in range(10000)
                            ],  # Large list
                            "matrix": [
                ...
                optimization_increase = pressure_report.get(
                    "optimization_stats", {}
                ...
                logger.info(
                    f"Optimization performed: {force_result.get('optimization_performed', False)}"
                )
                ...
                return {
                    ...
                    "optimization_performed": force_result.get(
...
async def main():
    ...
    try:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(report)
            f.write('\n"""\n\n')
            f.write("# Memory Optimization Test Results:\n")
