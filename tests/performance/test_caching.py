...
class IntelligentCachingTest:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        logger.info("=== Intelligent Caching Tests Completed ===")
        logger.info(
    ...
    async def _test_basic_cache_operations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Cache operations: Store={store_time:.4f}s, Retrieve={retrieve_time:.4f}s"
            )
            logger.info(
                f"Cache levels: L1={stats['l1_stats']['entries']}, "
    ...
    async def _test_llm_response_cache(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"LLM Cache: Hits={cache_hits}, Misses={cache_misses}, "
                f"Hit Rate={hit_rate:.1f}%"
            )
            logger.info(
                f"Performance: No cache={no_cache_time:.4f}s, "
    ...
    async def _test_cache_performance_under_load(self) -> Dict[str, Any]:
        ...
        try:
            ...
            access_keys = random.sample(
                list(load_data.keys()), 200
            )  # 200 random accesses
            ...
            logger.info(
                f"  Store: {len(load_data)} items in {store_time:.2f}s "
                f"({throughput_store:.0f} items/sec)"
            )
            logger.info(
                f"  Retrieve: {len(access_keys)} accesses in {retrieve_time:.2f}s "
            ...
            return {
                ...
                "performance_improvement_percent": min(
                    90, throughput_retrieve / 10
                ),  # Rough estimate
            }
    ...
    async def _test_adaptive_cache_strategy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for strategy in strategies_to_test:
                ...
                logger.info(
                    f"Strategy {strategy.value}: Hit rate={hit_rate:.1f}%, "
    ...
    async def _test_world_state_prefetching(self) -> Dict[str, Any]:
        ...
        try:
            ...
            predicted_key = "world_state_turn_12"
            ...
            hit_rate_improvement = (
                cache_stats_after["l1_stats"]["hit_rate"]
                - cache_stats_before["l1_stats"]["hit_rate"]
            )

            logger.info(
                f"Prefetching test: Worked={prefetching_worked}, "
    ...
    async def _test_cache_memory_efficiency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            compression_ratio = (
                theoretical_size_mb / memory_used if memory_used > 0 else 1
            )
            memory_efficiency = (
                (theoretical_size_mb - memory_used) / theoretical_size_mb
            ) * 100
...
async def main():
    ...
    try:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(report)
            f.write('\n"""\n\n')
            f.write("# Intelligent Caching Test Results:\n")
        ...
        logger.info(
            f"System caching status: {comprehensive_report.get('status', 'unknown')}"
        )
