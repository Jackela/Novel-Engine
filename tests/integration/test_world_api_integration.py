...
class TestWorldAPIIntegration:
    ...
    def test_world_delta_endpoint_structure(self, client):
        ...
        assert response.status_code in [422, 500, 501], f"Unexpected status code: {response.status_code}"
    ...
    def test_world_slice_endpoint_structure(self, client):
        ...
        response = client.get(f"/api/v1/{world_id}/slice?entities=entity1,entity2&include_metadata=true")
        assert response.status_code != 404, "World slice endpoint with params should exist"
    ...
    def test_world_history_endpoint_structure(self, client):
        ...
        assert response.status_code != 404, "World history endpoint with params should exist"
    ...
    def test_api_server_world_router_integration(self):
        ...
        try:
            ...
            if hasattr(api_server, 'app'):
                ...
                if WORLD_ROUTER_AVAILABLE and hasattr(api_server, 'WORLD_ROUTER_AVAILABLE') and api_server.WORLD_ROUTER_AVAILABLE:
                    # App should have routes from World router
                    route_paths = [route.path for route in app.routes]
                    world_routes_exist = any("/api/v1/" in path and "{world_id}" in path for path in route_paths)
                    assert world_routes_exist, "World router routes should be included in main app"
    ...
    try:
        ...
        try:
            ...
            if hasattr(api_server, 'app'):
                ...
                if WORLD_ROUTER_AVAILABLE:
                    world_routes = [route for route in app.routes if hasattr(route, 'path') and "/api/v1/" in getattr(route, 'path', '')]
                    if world_routes:
