...
async def test_component_initialization():
    ...
    components_to_test = [
        ...
        ("ConfigurationService", lambda: ConfigurationService(config_dir="config", logger=logger)),
    ...
    try:
        agent_manager = next((comp for name, comp in initialized_components if name == "AgentLifecycleManager"), None)
        if agent_manager:
            turn_engine = TurnExecutionEngine(agent_manager=agent_manager, logger=logger)
            print("✅ TurnExecutionEngine created successfully")
            initialized_components.append(("TurnExecutionEngine", turn_engine))
        else:
            print("❌ Cannot test TurnExecutionEngine - AgentLifecycleManager not available")
    except Exception as e:
...
async def test_component_protocols():
    ...
    try:
        from director_components.protocols import (
