...
from core_platform.persistence.database import DatabaseManager, Base

# Character context imports
from contexts.character import (
    Character, CharacterID, Gender, CharacterRace, CharacterClass,
    CharacterStats, SQLAlchemyCharacterRepository, CharacterBase
)
from contexts.character.domain.value_objects.character_stats import CoreAbilities

# Character ORM import with fallback
try:
    from contexts.character.infrastructure.persistence.character_models import CharacterORM
    ...
    class CharacterORM:
        ...
        def from_domain_entity(cls, entity): 
            ...
            return cls(
                ...
                character_class=entity.profile.character_class.value if entity.profile.character_class else "fighter",
...
try:
    ...
    class WorldState:
        @classmethod
        def create_new_world(cls, name, description): return cls()
...
try:
    from contexts.orchestration.application.services.turn_orchestrator import TurnOrchestrator
except ImportError:
    # Mock TurnOrchestrator if not available
    class TurnOrchestrator:
        def execute_turn(self, **kwargs): return {"success": True}

try:
    from contexts.orchestration.domain.value_objects.turn_configuration import TurnConfiguration
except ImportError:
...
try:
    ...
    async def mock_turns_run():
        return {"turn_id": str(uuid4()), "success": True, "phases_completed": [], "phase_results": {}, "performance_metrics": {}}
...
class TestDataFactory:
    ...
    @staticmethod
    def create_test_character_data(character_name: str, character_class: str = "fighter") -> Dict[str, Any]:
...
class TestTurnOrchestrationE2E:
    ...
    async def test_complete_turn_orchestration_e2e(
        ...
        assert "performance_metrics" in response_data, "Response missing performance_metrics"
        
        # Validate turn execution success
        assert response_data["success"] is True, f"Turn execution failed: {response_data.get('error_details')}"
        
        # Validate phase completion
        expected_phases = 5  # All 5 phases should complete
        assert len(response_data["phases_completed"]) == expected_phases, \
    ...
    async def test_turn_orchestration_with_validation_errors(
        ...
        response_data = response.json()
        assert "error" in response_data or "detail" in response_data, "Expected error details in response"
    ...
    async def test_turn_orchestration_async_execution(
        ...
        if response.status_code == 202:
            ...
            # Check status
            status_response = await client.get(f"{api_server_url}/v1/turns/{turn_id}/status")
    ...
    async def _validate_database_changes(
        ...
        assert final_worlds >= initial_worlds, \
        ...
        assert final_characters >= initial_characters, \
        ...
        assert final_narratives >= initial_narratives, \
        ...
        for expected_phase in expected_phases:
            assert expected_phase in phases_completed, \
        ...
        assert execution_time < 60000, f"Execution time too long: {execution_time}ms (max: 60s)"
        
        # If AI disabled, execution should be faster
        if not turn_response.get("configuration", {}).get("ai_integration_enabled", True):
            assert execution_time < 30000, \
        ...
        expected_metric_keys = ["events_processed", "phase_durations", "total_operations"]
        ...
        for character_data in final_state["characters"]:
            ...
            assert "character_id" in character_data or "id" in character_data, \
                "Character missing ID field"
            assert "name" in character_data or "character_name" in character_data, \
        ...
        for event_data in final_state["character_events"]:
            if "character_id" in event_data:
                ...
                character_ids = [c.get("character_id", c.get("id")) for c in final_state["characters"]]
        ...
        if initial_state["world_state"] and final_state["world_state"]:
            initial_world = initial_state["world_state"][-1] if initial_state["world_state"] else {}
            final_world = final_state["world_state"][-1] if final_state["world_state"] else {}
        ...
        self._log_validation_results(
            initial_state, final_state, turn_response, 
    ...
    def _log_validation_results(
        ...
        print(f"\n{'='*60}")
        print(f"🎯 E2E TURN ORCHESTRATION VALIDATION RESULTS")
        print(f"{'='*60}")
        ...
        print(f"  • World States:    {len(initial_state['world_state'])} → {len(final_state['world_state'])}")
        print(f"  • Characters:      {len(initial_state['characters'])} → {len(final_state['characters'])}")
        print(f"  • Character Events: {len(initial_state['character_events'])} → {len(final_state['character_events'])}")
        print(f"  • Narrative Arcs:  {len(initial_state['narrative_arcs'])} → {len(final_state['narrative_arcs'])}")
        print()
        ...
        if metrics:
            ...
            for key, value in metrics.items():
                if isinstance(value, (int, float)):
                    ...
                    print(f"  • {key.replace('_', ' ').title()}: {type(value).__name__}")
        
        # Quality assessment
        print("✅ QUALITY ASSESSMENT:")
        quality_score = self._calculate_quality_score(phases_completed, execution_time, total_changes)
