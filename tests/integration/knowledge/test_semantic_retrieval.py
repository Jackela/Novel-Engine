...
class TestSemanticKnowledgeRetrieval:
    ...
    async def test_semantic_search_retrieves_similar_entries_without_exact_match(
        ...
        repository.retrieve_for_agent_semantic = AsyncMock(
            return_value=semantically_similar_entries[
                :3
            ]  # Should return only space-related entries
        )
        ...
        assert all(
            entry.id in ["entry-001", "entry-002", "entry-003"] for entry in results
        )
    ...
    @pytest.mark.asyncio
    async def test_semantic_search_respects_access_control(
        self, semantically_similar_entries
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_semantic_search_top_k_limit(
        self, agent, semantically_similar_entries
    ):
        """
