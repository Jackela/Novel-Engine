...
"""
        ...
        with patch('os.path.exists', return_value=True), \
             ...
             patch('os.listdir', return_value=['character.md', 'stats.yaml']), \
             patch('builtins.open', mock_open()) as mock_file, \
             patch('yaml.safe_load', return_value=self.test_character_data):
            ...
            with patch.object(PersonaAgent, '_extract_core_identity'), \
    ...
    def test_load_character_context_missing_directory(self):
        ...
        with patch('os.path.exists', return_value=False), \
             patch.object(PersonaAgent, 'load_character_context', side_effect=FileNotFoundError("Character directory not found")):
    ...
    def test_derive_agent_id_from_path(self):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            result = agent._derive_agent_id_from_path('characters/imperial_guard/sergeant_johnson')
...
class TestPersonaAgentDecisionMaking(unittest.TestCase):
    ...
    def test_make_decision_with_world_state(self):
        ...
        with patch.object(self.agent, '_llm_enhanced_decision_making') as mock_llm, \
             patch.object(self.agent, '_process_world_state_update') as mock_process, \
             patch.object(self.agent, '_assess_current_situation') as mock_assess, \
             patch.object(self.agent, '_identify_available_actions') as mock_actions:
    ...
    def test_make_decision_ai_fallback(self):
        ...
        with patch.object(self.agent, '_llm_enhanced_decision_making', side_effect=Exception("AI Error")), \
             patch.object(self.agent, '_process_world_state_update'), \
             patch.object(self.agent, '_assess_current_situation', return_value={'threat_level': 'low'}), \
             patch.object(self.agent, '_identify_available_actions', return_value=[{'action_type': 'hold_position'}]), \
             patch.object(self.agent, '_select_best_action') as mock_select:
    ...
    def test_assess_threat_levels(self):
        ...
        for description, expected_level in test_cases:
            with patch.object(self.agent, '_assess_threat_from_description', return_value=expected_level):
    ...
    def test_decision_weight_application(self):
        ...
        with patch.object(self.agent, '_evaluate_action_option') as mock_evaluate, \
             patch.object(self.agent, '_process_world_state_update'), \
             patch.object(self.agent, '_assess_current_situation', return_value={'threat_level': 'high'}), \
             patch.object(self.agent, '_identify_available_actions') as mock_actions, \
             patch.object(self.agent, '_llm_enhanced_decision_making', return_value=None), \
             patch.object(self.agent, '_select_best_action') as mock_select:
            ...
            mock_select.return_value = CharacterAction(
                action_type="retreat",
                reasoning="Self-preservation priority"
            )
    ...
    def test_assess_current_situation(self):
        ...
        with patch.object(self.agent, '_assess_overall_threat_level', return_value=ThreatLevel.MODERATE), \
             patch.object(self.agent, '_assess_available_resources', return_value={'energy': 80}), \
             patch.object(self.agent, '_assess_social_obligations', return_value=[]), \
             patch.object(self.agent, '_assess_mission_status', return_value={'status': 'active'}), \
             patch.object(self.agent, '_assess_environmental_factors', return_value={'weather': 'clear'}):
...
class TestPersonaAgentWorldInterpretation(unittest.TestCase):
    ...
    def test_subjective_worldview_initialization(self):
        ...
        required_keys = [
            'known_entities', 'location_knowledge', 'faction_relationships',
            'recent_events', 'current_goals', 'active_threats'
        ]
    ...
    def test_world_event_processing(self):
        ...
        threat_level = self.agent._assess_threat_from_description("Major battle erupted")
...
class TestPersonaAgentAIIntegration(unittest.TestCase):
    ...
    def test_ai_query_with_valid_api_key(self):
        ...
        with patch.object(self.agent, '_call_llm', return_value=expected_response) as mock_llm:
    ...
    def test_api_error_handling(self):
        ...
        with patch('src.persona_agent._make_gemini_api_request', side_effect=Exception("Network error")):
            result = self.agent._call_llm(test_prompt)
