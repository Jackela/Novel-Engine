...
"""
    ...
    def test_validate_configuration_missing_file(self):
        ...
        assert any(
            "settings.yaml file not found" in error
    ...
    def test_validate_configuration_missing_sections(self):
        ...
        assert result is False
        assert any(
            "Missing configuration section" in error
            for error in self.startup_guard.errors
        )
    ...
    def test_validate_configuration_iron_laws_disabled(self):
        ...
        assert result is False
        assert any(
            "Iron Laws validation must be enabled" in error
            for error in self.startup_guard.errors
        )
    ...
    def test_validate_api_readiness_cors_warning(self):
        ...
        assert result is True
        assert any(
            "CORS enabled but no origins specified" in warning
...
class TestKnowledgeBaseBuilder:
    ...
    def setup_method(self):
        ...
        self.test_config = {"storage": {"kb_path": "private/knowledge_base/"}}
...
"""
    ...
    def test_system_validation_with_warnings(self):
        ...
        with patch("scripts.build_kb.__import__") as mock_import:
            ...
            assert any(
                "Legal safeguards are disabled" in warning
