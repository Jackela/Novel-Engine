...
@pytest.mark.skipif(not CHARACTER_FACTORY_AVAILABLE, reason="Character factory not available")
class TestCharacterFactory:
    ...
    def test_create_character_success(self, characters_directory):
        """测试角色创建 - 成功情况"""
        with patch('character_factory._get_characters_directory_path', return_value=str(characters_directory)):
    ...
    def test_create_character_not_found(self, temp_dir):
        """测试角色创建 - 角色不存在"""
        with patch('character_factory._get_characters_directory_path', return_value=str(temp_dir)):
