...
class TestCharacterFactory:
    ...
    def test_create_character_success(self, characters_directory):
        ...
        factory = CharacterFactory(self.mock_event_bus, str(characters_directory))
    ...
    def test_create_character_not_found(self, temp_dir):
        """测试角色创建 - 角色不存在"""
        # Create factory with temp directory that has no character subdirectories
        factory = CharacterFactory(self.mock_event_bus, str(temp_dir))
    ...
    def test_load_character_data_success(self, characters_directory):
        ...
        # CharacterFactory delegates loading to PersonaAgent, test through create_character
        with patch("character_factory.PersonaAgent") as mock_persona:
            mock_agent = Mock()
            mock_persona.return_value = mock_agent
            ...
            agent = factory.create_character("engineer")
            assert agent is not None
            # Verify PersonaAgent was called with correct directory
            call_args = mock_persona.call_args
            assert "engineer" in str(call_args[0][0])  # character directory path
    ...
    def test_validate_character_directory_success(self, characters_directory):
        ...
        # Test that creating character with valid directory works
        with patch("character_factory.PersonaAgent") as mock_persona:
            mock_agent = Mock()
            mock_persona.return_value = mock_agent
    ...
    def test_validate_character_directory_invalid(self, temp_dir):
        ...
        # Test that creating character with nonexistent directory fails
        with pytest.raises((FileNotFoundError, ValueError)):
    ...
    def test_character_factory_error_handling(self, characters_directory):
        ...
        with patch("character_factory.PersonaAgent") as mock_persona:
            ...
            with pytest.raises(Exception):
                factory.create_character("engineer")
    ...
    def test_multiple_character_creation(self, characters_directory):
        ...
        with patch("character_factory.PersonaAgent") as mock_persona:
            ...
            mock_persona.side_effect = lambda *args, **kwargs: create_mock_agent("test")
