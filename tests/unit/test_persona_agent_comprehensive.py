...
"""
    ...
    def test_initialization_with_valid_character_directory(self):
        ...
        with patch('os.path.exists', return_value=True), \
             ...
             patch('src.persona_agent.PersonaAgent._read_cached_file', return_value=self.test_character_content), \
    ...
    def test_event_bus_subscription(self):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            self.mock_event_bus.subscribe.assert_called_with("TURN_START", agent.handle_turn_start)
...
class TestPersonaAgentCharacterLoading:
    ...
    def create_test_agent(self, character_content: str):
        ...
        with patch('os.path.exists', return_value=True), \
             ...
             patch('src.persona_agent.PersonaAgent._read_cached_file', return_value=character_content), \
             patch.object(PersonaAgent, '_extract_core_identity'), \
             patch.object(PersonaAgent, '_extract_personality_traits'), \
             patch.object(PersonaAgent, '_extract_decision_weights'), \
             patch.object(PersonaAgent, '_extract_relationships'), \
...
Just random text"""
    ...
    def test_multiple_character_files_handling(self):
        ...
        with patch('os.path.exists', return_value=True), \
             patch('os.path.isdir', return_value=True), \
             patch('os.listdir', return_value=['character.md', 'background.md', 'equipment.yaml']), \
             patch('builtins.open', mock_open(read_data="# Test Character")), \
             patch('src.persona_agent.PersonaAgent._read_cached_file', return_value="# Test Character"), \
...
"""
    ...
    def test_handle_turn_start_basic_functionality(self):
        ...
        mock_action = CharacterAction(action_type="investigate", reasoning="Test reasoning")
...
"""
    ...
    def test_world_event_interpretation_methods(self):
        ...
        interpretation_methods = [
            '_interpret_event', '_assess_threat_level', 
            '_update_worldview', '_process_world_event'
        ]
...
"""
    ...
    def test_memory_system_initialization(self):
        ...
        memory_attributes = [
            'subjective_worldview', 'character_data', 'current_location'
        ]
    ...
    def test_character_evolution_tracking(self):
        ...
        evolution_attributes = [
            'experience_points', 'character_evolution', 'trait_evolution'
        ]
    ...
    def test_memory_persistence_capability(self):
        ...
        persistence_methods = [
            '_save_character_state', '_load_character_state',
            '_update_memory', '_store_experience'
        ]
...
def run_persona_initialization_tests():
    ...
    result = subprocess.run([
        ...
        "-v", "-k", "TestPersonaAgentInitialization",
        "--tb=short",
        __file__
...
def run_persona_decision_tests():
    ...
    result = subprocess.run([
        sys.executable, "-m", "pytest", 
        "-v", "-k", "TestPersonaAgentDecisionMaking",
        "--tb=short",
        __file__
    ], capture_output=True, text=True)
