...
from typing import Dict, Any, List, Set, Optional, AsyncIterator
import asyncio

from contexts.ai.domain.services.llm_provider import (
    ILLMProvider, LLMRequest, LLMResponse,
    LLMRequestType, LLMResponseStatus,
    LLMProviderError, RateLimitError, QuotaExceededError,
    ModelUnavailableError, InvalidRequestError
)
from contexts.ai.domain.value_objects.common import (
    ProviderId, ModelId, TokenBudget, ProviderType, ModelCapability
)
...
class TestLLMRequestTypeEnum:
    ...
    def test_all_request_types_exist(self):
        ...
        expected_types = {
            'COMPLETION', 'CHAT', 'CONVERSATION', 'SUMMARIZATION',
            'TRANSLATION', 'CODE_GENERATION', 'ANALYSIS', 
            'EMBEDDING', 'FUNCTION_CALL'
        }
...
class TestLLMResponseStatusEnum:
    ...
    def test_all_response_statuses_exist(self):
        ...
        expected_statuses = {
            'SUCCESS', 'PARTIAL_SUCCESS', 'FAILED', 'RATE_LIMITED',
            'QUOTA_EXCEEDED', 'MODEL_UNAVAILABLE', 'INVALID_REQUEST', 'TIMEOUT'
        }
...
class TestLLMRequestCreation:
    ...
    def test_request_validation_invalid_request_type(self):
        ...
        with pytest.raises(ValueError, match="request_type must be a LLMRequestType enum"):
            LLMRequest(
                request_id=self.request_id,
    ...
    def test_request_validation_empty_prompt(self):
        ...
        with pytest.raises(ValueError, match="prompt is required and must be a non-empty string"):
            LLMRequest(
                request_id=self.request_id,
                request_type=LLMRequestType.COMPLETION,
                model_id=self.model_id,
...
class TestLLMRequestBusinessMethods:
    ...
    def test_estimate_input_tokens(self):
        ...
        expected_tokens = (len(self.request.prompt) + len(self.request.system_prompt)) // 4
    ...
    def test_is_compatible_with_model_context_too_large(self):
        ...
        long_prompt = "x" * (self.model_id.max_context_tokens * 4)  # Much longer than context allows
...
class TestLLMResponseCreation:
    ...
    def test_basic_response_creation(self):
        ...
        assert response.usage_stats == {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0}
    ...
    def test_response_validation_failed_without_error_details(self):
        ...
        with pytest.raises(ValueError, match="Failed responses must have error_details"):
            LLMResponse(
                request_id=self.request_id,
                response_id=self.response_id,
    ...
    def test_response_validation_invalid_usage_stats(self):
        ...
        with pytest.raises(ValueError, match="usage_stats.*must be a non-negative integer"):
            LLMResponse(
                request_id=self.request_id,
                response_id=self.response_id,
                status=LLMResponseStatus.SUCCESS,
                content="Test content",
                usage_stats={"input_tokens": -10, "output_tokens": 50, "total_tokens": 40}
            )
    
    def test_response_validation_invalid_cost_estimate(self):
        """Test response validation with invalid cost estimate."""
        with pytest.raises(ValueError, match="cost_estimate must be a non-negative Decimal"):
            LLMResponse(
                request_id=self.request_id,
                response_id=self.response_id,
                status=LLMResponseStatus.SUCCESS,
...
class TestLLMResponseFactoryMethods:
    ...
    def test_create_success_response(self):
        ...
        assert response.cost_estimate == self.model_id.estimate_cost(input_tokens, output_tokens)
...
class MockLLMProvider(ILLMProvider):
    ...
    async def generate_async(self, request: LLMRequest, budget: Optional[TokenBudget] = None) -> LLMResponse:
    ...
    async def generate_stream_async(self, request: LLMRequest, budget: Optional[TokenBudget] = None) -> AsyncIterator[str]:
...
class TestILLMProviderWithBudget:
    ...
    def setup_method(self):
        ...
        self.budget = TokenBudget.create_daily_budget("test-budget", 1000, Decimal('10.00'))
    ...
    async def test_generate_stream_async_with_budget(self):
        ...
        chunks = []
        async for chunk in self.provider.generate_stream_async(self.request, self.budget):
            chunks.append(chunk)
