...
from contexts.character.domain.value_objects.skills import (
    Skills, Skill, SkillCategory, ProficiencyLevel
)
...
class TestCharacterAggregate:
    ...
    def sample_skills(self) -> Skills:
        ...
        skills.get_skills_by_category = Mock(side_effect=lambda cat: skills.skill_groups.get(cat, []))
        skills.get_skill_summary = Mock(return_value={"total_skills": 2, "trained_skills": 2})
    ...
    def test_character_validation_insufficient_health(self, sample_character_profile, sample_character_stats, sample_skills):
    ...
    def test_character_validation_class_skills(self, sample_character_profile, sample_character_stats):
        ...
        invalid_skills.get_skills_by_category = Mock(side_effect=lambda cat: invalid_skills.skill_groups.get(cat, []))
    ...
    def test_update_stats_exceed_maximum_values_fails(self, sample_character):
        ...
        object.__setattr__(new_vital_stats, 'current_health', new_vital_stats.max_health + 10)
    ...
    def test_level_up_success(self, sample_character):
        ...
        assert sample_character.stats.vital_stats.max_health == initial_health + 10 + sample_character.stats.core_abilities.constitution
        assert sample_character.stats.vital_stats.max_mana == initial_mana + 5 + sample_character.stats.core_abilities.intelligence
    ...
    def test_take_damage_success(self, sample_character):
        ...
        assert sample_character.stats.vital_stats.current_health == initial_health - expected_damage
        assert sample_character.version == initial_version + 1
    ...
    def test_update_skills_success(self, sample_character):
        ...
        new_skills.get_skills_by_category = Mock(side_effect=lambda cat: new_skills.skill_groups.get(cat, []))
    ...
    def test_character_business_rules_consistency(self, sample_character):
        ...
        min_expected_health = max(1, original_constitution + sample_character.profile.level)
