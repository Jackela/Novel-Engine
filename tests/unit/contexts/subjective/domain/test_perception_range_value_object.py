...
class TestPerceptionRangeValidation:
    ...
    def test_accuracy_modifier_below_zero(self):
        ...
        with pytest.raises(
            ValueError, match="Accuracy modifier must be between 0.0 and 1.0"
        ):
            PerceptionRange(
                perception_type=PerceptionType.VISUAL,
                base_range=100.0,
    ...
    def test_non_numeric_environmental_modifier_value(self):
        ...
        with pytest.raises(
            ValueError, match="Environmental modifier 'fog' must be numeric"
        ):
            PerceptionRange(
                perception_type=PerceptionType.VISUAL,
                base_range=100.0,
    ...
    def test_visibility_with_environmental_degradation(self):
        ...
        degraded_perception = PerceptionRange(
            ...
            environmental_modifiers={
                "fog": 0.3,
                "rain": 0.7,
            },  # Combined modifier: 0.3 * 0.7 = 0.21
        )
...
class TestPerceptionCapabilitiesValidation:
    ...
    def test_empty_perception_ranges_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Entity must have at least one perception range"
        ):
    ...
    def test_perception_type_mismatch(self):
        ...
        with pytest.raises(ValueError, match="Perception range type mismatch"):
            PerceptionCapabilities(
                perception_ranges={
                    PerceptionType.VISUAL: auditory_range
    ...
    def test_negative_passive_awareness_bonus(self):
        ...
        with pytest.raises(
            ValueError, match="Passive awareness bonus cannot be negative"
        ):
            PerceptionCapabilities(
                perception_ranges={PerceptionType.VISUAL: visual_range},
    ...
    def test_zero_focused_perception_multiplier(self):
        ...
        with pytest.raises(
            ValueError, match="Focused perception multiplier must be positive"
        ):
            PerceptionCapabilities(
                perception_ranges={PerceptionType.VISUAL: visual_range},
...
class TestComplexPerceptionScenarios:
    ...
    def test_stealth_detection_scenario(self):
        ...
        close_visibility = guard_capabilities.get_best_visibility_at_distance(
            close_distance
        )
        assert close_visibility in [
            VisibilityLevel.CLEAR,
            VisibilityLevel.PARTIAL,
            VisibilityLevel.OBSCURED,
        ]
        ...
        medium_visibility = guard_capabilities.get_best_visibility_at_distance(
            medium_distance
        )
        assert (
            medium_visibility != VisibilityLevel.INVISIBLE
        )  # Should still detect something

        # At far distance, might struggle to detect
        far_visibility = guard_capabilities.get_best_visibility_at_distance(
            far_distance
        )
        ...
        focused_auditory = guard_capabilities.get_best_visibility_at_distance(
            medium_distance, PerceptionType.AUDITORY
        )
    ...
    def test_focused_perception_effectiveness(self):
        ...
        focused_edge = capabilities.get_best_visibility_at_distance(
            edge_distance, PerceptionType.VISUAL
        )
        ...
        focused_beyond = capabilities.get_best_visibility_at_distance(
            beyond_distance, PerceptionType.VISUAL
        )
        ...
        focused_far_beyond = capabilities.get_best_visibility_at_distance(
            far_beyond_distance, PerceptionType.VISUAL
        )
    ...
    def test_multi_sense_compensation_scenario(self):
        ...
        visual_only_visibility = visual_only.get_best_visibility_at_distance(
            test_distance
        )
        multi_sense_visibility = multi_sense.get_best_visibility_at_distance(
            test_distance
        )
...
class TestEdgeCasesAndBoundaryConditions:
    ...
    def test_focused_perception_on_nonexistent_type(self):
        ...
        assert (
            visibility != VisibilityLevel.INVISIBLE
        )  # Visual should still work at this distance
