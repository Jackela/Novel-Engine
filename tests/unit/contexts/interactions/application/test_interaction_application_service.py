...
from contexts.interactions.application.services.interaction_application_service import InteractionApplicationService
from contexts.interactions.domain.repositories.negotiation_session_repository import NegotiationSessionRepository
from contexts.interactions.domain.services.negotiation_service import NegotiationService
from contexts.interactions.domain.aggregates.negotiation_session import NegotiationSession
from contexts.interactions.domain.value_objects.interaction_id import InteractionId
from contexts.interactions.domain.value_objects.negotiation_party import NegotiationParty
from contexts.interactions.domain.value_objects.negotiation_status import (
    NegotiationPhase, NegotiationOutcome, TerminationReason
)
from contexts.interactions.domain.value_objects.proposal_terms import ProposalTerms
from contexts.interactions.domain.value_objects.proposal_response import ProposalResponse
from contexts.interactions.application.commands.interaction_command_handlers import InteractionCommandHandler
...
class TestInteractionApplicationServiceInitialization:
    ...
    def test_command_handler_initialization(self):
        ...
        with patch('contexts.interactions.application.services.interaction_application_service.InteractionCommandHandler') as mock_handler_class:
            mock_handler = Mock()
...
class TestNegotiationSessionOperations:
    ...
    async def test_create_negotiation_session_success(self, mock_dependencies):
        ...
        mock_dependencies['command_handler'].handle_create_negotiation_session.return_value = mock_result
        ...
        mock_dependencies['command_handler'].handle_create_negotiation_session.assert_called_once()
    ...
    async def test_create_negotiation_session_with_defaults(self, mock_dependencies):
        ...
        call_args = mock_dependencies['command_handler'].handle_create_negotiation_session.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_add_party_to_negotiation_success(self, mock_dependencies, sample_negotiation_party):
        ...
        mock_dependencies['command_handler'].handle_add_party_to_session.return_value = mock_result
        ...
        mock_dependencies['command_handler'].handle_add_party_to_session.assert_called_once()
        call_args = mock_dependencies['command_handler'].handle_add_party_to_session.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_add_party_insufficient_parties(self, mock_dependencies, sample_negotiation_party):
    ...
    async def test_advance_negotiation_phase_success(self, mock_dependencies):
        ...
        mock_dependencies['command_handler'].handle_advance_negotiation_phase.return_value = mock_result
        ...
        # Verify command was handled
        mock_dependencies['command_handler'].handle_advance_negotiation_phase.assert_called_once()
        call_args = mock_dependencies['command_handler'].handle_advance_negotiation_phase.call_args[0][0]
        assert call_args.session_id == session_id
...
class TestProposalOperations:
    ...
    @pytest.mark.asyncio
    async def test_submit_proposal_success(self, mock_dependencies, sample_proposal_terms):
        ...
        mock_dependencies['command_handler'].handle_analyze_proposal_viability.return_value = mock_analysis_result
        ...
        mock_dependencies['command_handler'].handle_submit_proposal.return_value = mock_submission_result
        ...
        mock_dependencies['command_handler'].handle_analyze_proposal_viability.assert_called_once()
        ...
        assert result['pre_submission_analysis'] == mock_analysis_result['analysis_result']
    
    @pytest.mark.asyncio
    async def test_submit_proposal_response_success(self, mock_dependencies, sample_proposal_response):
        ...
        mock_dependencies['command_handler'].handle_submit_proposal_response.return_value = mock_response_result
        ...
        mock_dependencies['command_handler'].handle_calculate_negotiation_momentum.return_value = mock_momentum_result
        ...
        mock_dependencies['command_handler'].handle_submit_proposal_response.assert_called_once()
        mock_dependencies['command_handler'].handle_calculate_negotiation_momentum.assert_called_once()
        ...
        assert result['session_status']['momentum'] == mock_momentum_result['momentum_analysis']
    
    @pytest.mark.asyncio
    async def test_submit_proposal_response_negative_momentum(self, mock_dependencies, sample_proposal_response):
...
class TestNegotiationCompletion:
    ...
    async def test_complete_negotiation_success(self, mock_dependencies):
        ...
        mock_dependencies['command_handler'].handle_detect_negotiation_conflicts.return_value = mock_conflicts_result
        ...
        mock_dependencies['command_handler'].handle_terminate_negotiation_session.return_value = mock_termination_result
        ...
        assert mock_dependencies['command_handler'].handle_calculate_negotiation_momentum.call_count == 1
        assert mock_dependencies['command_handler'].handle_detect_negotiation_conflicts.call_count == 1
        assert mock_dependencies['command_handler'].handle_terminate_negotiation_session.call_count == 1
        ...
        assert result['completion_summary']['completion_notes'] == "Successfully reached agreement"
        assert result['final_analysis']['momentum'] == mock_momentum_result['momentum_analysis']
        assert result['final_analysis']['conflicts'] == mock_conflicts_result['conflicts_detected']
    ...
    async def test_complete_negotiation_with_conflicts(self, mock_dependencies):
        ...
        mock_conflicts_result = {
            'conflicts_detected': [
                ...
                {'severity': 'medium', 'resolved': False, 'type': 'timeline_disagreement'},
...
class TestAnalyticalOperations:
    ...
    async def test_get_negotiation_insights_comprehensive(self, mock_dependencies):
        ...
        mock_dependencies['command_handler'].handle_assess_party_compatibility.return_value = mock_compatibility_result
        ...
        mock_dependencies['command_handler'].handle_recommend_negotiation_strategy.return_value = mock_strategy_result
        
        # Mock conflicts detection
        mock_conflicts_result = {
            'conflicts_detected': [
                {'severity': 'medium', 'type': 'timeline_pressure', 'resolution_suggestions': ['extend_deadline']}
            ]
        }
        mock_dependencies['command_handler'].handle_detect_negotiation_conflicts.return_value = mock_conflicts_result
        ...
        mock_dependencies['command_handler'].handle_assess_party_compatibility.assert_called_once()
        mock_dependencies['command_handler'].handle_recommend_negotiation_strategy.assert_called_once()
        mock_dependencies['command_handler'].handle_detect_negotiation_conflicts.assert_called_once()
        mock_dependencies['command_handler'].handle_calculate_negotiation_momentum.assert_called_once()
        ...
        assert result['insights']['recommended_strategy'] == mock_strategy_result['strategy_recommendation']
        assert result['insights']['detected_conflicts'] == mock_conflicts_result['conflicts_detected']
        assert result['insights']['momentum_analysis'] == mock_momentum_result['momentum_analysis']
    ...
    async def test_get_negotiation_insights_low_compatibility(self, mock_dependencies):
        ...
        assert result['overall_assessment']['conflict_level'] == 3  # High number of conflicts
        ...
        assert success_prob < 50  # Should be low due to poor compatibility, high conflicts, low momentum
...
class TestProposalOptimization:
    ...
    @pytest.mark.asyncio
    async def test_optimize_active_proposals_success(self, mock_dependencies, mock_negotiation_session):
        ...
        session_id = uuid4()
        mock_dependencies['repository'].get_by_id.return_value = mock_negotiation_session
        ...
        for i, proposal_id in enumerate(proposal_ids):
            mock_result = {
                'analysis_result': {
                    ...
                    'optimization_suggestions': [f'suggestion_{i}_1', f'suggestion_{i}_2'],
        ...
        mock_dependencies['command_handler'].handle_analyze_proposal_viability.side_effect = analysis_results
        ...
        mock_dependencies['repository'].get_by_id.assert_called_once_with(InteractionId(session_id))
        
        # Verify analysis was called for each proposal
        assert mock_dependencies['command_handler'].handle_analyze_proposal_viability.call_count == 3
    ...
    async def test_optimize_active_proposals_no_proposals(self, mock_dependencies):
        ...
        mock_dependencies['command_handler'].handle_analyze_proposal_viability.assert_not_called()
...
class TestSessionHealthMonitoring:
    ...
    def mock_unhealthy_session(self):
        ...
        session = Mock(spec=NegotiationSession)
        session.created_at = datetime.now(timezone.utc) - timedelta(days=7)  # Old session
        session.parties = [Mock()]  # Only 1 party
        session.active_proposals = {}  # No proposals
        session.total_responses = 2
        session.status = Mock()
        session.status.phase = NegotiationPhase.BARGAINING
        session.status.time_since_last_activity = 3600 * 6  # 6 hours ago (exceeds threshold)
        session.is_timeout_approaching.return_value = True  # Approaching timeout
        return session
    
    @pytest.mark.asyncio
    async def test_monitor_session_health_healthy_session(self, mock_dependencies, mock_healthy_session):
    ...
    @pytest.mark.asyncio
    async def test_monitor_session_health_unhealthy_session(self, mock_dependencies, mock_unhealthy_session):
...
class TestPrivateHelperMethods:
    ...
    def test_generate_recommendations(self):
        ...
        compatibility_result = {
            'overall_compatibility': 45.0  # Below 50 threshold
        }
    ...
    def test_calculate_health_score(self):
        ...
        healthy_session.active_proposals = {uuid4(): Mock(), uuid4(): Mock()}  # Has proposals
        ...
        unhealthy_session.status.time_since_last_activity = 3600 * 25  # 25 hours (> 24h threshold)
    ...
    def test_generate_health_recommendations(self):
        ...
        recommendations = service._generate_health_recommendations(
            health_score=45.0,  # Poor health
            health_alerts=health_alerts
        )
        
        # Should address each alert type
        assert any('timeline' in rec.lower() or 'extend' in rec.lower() for rec in recommendations)
        assert any('follow-up' in rec.lower() or 'meeting' in rec.lower() for rec in recommendations)
        assert any('conflict resolution' in rec.lower() for rec in recommendations)
        assert any('momentum' in rec.lower() or 'approach' in rec.lower() for rec in recommendations)
