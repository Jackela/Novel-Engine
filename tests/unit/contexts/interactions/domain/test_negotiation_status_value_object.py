...
class TestNegotiationStatusValidation:
    ...
    def test_timezone_validation_expected_completion_at(self):
        ...
        with pytest.raises(ValueError, match="expected_completion_at must be timezone-aware"):
            NegotiationStatus(
                phase=NegotiationPhase.INITIATION,
                outcome=NegotiationOutcome.PENDING,
                started_at=aware_datetime,
    ...
    def test_timezone_validation_actual_completion_at(self):
        ...
        with pytest.raises(ValueError, match="actual_completion_at must be timezone-aware"):
            NegotiationStatus(
                phase=NegotiationPhase.TERMINATED,
                outcome=NegotiationOutcome.AGREEMENT_REACHED,
                started_at=aware_datetime,
    ...
    def test_started_at_after_last_activity_validation(self):
        ...
        with pytest.raises(ValueError, match="started_at cannot be later than last_activity_at"):
            NegotiationStatus(
                phase=NegotiationPhase.INITIATION,
                outcome=NegotiationOutcome.PENDING,
                started_at=now,
    ...
    def test_started_at_after_expected_completion_validation(self):
        ...
        with pytest.raises(ValueError, match="started_at cannot be later than expected_completion_at"):
            NegotiationStatus(
                phase=NegotiationPhase.INITIATION,
                outcome=NegotiationOutcome.PENDING,
                started_at=now,
    ...
    def test_started_at_after_actual_completion_validation(self):
        ...
        with pytest.raises(ValueError, match="started_at cannot be later than actual_completion_at"):
            NegotiationStatus(
                phase=NegotiationPhase.TERMINATED,
                outcome=NegotiationOutcome.AGREEMENT_REACHED,
                started_at=now,
    ...
    def test_terminated_phase_with_pending_outcome_validation(self):
        ...
        with pytest.raises(ValueError, match="Terminated negotiations cannot have pending outcome"):
            NegotiationStatus(
                phase=NegotiationPhase.TERMINATED,
                outcome=NegotiationOutcome.PENDING,
                started_at=now,
    ...
    def test_terminated_phase_without_termination_reason_validation(self):
        ...
        with pytest.raises(ValueError, match="Terminated negotiations must have a termination reason"):
            NegotiationStatus(
                phase=NegotiationPhase.TERMINATED,
                outcome=NegotiationOutcome.AGREEMENT_REACHED,
                started_at=now,
    ...
    def test_terminated_phase_without_actual_completion_validation(self):
        ...
        with pytest.raises(ValueError, match="Terminated negotiations must have actual completion time"):
            NegotiationStatus(
                phase=NegotiationPhase.TERMINATED,
                outcome=NegotiationOutcome.AGREEMENT_REACHED,
                started_at=now,
...
class TestNegotiationStatusFactoryMethods:
    ...
    def test_create_initial_default_time(self):
        ...
        with patch('contexts.interactions.domain.value_objects.negotiation_status.datetime') as mock_datetime:
...
class TestNegotiationStatusPhaseTransitions:
    ...
    def test_valid_phase_transitions(self):
        ...
        assert status._is_valid_phase_transition(NegotiationPhase.INITIATION, NegotiationPhase.PREPARATION)
        assert status._is_valid_phase_transition(NegotiationPhase.INITIATION, NegotiationPhase.TERMINATED)
        
        # Valid transitions from PREPARATION
        assert status._is_valid_phase_transition(NegotiationPhase.PREPARATION, NegotiationPhase.OPENING)
        assert status._is_valid_phase_transition(NegotiationPhase.PREPARATION, NegotiationPhase.TERMINATED)
        
        # Valid transitions from OPENING
        assert status._is_valid_phase_transition(NegotiationPhase.OPENING, NegotiationPhase.BARGAINING)
        assert status._is_valid_phase_transition(NegotiationPhase.OPENING, NegotiationPhase.TERMINATED)
        
        # Valid transitions from BARGAINING
        assert status._is_valid_phase_transition(NegotiationPhase.BARGAINING, NegotiationPhase.CLOSING)
        assert status._is_valid_phase_transition(NegotiationPhase.BARGAINING, NegotiationPhase.TERMINATED)
        
        # Valid transitions from CLOSING
        assert status._is_valid_phase_transition(NegotiationPhase.CLOSING, NegotiationPhase.IMPLEMENTATION)
        assert status._is_valid_phase_transition(NegotiationPhase.CLOSING, NegotiationPhase.TERMINATED)
        
        # Valid transitions from IMPLEMENTATION
        assert status._is_valid_phase_transition(NegotiationPhase.IMPLEMENTATION, NegotiationPhase.TERMINATED)
        
        # No valid transitions from TERMINATED
        assert not status._is_valid_phase_transition(NegotiationPhase.TERMINATED, NegotiationPhase.INITIATION)
    ...
    def test_invalid_phase_transitions(self):
        ...
        # Cannot skip phases
        assert not status._is_valid_phase_transition(NegotiationPhase.INITIATION, NegotiationPhase.BARGAINING)
        assert not status._is_valid_phase_transition(NegotiationPhase.PREPARATION, NegotiationPhase.CLOSING)
        
        # Cannot go backwards
        assert not status._is_valid_phase_transition(NegotiationPhase.BARGAINING, NegotiationPhase.OPENING)
        assert not status._is_valid_phase_transition(NegotiationPhase.CLOSING, NegotiationPhase.BARGAINING)
        
        # Cannot transition from terminal state
        assert not status._is_valid_phase_transition(NegotiationPhase.TERMINATED, NegotiationPhase.IMPLEMENTATION)
    ...
    def test_advance_to_phase_success(self):
        ...
        new_status = status.advance_to_phase(NegotiationPhase.PREPARATION, activity_time)
        ...
        assert new_status.expected_completion_at == status.expected_completion_at  # Unchanged
    ...
    def test_advance_to_phase_invalid_transition(self):
        ...
        with pytest.raises(ValueError, match="Invalid phase transition from initiation to bargaining"):
...
class TestNegotiationStatusCompletion:
    ...
    def test_complete_different_outcomes(self):
        ...
        outcomes_reasons = [
            ...
            (NegotiationOutcome.STALEMATE, TerminationReason.IRRECONCILABLE_DIFFERENCES),
...
class TestNegotiationStatusProperties:
    ...
    def test_duration_property(self):
        ...
        start_time = datetime(2023, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        completion_time = datetime(2023, 1, 1, 15, 30, 0, tzinfo=timezone.utc)  # 3.5 hours later
    ...
    def test_time_since_last_activity_property(self):
        ...
        last_activity = datetime(2023, 1, 1, 12, 0, 0, tzinfo=timezone.utc)  # 3 hours ago
...
class TestNegotiationStatusStringRepresentation:
    ...
    def test_str_representation_completed(self):
        ...
        str_repr = str(status)
        assert str_repr == "NegotiationStatus(phase=terminated, outcome=agreement_reached)"
...
class TestNegotiationStatusEdgeCases:
    ...
    def test_very_long_negotiation(self):
        ...
        start_time = datetime(2023, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        completion_time = datetime(2024, 1, 1, 12, 0, 0, tzinfo=timezone.utc)  # 1 year later
    ...
    def test_phase_transition_edge_cases(self):
        ...
        assert not status._is_valid_phase_transition(NegotiationPhase.INITIATION, NegotiationPhase.INITIATION)
        
        # Direct to terminated from any phase should be valid
        for phase in [NegotiationPhase.INITIATION, NegotiationPhase.PREPARATION, 
