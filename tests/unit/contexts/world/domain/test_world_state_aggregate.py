...
class TestWorldStateAggregate:
    ...
    def test_world_state_initialization(self):
        ...
        assert world.version == 2  # Version increments during creation (1) and activation (2)
    ...
    def test_activate_world_state(self, world_state):
        ...
        assert len(events) > 0
        activation_event = next((e for e in events if "activated" in e.change_reason), None)
    ...
    def test_add_entity_success(self, world_state, sample_entity_data):
        ...
        # Check domain event was raised
        events = world_state.get_domain_events()
        add_event = next((e for e in events if e.change_type.value == "entity_added"), None)
    ...
    def test_add_duplicate_entity_fails(self, world_state, sample_entity_data):
        ...
        with pytest.raises(ValueError) as exc_info:
            world_state.add_entity(**sample_entity_data)
        assert f"Entity with ID {sample_entity_data['entity_id']} already exists" in str(exc_info.value)
    ...
    def test_remove_entity_success(self, world_state, sample_entity_data):
        ...
        removed_entity = world_state.remove_entity(sample_entity_data["entity_id"], "Test removal")
        ...
        # Check spatial index was updated
        grid_key = world_state._get_spatial_grid_key(sample_entity_data["coordinates"])
        assert grid_key not in world_state.spatial_index or \
               sample_entity_data["entity_id"] not in world_state.spatial_index.get(grid_key, [])
        
        # Check domain event was raised
        events = world_state.get_domain_events()
        remove_event = next((e for e in events if e.change_type.value == "entity_removed"), None)
    ...
    def test_move_entity_success(self, world_state, sample_entity_data):
        ...
        success = world_state.move_entity(sample_entity_data["entity_id"], new_coordinates, "Test move")
        ...
        # Check spatial index was updated
        old_grid_key = world_state._get_spatial_grid_key(sample_entity_data["coordinates"])
        new_grid_key = world_state._get_spatial_grid_key(new_coordinates)
        
        assert sample_entity_data["entity_id"] not in world_state.spatial_index.get(old_grid_key, [])
        assert sample_entity_data["entity_id"] in world_state.spatial_index[new_grid_key]
        
        # Check domain event was raised
        events = world_state.get_domain_events()
        move_event = next((e for e in events if e.change_type.value == "entity_moved"), None)
    ...
    def test_update_entity_success(self, world_state, sample_entity_data):
        ...
        # Check domain event was raised
        events = world_state.get_domain_events()
        update_event = next((e for e in events if e.change_type.value == "entity_updated"), None)
        assert update_event is not None

    def test_update_nonexistent_entity(self, world_state):
        """Test updating non-existent entity returns False."""
        success = world_state.update_entity("nonexistent-id", properties={"test": "value"})
        assert success is False
    ...
    def test_get_entities_in_area_with_type_filter(self, world_state):
        ...
        world_state.add_entity("near_char", EntityType.CHARACTER, "Near Char", coords_near)
    ...
    def test_get_entities_at_coordinates_exact(self, world_state):
        ...
        world_state.add_entity("other_entity", EntityType.CHARACTER, "Other", Coordinates(15, 25, 5))
    ...
    def test_get_entities_at_coordinates_with_tolerance(self, world_state):
        ...
        world_state.add_entity("exact_entity", EntityType.CHARACTER, "Exact", coords)
        world_state.add_entity("nearby_entity", EntityType.CHARACTER, "Nearby", nearby_coords)
    ...
    def test_advance_time_success(self, world_state):
        ...
        # Check domain event was raised
        events = world_state.get_domain_events()
        time_event = next((e for e in events if e.change_type.value == "time_advanced"), None)
    ...
    def test_update_environment_success(self, world_state):
        ...
        # Check domain event was raised
        events = world_state.get_domain_events()
        env_event = next((e for e in events if e.change_type.value == "environment_changed"), None)
    ...
    def test_get_entities_from_spatial_index(self, world_state):
        ...
        world_state._add_to_spatial_index("entity2", Coordinates(150, 150, 0)) # Grid 1,1
        world_state._add_to_spatial_index("entity3", Coordinates(250, 250, 0)) # Grid 2,2
        
        # Query center at (100, 100) with radius 100 should cover grids 0,0 to 2,2
        center = Coordinates(100, 100, 0)
        candidate_ids = world_state._get_entities_from_spatial_index(center, radius=100.0)
    ...
    def test_create_snapshot(self, world_state, sample_entity_data):
        ...
        # Check domain event was raised
        events = world_state.get_domain_events()
        snapshot_event = next((e for e in events if e.change_type.value == "state_snapshot"), None)
    ...
    def test_reset_state_preserve_entities(self, world_state, sample_entity_data):
        ...
        assert world_state.version == initial_version + 2  # +1 for reset, +1 for reactivation
        
        # Check domain event was raised
        events = world_state.get_domain_events()
        reset_event = next((e for e in events if e.change_type.value == "state_reset"), None)
    ...
    def test_concurrent_entity_modifications(self, world_state, sample_entity_data):
        ...
        world_state.update_entity(sample_entity_data["entity_id"], properties={"test": "value"})
    ...
    def test_complete_entity_lifecycle(self, world_state):
        ...
        entity = world_state.add_entity(
            entity_id, EntityType.CHARACTER, "Lifecycle Test", coords,
            properties={"health": 100}, metadata={"test": True}
        )
        assert entity.id == entity_id
        
        # Update entity properties
        success = world_state.update_entity(entity_id, properties={"health": 150, "mana": 50})
        assert success
    ...
    def test_spatial_query_integration(self, world_state):
        ...
        world_state.add_entity("close1", EntityType.CHARACTER, "Close 1", Coordinates(1, 1, 0))
        world_state.add_entity("close2", EntityType.OBJECT, "Close 2", Coordinates(2, 2, 0))
        world_state.add_entity("far1", EntityType.CHARACTER, "Far 1", Coordinates(20, 20, 0))
        ...
        # Query again
        nearby_entities_after_move = world_state.get_entities_in_area(center, radius=5.0)
    ...
    def test_environment_and_time_integration(self, world_state):
        ...
        world_state.update_environment({"weather": "sunny", "temperature": 25}, "Day setup")
        ...
        world_state.update_environment({"weather": "cloudy", "temperature": 18}, "Evening weather")
