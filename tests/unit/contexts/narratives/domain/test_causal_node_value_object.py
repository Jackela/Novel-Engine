...
class TestCausalRelationTypeEnum:
    ...
    def test_all_relation_types_exist(self):
        ...
        expected_types = {
            'DIRECT_CAUSE', 'INDIRECT_CAUSE', 'NECESSARY_CONDITION', 'SUFFICIENT_CONDITION',
            'CONTRIBUTING_FACTOR', 'PREVENTING_FACTOR', 'CATALYST', 'INHIBITOR',
            'TRIGGER', 'ENABLING_CONDITION', 'COINCIDENTAL', 'FEEDBACK_LOOP'
        }
...
class TestCausalStrengthEnum:
    ...
    def test_all_strength_levels_exist(self):
        ...
        expected_levels = {
            'ABSOLUTE', 'VERY_STRONG', 'STRONG', 'MODERATE', 
            'WEAK', 'VERY_WEAK', 'NEGLIGIBLE'
        }
    ...
    def test_strength_logical_ordering(self):
        ...
        assert strength_order[CausalStrength.ABSOLUTE] > strength_order[CausalStrength.VERY_STRONG]
        assert strength_order[CausalStrength.VERY_STRONG] > strength_order[CausalStrength.STRONG]
        assert strength_order[CausalStrength.STRONG] > strength_order[CausalStrength.MODERATE]
        assert strength_order[CausalStrength.MODERATE] > strength_order[CausalStrength.WEAK]
        assert strength_order[CausalStrength.WEAK] > strength_order[CausalStrength.VERY_WEAK]
        assert strength_order[CausalStrength.VERY_WEAK] > strength_order[CausalStrength.NEGLIGIBLE]
...
class TestCausalNodeValidation:
    ...
    def test_missing_title_and_description_validation(self):
        ...
        with pytest.raises(ValueError, match="Causal node must have either title or description"):
            CausalNode(
                node_id="no-content-test",
                title="",
                description=""
            )
    ...
    def test_occurrence_probability_below_minimum_validation(self):
        ...
        with pytest.raises(ValueError, match="Occurrence probability must be between 0 and 1"):
            CausalNode(
    ...
    def test_causal_certainty_boundary_validation(self):
        ...
        with pytest.raises(ValueError, match="Causal certainty must be between 0 and 1"):
            CausalNode(
    ...
    def test_character_impact_boundary_validation(self):
        ...
        with pytest.raises(ValueError, match="character_impact_level must be between 1 and 10"):
            CausalNode(
    ...
    def test_story_arc_impact_boundary_validation(self):
        ...
        with pytest.raises(ValueError, match="story_arc_impact must be between 1 and 10"):
            CausalNode(
    ...
    def test_string_length_validations(self):
        ...
        # Node ID too long
        with pytest.raises(ValueError, match="Node ID too long \\(max 100 characters\\)"):
            CausalNode(
                node_id="x" * 101,
                title="Valid Title"
            )
        ...
        # Title too long
        with pytest.raises(ValueError, match="Node title too long \\(max 200 characters\\)"):
            CausalNode(
                node_id="valid-id",
                title="x" * 201
            )
        ...
        with pytest.raises(ValueError, match="Node description too long \\(max 1000 characters\\)"):
            CausalNode(
                node_id="valid-id",
                title="Valid Title",
                description="x" * 1001
...
class TestCausalNodeProperties:
    ...
    def test_has_causes_false(self):
        ...
        node = CausalNode(
            node_id="no-causes-test",
            title="No Causes Test"
        )
...
class TestCausalComplexityScore:
    ...
    def test_complexity_isolated_node(self):
        ...
        node = CausalNode(
            node_id="isolated-complexity-test",
            title="Isolated Node"
        )
...
class TestCausalNodeInstanceMethods:
    ...
    def test_get_relationship_info_non_existing(self):
        ...
        node = CausalNode(
            node_id="no-relationship-test",
            title="No Relationship Test"
        )
    ...
    def test_get_relationship_type_non_existing(self):
        ...
        node = CausalNode(
            node_id="no-rel-type-test",
            title="No Relationship Type Test"
        )
...
class TestCausalNodeFactoryMethods:
    ...
    def test_with_additional_cause_non_direct_relationship(self):
        ...
        original = CausalNode(
            node_id="add-catalyst-test",
            title="Add Catalyst Test"
        )
...
class TestCausalNodeEdgeCasesAndBoundaryConditions:
    ...
    def test_complex_metadata_handling(self):
        ...
        assert node.metadata["causal_analysis"]["confidence_intervals"]["occurrence"] == [0.7, 0.9]
...
class TestCausalNodeCollectionsAndComparison:
    ...
    def test_nodes_sorting_by_complexity_score(self):
        ...
        sorted_nodes = sorted(nodes, key=lambda n: n.causal_complexity_score, reverse=True)
        
        # Most complex should be first
        assert sorted_nodes[0].causal_complexity_score >= sorted_nodes[1].causal_complexity_score
        assert sorted_nodes[1].causal_complexity_score >= sorted_nodes[2].causal_complexity_score
        assert sorted_nodes[2].causal_complexity_score >= sorted_nodes[3].causal_complexity_score
    
    def test_node_equality_identity(self):
        """Test that identical CausalNode objects are considered equal."""
        node1 = CausalNode(
            node_id="equality-test",
            title="Equal Node",
            node_type="action"
        )
    ...
    def test_nodes_in_set(self):
        ...
        node1 = CausalNode(
            node_id="set-test-1",
            title="First Set Node"
        )
        
        node2 = CausalNode(
            node_id="set-test-2",
            title="Second Set Node"
        )
    ...
    def test_nodes_as_dict_keys(self):
        ...
        node_dict = {
            node1: "first_node_data",
            node2: "second_node_data"
        }
