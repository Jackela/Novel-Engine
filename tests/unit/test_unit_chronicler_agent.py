...
@pytest.mark.skipif(not CHRONICLER_AGENT_AVAILABLE, reason="Chronicler agent not available")
class TestChroniclerAgent:
    ...
    def test_transcribe_log_success(self):
        ...
        with patch('chronicler_agent._make_gemini_api_request') as mock_gemini:
            mock_response = Mock()
            mock_response.text = "In a realm of adventure, our heroes faced great challenges..."
    ...
    def test_log_content_processing(self):
        """测试日志内容处理"""
        # 测试日志文件是否能正确读取
        if hasattr(self.chronicler, '_read_log_file') or hasattr(self.chronicler, 'transcribe_log'):
            try:
                # 直接读取文件内容测试
    ...
    def test_character_name_integration(self):
        ...
        if hasattr(self.chronicler, 'transcribe_log'):
            with patch('chronicler_agent._make_gemini_api_request') as mock_gemini:
                mock_response = Mock()
                mock_response.text = "The hero, villain, and sage embark on their journey..."
                mock_gemini.return_value = mock_response
                ...
                if mock_gemini.called:
                    ...
                    if call_args:
                        ...
                        # 角色信息可能在提示文本中
                        has_character_context = any(name.lower() in call_str.lower() 
...
class TestChroniclerAgentAdvanced:
    ...
    def test_event_bus_integration(self):
        ...
        # 检查是否有事件监听功能
        if hasattr(self.chronicler, 'subscribe') or hasattr(self.chronicler, 'on_event'):
            # 如果有事件监听，测试订阅
            try:
        ...
        try:
            ...
            with patch('chronicler_agent._make_gemini_api_request') as mock_gemini:
                ...
                if story:
                    ...
                    assert "protagonist" in story.lower() or "antagonist" in story.lower()
                    # 检查是否有叙述结构
                    assert any(word in story.lower() for word in 
                             ["tale", "story", "journey", "struggle", "battle"])
    ...
    def test_different_character_configurations(self):
        ...
        for char_names in configurations:
            try:
                ...
                pytest.skip(f"Character configuration test failed for {len(char_names)} characters: {e}")
        ...
        try:
            ...
            with open(large_log_file, 'w', encoding='utf-8') as f:
                ...
                for turn in range(20):  # 20个回合
                    f.write(f"## Turn {turn + 1}\n")
                    f.write(f"Protagonist: Complex action {turn + 1} with detailed description...\n")
                    f.write(f"Antagonist: Elaborate response {turn + 1} with intricate details...\n\n")
...
class TestChroniclerAgentPerformance:
    ...
    def test_transcription_performance(self):
        ...
        try:
            ...
            with patch('chronicler_agent._make_gemini_api_request') as mock_gemini:
                mock_response = Mock()
                mock_response.text = "A performance test story with multiple characters..."
                mock_gemini.return_value = mock_response
