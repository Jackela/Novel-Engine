...
class CodeQualityTest:
    ...
    def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        try:
                ...
                if full_path.exists():
                    try:
                        ...
                        priority_results[file_path] = {
                            ...
                            'critical_issues': len([i for i in metrics.issues if i.severity.value == 'critical']),
                            'high_issues': len([i for i in metrics.issues if i.severity.value == 'high']),
                            'analysis_time_ms': int(analysis_time * 1000)
                        }
                        logger.info(f"✓ {file_path}: {metrics.calculate_quality_score():.1f}/100 ({len(metrics.issues)} issues)")
            ...
            report = analyzer.analyze_project(exclude_patterns=[
                'test_*', 
                '*_test.py', 
                'tests/*', 
                '__pycache__',
            ...
            logger.info(f"Overall Quality Score: {report['summary']['overall_quality_score']:.1f}/100")
            logger.info(f"Technical Debt: {report['summary']['technical_debt_hours']:.1f} hours")
            logger.info(f"Priority Files Analyzed: {len([r for r in priority_results.values() if 'error' not in r])}")
...
"""
        ...
        high_issues = [issue for issue in report['detailed_issues'] 
...
"""
            ...
            for issue_type, issues in sorted(issue_types.items(), key=lambda x: len(x[1]), reverse=True):
                ...
                if len(issues) > 3:
                    detailed_report += f" (showing top 3):"
...
"""
        ...
        for category, minutes in sorted(debt_by_category.items(), key=lambda x: x[1], reverse=True):
            hours = minutes / 60
...
"""
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write('\n"""\n\n')
            f.write(f"# Code Quality Analysis Results:\n")
            f.write(f"TECHNICAL_DEBT_ANALYSIS = {json.dumps(report, indent=2, default=str)}\n")
    ...
    try:
        ...
        if component_results['success']:
            ...
            if analysis_results['success']:
                logger.info("✅ Comprehensive code quality analysis completed successfully")
                
                # Summary of results
                report = analysis_results['report']
                logger.info(f"📊 Analysis Summary:")
                logger.info(f"   Quality Score: {report['summary']['overall_quality_score']:.1f}/100")
                logger.info(f"   Grade: {report['summary']['maintainability_grade']}")
                logger.info(f"   Technical Debt: {report['summary']['technical_debt_hours']:.1f} hours")
                logger.info(f"   Critical Issues: {report['issues_by_severity']['critical']}")
