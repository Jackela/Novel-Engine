...
class TestAPIServerEndpoints:
    ...
    def test_characters_endpoint_empty_directory(self, temp_dir):
        ...
        with patch('api_server._get_characters_directory_path', return_value=str(chars_dir)):
    ...
    def test_characters_endpoint_populated_directory(self, characters_directory):
        """测试角色列表端点 - 有角色数据"""
        with patch('api_server._get_characters_directory_path', return_value=str(characters_directory)):
            response = self.client.get("/characters")
    ...
    def test_characters_endpoint_directory_not_found(self, temp_dir):
        ...
        with patch('api_server._get_characters_directory_path', return_value=str(non_existent_dir)):
            response = self.client.get("/characters")
    ...
    def test_character_detail_endpoint_success(self, characters_directory):
        ...
        with patch('api_server._get_characters_directory_path', return_value=str(characters_directory)):
            with patch('api_server.EventBus') as mock_bus, \
                 patch('api_server.CharacterFactory') as mock_factory:
                
                # 模拟CharacterFactory失败，但仍应返回基础信息
                mock_factory.return_value.create_character.side_effect = Exception("Factory error")
    ...
    @pytest.mark.api
    @pytest.mark.slow
    def test_simulations_endpoint_valid_request(self, characters_directory, sample_simulation_request):
        """测试模拟端点 - 有效请求"""
        # 确保使用正确的字符目录路径 (不是 parent)
        with patch('api_server._get_characters_directory_path', return_value=str(characters_directory)):
            with patch('api_server.EventBus'), \
                 patch('api_server.CharacterFactory') as mock_factory, \
                 patch('api_server.DirectorAgent') as mock_director, \
                 patch('api_server.ChroniclerAgent') as mock_chronicler:
                ...
                mock_chronicler_instance.transcribe_log.return_value = "Generated test story"
                mock_chronicler.return_value = mock_chronicler_instance
                
                response = self.client.post("/simulations", json=sample_simulation_request)
                ...
                assert data["participants"] == sample_simulation_request["character_names"]
    ...
    def test_simulations_endpoint_invalid_request_too_many_characters(self):
        """测试模拟端点 - 无效请求（角色过多）"""
        invalid_request = {
            "character_names": ["char1", "char2", "char3", "char4", "char5", "char6", "char7"],  # 超过6个
            "turns": 3
        }
    ...
    def test_simulations_endpoint_character_not_found(self, temp_dir):
        """测试模拟端点 - 角色不存在"""
        with patch('api_server._get_characters_directory_path', return_value=str(temp_dir)):
            # 创建一个存在的角色目录
            char1_dir = temp_dir / "valid_char1"
            char1_dir.mkdir()
            
            request_data = {
                "character_names": ["valid_char1", "nonexistent_character"],  # 需要至少2个角色来满足验证
                "turns": 3
            }
            ...
            assert "error" in data
            # 可能任一不存在的字符都会被检测到
            assert ("nonexistent_character" in data["detail"] or "not found" in data["detail"].lower())
    ...
    def test_characters_endpoint_permission_error(self):
        """测试角色列表端点 - 权限错误"""
        with patch('api_server._get_characters_directory_path', return_value="/restricted/path"):
    ...
    def test_characters_endpoint_unexpected_error(self):
        """测试角色列表端点 - 意外错误"""
        with patch('api_server._get_characters_directory_path', return_value="/valid/path"):
            with patch('os.path.isdir', return_value=True):  # 让目录存在检查通过
                with patch('os.listdir') as mock_listdir:
    ...
    def test_simulations_endpoint_character_loading_failure(self, temp_dir):
        ...
        with patch('api_server._get_characters_directory_path', return_value=str(temp_dir)):
            ...
            with patch('api_server.CharacterFactory') as mock_factory:
                mock_factory.return_value.create_character.side_effect = Exception("Character loading failed")
                
                request_data = {
                    "character_names": ["test_char1", "test_char2"],  # 满足最少两个角色的验证
                    "turns": 3
                }
    ...
    def test_simulations_endpoint_director_turn_failure(self, characters_directory):
        ...
        with patch('api_server._get_characters_directory_path', return_value=str(characters_directory)):
            with patch('api_server.EventBus'), \
                ...
                mock_director_instance.run_turn.side_effect = Exception("Turn execution failed")
    ...
    def test_simulations_endpoint_story_generation_failure(self, characters_directory):
        ...
        with patch('api_server._get_characters_directory_path', return_value=str(characters_directory)):
            with patch('api_server.EventBus'), \
                ...
                # 设置故事生成失败
                mock_chronicler_instance = Mock()
                mock_chronicler_instance.transcribe_log.side_effect = Exception("Story generation failed")
                mock_chronicler.return_value = mock_chronicler_instance
                ...
                data = response.json()
                assert "story" in data
                assert "was generated, but detailed transcription failed" in data["story"]
    ...
    def test_character_detail_endpoint_unexpected_error(self, characters_directory):
        ...
        with patch('api_server._get_characters_directory_path', return_value=str(characters_directory)):
            with patch('api_server.EventBus'), \
                ...
                mock_factory.return_value.create_character.side_effect = Exception("Unexpected character error")
...
class TestAPIServerSecurity:
    ...
    def test_large_payload_handling(self):
        """测试大载荷处理"""
        large_payload = {
            "character_names": ["test"] * 1000,  # 大量角色名
            "turns": 10
        }
