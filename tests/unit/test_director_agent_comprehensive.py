...
class TestDirectorAgentInitialization:
    ...
    def teardown_method(self):
        ...
        for temp_file in [self.temp_campaign_log, self.temp_world_state, self.temp_campaign_brief]:
            try:
...
characters:
  - name: "Hero"
    ...
    def test_initialization_event_bus_subscription(self):
        ...
        with patch.object(self.mock_event_bus, 'subscribe') as mock_subscribe:
            director = DirectorAgent(
                event_bus=self.mock_event_bus,
                campaign_log_path=self.temp_campaign_log
            )
...
class TestDirectorAgentTurnExecution:
    ...
    def setup_method(self):
        ...
        self.director = DirectorAgent(
            event_bus=self.mock_event_bus,
            campaign_log_path="test_turns.md"
        )
    ...
    def test_run_turn_with_registered_agents(self):
        ...
        if not hasattr(self.director, 'run_turn') or not hasattr(self.director, 'register_agent'):
            ...
            try:
                ...
                if os.path.exists(self.temp_log_path):
                    with open(self.temp_log_path, 'r') as f:
                        ...
                        assert any(word in content.lower() for word in ["turn", "action", "character"])
            except Exception:
...
def run_director_initialization_tests():
    ...
    result = subprocess.run([
        ...
        "-v", "-k", "TestDirectorAgentInitialization",
        "--tb=short",
        __file__
...
def run_director_registration_tests():
    ...
    result = subprocess.run([
        sys.executable, "-m", "pytest", 
        "-v", "-k", "TestDirectorAgentRegistration",
        "--tb=short",
        __file__
    ], capture_output=True, text=True)
