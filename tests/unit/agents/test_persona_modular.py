...
def create_test_character_data():
    ...
    return {
        ...
        "goals": [
            {"title": "Protect the innocent", "description": "Defend those who cannot defend themselves", "priority": "high"},
            {"title": "Seek knowledge", "description": "Learn about the world and grow wiser", "priority": "medium"},
            {"title": "Unite the clans", "description": "Bring peace to the northern lands", "priority": "high"}
        ],
        "decision_weights": {
            "safety": 0.8,
            "honor": 0.9,
...
async def test_decision_making():
    ...
    try:
        ...
        valid_actions = ["attack", "defend", "negotiate", "retreat", "observe", "wait", "communicate"]
        assert decision['action_type'] in valid_actions, f"Action type should be valid: {decision['action_type']}"
...
async def test_world_event_processing():
    ...
    try:
        ...
        assert 'processed_events' in results, "Results should include processed event count"
        assert 'processing_results' in results, "Results should include processing results"
        assert results['processed_events'] == len(events), "Should process all events"
        
        print(f"✅ World event processing: SUCCESS - {results['processed_events']} events")
        return True
...
async def test_data_access_methods():
    ...
    try:
        ...
        assert char_data['basic_info']['name'] == "TestCharacter", "Should have correct name"
...
async def test_integration_statistics():
    ...
    try:
        ...
        expected_components = [
            'character_data_manager', 'agent_state_manager', 'decision_processor',
            'threat_assessor', 'goal_manager', 'world_interpreter', 'memory_manager',
            'llm_client', 'response_processor', 'validator', 'response_generator'
        ]
        
        for component in expected_components:
            assert component in stats['component_status'], f"Missing component: {component}"
...
async def test_backward_compatibility():
    ...
    try:
        ...
        try:
            ...
            assert isinstance(char_data, dict), "Legacy character data access should work"
