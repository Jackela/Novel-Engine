...
class TestNarrativeGenerationFlow:
    ...
    def test_orchestration_lifecycle(
        ...
        story_request = data_factory.create_story_request(
            characters=["captain_nova", "engineer_vex"],
            title="Space Station Crisis"
        )

        response = client.post("/api/stories/generate", json=story_request)
        assert response.status_code in [200, 202], f"Failed to start orchestration: {response.text}"
        ...
        performance_tracker.record("orchestration_monitoring", monitoring_duration, {
            "checks": check_count,
    ...
    def test_sse_event_streaming(self, client, performance_tracker):
        ...
        performance_tracker.record("sse_streaming", sse_duration, {
    ...
    def test_narrative_progress_tracking(
        self,
        client,
        data_factory,
        api_helper
    ...
    def test_orchestration_error_handling(
        ...
        assert response.status_code in [400, 404, 422], "Should reject non-existent characters"
    ...
    def test_concurrent_orchestration_sessions(
        ...
        for i in range(2):
            char_data = data_factory.create_character_data(
                name=f"Concurrent Char {i}",
                agent_id=f"concurrent_char_{i}"
            )
            response = client.post("/api/characters", json=char_data)
            assert response.status_code in [200, 201]
        ...
        performance_tracker.record("concurrent_start", time.time() - start_time, {
        ...
        assert len(generation_ids) == len(set(generation_ids)), "Generation IDs should be unique"

    def test_narrative_quality_metrics(
        self,
        client,
        data_factory,
        api_helper
        ...
        # Create character
        char_data = data_factory.create_character_data(
            name="Quality Test Character",
            agent_id="quality_char"
        )
        response = client.post("/api/characters", json=char_data)
        assert response.status_code in [200, 201]
    ...
    def test_orchestration_resource_cleanup(
        self,
        client,
        data_factory,
        api_helper
