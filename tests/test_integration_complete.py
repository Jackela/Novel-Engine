...
try:
    from evaluate_baseline import BaselineEvaluator, SeedLoader, NovelEngineRunner
...
try:
    from src.caching import (
        StateHasher, HashingConfig, 
        SemanticCache, SemanticCacheConfig,
        TokenBudgetManager, TokenBudgetConfig, OperationType, BudgetPeriod
    )
...
class TestCompleteSystemIntegration:
    ...
    @pytest.mark.skipif(not CACHING_SYSTEM_AVAILABLE, reason="Caching system not available")
    ...
    def test_semantic_caching_integration(self, temp_directory):
        ...
        similar_value = semantic_cache.get("nonexistent_key", similar_query)
    ...
    @pytest.mark.skipif(not EVALUATION_SYSTEM_AVAILABLE, reason="Evaluation system not available")
    ...
    @pytest.mark.skipif(not (CORE_ENGINE_AVAILABLE and EVALUATION_SYSTEM_AVAILABLE and CACHING_SYSTEM_AVAILABLE), 
    ...
    def test_end_to_end_integration(self, temp_directory, mock_character_data):
        ...
        budget_manager = TokenBudgetManager(TokenBudgetConfig(
            enable_persistence=True,
        ...
        test_response = "The character carefully examines the object, looking for clues."
        ...
        integration_report = {
            ...
            "caching_system": {
                ...
                "semantic_matching": "available" if cache.config.enable_semantic_matching else "disabled"
            },
            "budget_system": {
    ...
    def test_performance_integration(self, temp_directory):
        ...
        hasher = StateHasher(HashingConfig(
        ...
        print(f"✅ State hashing: {len(test_characters)} characters in {hashing_time:.3f}s")
        ...
        print(f"✅ Caching operations: {len(test_characters)} ops in {caching_time:.3f}s")
        ...
        for i in range(5):
            world_state = {
                'characters': {char['character_id']: char for char in test_characters[:5]},
                'environment': {'turn': i, 'time': f'hour_{i}'},
                'locations': [{'id': f'loc_{j}', 'name': f'Location {j}'} for j in range(3)]
            }
        ...
        performance_report = {
            ...
            "throughput_metrics": {
                ...
                "cache_operations_per_second": len(test_characters) * 2 / caching_time,  # put + get
            ...
            "performance_acceptable": all([
                ...
                budget_time < 0.5,   # Should track 20 operations in under 0.5 seconds
                combined_time < 2.0  # Combined operations should take under 2 seconds
            ])
        }
        
        print(f"\n⚡ Performance Integration Test Results:")
        print(f"   Hashing: {performance_report['throughput_metrics']['characters_hashed_per_second']:.1f} chars/sec")
        print(f"   Caching: {performance_report['throughput_metrics']['cache_operations_per_second']:.1f} ops/sec")
        print(f"   Budget: {performance_report['throughput_metrics']['budget_operations_per_second']:.1f} ops/sec")
        print(f"   Combined: {performance_report['throughput_metrics']['world_states_per_second']:.1f} states/sec")
        print(f"   Overall: {'✅ ACCEPTABLE' if performance_report['performance_acceptable'] else '❌ NEEDS OPTIMIZATION'}")
