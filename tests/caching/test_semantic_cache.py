...
class TestSemanticCacheSimilarity:
    ...
    def test_get_by_similar_query_text(self, semantic_cache: SemanticCache) -> None:
        ...
        result = semantic_cache.get(
            "different_key", "the quick brown fox jumps over lazy dog"
        )

        # Should find similar entry due to term overlap
        assert result == "value1"

    def test_similarity_below_threshold_returns_none(
    ...
    def test_custom_similarity_threshold(self) -> None:
        ...
        cache.put(
            "key1", "value1", "machine learning algorithms and data science methods"
        )
...
class TestSemanticCachePersistence:
    ...
    def test_save_without_persistence_file_does_nothing(
        self, semantic_cache: SemanticCache
    ) -> None:
    ...
    def test_load_expired_entries_not_loaded(self, tmp_path: Path) -> None:
        ...
        config1 = SemanticCacheConfig(persistence_file=cache_file, ttl_seconds=0.1)
        cache1 = SemanticCache(config1)
        cache1.put("key1", "value1", "query text")
        cache1.save_cache()
        ...
        # Load cache
        config2 = SemanticCacheConfig(persistence_file=cache_file, ttl_seconds=0.1)
        cache2 = SemanticCache(config2)
...
class TestSemanticCacheConfig:
    ...
    def test_config_custom_values(self) -> None:
        ...
        config = SemanticCacheConfig(
            max_cache_size=100, similarity_threshold=0.75, ttl_seconds=600
        )
