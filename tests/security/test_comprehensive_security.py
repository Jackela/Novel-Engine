...
from src.security.input_validation import InputValidator, ValidationError, InputType
from src.security.rate_limiting import RateLimitMiddleware, InMemoryRateLimitBackend, RateLimitStrategy
from src.security.security_headers import SecurityHeaders, get_production_security_config
...
class SecurityTestSuite:
    ...
    async def _create_test_users(self):
        ...
        test_users = [
            ...
            ("creator_user", "creator@test.com", "SecurePass123!", UserRole.CONTENT_CREATOR),
...
class TestAuthorization:
    ...
    async def test_role_based_access_control(self, security_suite):
        ...
        assert not auth_manager.has_permission(reader_user["role"], Permission.SYSTEM_ADMIN)
        assert not auth_manager.has_permission(reader_user["role"], Permission.USER_DELETE)
    ...
    async def test_permission_escalation_prevention(self, security_suite):
        ...
        assert not auth_manager.has_permission(creator_user["role"], Permission.SYSTEM_ADMIN)
        assert not auth_manager.has_permission(creator_user["role"], Permission.USER_MANAGE_ROLES)
        
        # API user should not have content creation permissions
        api_user = security_suite.test_users["api_user"]
        assert not auth_manager.has_permission(api_user["role"], Permission.STORY_CREATE)
        assert not auth_manager.has_permission(api_user["role"], Permission.CHARACTER_CREATE)
...
class TestInputValidation:
    ...
    def test_sql_injection_detection(self, input_validator):
        ...
        for malicious_input in malicious_inputs:
            ...
            assert "sql" in exc_info.value.message.lower() or "injection" in exc_info.value.message.lower()
    ...
    async def test_ddos_detection(self, rate_limiter):
        ...
        if ddos_detector:
            ...
            for i in range(600):  # Exceed attack threshold
                is_allowed, reason = await ddos_detector.analyze_request("192.168.1.100")
                if not is_allowed:
...
class TestSecurityIntegration:
    ...
    async def test_end_to_end_security_flow(self, security_suite):
        ...
        response = await security_suite.client.get("/api/v1/characters", headers=headers)
