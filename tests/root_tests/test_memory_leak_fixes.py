    ...
    def simulate_decision_accumulation(self, decision_count: int):
        ...
        for i in range(decision_count):
            decision = {
                ...
                "action": f"action_{i}",
                "reasoning": f"Long reasoning text for decision {i} " * 20,  # Make it memory-heavy
...
class MemoryLeakFixTest:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        logger.info(f"=== Memory Leak Fix Tests Completed ===")
    ...
    async def _test_decision_history_memory_fix(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Before fix: {footprint_before['decision_history']} decisions, "
            ...
            decisions_reduced = footprint_before['decision_history'] - footprint_after['decision_history']
            
            logger.info(f"After fix: {footprint_after['decision_history']} decisions remaining, "
                       f"{memory_after:.1f}MB memory ({memory_improvement:.1f}MB saved)")
            
            # Verify fix effectiveness
            success = (
                footprint_after['decision_history'] <= 500 and  # Should be capped at 500
                'decision_history_sliding_window' in fix_result['fixes_applied'] and
                fix_result['items_archived']['decision_history'] > 0
            )
    ...
    async def _test_sliding_window_memory_manager(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Sliding window: {original_size} → {final_size} items "
                       f"({archived_count} archived)")
            
            # Verify effectiveness
            success = (
                final_size <= 100 and  # Should be limited to max_size
                archived_count > 0 and  # Should have archived items
                final_size == (original_size - archived_count)  # Math should work out
            )
    ...
    async def _test_memory_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success = (
                'agent_id' in report and
                'current_memory_mb' in report and
                'monitoring_active' in report and
                report['agent_id'] == 'monitored_agent'
            )
    ...
    async def _test_system_wide_memory_management(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success = (
                'system_memory' in comprehensive_report and
                'python_info' in comprehensive_report and
                'status' in comprehensive_report and
                setup_result is not None
            )
            
            logger.info(f"System memory status: {comprehensive_report['status']}")
            logger.info(f"Current memory: {comprehensive_report['system_memory']['current_memory_mb']:.1f}MB")
            
            return {
                'test_name': 'System-Wide Memory Management',
                'success': success,
                'comprehensive_report': comprehensive_report,
                'manager_active': comprehensive_report['system_memory']['monitoring_active']
            }
    ...
    async def _test_memory_leak_prevention_under_load(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_decisions_before = sum(len(agent.decision_history) for agent in agents)
            
            logger.info(f"Before load test fixes: {len(agents)} agents, "
            ...
            logger.info(f"After load test fixes: {total_decisions_after} decisions remaining, "
                       f"{memory_after:.1f}MB ({memory_improvement:.1f}MB saved)")
            
            # Verify load test effectiveness
            success = (
                fixes_applied == len(agents) and  # All agents should be fixed
                total_decisions_after < total_decisions_before and  # Should have fewer decisions
                memory_improvement >= 0  # Should save some memory
            )
...
async def main():
    ...
    try:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(report)
            f.write('\n"""\n\n')
            f.write(f"# Memory Leak Fix Test Results:\n")
