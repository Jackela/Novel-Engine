...
class MockDirectorAgent:
    ...
    def _generate_mock_world_state(self, agent_count: int) -> Dict[int, Dict[str, List[str]]]:
    ...
    def get_world_state_feedback_original(self, requesting_agent_id: str, current_turn: int):
        """
            ...
            if turn_num in self.world_state_tracker['agent_discoveries']:       
                ...
                for other_agent_id, discoveries in turn_discoveries.items():    # O(m)
                    if other_agent_id != requesting_agent_id:
                        for agent in self.registered_agents:                    # O(k) - NESTED AGENT LOOKUP
                            if agent.agent_id == other_agent_id:               # O(j) - STRING COMPARISON
                                # TOTAL COMPLEXITY: O(n*m*k*j) - EXPONENTIAL GROWTH
                                if other_agent_id not in feedback['other_agent_discoveries']:
                                    feedback['other_agent_discoveries'][other_agent_id] = []
                                feedback['other_agent_discoveries'][other_agent_id].extend(discoveries)
                                break
...
class DirectorAgentLoopOptimizationTest:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for config in test_configs:
            ...
            result1 = await self._test_agent_registry_optimization(config['agent_count'])
            ...
            result2 = await self._test_world_state_feedback_optimization(config['agent_count'])
        ...
        test_suite_results['passed_tests'] = sum(1 for r in self.test_results if r.success)
        test_suite_results['failed_tests'] = sum(1 for r in self.test_results if not r.success)
        test_suite_results['overall_success'] = test_suite_results['failed_tests'] == 0
        
        # Calculate overall performance improvements
        avg_improvement = sum(r.improvement_percentage for r in self.test_results if r.success) / max(1, test_suite_results['passed_tests'])
        test_suite_results['average_performance_improvement'] = avg_improvement
        
        logger.info(f"=== Test Suite Completed ===")
        logger.info(f"Passed: {test_suite_results['passed_tests']}/{test_suite_results['total_tests']}")
        logger.info(f"Average Performance Improvement: {avg_improvement:.1f}%")
        
        return test_suite_results
    
    async def _test_agent_registry_optimization(self, agent_count: int) -> PerformanceTestResult:
        ...
        try:
            ...
            for _ in range(1000):
                target_id = f"agent_{random.randint(0, agent_count-1)}"
                found = optimized_registry.find_agent(target_id)  # O(1) hash lookup
            ...
            logger.info(f"Agent Registry: {original_time:.4f}s → {optimized_time:.4f}s ({improvement:.1f}% improvement)")
    ...
    async def _test_world_state_feedback_optimization(self, agent_count: int) -> PerformanceTestResult:
        """
        ...
        """
        logger.info(f"Testing World State Feedback Optimization with {agent_count} agents")
        ...
        try:
            ...
            for _ in range(100):  # 100 world state feedback calls
                ...
                feedback = mock_director.get_world_state_feedback_original(requesting_agent, current_turn)
            original_time = time.time() - start_time
            ...
            for turn, turn_data in mock_director.world_state_tracker['agent_discoveries'].items():
            ...
            for _ in range(100):  # Same 100 calls with optimized version
                ...
                feedback = optimized_tracker.get_world_state_feedback(requesting_agent, current_turn)
            optimized_time = time.time() - start_time
            ...
            logger.info(f"World State Feedback: {original_time:.4f}s → {optimized_time:.4f}s ({improvement:.1f}% improvement)")
    ...
    async def _test_full_director_optimization(self, agent_count: int) -> PerformanceTestResult:
        ...
        try:
            ...
            for _ in range(50):
                ...
                found_agent = mock_director.find_agent_original(agent_id)
                feedback = mock_director.get_world_state_feedback_original(agent_id, 8)
                mock_director.log_event(f"Test operation for {agent_id}")
            original_time = time.time() - start_time
            
            # Apply full optimization
            optimization_result = DirectorAgentPerformanceOptimizer.optimize_director_agent(mock_director)
            ...
            for _ in range(50):
                ...
                agent_id = f"agent_{random.randint(0, agent_count-1)}"
                found_agent = mock_director.find_agent(agent_id)  # Now uses optimized registry
                feedback = mock_director.get_world_state_feedback(agent_id, 8)  # Now uses optimized tracker
                mock_director.log_event(f"Test operation for {agent_id}")  # Now uses async logger
            optimized_time = time.time() - start_time
            ...
            logger.info(f"Full Director Optimization: {original_time:.4f}s → {optimized_time:.4f}s ({improvement:.1f}% improvement)")
    ...
    async def _test_async_campaign_logger(self) -> PerformanceTestResult:
        ...
        try:
            from src.performance_optimizations.director_agent_loop_optimizer import AsyncCampaignLogger
            
            # Test synchronous logging (original)
            log_entries = [f"Test event {i} with some descriptive content" for i in range(1000)]
            ...
            logger.info(f"Async Logger: {original_time:.4f}s → {optimized_time:.4f}s ({improvement:.1f}% improvement)")
        ...
        try:
            ...
            for turn in range(1, 100):  # 100 turns
                for agent_id in [f"agent_{i}" for i in range(50)]:  # 50 agents per turn
                    for discovery_idx in range(random.randint(1, 5)):  # 1-5 discoveries per agent
            ...
            logger.info(f"Memory Management Test:")
            ...
            success = perf_stats['archived_turns'] > 0 and memory_delta < 100  # Less than 100MB growth
...
"""
        ...
        if successful_tests:
            avg_improvement = sum(r.improvement_percentage for r in successful_tests) / len(successful_tests)
...
"""
            ...
            for result in successful_tests:
                improvement_emoji = "🔥" if result.improvement_percentage > 80 else "⚡" if result.improvement_percentage > 50 else "✅"
