...
async def test_async_llm_client_performance():
    ...
    try:
        ...
        async with AsyncLLMClient(
            ...
            cached_response = await client.call_llm_async(
                "test_agent_001", 
                "SITUATION: Enemy forces detected nearby. THREAT_LEVEL: HIGH. What action do you take?",
                test_context
            )
            ...
            print(f"   • Cache hit time: {cache_request_time:.6f}s ({cache_request_time/single_request_time:.1%} of original)")
            print(f"   • Concurrent 5 requests: {concurrent_total_time:.3f}s")
            print(f"   • Average per concurrent request: {concurrent_total_time/5:.3f}s")
            ...
            return {
                ...
                'cache_speedup': single_request_time / max(cache_request_time, 0.000001),
                'stats': stats,
                'success': True
            }
...
def test_persona_agent_patch():
    ...
    try:
        ...
        original_response = test_agent._call_llm("Test prompt for performance measurement")
        original_time = time.perf_counter() - original_start
        
        # Apply async patch
        from src.performance_optimizations.persona_agent_async_patch import PersonaAgentAsyncPatch
        ...
        patched_response = test_agent._call_llm("Test prompt for performance measurement")
...
async def test_concurrent_agent_performance():
    ...
    try:
        from src.performance_optimizations.persona_agent_async_patch import apply_async_optimization_to_agent_collection
        ...
        agents = {
            f"concurrent_agent_{i:03d}": MockAgentForConcurrency(f"concurrent_agent_{i:03d}")
            for i in range(10)
        }
        ...
        concurrent_improvement_pct = ((sequential_time - optimized_time) / sequential_time) * 100
        ...
        print(f"   • Successfully optimized agents: {optimization_result['successfully_patched']}/10")
        print(f"   • Estimated API cost reduction: {optimization_result['estimated_cost_reduction']}")
...
async def run_comprehensive_performance_tests():
    ...
    if test_results['async_llm_client']['success']:
        ...
        print(f"🚀 Single Request: {single_time:.3f}s (vs ~30s original = {30/single_time:.0f}x faster)")
    ...
    if test_results['concurrent_agents']['success']:
        ...
        print(f"🏭 Concurrent Processing: {concurrent_improvement:.1f}x faster for multiple agents")
