...
class PerformanceTracker:
    ...
    def detect_regression(self, results: PipelineResults) -> Tuple[bool, List[str]]:
        ...
        if violation_rate_regression > 0.05:  # 5% increase in violations
            regressions.append(f"Violation rate regression: {violation_rate_regression*100:.1f}% higher")
...
class ReportGenerator:
    ...
    def generate_html_report(self, results: PipelineResults, 
                           individual_results: List[Dict[str, Any]],
                           trend_data: Optional[List[Dict[str, Any]]] = None) -> Path:
        """
    ...
    def _build_html_report(self, results: PipelineResults,
                          individual_results: List[Dict[str, Any]], 
                          trend_data: Optional[List[Dict[str, Any]]]) -> str:
...
<body>
    ...
    def generate_json_summary(self, results: PipelineResults, 
                            individual_results: List[Dict[str, Any]]) -> Path:
...
class AlertSystem:
    ...
    def send_alerts(self, results: PipelineResults, regressions: List[str]):
        ...
        alert_level = "CRITICAL" if not results.pipeline_success else "WARNING" if regressions else "INFO"
    ...
    def _generate_alert_body(self, results: PipelineResults, 
                            regressions: List[str], alert_level: str) -> str:
...
class EvaluationPipeline:
    ...
    def run_pipeline(self) -> PipelineResults:
        ...
        try:
            ...
            if has_regression:
                results.performance_regression = max(0.1, results.performance_regression)  # Set minimum regression indicator
            ...
            logger.info(f"✅ Pipeline completed: {results.execution_time:.1f}s - "
    ...
    def _run_baseline_evaluation(self) -> List[Dict[str, Any]]:
        ...
        cmd = [
            ...
            "--suite", str(seeds_dir),
            "--output", str(self.config.results_directory)
        ]
            ...
            try:
                ...
                if "evaluation_summary" in data:
                    ...
                    results.append({
                        ...
                        "violations": data.get("iron_laws_compliance", {}).get("total_violations", 0),
                        "execution_time": data.get("execution_metrics", {}).get("execution_time_seconds", 0)
                    })
    ...
    def _calculate_pipeline_results(self, timestamp: datetime, execution_time: float,
                                  individual_results: List[Dict[str, Any]]) -> PipelineResults:
    ...
    def _check_quality_gates(self, results: PipelineResults) -> Tuple[bool, List[str]]:
        ...
        if results.pass_rate < self.config.minimum_pass_rate:
            failures.append(f"Pass rate {results.pass_rate:.1%} below minimum {self.config.minimum_pass_rate:.1%}")
        
        # Check violation rate
        if results.violation_rate > self.config.maximum_violation_rate:
            failures.append(f"Violation rate {results.violation_rate:.1%} exceeds maximum {self.config.maximum_violation_rate:.1%}")
        
        # Check critical violations
        if results.total_violations > self.config.critical_violation_threshold:
            failures.append(f"Total violations {results.total_violations} exceeds threshold {self.config.critical_violation_threshold}")
...
def load_pipeline_config(config_path: Path) -> PipelineConfig:
    ...
    if config_path.exists():
        ...
        # Convert paths
        for field in ['seeds_directory', 'results_directory', 'archive_directory', 'performance_baseline_file']:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Novel Engine Automated Evaluation Pipeline"
    )
    
    parser.add_argument("--config", type=Path, default=Path("evaluation/pipeline_config.yaml"),
                       help="Pipeline configuration file")
    parser.add_argument("--mode", choices=["manual", "daily", "weekly", "ci"],
                       default="manual", help="Pipeline execution mode")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument("--dry-run", action="store_true", help="Dry run mode - no actual evaluation")
