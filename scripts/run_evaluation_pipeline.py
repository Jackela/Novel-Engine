...
class PerformanceTracker:
    ...
    def detect_regression(
        ...
        if time_regression > 0.2:  # 20% slower
            regressions.append(
                f"Execution time regression: {time_regression*100:.1f}% slower"
            )
        ...
        if pass_rate_regression > 0.1:  # 10% drop in pass rate
            regressions.append(
                f"Pass rate regression: {pass_rate_regression*100:.1f}% lower"
            )
...
class ReportGenerator:
    ...
    def generate_html_report(
        ...
        report_path = (
            self.config.results_directory / f"pipeline_report_{timestamp}.html"
        )
