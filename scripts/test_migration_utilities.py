...
class TestMigrationUtilities:
    ...
    def __init__(self):
        ...
        self.migration_map = {
            ...
            "core": {
                ...
                "patterns": ["config", "integration", "foundation", "event", "logging", "data_models"],
                "files": [
    ...
    def analyze_file_content(self, file_path: Path) -> Dict[str, any]:
        ...
        try:
            ...
            keyword_patterns = [
                "api", "server", "endpoint", "fastapi", "uvicorn",
                "agent", "persona", "director", "character",
                "integration", "config", "event", "logging",
                "bridge", "llm", "multi_agent", 
                "interaction", "equipment", "dynamic",
                "performance", "optimization", "async", "cache",
                "frontend", "ui", "browser", "playwright",
                "quality", "security", "narrative", "story"
            ]
    ...
    def get_migration_plan(self) -> Dict[str, List[Dict]]:
        ...
        for test_file in all_test_files:
            ...
            if any(part in str(test_file) for part in [
                "integration/api", "integration/core", "integration/bridges",
                "integration/frontend", "integration/interactions", 
                "integration/agents", "integration/components",
                "unit/agents", "unit/interactions", "unit/quality"
            ]):
                continue
        ...
        plan["statistics"] = {
            ...
            "high_confidence": len([f for f in plan["files_to_migrate"] if f["confidence"] > 0.7]),
            "medium_confidence": len([f for f in plan["files_to_migrate"] if 0.3 <= f["confidence"] <= 0.7]),
            "low_confidence": len([f for f in plan["files_to_migrate"] if f["confidence"] < 0.3]),
            "categories": {}
        }
        ...
        try:
            ...
            for dir_path in plan["directories_to_create"]:
                ...
                if not dry_run:
                    ...
                    results["actions_taken"].append(f"[DRY RUN] Would create directory: {dir_path}")
                ...
                if migration["target"] and migration["confidence"] > 0.3:
                    ...
                    if source_path.exists():
                        if not dry_run:
                            ...
                            results["actions_taken"].append(f"Migrated: {migration['source']} -> {migration['target']}")
                        else:
                            results["actions_taken"].append(f"[DRY RUN] Would migrate: {migration['source']} -> {migration['target']}")
