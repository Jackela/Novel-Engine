...
def fix_missing_return_types(file_path: Path) -> int:
    ...
    # 首先找到所有函数定义
    func_pattern = r'^(\s*)def\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)(?!\s*->)(\s*:)'  
            ...
            if close_match and '->' not in params:
                ...
                if last_paren_idx != -1:
                    ...
                    if '\n' in params:
                        ...
                        # 保持原始缩进
                        new_lines.append(indent + 'def ' + func_name + '(' + param_lines[0])
...
def fix_lowercase_any(file_path: Path) -> int:
    ...
    if fixes > 0 and 'from typing import' in content:
        # 检查是否已有 Any
        if 'Any' not in content or not re.search(r'from typing import.*\bAny\b', content):
            # 添加到现有的 typing 导入
            content = re.sub(
                r'(from typing import [^\n]+)',
                lambda m: m.group(1) + ', Any' if 'Any' not in m.group(1) else m.group(1),
                content
            )
    elif fixes > 0:
        # 添加新的 typing 导入
