...
class TermGuard:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        logger.info(f"🛡️ Term Guard initialized with {len(self.filter_rules)} filter rules")
    ...
    def _load_configuration(self):
        ...
        try:
            if self.config_path.exists():
                ...
                logger.warning(f"⚠️ Configuration file {self.config_path} not found, using defaults")
    ...
    def analyze_content(self, content: str) -> List[IPViolation]:
        ...
        for rule in self.filter_rules:
            ...
            if any(whitelist_term.lower() in content.lower() 
            ...
            try:
                ...
                for match in matches:
                    ...
                    if matched_term.lower() in [term.lower() for term in self.whitelist_terms]:
                        continue
    ...
    def clean_content(self, content: str, apply_fixes: bool = True) -> Tuple[str, CleaningReport]:
        """
        ...
        if apply_fixes:
            ...
            for violation in violations:
                ...
                if matching_rule.action == FilterAction.REMOVE:
                ...
                elif matching_rule.action == FilterAction.REPLACE and matching_rule.replacement:
                    content = content[:start] + matching_rule.replacement + content[end:]
                    actions_taken.append(f"Replaced '{violation.term}' with '{matching_rule.replacement}'")
                    ...
                    actions_taken.append(f"Genericized '{violation.term}' to '{generic_replacement}'")
                    ...
                    actions_taken.append(f"Warning: '{violation.term}' flagged for review")
        ...
        logger.info(f"🧹 Cleaned content: {len(violations)} violations processed, "
    ...
    def generate_compliance_report(self, reports: List[CleaningReport]) -> Dict[str, Any]:
        """
        ...
        safe_percentage = (sum(1 for r in reports if r.safe_for_use) / len(reports)) * 100
    ...
    def _generate_compliance_recommendations(self, reports: List[CleaningReport]) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("Content analysis shows good compliance - continue current practices")
...
def main():
    ...
    parser.add_argument("--output", "-o", type=Path, help="Output file for cleaned content")
