...
class EnterpriseHealthChecker:
    ...
    async def check_system_resources(self) -> HealthStatus:
        ...
        try:
            ...
            if (memory.percent > self.config.performance_thresholds["memory_usage_percent"] or
                cpu_percent > self.config.performance_thresholds["cpu_usage_percent"]):
                status = "degraded"
            else:
                status = "healthy"
    ...
    async def run_comprehensive_health_check(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Health check completed: {overall_status} ({len(health_checks)} checks)")
        return result
