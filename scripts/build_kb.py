...
class StartupGuard:
    ...
    def _validate_configuration(self) -> bool:
        ...
        try:
            ...
            required_sections = [
                'system', 'legal', 'api', 'storage', 'ai', 
                'simulation', 'performance', 'security'
            ]
            ...
            if not legal_config.get('enable_safeguards', False):
                self.warnings.append("Legal safeguards are disabled - this may cause compliance issues")
            
            # Validate simulation configuration
            simulation_config = self.config.get('simulation', {})
            if not simulation_config.get('iron_laws', {}).get('enabled', False):
                self.errors.append("Iron Laws validation must be enabled for safe operation")
                return False
    ...
    def _validate_legal_compliance(self) -> bool:
        ...
        try:
            ...
            if not legal_path.exists():
                self.errors.append("LEGAL.md file not found - legal compliance documentation required")
                return False
            
            # Check NOTICE file exists
            notice_path = Path("NOTICE")
            if not notice_path.exists():
                self.errors.append("NOTICE file not found - third-party attribution required")
                return False
            
            # Validate fan mode compliance if enabled
            if self.config and self.config.get('legal', {}).get('compliance_mode') == 'fan':
                registry_file = self.config.get('legal', {}).get('registry_file', 'private/registry.yaml')
                registry_path = Path(registry_file)
                
                if not registry_path.exists():
                    self.warnings.append(f"Fan mode enabled but registry file {registry_file} not found")
                    ...
                    self.logger.info(f"📝 Created template registry file: {registry_file}")
    ...
    def _validate_file_structure(self) -> bool:
        ...
        try:
            ...
            required_dirs = [
                'src', 'tests', 'docs', 'docs/ADRs', 'scripts',
                'private', 'logs', 'evaluation'
            ]
            ...
            # Required files  
            required_files = [
                'README.md', 'LEGAL.md', 'NOTICE', 'settings.yaml'
            ]
    ...
    def _validate_external_dependencies(self) -> bool:
        ...
        try:
            ...
            required_packages = [
                'fastapi', 'uvicorn', 'pydantic', 'yaml', 'requests'
            ]
            ...
            if not available_keys:
                self.warnings.append("No AI API keys found - system will run in offline mode")
            else:
    ...
    def _validate_knowledge_base(self) -> bool:
        ...
        try:
            if not self.config:
                self.errors.append("Configuration not loaded - cannot validate knowledge base")
                return False
            ...
            for subdir in kb_subdirs:
                ...
                if not readme_path.exists():
                    with open(readme_path, 'w', encoding='utf-8') as f:
                        ...
                        f.write(f"This directory contains {subdir} data for the Novel Engine.\n")
