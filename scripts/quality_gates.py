...
class LintingGate(QualityGate):
    ...
    def run(self) -> bool:
        ...
        self.output = "\n".join(f"{name}: {'✓' if passed else '✗'}" for name, passed, _, _ in checks)
        self.error = "\n".join(f"{name}: {stderr}" for name, passed, _, stderr in checks if not passed and stderr)
...
class SecurityGate(QualityGate):
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if result.stdout:
                try:
                    ...
                    medium_severity = sum(1 for issue in bandit_data.get("results", []) 
                                        if issue.get("issue_severity") == "MEDIUM")
                    
                    bandit_passed = high_severity == 0  # No high severity issues
                    checks.append(("Bandit", bandit_passed, 
        ...
        # Compile results
        self.passed = all(passed for _, passed, _, _ in checks)
        self.output = "\n".join(f"{name}: {'✓' if passed else '✗'} - {output}" 
                               for name, passed, output, _ in checks)
        self.error = "\n".join(f"{name}: {error}" for name, passed, _, error in checks if not passed and error)
...
def main():
    ...
    parser.add_argument("--report", default="quality_report.json",
