...
class CoverageGate(QualityGate):
    ...
    def __init__(self, min_coverage: float = 90.0):
        super().__init__(
            "Test Coverage", f"Ensure test coverage is at least {min_coverage}%", required=True
        )
...
class LintingGate(QualityGate):
    ...
    def __init__(self):
        super().__init__("Code Linting", "Ensure code follows style guidelines", required=True)
    ...
    def run(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "flake8", "src", "tests"], capture_output=True, text=True
            )
        ...
        self.error = "\n".join(
            f"{name}: {stderr}" for name, passed, _, stderr in checks if not passed and stderr
        )
...
class SecurityGate(QualityGate):
    ...
    def __init__(self):
        super().__init__(
            "Security Scan", "Ensure code passes security vulnerability scanning", required=True
        )
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if result.stdout:
                try:
                    ...
                    checks.append(("Bandit", False, "", "Failed to parse Bandit output"))
            else:
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "safety", "check", "--json"], capture_output=True, text=True
            )
        ...
        self.passed = all(passed for _, passed, _, _ in checks)
        self.output = "\n".join(
            f"{name}: {'✓' if passed else '✗'} - {output}" for name, passed, output, _ in checks
        )
        self.error = "\n".join(
            f"{name}: {error}" for name, passed, _, error in checks if not passed and error
        )
...
class TestGate(QualityGate):
    ...
    def __init__(self):
        super().__init__("Test Execution", "Ensure all tests pass successfully", required=True)
...
class QualityGateRunner:
    ...
    def run_all(self, fail_fast: bool = False) -> Tuple[bool, Dict]:
        ...
        results = {
            "overall_passed": True,
            "gates": {},
            "summary": {"total": len(self.gates), "passed": 0, "failed": 0, "required_failed": 0},
        }
...
def main():
    ...
    parser.add_argument(
        "--report", default="quality_report.json", help="Output file for detailed report"
    )
