...
def demo_pipeline_execution():
    ...
    try:
        ...
        violation_rate = total_violations / sum(
            r.get("total_actions", 100) for r in mock_results
        )

        if pass_rate >= config.minimum_pass_rate:
            print(
                f"  ✅ Pass Rate Gate: {pass_rate:.1%} >= {config.minimum_pass_rate:.1%}"
            )
        else:
            print(
                f"  ❌ Pass Rate Gate: {pass_rate:.1%} < {config.minimum_pass_rate:.1%}"
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print(
                "   Use 'python scripts/run_evaluation_pipeline.py --help' for full options."
            )
            return 0
        else:
