...
class TestMigrationValidator:
    ...
    def validate_pytest_discovery(self) -> Dict[str, any]:
        ...
        test_commands = [
            (
                "integration",
                ["python", "-m", "pytest", "--collect-only", "tests/integration/", "--quiet"],
            ),
            ("unit", ["python", "-m", "pytest", "--collect-only", "tests/unit/", "--quiet"]),
            (
                "performance",
                ["python", "-m", "pytest", "--collect-only", "tests/performance/", "--quiet"],
            ),
    ...
    def count_test_files(self) -> Dict[str, int]:
        ...
        root_test_files = [
            f
            for f in self.tests_dir.iterdir()
    ...
    def determine_migration_status(self):
        ...
        dir_issues = sum(
            1 for v in self.results["validation_results"]["directory_structure"].values() if not v
        )
        pytest_issues = sum(
            1
