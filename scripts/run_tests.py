...
class TestRunner:
        ...
        if os.path.exists("coverage.json"):
            try:
                with open("coverage.json", "r") as f:
                    ...
                    self.log(f"Coverage: {self.results['coverage'].get('percent_covered', 0):.2f}%")
            except Exception as e:
    ...
    def run_integration_tests(self) -> bool:
        ...
        success, stdout, stderr = self.run_command(command, "Integration Tests", timeout=300)
    ...
    def run_performance_tests(self) -> bool:
        ...
        success, stdout, stderr = self.run_command(command, "Performance Tests", timeout=300)
    ...
    def run_security_tests(self) -> bool:
        ...
        command = [sys.executable, "-m", "pytest", "tests/", "-m", "security", "-v", "--tb=short"]

        success, stdout, stderr = self.run_command(command, "Security Tests", timeout=180)
    ...
    def generate_report(self, output_file: str = "test_report.json"):
        ...
        passed_suites = sum(
            1 for result in self.results["test_results"].values() if result.get("success", False)
        )
    ...
    def print_summary(self):
        ...
        status = "✅ ALL TESTS PASSED" if summary["overall_success"] else "❌ SOME TESTS FAILED"
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["all", "unit", "integration", "api", "performance", "security", "smoke"],
        default="all",
        help="Test execution mode",
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument("--report", default="test_report.json", help="Output file for test report")
    parser.add_argument(
