...
def validate_pipeline_components():
    ...
    for file_path in required_files:
        ...
        if full_path.exists():
            print(f"  ✅ {file_path}")
            validation_results[f"file_{file_path.replace('/', '_').replace('.', '_')}"] = True
        else:
            print(f"  ❌ {file_path} - MISSING")
            validation_results[f"file_{file_path.replace('/', '_').replace('.', '_')}"] = False
    ...
    try:
        ...
        if config_path.exists():
            ...
            print(f"    📋 Run mode: {config.get('execution', {}).get('run_mode', 'unknown')}")
            print(f"    🎯 Quality gate pass rate: {config.get('quality_gates', {}).get('minimum_pass_rate', 'unknown')}")
