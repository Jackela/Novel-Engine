...
async def test_performance_engine():
    ...
    try:
        ...
        results = await performance_engine.optimize_batch_operations(operations, 'io_bound')
        ...
        for method, endpoint, description in endpoints:
            try:
                ...
                async with session.request(method, f"{base_url}{endpoint}") as response:
                    ...
                    status = "PASS" if result["success"] and result["meets_target"] else "FAIL"
                    logger.info(f"{status} {description}: {result['response_time_ms']:.1f}ms (HTTP {result['status_code']})")
...
async def run_quick_load_test(base_url: str = "http://localhost:8000", concurrent_users: int = 50, requests_per_user: int = 5):
    """Run a quick load test."""
    logger.info(f"Running quick load test: {concurrent_users} users, {requests_per_user} requests each...")
    ...
    load_test_results = {
        ...
        "meets_targets": {
            ...
            "error_rate": (total_errors / total_requests * 100) < 1 if total_requests > 0 else True  # <1% error rate
        }
    }
    ...
    logger.info(f"{status} Load test: {load_test_results['avg_response_time_ms']:.1f}ms avg, "
...
async def validate_performance():
    ...
    try:
        ...
        validation_results["api_endpoint_tests"] = {
            ...
            "targets_met": all(r.get("meets_target", False) for r in api_results if r.get("success", False))
        }
    except Exception as e:
    ...
    load_ok = validation_results["load_test_results"].get("meets_targets", {}).get("response_time", True) if isinstance(validation_results["load_test_results"], dict) else True
        ...
        if "results" in api_tests:
            for result in api_tests["results"]:
                if result.get("success", False):
                    target_status = "PASS" if result.get("meets_target", False) else "FAIL"
                    print(f"  {target_status} {result['description']}: {result.get('response_time_ms', 0):.1f}ms")
                else:
