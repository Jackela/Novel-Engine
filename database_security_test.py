    ...
    def test_database_content(self, db_path: Path) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                sensitive_tables = ["users", "passwords", "auth", "credentials", "secrets", "tokens"]
                for table_name in result["tables"]:
                    if any(sensitive in table_name.lower() for sensitive in sensitive_tables):
                        result["sensitive_data"].append(f"Sensitive table name: {table_name}")
                ...
                for table_name in result["tables"]:
                    try:
                        ...
                        sensitive_columns = ["password", "secret", "token", "key", "hash", "salt"]
                        for column in columns:
                            column_name = column[1].lower()
                            if any(sensitive in column_name for sensitive in sensitive_columns):
                                result["sensitive_data"].append(f"Sensitive column: {table_name}.{column[1]}")
                        ...
                        for row in sample_data:
                            for value in row:
                                if isinstance(value, str):
                                    if len(value) > 8 and value.isalnum() and not value.isdigit():
                                        # Could be a hash or sensitive string
                                        if len(value) in [32, 40, 64, 128]:  # Common hash lengths
                                            continue  # Likely hashed, which is good
                                        elif "password" in str(value).lower():
                                            result["vulnerabilities"].append(f"Potential cleartext password in {table_name}")
...
def main():
    ...
    print(f"Vulnerable databases: {results['security_summary']['vulnerable_databases']}")
