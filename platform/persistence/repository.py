...
class BaseRepository(Generic[T], ABC):
    ...
    def create_many(self, entities: List[T]) -> List[T]:
        ...
        try:
            ...
            for entity in entities:
                validation_errors = entity.validate()
                if validation_errors:
                    raise RepositoryOperationException(f"Validation failed for entity {entity}: {validation_errors}")
    ...
    def get_all(self, offset: int = 0, limit: int = 100, order_by: str = "created_at", 
                order_direction: str = "desc") -> List[T]:
    ...
    def exists(self, **filters) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to check existence of {self.model.__name__} with filters {filters}: {e}")
    ...
    def delete_many(self, **filters) -> int:
        ...
        try:
            ...
            logger.error(f"Failed to delete {self.model.__name__} entities with filters {filters}: {e}")
...
class AsyncBaseRepository(Generic[T], ABC):
    ...
    async def get_all(self, offset: int = 0, limit: int = 100, order_by: str = "created_at", 
                     order_direction: str = "desc") -> List[T]:
