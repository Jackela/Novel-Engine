...
from sqlalchemy.exc import IntegrityError, OptimisticLockError
...
class SQLAlchemyNegotiationSessionRepository(NegotiationSessionRepository):
    ...
    async def save(self, session: NegotiationSession) -> None:
        ...
        try:
            ...
            existing = await self.session.get(NegotiationSessionModel, session.session_id.value)
            ...
            logger.error(f"Integrity error saving negotiation session {session.session_id}: {e}")
    ...
    async def find_by_filters(self,
                             filters: Dict[str, Any],
                             limit: int = 50,
                             offset: int = 0,
                             order_by: str = "created_at",
                             order_direction: str = "desc") -> List[NegotiationSession]:
        """
        ...
        try:
            ...
            if 'created_by' in filters:
                conditions.append(NegotiationSessionModel.created_by == filters['created_by'])
            
            if 'session_type' in filters:
                conditions.append(NegotiationSessionModel.session_type == filters['session_type'])
            
            if 'negotiation_domain' in filters:
                conditions.append(NegotiationSessionModel.negotiation_domain == filters['negotiation_domain'])
            
            if 'priority_level' in filters:
                conditions.append(NegotiationSessionModel.priority_level == filters['priority_level'])
            
            if 'confidentiality_level' in filters:
                conditions.append(NegotiationSessionModel.confidentiality_level == filters['confidentiality_level'])
            
            # Date range filters
            if 'created_after' in filters:
                conditions.append(NegotiationSessionModel.created_at >= filters['created_after'])
            
            if 'created_before' in filters:
                conditions.append(NegotiationSessionModel.created_at <= filters['created_before'])
            
            if 'updated_after' in filters:
                conditions.append(NegotiationSessionModel.updated_at >= filters['updated_after'])
            
            if 'updated_before' in filters:
                conditions.append(NegotiationSessionModel.updated_at <= filters['updated_before'])
            
            # Numeric range filters
            if 'min_parties' in filters:
                conditions.append(NegotiationSessionModel.total_parties >= filters['min_parties'])
            
            if 'max_parties' in filters:
                conditions.append(NegotiationSessionModel.total_parties <= filters['max_parties'])
            
            if 'min_proposals' in filters:
                conditions.append(NegotiationSessionModel.total_proposals >= filters['min_proposals'])
            ...
            if 'outcome' in filters:
                conditions.append(
                    text("status->>'outcome' = :outcome").bindparam(outcome=filters['outcome'])
                )
            ...
            # Apply ordering
            order_column = getattr(NegotiationSessionModel, order_by, NegotiationSessionModel.created_at)
            ...
            logger.error(f"Error finding negotiation sessions with filters {filters}: {e}")
            raise
    ...
    async def find_by_participant(self, participant_id: UUID) -> List[NegotiationSession]:
        ...
        try:
            ...
            query = select(NegotiationSessionModel).where(
            ...
            logger.error(f"Error finding sessions for participant {participant_id}: {e}")
            raise
    ...
    async def find_sessions_by_status(self,
                                    phase: Optional[str] = None,
                                    outcome: Optional[str] = None) -> List[NegotiationSession]:
        """
        ...
        try:
            ...
            logger.error(f"Error finding sessions by status (phase={phase}, outcome={outcome}): {e}")
            raise
    ...
    async def find_sessions_requiring_attention(self,
                                              hours_until_timeout: int = 24) -> List[NegotiationSession]:
        """
        ...
        """
        try:
            cutoff_time = datetime.now(timezone.utc) + timedelta(hours=hours_until_timeout)
            stagnant_time = datetime.now(timezone.utc) - timedelta(hours=48)  # No activity for 2 days
                ...
                and_(
                    ...
                    or_(
                        ...
                        func.date_part('epoch', 
                                       ...
                                       func.make_interval(hours=NegotiationSessionModel.session_timeout_hours)
                        ...
                        and_(
                            ...
                            NegotiationSessionModel.total_responses < NegotiationSessionModel.total_parties
                        )
                    )
                )
    ...
    async def exists(self, session_id: InteractionId) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking existence of negotiation session {session_id}: {e}")
            raise
    ...
    async def get_session_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_query = select(
                ...
                func.avg(NegotiationSessionModel.total_proposals).label('avg_proposals'),
                func.avg(NegotiationSessionModel.total_responses).label('avg_responses')
            )
    ...
    async def find_sessions_by_date_range(self,
                                        start_date,
                                        end_date,
                                        date_field: str = "created_at") -> List[NegotiationSession]:
        """
        ...
        return await self.find_by_filters(filters, limit=1000)  # Large limit for date ranges
    
    async def batch_update_sessions(self,
                                  session_updates: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        try:
            for update_data in session_updates:
                ...
                stmt = update(NegotiationSessionModel).where(
