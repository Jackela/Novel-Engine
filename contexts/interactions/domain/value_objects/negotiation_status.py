...
class NegotiationStatus:
    ...
    def __post_init__(self):
        ...
        for field_name in ['started_at', 'last_activity_at', 'expected_completion_at', 'actual_completion_at']:
        ...
        if self.outcome != NegotiationOutcome.PENDING and self.phase != NegotiationPhase.TERMINATED:
            if self.outcome in [NegotiationOutcome.AGREEMENT_REACHED, NegotiationOutcome.PARTIAL_AGREEMENT]:
                if self.phase not in [NegotiationPhase.CLOSING, NegotiationPhase.IMPLEMENTATION]:
                    raise ValueError(f"Outcome {self.outcome.value} inconsistent with phase {self.phase.value}")
    
    @classmethod
    def create_initial(cls, started_at: Optional[datetime] = None, 
                      expected_completion_at: Optional[datetime] = None) -> 'NegotiationStatus':
    ...
    def advance_to_phase(self, new_phase: NegotiationPhase, 
                        activity_time: Optional[datetime] = None) -> 'NegotiationStatus':
        """Advance negotiation to a new phase."""
        if not self._is_valid_phase_transition(self.phase, new_phase):
            raise ValueError(f"Invalid phase transition from {self.phase.value} to {new_phase.value}")
    ...
    def complete_with_outcome(self, outcome: NegotiationOutcome,
                             completion_time: Optional[datetime] = None,
                             termination_reason: Optional[TerminationReason] = None) -> 'NegotiationStatus':
    ...
    def _is_valid_phase_transition(self, from_phase: NegotiationPhase, 
                                  to_phase: NegotiationPhase) -> bool:
        """Check if phase transition is valid."""
        valid_transitions = {
            NegotiationPhase.INITIATION: [NegotiationPhase.PREPARATION, NegotiationPhase.TERMINATED],
            NegotiationPhase.PREPARATION: [NegotiationPhase.OPENING, NegotiationPhase.TERMINATED],
            NegotiationPhase.OPENING: [NegotiationPhase.BARGAINING, NegotiationPhase.TERMINATED],
            NegotiationPhase.BARGAINING: [NegotiationPhase.CLOSING, NegotiationPhase.TERMINATED],
            NegotiationPhase.CLOSING: [NegotiationPhase.IMPLEMENTATION, NegotiationPhase.TERMINATED],
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (
            ...
            self.last_activity_at == other.last_activity_at and
            self.expected_completion_at == other.expected_completion_at and
            self.actual_completion_at == other.actual_completion_at and
