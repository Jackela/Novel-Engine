...
class TermResponse:
    ...
    def __post_init__(self):
        ...
        if self.response_type == ResponseType.REQUEST_MODIFICATION:
            if not self.suggested_modification:
                raise ValueError(
                    "REQUEST_MODIFICATION responses must include suggested_modification"
                )
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (
            ...
            and self.suggested_modification == other.suggested_modification
...
class ProposalResponse:
    ...
    def _validate_response_consistency(self):
        ...
        if self.overall_response == ResponseType.ACCEPT:
            ...
            if rejection_count == 0:
                raise ValueError(
                    "REJECT overall response requires at least one term to be rejected"
                )
    ...
    def is_complete_acceptance(self) -> bool:
        ...
        return self.overall_response == ResponseType.ACCEPT and all(
            tr.is_acceptance() for tr in self.term_responses
        )

    def is_complete_rejection(self) -> bool:
        """Check if response represents complete rejection."""
        return self.overall_response == ResponseType.REJECT and all(
            tr.is_rejection() for tr in self.term_responses
        )
    ...
    def with_updated_term_response(
        self, term_id: str, term_response: TermResponse
    ) -> "ProposalResponse":
    ...
    def with_updated_overall_response(
        self,
        overall_response: ResponseType,
        overall_reason: Optional[ResponseReason] = None,
    ) -> "ProposalResponse":
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (
            ...
            and self.proposal_id == other.proposal_id
            and self.responding_party_id == other.responding_party_id
            and self.overall_response == other.overall_response
            and self.term_responses == other.term_responses
            and self.overall_reason == other.overall_reason
            and self.overall_comments == other.overall_comments
            and self.confidence_level == other.confidence_level
            and self.response_timestamp == other.response_timestamp
    ...
    def __str__(self) -> str:
        ...
        return (
            f"ProposalResponse(id={self.response_id}, "
