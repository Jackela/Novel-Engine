...
from ..value_objects.proposal_response import ProposalResponse
from ..value_objects.proposal_terms import ProposalTerms
...
class NegotiationSession:
    ...
    def add_party(self, party: NegotiationParty) -> None:
        ...
        for existing_party in self.parties.values():
            if not party.is_compatible_with(existing_party):
                raise ValueError(
                    f"Party {party.party_name} incompatible with existing party {existing_party.party_name}"
                )
    ...
    def advance_phase(self, target_phase: NegotiationPhase, forced: bool = False) -> None:
        ...
        if not forced and not self.auto_advance_phases:
            raise ValueError(
                "Manual phase advancement not allowed when auto_advance_phases is False"
            )
            ...
            try:
                ...
                if not self.require_unanimous_agreement:
                    ...
                    acceptances = len(
                        [
                            r
                            for r in self.get_proposal_responses(proposal_id)
    ...
    def __str__(self) -> str:
        ...
        return (
            f"NegotiationSession(id={self.session_id}, name={self.session_name}, "
