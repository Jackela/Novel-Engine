...
from ...domain.value_objects.negotiation_status import NegotiationPhase, NegotiationOutcome, TerminationReason
...
class InteractionApplicationService:
    ...
    async def create_negotiation_session(self, 
                                       session_name: str,
                                       session_type: str,
                                       created_by: UUID,
                                       **kwargs) -> Dict[str, Any]:
        ...
        command = CreateNegotiationSessionCommand(
            ...
            require_unanimous_agreement=kwargs.get('require_unanimous_agreement', False),
    ...
    async def add_party_to_negotiation(self, 
                                     session_id: UUID,
    ...
    async def submit_proposal(self, 
        ...
        analysis_result = await self.command_handler.handle_analyze_proposal_viability(analysis_command)
        ...
        return {
            ...
            'proposal_submitted': {
                ...
                'viability_score': analysis_result['analysis_result']['overall_viability_score'],
                'acceptance_probability': analysis_result['analysis_result']['acceptance_probability']
            },
            'session_status': {
    ...
    async def submit_proposal_response(self, 
                                     session_id: UUID,
                                     response: ProposalResponse,
                                     submitted_by: UUID) -> Dict[str, Any]:
        """
        ...
        momentum_result = await self.command_handler.handle_calculate_negotiation_momentum(momentum_command)
    ...
    async def advance_negotiation_phase(self, 
                                      session_id: UUID,
                                      target_phase: NegotiationPhase,
                                      initiated_by: UUID,
                                      force_advancement: bool = False) -> Dict[str, Any]:
        """
    ...
    async def complete_negotiation(self, 
                                 session_id: UUID,
        ...
        final_momentum = await self.command_handler.handle_calculate_negotiation_momentum(momentum_command)
        ...
        final_conflicts = await self.command_handler.handle_detect_negotiation_conflicts(conflicts_command)
        ...
        result = await self.command_handler.handle_terminate_negotiation_session(command)
    ...
    async def get_negotiation_insights(self, 
                                     session_id: UUID,
                                     initiated_by: UUID,
        ...
        compatibility_result = await self.command_handler.handle_assess_party_compatibility(compatibility_command)
        ...
        strategy_result = await self.command_handler.handle_recommend_negotiation_strategy(strategy_command)
        ...
        conflicts_result = await self.command_handler.handle_detect_negotiation_conflicts(conflicts_command)
    ...
    async def optimize_active_proposals(self, 
                                      session_id: UUID,
                                      initiated_by: UUID,
                                      optimization_target: str = 'maximize_acceptance') -> Dict[str, Any]:
        """
        ...
        for proposal_id in session.active_proposals.keys():
            ...
            optimization_results.append({
                ...
                'current_viability': analysis_result['analysis_result']['overall_viability_score'],
                'optimization_suggestions': analysis_result['analysis_result']['optimization_suggestions'],
                'risk_factors': analysis_result['analysis_result']['risk_factors'],
                'success_factors': analysis_result['analysis_result']['success_factors']
            })
        
        # Calculate overall optimization potential
        total_viability = sum(result['current_viability'] for result in optimization_results)
        avg_viability = total_viability / len(optimization_results) if optimization_results else 0
        ...
        return {
            ...
            'overall_recommendations': self._generate_proposal_optimization_recommendations(optimization_results)
        }
    
    async def monitor_session_health(self, 
                                   session_id: UUID,
                                   initiated_by: UUID) -> Dict[str, Any]:
        """
        ...
        health_score = self._calculate_health_score(
            session, conflicts_result['conflicts_detected'], 
        ...
        return {
            ...
            'health_summary': {
                ...
                'session_age_hours': int((datetime.now(timezone.utc) - session.created_at).total_seconds() // 3600),
            ...
            'recommendations': self._generate_health_recommendations(health_score, health_alerts)
        }
    ...
    def _generate_recommendations(self, 
                                ...
                                momentum_result: Dict[str, Any]) -> List[str]:
        """Generate actionable recommendations based on analysis results."""
        recommendations = []
        
        # Compatibility recommendations
        if compatibility_result['overall_compatibility'] < 50:
            recommendations.append("Consider mediation or conflict resolution strategies due to low party compatibility")
        
        # Strategy recommendations
        recommendations.extend(strategy_result['strategy_recommendation'].get('key_tactics', []))
    ...
    def _generate_proposal_optimization_recommendations(self, 
                                                      optimization_results: List[Dict[str, Any]]) -> List[str]:
        ...
        for suggestion, count in common_suggestions[:5]:
            if count > 1:  # Appears in multiple proposals
                recommendations.append(f"Common issue across {count} proposals: {suggestion}")
        
        # Add viability-based recommendations
        low_viability_count = sum(1 for result in optimization_results if result['current_viability'] < 50)
        if low_viability_count > 0:
            recommendations.append(f"{low_viability_count} proposals have low viability - consider major revisions")
    ...
    def _calculate_health_score(self, 
        ...
        if session.status.phase.value in ['bargaining', 'closing'] and len(session.active_proposals) == 0:
    ...
    def _generate_health_recommendations(self, 
                                       health_score: float,
                                       health_alerts: List[Dict[str, Any]]) -> List[str]:
        ...
        for alert in health_alerts:
            if alert['type'] == 'timeout_warning':
                recommendations.append("Extend session timeline or accelerate decision-making")
            elif alert['type'] == 'inactivity_warning':
                recommendations.append("Schedule immediate follow-up meeting or send status inquiry")
            elif alert['type'] == 'conflict_warning':
                recommendations.append("Implement conflict resolution procedures immediately")
            elif alert['type'] == 'momentum_warning':
                recommendations.append("Introduce new proposals or change negotiation approach")
