...
from .interaction_queries import (  # Session Queries; Party Queries; Proposal Queries; Analysis Queries; Performance Queries; Search Queries; Historical Queries; Monitoring Queries
...
class InteractionQueryHandler:
    ...
    async def handle_get_negotiation_session(
        ...
        if query.include_parties:
            result["parties"] = cast(List[Dict[str, Any]], [])
            for party_id, party in session.parties.items():
                ...
                cast(List[Dict[str, Any]], result["parties"]).append(
                    party_data
                )
        ...
        if query.include_proposals:
            ...
            for proposal_id, proposal in session.active_proposals.items():
                ...
                cast(
                    List[Dict[str, Any]], result["proposals"]["active"]
        ...
        if query.include_responses:
            ...
            for party_id, responses in session.responses.items():
                cast(Dict[str, Any], result["responses"])["by_party"][
                    str(party_id)
    ...
    async def handle_list_negotiation_sessions(
        self, query: ListNegotiationSessionsQuery
    ) -> Dict[str, Any]:
        ...
        for session in sessions:
            ...
            if query.status_filter:
                if (
                    ...
                    and not session.status.is_active
                ):
                    continue
                elif (
                    ...
                    query.status_filter == "terminated"
                    and session.status.phase.value != "terminated"
                ):
                    continue
            ...
            if (
                ...
                and query.participant_id not in session.parties
            ):
                continue
    ...
    async def handle_get_session_summary(
            ...
            if query.include_analytics and session.parties:
                ...
                summary["analytics"] = {
                    ...
                    "average_compatibility": (
                        sum(compatibility_scores.values())
                ...
                if session.total_responses > 0:
                    ...
                    momentum = self.negotiation_service.calculate_negotiation_momentum(
                        responses=all_responses, phase=session.status.phase
                    )
                    ...
                    summary["analytics"]["momentum"] = {
                        ...
                        "trajectory_prediction": momentum[
        ...
        if query.summary_depth == "detailed":
                    ...
                    party_detail = {
                        ...
                        "preferences": {
                            ...
                            "risk_tolerance": float(
                                party.preferences.risk_tolerance
                            ),
                        ...
                        "negotiation_power": (
                            float(
                                party.get_negotiation_power(
                                    session.negotiation_domain
                                )
                            )
            ...
            if query.include_timeline:
                ...
                # Sort by timestamp
                timeline_events.sort(key=lambda x: cast(Any, x["timestamp"]))
    ...
    async def handle_get_session_timeline(
        ...
        for party_id, responses in session.responses.items():
            ...
            party_name = (
                party_obj.party_name if party_obj is not None else "Unknown"
            )
            for response in responses:
                timeline_events.append(
                    {
        ...
        if query.start_date:
            timeline_events = [
                ...
                if cast(Any, e["timestamp"]) >= query.start_date
            ]
        ...
        if query.end_date:
            timeline_events = [
                e
                for e in timeline_events
                if cast(Any, e["timestamp"]) <= query.end_date
            ]
    ...
    async def handle_get_session_parties(
        ...
        for party in session.parties.values():
            ...
            if query.include_capabilities:
                ...
                party_data["average_proficiency"] = float(
                    party.average_proficiency
                )
            ...
            if query.include_activity_summary:
                ...
                party_data["activity_summary"] = {
                    ...
                    "avg_acceptance_rate": (
                        sum(
                            r.get_acceptance_percentage()
                            for r in party_responses
                        )
                        / len(party_responses)
                        if party_responses
                        else 0
                    ...
                    "last_activity": (
                        max(r.response_timestamp for r in party_responses)
                        if party_responses
                        else None
                    ),
                }
    ...
    async def handle_get_party_compatibility(
        ...
        if query.party_ids:
            parties = [
                session.parties[party_id]
                for party_id in query.party_ids
        ...
        if query.matrix_format:
            ...
            for party in parties:
                party_compatibilities: List[Dict[str, Any]] = [
                    cm
                    for cm in compatibility_matrix
                    if cm["party1_id"] == str(party.party_id)
                    or cm["party2_id"] == str(party.party_id)
                ]
                if party_compatibilities:
                    avg_compatibility = sum(
                        cm["compatibility_score"]
                        for cm in party_compatibilities
                    ) / len(party_compatibilities)
        ...
        if query.include_recommendations:
            ...
            if overall_compatibility < 50:
                recommendations.append(
                    "Overall compatibility is low - consider restructuring negotiation approach"
                )
    ...
    async def handle_get_proposal_details(
            ...
            if responses:
                ...
                for response in responses:
                    ...
                    response_types[resp_type] = (
                        response_types.get(resp_type, 0) + 1
                    )
        ...
        if query.include_optimization_suggestions:
            ...
            proposal_data["optimization_suggestions"] = {
                ...
                "implementation_difficulty": optimization[
    ...
    async def handle_get_momentum_analysis(
        ...
        cutoff_time = query.timestamp - timedelta(
            hours=query.analysis_window_hours
        )
        recent_responses = []
