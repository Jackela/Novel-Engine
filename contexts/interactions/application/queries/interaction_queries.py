...
class ListNegotiationSessionsQuery(InteractionQuery):
    ...
    def __post_init__(self):
        ...
        if self.order_by not in ["created_at", "session_name", "status", "priority_level"]:
            raise ValueError("order_by must be one of: created_at, session_name, status, priority_level")
...
class GetNegotiationAnalyticsQuery(InteractionQuery):
    ...
    def __post_init__(self):
        ...
        if self.analytics_types is None:
            self.analytics_types = ["momentum", "conflicts", "viability", "compatibility"]
...
class GetStrategyRecommendationsQuery(InteractionQuery):
    ...
    def __post_init__(self):
        super().__post_init__()
        if self.strategy_focus not in ["aggressive", "balanced", "conservative", "collaborative"]:
            raise ValueError("strategy_focus must be one of: aggressive, balanced, conservative, collaborative")
...
class GenerateSessionReportQuery(InteractionQuery):
    ...
    def __post_init__(self):
        super().__post_init__()
        if self.report_type not in ["summary", "comprehensive", "analytical", "executive"]:
            raise ValueError("report_type must be one of: summary, comprehensive, analytical, executive")
        if self.target_audience not in ["technical", "business", "executive", "general"]:
            raise ValueError("target_audience must be one of: technical, business, executive, general")
...
class GetNegotiationTrendsQuery(InteractionQuery):
    ...
    def __post_init__(self):
        super().__post_init__()
        if self.time_period not in ["last_7_days", "last_30_days", "last_90_days", "custom"]:
            raise ValueError("time_period must be one of: last_7_days, last_30_days, last_90_days, custom")
        if self.time_period == "custom":
            if not self.custom_start_date or not self.custom_end_date:
                raise ValueError("custom_start_date and custom_end_date required for custom time_period")
...
class GetHistoricalAnalysisQuery(InteractionQuery):
    ...
    def __post_init__(self):
        ...
        if self.analysis_type not in [
            "success_factors", "failure_patterns", "duration_analysis", 
            "party_behavior", "proposal_effectiveness"
        ]:
            raise ValueError("analysis_type must be one of: success_factors, failure_patterns, duration_analysis, party_behavior, proposal_effectiveness")
...
class GetNotificationsQuery(InteractionQuery):
    ...
    def __post_init__(self):
        ...
        if self.offset < 0:
            raise ValueError("offset must be non-negative")
        if self.severity_filter is not None and self.severity_filter not in ["low", "medium", "high", "critical"]:
