...
class CreateNegotiationSessionCommand(InteractionCommand):
    ...
    def __post_init__(self):
        ...
        if self.confidentiality_level not in ["public", "standard", "confidential", "secret"]:
            raise ValueError("confidentiality_level must be one of: public, standard, confidential, secret")
...
class UpdateSessionConfigurationCommand(InteractionCommand):
    ...
    def __post_init__(self):
        ...
        if self.session_timeout_hours is not None and self.session_timeout_hours <= 0:
            raise ValueError("session_timeout_hours must be positive")
        if self.priority_level is not None and self.priority_level not in ["low", "medium", "high", "critical"]:
            raise ValueError("priority_level must be one of: low, medium, high, critical")
        if self.confidentiality_level is not None and self.confidentiality_level not in ["public", "standard", "confidential", "secret"]:
            raise ValueError("confidentiality_level must be one of: public, standard, confidential, secret")
...
class OptimizeProposalCommand(InteractionCommand):
    ...
    def __post_init__(self):
        ...
        if self.optimization_target not in [
            "maximize_acceptance", "minimize_risk", "balance_interests", "speed_resolution"
        ]:
            raise ValueError("optimization_target must be one of: maximize_acceptance, minimize_risk, balance_interests, speed_resolution")
...
class UpdateResponseCommand(InteractionCommand):
    ...
    def __post_init__(self):
        super().__post_init__()
        if (not self.updated_term_responses and 
            not self.updated_overall_response and 
            not self.updated_conditions):
...
class SynchronizeExternalDataCommand(InteractionCommand):
    ...
    def __post_init__(self):
        ...
        if self.conflict_resolution_strategy not in ["overwrite", "merge", "manual"]:
            raise ValueError("conflict_resolution_strategy must be one of: overwrite, merge, manual")
