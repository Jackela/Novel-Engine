...
class NarrativeContext:
    ...
    character_knowledge_required: Set[UUID] = None  # Characters who must know this context
    ...
    def _validate_constraints(self):
        ...
        if (self.applies_from_sequence is not None and 
            self.applies_to_sequence is not None and
        ...
        for influence_dict in [self.mood_influences, self.tension_modifiers, self.pacing_effects]:
            for key, value in influence_dict.items():
                if not (-Decimal('10') <= value <= Decimal('10')):
                    raise ValueError(f"Influence values must be between -10 and 10, got {value} for {key}")
    ...
    def has_sequence_range(self) -> bool:
        ...
        return self.applies_from_sequence is not None or self.applies_to_sequence is not None
    ...
    def overall_influence_strength(self) -> Decimal:
        ...
        """
        base_strength = self.narrative_importance * self.visibility_level / Decimal('10')
    ...
    def contextual_complexity_score(self) -> Decimal:
        ...
        return min(Decimal('10'), base_complexity + relationship_complexity + information_complexity)
    ...
    def applies_at_sequence(self, sequence_number: int) -> bool:
        ...
        if self.applies_from_sequence is not None and sequence_number < self.applies_from_sequence:
            return False
        
        if self.applies_to_sequence is not None and sequence_number > self.applies_to_sequence:
            return False
    ...
    def get_contextual_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'sequence_range': [self.applies_from_sequence, self.applies_to_sequence] if self.has_sequence_range else None,
