...
class CausalNode:
    ...
    def causal_complexity_score(self) -> Decimal:
        ...
        complexity_bonus += Decimal(str(
            len(self.prerequisite_conditions) + len(self.blocking_conditions)
        )) * Decimal('0.5')
    ...
    def with_additional_cause(self, 
                            cause_node_id: str, 
                            relationship_type: CausalRelationType = CausalRelationType.DIRECT_CAUSE,
                            strength: CausalStrength = CausalStrength.MODERATE) -> 'CausalNode':
        """
