...
class NarrativeTheme:
    ...
    def _validate_constraints(self):
        ...
        for audience, relevance in self.target_audience_relevance.items():
            if not (Decimal('0') <= relevance <= Decimal('10')):
                raise ValueError(f"Audience relevance for '{audience}' must be between 0 and 10")
    ...
    def is_major_theme(self) -> bool:
        ...
        return self.intensity in [ThemeIntensity.PROMINENT, ThemeIntensity.CENTRAL, ThemeIntensity.OVERWHELMING]
    ...
    def spans_full_narrative(self) -> bool:
        ...
        return (self.introduction_sequence is not None and 
    ...
    def thematic_complexity_score(self) -> Decimal:
        ...
        relationship_bonus = Decimal(str(
            (len(self.conflicts_with_themes) + len(self.reinforces_themes)) * 0.3
        ))
        
        # Cap at 10
        return min(Decimal('10'), base_complexity + expression_bonus + relationship_bonus)
