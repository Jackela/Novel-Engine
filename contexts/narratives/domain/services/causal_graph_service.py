...
class CausalGraphService:
    ...
    def establish_causal_link(self,
        ...
        self._invalidate_cache()
        logger.info(f"Established causal link: {cause_id} --{relationship_type.value}--> {effect_id}")
        return True
    ...
    def _find_paths_recursive(self,
                             current: str,
                             target: str,
                             path: List[str],
        ...
        for next_node in node.direct_effects.union(node.indirect_effects):
            ...
            if next_node == target:
                ...
                # Continue searching
                self._find_paths_recursive(
                    next_node, target, new_path, new_rel_types,
                    new_strength, remaining_depth - 1, paths
                )
    ...
    def _get_chains_from_node(self, start_node: str, visited: Optional[Set[str]] = None) -> List[CausalPath]:
    ...
    def _calculate_graph_complexity(self) -> Decimal:
        ...
        total_relationships = sum(
            ...
            len(node.indirect_causes) + len(node.indirect_effects)
            for node in self.nodes.values()
        ) / 2  # Divide by 2 since each relationship is counted twice
        ...
        for node in self.nodes.values():
            ...
            for cause_id in node.direct_causes.union(node.indirect_causes):
                if cause_id in self.nodes:
                    cause_node = self.nodes[cause_id]
                    if (cause_node.sequence_order is not None and
                        cause_node.sequence_order >= node.sequence_order):
                        issues += 1
    ...
    def _calculate_narrative_flow_score(self) -> Decimal:
        ...
        avg_connectivity = sum(n.total_causes + n.total_effects for n in self.nodes.values()) / len(self.nodes)
    ...
    def get_node_influence_score(self, node_id: str) -> Decimal:
        ...
        total_influence = base_influence + connectivity_bonus + junction_bonus + downstream_influence
        return min(Decimal('10.0'), total_influence)
    
    def _calculate_downstream_influence(self, node_id: str, visited: Set[str], depth: int = 0) -> Decimal:
        ...
        for effect_id in node.direct_effects:
            if effect_id in self.nodes and effect_id not in visited:
                ...
                recursive_influence = self._calculate_downstream_influence(effect_id, visited, depth + 1)
    ...
    def get_graph_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'longest_chain_length': max((c.path_length for c in analysis.longest_chains), default=0),
