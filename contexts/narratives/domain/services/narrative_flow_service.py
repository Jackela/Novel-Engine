...
from ..value_objects.story_pacing import StoryPacing, PacingType
...
class NarrativeFlowService:
    ...
    def _analyze_pacing_quality(self, arc: NarrativeArc) -> Decimal:
        ...
        ) / Decimal(str(total_coverage)) if total_coverage > 0 else Decimal('5.0')
    ...
    def _evaluate_pacing_segment(self, pacing: StoryPacing) -> Decimal:
        ...
        segment_score = (base_score * Decimal('0.6') + 
    ...
    def _assess_content_balance(self, pacing: StoryPacing) -> Decimal:
        ...
        if any(ratio > Decimal('0.8') for ratio in [pacing.dialogue_ratio, pacing.action_ratio, pacing.reflection_ratio]):
            content_score -= Decimal('2.0')
        
        if any(ratio < Decimal('0.05') for ratio in [pacing.dialogue_ratio, pacing.action_ratio, pacing.reflection_ratio]):
    ...
    def _assess_resolution_quality(self, arc: NarrativeArc) -> Decimal:
        ...
        if arc.themes:
            themes_with_resolution = sum(
                1 for theme in arc.themes.values()
                if theme.resolution_sequence is not None
            )
            theme_resolution_ratio = Decimal(str(themes_with_resolution)) / Decimal(str(len(arc.themes)))
    ...
    def _calculate_pacing_momentum(self, pacing_segments: Dict[str, StoryPacing]) -> Decimal:
        ...
        sorted_segments = sorted(
            pacing_segments.values(),
            key=lambda p: p.start_sequence
        )
    ...
    def _calculate_character_momentum(self, arc: NarrativeArc, plot_points: List[PlotPoint]) -> Decimal:
            ...
            if total_prereqs > 0:
                for prereq in plot_point.prerequisite_events:
                    ...
                    for earlier_point in plot_points:
                        if (earlier_point.sequence_order < plot_point.sequence_order and
                            earlier_point.plot_point_id == prereq):
                            satisfied_prereqs += 1
                            break
    ...
    def optimize_sequence_order(self, arc: NarrativeArc) -> SequenceOptimization:
        ...
        improvement_score = self._calculate_improvement_score(arc, original_sequence, optimized_sequence)
    ...
    def _identify_sequence_changes(self, original: List[str], optimized: List[str]) -> List[Dict[str, Any]]:
    ...
    def _calculate_improvement_score(self, arc: NarrativeArc, original: List[str], optimized: List[str]) -> Decimal:
    ...
    def _generate_optimization_rationale(self, changes: List[Dict[str, Any]], improvement_score: Decimal) -> str:
        ...
        if len(changes) == 1:
            ...
            rationale_parts.append(f"{len(changes)} plot points repositioned for better narrative flow.")
