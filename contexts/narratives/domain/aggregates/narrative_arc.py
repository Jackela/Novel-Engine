...
class NarrativeArc:
    ...
    def add_plot_point(self, plot_point: PlotPoint) -> None:
        ...
        for existing_id, existing_plot in self.plot_points.items():
            if existing_plot.sequence_order == plot_point.sequence_order:
                raise ValueError(
                    f"Sequence {plot_point.sequence_order} already occupied by {existing_id}"
                )
    ...
    def _sequences_overlap(
        ...
        return not (
            ...
            or pacing2.end_sequence < pacing1.start_sequence
        )
    ...
    def get_active_contexts(self) -> List[NarrativeContext]:
        ...
        return [
            self.narrative_contexts[cid]
            for cid in self.active_contexts
