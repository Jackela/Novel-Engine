...
class NarrativeArcQueryHandler:
    ...
    def handle_get_narrative_arcs_by_type(self, query: GetNarrativeArcsByTypeQuery) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for arc in arcs:
                results.append({
                    ...
                    'arc_type': arc.arc_type,
                    'status': arc.status,
                    'description': arc.description[:200] + "..." if len(arc.description) > 200 else arc.description,
                    'completion_percentage': float(arc.completion_percentage),
                    'complexity_score': float(arc.complexity_score),
                    'plot_points_count': len(arc.plot_points),
                    'themes_count': len(arc.themes),
    ...
    def handle_get_plot_point(self, query: GetPlotPointQuery) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'pov_character': str(plot_point.pov_character) if plot_point.pov_character else None,
    ...
    def handle_get_plot_points_in_sequence(self, query: GetPlotPointsInSequenceQuery) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if query.start_sequence is not None or query.end_sequence is not None:
                ...
                for pp in plot_points:
                    if query.start_sequence is not None and pp.sequence_order < query.start_sequence:
                        continue
    ...
    def handle_get_plot_points_by_type(self, query: GetPlotPointsByTypeQuery) -> List[Dict[str, Any]]:
    ...
    def handle_get_themes_at_sequence(self, query: GetThemesAtSequenceQuery) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if query.include_statistics:
                ...
                for plot_point in arc.plot_points.values():
                    ...
                    plot_types_distribution[plot_type] = plot_types_distribution.get(plot_type, 0) + 1
                
                theme_types_distribution = {}
                for theme in arc.themes.values():
                    theme_type = theme.theme_type.value
                    theme_types_distribution[theme_type] = theme_types_distribution.get(theme_type, 0) + 1
                ...
                summary['statistics'] = {
                    ...
                    'average_emotional_intensity': float(
                        ...
                        len(arc.plot_points) if arc.plot_points else Decimal('0')
                    ),
    ...
    def handle_get_narrative_flow_analysis(self, query: GetNarrativeFlowAnalysisQuery) -> Dict[str, Any]:
