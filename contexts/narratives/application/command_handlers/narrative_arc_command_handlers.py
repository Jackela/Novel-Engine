...
class NarrativeArcCommandHandler:
    ...
    def handle_update_plot_point(self, command: UpdatePlotPointCommand) -> None:
        ...
        try:
            ...
            updated_plot = PlotPoint(
                ...
                description=command.description if command.description is not None else plot_point.description,
                sequence_order=command.sequence_order if command.sequence_order is not None else plot_point.sequence_order,
                emotional_intensity=command.emotional_intensity if command.emotional_intensity is not None else plot_point.emotional_intensity,
                dramatic_tension=command.dramatic_tension if command.dramatic_tension is not None else plot_point.dramatic_tension,
                story_significance=command.story_significance if command.story_significance is not None else plot_point.story_significance,
    ...
    def handle_add_character_to_arc(self, command: AddCharacterToArcCommand) -> None:
        ...
        try:
            ...
            logger.info(f"Added {command.role} character {command.character_id} to arc {command.arc_id}")
    ...
    def handle_start_narrative_arc(self, command: StartNarrativeArcCommand) -> None:
        ...
        try:
            ...
            logger.info(f"Started narrative arc {command.arc_id} at sequence {command.start_sequence}")
    ...
    def handle_complete_narrative_arc(self, command: CompleteNarrativeArcCommand) -> None:
        ...
        try:
            ...
            logger.info(f"Completed narrative arc {command.arc_id} at sequence {command.end_sequence}")
