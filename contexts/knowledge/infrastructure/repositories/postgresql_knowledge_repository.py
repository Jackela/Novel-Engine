...
class PostgreSQLKnowledgeRepository(IKnowledgeRepository):
    ...
    def __init__(self, session: AsyncSession, embedding_generator: EmbeddingGeneratorAdapter | None = None):
        """
    ...
    async def save(self, entry: KnowledgeEntry) -> None:
        ...
        await self._session.execute(
            upsert_sql,
            {
                ...
                "allowed_roles": list(entry.access_control.allowed_roles) if entry.access_control.allowed_roles else None,
                "allowed_character_ids": list(entry.access_control.allowed_character_ids) if entry.access_control.allowed_character_ids else None,
    ...
    async def retrieve_for_agent_semantic(
        ...
        query_embedding = await self._embedding_generator.generate_embedding(semantic_query)
    ...
    def _row_to_domain_model(self, row) -> KnowledgeEntry:
        ...
        access_control = AccessControlRule(
            ...
            allowed_character_ids=tuple(row.allowed_character_ids) if row.allowed_character_ids else (),
        )
