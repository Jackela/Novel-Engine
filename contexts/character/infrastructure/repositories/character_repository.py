...
from ...domain.repositories.character_repository import (
    ICharacterRepository, RepositoryException, ConcurrencyException, NotSupportedException
)
...
class SQLAlchemyCharacterRepository(ICharacterRepository):
    ...
    async def get_by_id(self, character_id: CharacterID) -> Optional[Character]:
        ...
        try:
            with self.session_factory() as session:
                character_orm = session.query(CharacterORM)\
    ...
    async def save(self, character: Character) -> None:
        ...
        try:
            with self.session_factory() as session:
                ...
                existing_orm = session.query(CharacterORM)\
    ...
    async def find_by_name(self, name: str) -> List[Character]:
        ...
        try:
            with self.session_factory() as session:
                character_orms = session.query(CharacterORM)\
    ...
    async def find_by_level_range(self, min_level: int, max_level: int) -> List[Character]:
        ...
        try:
            ...
            self.logger.error(f"Error finding characters by level range {min_level}-{max_level}: {e}")
    ...
    async def find_by_created_date_range(self, start_date: datetime, end_date: datetime) -> List[Character]:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.session_factory() as session:
                ...
                ).group_by(CharacterORM.character_class).all()
                
                # Characters by race
                race_stats = session.query(
                    CharacterORM.race,
                    func.count(CharacterORM.character_id)
                ).group_by(CharacterORM.race).all()
                ...
                level_distribution = session.query(
                    CharacterORM.level,
                    func.count(CharacterORM.character_id)
                ).group_by(CharacterORM.level).all()
                
                # Alive characters
                alive_characters = session.query(CharacterORM)\
                    .filter(CharacterORM.is_alive == True).count()
    ...
    async def delete_multiple(self, character_ids: List[CharacterID]) -> int:
        ...
        try:
            with self.session_factory() as session:
                ...
                deleted_count = session.query(CharacterORM)\
    ...
    async def get_version(self, character_id: CharacterID) -> Optional[int]:
        ...
        try:
            with self.session_factory() as session:
                version = session.query(CharacterORM.version)\
                    .filter(CharacterORM.character_id == character_id.value)\
                    .scalar()
    ...
    async def get_character_history(self, character_id: CharacterID, 
                                   limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Get the change history for a character."""
        try:
            with self.session_factory() as session:
                query = session.query(CharacterEventORM)\
    ...
    def _update_orm_from_domain(self, character_orm: CharacterORM, character: Character, session: Session) -> None:
