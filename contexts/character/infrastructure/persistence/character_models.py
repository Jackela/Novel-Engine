...
class CharacterORM(Base):
    ...
    profile = relationship("CharacterProfileORM", back_populates="character", uselist=False, cascade="all, delete-orphan")
    stats = relationship("CharacterStatsORM", back_populates="character", uselist=False, cascade="all, delete-orphan")
    skills = relationship("CharacterSkillsORM", back_populates="character", uselist=False, cascade="all, delete-orphan")
...
class CharacterProfileORM(Base):
    ...
    character_id = Column(UUID(as_uuid=True), ForeignKey("characters.character_id"), nullable=False, unique=True)
...
class CharacterSkillsORM(Base):
    ...
    def set_skill(self, category: str, skill_name: str, proficiency_level: int, 
                  modifier: int = 0, description: str = None) -> None:
...
class CharacterEventORM(Base):
    ...
    character_id = Column(UUID(as_uuid=True), ForeignKey("characters.character_id"), nullable=False, index=True)
