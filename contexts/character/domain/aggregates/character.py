...
from ..value_objects.character_profile import CharacterProfile, Gender, CharacterRace, CharacterClass
from ..value_objects.character_stats import CharacterStats, CoreAbilities, VitalStats, CombatStats, AbilityScore
from ..value_objects.skills import Skills, SkillCategory, ProficiencyLevel
from ..events.character_events import CharacterEvent, CharacterCreated, CharacterUpdated, CharacterStatsChanged
...
class Character:
    ...
    def __post_init__(self):
        ...
        if self.version == 1 and not self._events:
            self._add_event(CharacterCreated.create(
    ...
    def _validate_character_consistency(self) -> None:
        ...
        if self.stats.vital_stats.max_health < expected_min_health:
            errors.append(f"Character health too low for constitution and level (minimum: {expected_min_health})")
        ...
        for ability, min_score in minimums.items():
            if hasattr(self.stats.core_abilities, ability):
                ...
                if actual_score < min_score:
                    errors.append(f"{self.profile.race.value.title()} should have {ability} >= {min_score}")
    ...
    def _validate_class_skills(self, errors: List[str]) -> None:
        ...
        class_skills = {
            ...
            CharacterClass.SCIENTIST: [SkillCategory.INTELLECTUAL, SkillCategory.TECHNICAL],
            CharacterClass.ENGINEER: [SkillCategory.TECHNICAL, SkillCategory.INTELLECTUAL],
        }
        
        expected_categories = class_skills.get(self.profile.character_class, [])
        for category in expected_categories:
            if category not in self.skills.skill_groups:
                errors.append(f"{self.profile.character_class.value.title()} should have {category.value} skills")
            elif not any(skill.is_trained() for skill in self.skills.get_skills_by_category(category)):
                errors.append(f"{self.profile.character_class.value.title()} should be trained in at least one {category.value} skill")
    ...
    def _validate_age_consistency(self, errors: List[str]) -> None:
        ...
        for level_range, min_age in min_age_by_level.items():
            if self.profile.level in level_range and self.profile.age < min_age:
                errors.append(f"Character too young ({self.profile.age}) for level {self.profile.level} (minimum age: {min_age})")
                break
    ...
    def update_profile(self, new_profile: CharacterProfile) -> None:
        ...
        # Record update event
        self._add_event(CharacterUpdated.create(
            character_id=self.character_id,
    ...
    def update_stats(self, new_stats: CharacterStats) -> None:
        ...
        if (new_stats.vital_stats.current_health < old_stats.vital_stats.current_health and
            new_stats.vital_stats.current_health < old_stats.vital_stats.current_health / 2):
            raise ValueError("Cannot lose more than half health in a single update")
        
        # Business rule: Cannot exceed maximum values
        if (new_stats.vital_stats.current_health > new_stats.vital_stats.max_health or
            new_stats.vital_stats.current_mana > new_stats.vital_stats.max_mana or
            new_stats.vital_stats.current_stamina > new_stats.vital_stats.max_stamina):
        ...
        self._add_event(CharacterStatsChanged.create(
            character_id=self.character_id,
    ...
    def can_level_up(self, required_xp: int) -> bool:
        ...
        return (self.stats.experience_points >= required_xp and 
                self.profile.level < 100)
