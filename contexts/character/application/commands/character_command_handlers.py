...
class CreateCharacterCommandHandler:
    ...
    async def handle(self, command: CreateCharacterCommand) -> CharacterID:
        ...
        try:
            ...
            if any([command.title, command.affiliation, command.languages,
                   command.height_cm, command.weight_kg, command.hair_color,
                   command.eye_color, command.skin_tone, command.physical_description,
                   command.backstory, command.homeland, command.education]):
    ...
    async def handle(self, command: UpdateCharacterStatsCommand) -> None:
        ...
        try:
            ...
            if command.expected_version and character.version != command.expected_version:
                raise ValueError(f"Version conflict: expected {command.expected_version}, got {character.version}")
            ...
            new_vital_stats = VitalStats(
                ...
                current_health=command.current_health if command.current_health is not None else current_vital_stats.current_health,
                max_mana=command.max_mana or current_vital_stats.max_mana,
                current_mana=command.current_mana if command.current_mana is not None else current_vital_stats.current_mana,
                max_stamina=command.max_stamina or current_vital_stats.max_stamina,
                current_stamina=command.current_stamina if command.current_stamina is not None else current_vital_stats.current_stamina,
            ...
            new_combat_stats = CombatStats(
                base_attack_bonus=command.base_attack_bonus or current_combat_stats.base_attack_bonus,
                initiative_modifier=command.initiative_modifier or current_combat_stats.initiative_modifier,
    ...
    async def handle(self, command: LevelUpCharacterCommand) -> None:
        ...
        try:
            ...
            if command.ability_score_improvements:
                ...
                self.logger.info(f"Ability improvements requested: {command.ability_score_improvements}")
...
class HealCharacterCommandHandler:
    ...
    async def handle(self, command: HealCharacterCommand) -> None:
        ...
        """
        character_id = CharacterID.from_string(command.character_id)
        self.logger.info(f"Healing character {character_id} for {command.healing_amount} points ({command.healing_type})")
...
class DamageCharacterCommandHandler:
    ...
    async def handle(self, command: DamageCharacterCommand) -> None:
        ...
        """
        character_id = CharacterID.from_string(command.character_id)
        self.logger.info(f"Applying {command.damage_amount} {command.damage_type} damage to character {character_id}")
