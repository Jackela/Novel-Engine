...
class ContextLoaderService:
    ...
    capabilities for the Character Context Engineering Framework.
    It handles multiple file formats, graceful error recovery, and
    ...
    def __init__(
        ...
        self.logger.info(
            ...
            f"concurrent_limit={max_concurrent_loads}"
        )
    ...
    async def load_character_context(
        ...
        self.logger.info(
            f"Loading character context for: {character_identifier}"
        )
        ...
        async with self._load_semaphore:
            try:
                ...
                try:
                    ...
                    if self.enable_caching:
                        ...
                        if cached_result:
                            ...
                            # Cache hits are also successful operations
                            if cached_result.load_success:
                                self._load_stats["successful_loads"] += 1
                            self.logger.debug(
                                f"Cache hit for {character_identifier}"
                            )
                    ...
                    character_dir = await self._find_character_directory(
                        sanitized_id
                    )

                    # Load all context files with timeout
                    context_data = await asyncio.wait_for(
                        self._load_all_context_files(
                            character_dir, sanitized_id
                        ),
                    ...
                    if self.enable_caching and character_context.load_success:
                        await self._store_in_cache(
                            sanitized_id, character_context
                        )
                    ...
                    self.logger.info(
                        f"Context loading completed for "
                        f"{character_identifier} in {load_duration:.2f}s - "
                ...
                await self._record_failure()
                self.logger.error(
                    f"Context loading timeout for {character_identifier}"
                )
                ...
                self.logger.error(
                    f"Security violation for {character_identifier}: {e}"
                )
                raise
                ...
                raise ContextLoaderError(
                    f"Context loading failed: {str(e)}"
                ) from e
    ...
    async def _find_character_directory(self, character_id: str) -> Path:
        ...
        if not character_dir.exists():
            self.logger.warning(
                f"Character directory not found: {character_dir}"
            )
            raise ContextLoaderError(
    ...
    async def _load_all_context_files(
        ...
        """
        context_data: Dict[str, Any] = {"loaded_files": [], "contexts": {}}
        ...
        for context_type, file_suffix in self.file_patterns.items():
            try:
                ...
                if file_path.exists():
                    self.logger.debug(
                        f"Loading {context_type} file: {file_path}"
                    )

                    # Load and parse file based on type
                    if context_type == "stats":
                        parsed_data, file_info = await self._load_yaml_file(
                            file_path
                        )
                    else:
                        (
                            parsed_data,
                            file_info,
                        ) = await self._load_markdown_file(
                ...
                context_data["loaded_files"].append(
                    LoadedFileInfo(
                        file_name=f"{character_id}{file_suffix}",
                        file_path=str(
                            character_dir / f"{character_id}{file_suffix}"
                        ),
                        loaded_successfully=False,
                        error_message=str(e),
                    )
    ...
    async def _load_yaml_file(
        ...
        try:
            ...
            if not content.strip():
                # Handle empty file as partial load scenario rather than complete failure
                file_info.error_message = "YAML file is empty"
                self.logger.warning(
                    f"Empty YAML file: {file_path} - treating as partial load"
                )
            ...
            if not yaml_data:
                # Handle empty YAML data as partial load scenario
                file_info.error_message = "YAML file contains no data"
                self.logger.warning(
                    f"No data in YAML file: {file_path} - treating as partial load"
                )
                return None, file_info
            ...
            self.logger.error(
                f"Failed to load YAML file {file_path}: {error_msg}"
            )
            return None, file_info
    ...
    async def _load_markdown_file(
        ...
        try:
            ...
            file_info.loaded_successfully = True
            self.logger.debug(
                f"Successfully loaded {context_type} file: {file_path}"
            )
    ...
    async def _parse_memory_markdown(self, content: str) -> MemoryContext:
        ...
        try:
            ...
            for match in re.finditer(
                relationship_pattern, content, re.IGNORECASE
            ):
                ...
                try:
                    ...
                    relationships.append(
                        RelationshipMemory(
                            ...
                            relationship_type=(
                                RelationshipType.PROFESSIONAL_NETWORK
                            ),  # Default type
                            memory_foundation="Documented in memory context",
                            trust_level=TrustLevel(
                                score=trust_score, category="Medium"
                            ),
                    ...
                    if (
                        ...
                        formative_events.append(
                            FormativeEvent(
                                ...
                                emotional_impact=(
                                    "Documented in memory context"
                                ),
                                decision_influence=(
                                    "Influences current behavior patterns"
                                ),
                                trigger_phrases=[],
                                key_lesson=None,
                            )
    ...
    async def _parse_objectives_markdown(
        self, content: str
    ) -> ObjectivesContext:
        """
        ...
        """
        try:
            core_objectives: List[Objective] = []
            strategic_objectives: List[Objective] = []
            tactical_objectives: List[Objective] = []
            ...
            raise FileParsingError(
                f"Objectives markdown parsing error: {str(e)}"
            )
    ...
    async def _parse_profile_markdown(self, content: str) -> ProfileContext:
        ...
        try:
            ...
            age_match = re.search(
                r"\*\*Age\*\*:\s*(\d+)", content, re.IGNORECASE
            )
            ...
            for match in re.finditer(drive_pattern, content):
                ...
                if (
                    "drive" in drive_name.lower()
                    or "emotional" in drive_name.lower()
                ):
                    emotional_drives.append(
                        EmotionalDrive(
                            ...
                            positive_expression=(
                                "Positive manifestation documented"
                            ),
                            negative_expression=(
                                "Negative manifestation documented"
                            ),
                            trigger_events=[],
                            soothing_behaviors=[],
                        )
            ...
            return ProfileContext(
                ...
                gender=(
                    gender_match.group(1).strip()
                    if gender_match
                    else "Unknown"
                ),
                race=(
                    race_match.group(1).strip() if race_match else "Unknown"
                ),
                character_class=(
    ...
    async def _create_character_context(
        ...
        try:
            ...
            if contexts.get("profile") and hasattr(
                contexts["profile"], "name"
            ):
            ...
            self.logger.info(
                ...
                f"Partial={character_context.partial_load}, "
                f"Files loaded: "
                f"{len([f for f in loaded_files if f.loaded_successfully])}"
                f"/{len(loaded_files)}"
            )
            ...
            raise ContextLoaderError(
                f"Failed to create character context: {str(e)}"
            )
    ...
    async def _check_circuit_breaker(self):
        ...
        if self._circuit_breaker["state"] == "OPEN":
            time_since_failure = now - self._circuit_breaker["last_failure_time"]  # type: ignore[operator]
            ...
            if time_since_failure > self._circuit_breaker["recovery_timeout"]:
                ...
                raise ServiceUnavailableError(
                    ...
                    f"Recovery in {remaining_time.total_seconds():.0f}s"
                )
    ...
    async def _record_failure(self):
        ...
        self._circuit_breaker["failure_count"] += 1  # type: ignore[operator]
        self._circuit_breaker["last_failure_time"] = datetime.now(timezone.utc)

        if (
            self._circuit_breaker["failure_count"]  # type: ignore[operator]
    ...
    async def _record_partial_failure(self):
        ...
        self._circuit_breaker["failure_count"] += 0.5  # type: ignore[operator]
        self._circuit_breaker["last_failure_time"] = datetime.now(timezone.utc)
    ...
    async def _security_check(self, character_id: str):
        ...
        if len(character_id) > 100:
            raise SecurityError(
                f"Character ID too long: {len(character_id)} chars"
            )
        ...
        if not safe_pattern.match(character_id):
            raise SecurityError(
                f"Invalid characters in character ID: {character_id}"
            )
        ...
        try:
            ...
            if not str(resolved_path).startswith(str(base_resolved)):
                raise SecurityError(
                    "Path resolution outside base directory detected"
                )
    ...
    async def _validate_context_integrity(
        ...
        if (
            character_context.memory_context
            and character_context.profile_context
        ):
            {
                rel.character_name
        ...
        if warnings:
            ...
            if (
                len(warnings) > 5
            ):  # Too many warnings indicate serious problems
                character_context.context_integrity = False
                raise ValidationError(
                    f"Critical data integrity issues: {warnings[:3]}"
                )
    ...
    async def _get_from_cache(
        ...
        if (
            not cache_time
            or (datetime.now(timezone.utc) - cache_time) > self.cache_ttl
        ):
            # Cache expired
    ...
    async def _store_in_cache(
        ...
        cache_entry = CharacterContext.model_validate(
            character_context.model_dump()
        )
    ...
    async def _generate_cache_key(
        ...
        for file_info in files_info:
            if file_info.loaded_successfully:
                ...
                file_hashes.append(
                    f"{file_info.file_path}:{file_info.load_timestamp}"
                )
    ...
    async def validate_character_directory_structure(
        ...
        try:
            ...
            if not character_dir.exists():
                validation_result[
                    "error"
                ] = "Character directory does not exist"
