...
class AwarenessState:
    ...
    def calculate_effective_alertness(self) -> AlertnessLevel:
        ...
        for modifier, strength in self.awareness_modifiers.items():
            if modifier in [AwarenessModifier.FATIGUE, AwarenessModifier.INJURY, 
                           AwarenessModifier.DISTRACTION, AwarenessModifier.DRUG_EFFECT]:
            ...
            elif modifier in [AwarenessModifier.CONFIDENCE, AwarenessModifier.TRAINING,
                             AwarenessModifier.EXPERIENCE, AwarenessModifier.MAGICAL_ENHANCEMENT]:
    ...
    def can_detect_stealth(self) -> bool:
        ...
        if self.attention_focus in [AttentionFocus.THREAT_SCANNING, AttentionFocus.ENVIRONMENTAL]:
            return True
        ...
        return effective_alertness in [AlertnessLevel.ALERT, AlertnessLevel.VIGILANT, AlertnessLevel.PARANOID]
    ...
    def is_surprised_by_combat(self) -> bool:
        ...
        if self.attention_focus == AttentionFocus.TASK_FOCUSED:
            return effective_alertness in [AlertnessLevel.DROWSY, AlertnessLevel.RELAXED]
    ...
    def with_focus_change(self, new_focus: AttentionFocus, 
                         new_target: Optional[str] = None) -> 'AwarenessState':
    ...
    def with_added_modifier(self, modifier: AwarenessModifier, 
                           strength: float) -> 'AwarenessState':
