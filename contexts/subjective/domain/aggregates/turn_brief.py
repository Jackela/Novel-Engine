...
from ..value_objects.awareness import AwarenessState, AlertnessLevel, AttentionFocus, AwarenessModifier
from ..events.subjective_events import (
    SubjectiveDomainEvent, TurnBriefCreated, TurnBriefUpdated, PerceptionRangeUpdated,
    NewPerceptionAcquired, KnowledgeRevealed, KnowledgeUpdated, AlertnessChanged,
    AttentionFocusChanged, ThreatDetected, ThreatLost, FogOfWarUpdated
)
...
class TurnBrief:
    ...
    @classmethod
    def create_for_entity(cls, entity_id: str, 
                         perception_capabilities: PerceptionCapabilities,
                         world_state_version: int,
                         initial_alertness: AlertnessLevel = AlertnessLevel.RELAXED) -> 'TurnBrief':
        """
    ...
    def update_perception_capabilities(self, new_capabilities: PerceptionCapabilities,
    ...
    def add_perception(self, subject: str, perception_type: PerceptionType,
                      visibility_level: VisibilityLevel, distance: float,
    ...
    def detect_threat(self, threat_subject: str, threat_type: str, threat_level: str,
                     confidence: float, detection_method: PerceptionType,
                     estimated_distance: Optional[float] = None) -> None:
        """
    ...
    def update_fog_of_war(self, newly_revealed: List[str], newly_concealed: List[str],
        ...
        affected_subjects = list(set(newly_revealed + newly_concealed + list(visibility_changes.keys())))
    ...
    def can_perceive_at_distance(self, distance: float, 
                                perception_type: Optional[PerceptionType] = None) -> VisibilityLevel:
        """
        ...
        if perception_type:
            ...
            if (self.awareness_state.attention_focus == AttentionFocus.TARGET_SPECIFIC and
                self.awareness_state.focus_target):
    ...
    def has_knowledge_about(self, subject: str, 
                           min_certainty: CertaintyLevel = CertaintyLevel.MINIMAL) -> bool:
        """
    ...
    def get_current_threats(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {subject: info for subject, info in self.known_threats.items() 
