...
from ..value_objects.perception_range import (
    PerceptionCapabilities, PerceptionType, VisibilityLevel
)
from ..value_objects.knowledge_level import (
    KnowledgeBase, KnowledgeItem, KnowledgeType, CertaintyLevel, KnowledgeSource
)
from ..value_objects.awareness import AwarenessState, AlertnessLevel
...
class IVisibilityCalculator(ABC):
    ...
    @abstractmethod
    def calculate_visibility(self, observer_position: Tuple[float, float, float],
                           target_position: Tuple[float, float, float],
                           perception_capabilities: PerceptionCapabilities,
                           awareness_state: AwarenessState,
                           environmental_conditions: Dict[str, Any]) -> Dict[PerceptionType, VisibilityLevel]:
        """
...
class BasicVisibilityCalculator(IVisibilityCalculator):
    ...
    def calculate_visibility(self, observer_position: Tuple[float, float, float],
        ...
        for perception_type, perception_range in perception_capabilities.perception_ranges.items():
            ...
            modified_range = PerceptionRange(
                ...
                accuracy_modifier=perception_range.accuracy_modifier * (1.0 + awareness_bonus * 0.5),
    ...
    def _get_environmental_modifier(self, perception_type: PerceptionType,
        ...
        light_level = environmental_conditions.get("light_level", "normal")  # "bright", "normal", "dim", "dark"
        weather = environmental_conditions.get("weather", "clear")  # "clear", "fog", "rain", "storm"
        terrain = environmental_conditions.get("terrain", "open")  # "open", "forest", "urban", "underground"
...
class FogOfWarService:
    ...
    def update_visible_subjects_for_turn_brief(
        ...
        if turn_brief.entity_id not in world_positions:
            self.logger.warning(f"Observer entity {turn_brief.entity_id} not found in world positions")
            return [], [], {}
    ...
    def filter_knowledge_by_reliability(
        ...
        for subject, items in knowledge_base.knowledge_items.items():
            ...
            for item in items:
                if (item.is_current(check_time) and 
                    item.get_reliability_score() >= min_reliability_score):
    ...
    def propagate_knowledge_between_entities(
        ...
        if (source_turn_brief.awareness_state.current_alertness == AlertnessLevel.UNCONSCIOUS or
            target_turn_brief.awareness_state.current_alertness == AlertnessLevel.UNCONSCIOUS):
            return []
        ...
        for subject, items in source_turn_brief.knowledge_base.knowledge_items.items():
            for item in items:
                ...
                if (item.is_current() and 
                    item.get_reliability_score() >= 0.5):  # Minimum for sharing
    ...
    def calculate_information_decay(
        ...
        closest_certainty = min(reverse_mapping.keys(), 
                               key=lambda x: abs(x - new_certainty_value))
