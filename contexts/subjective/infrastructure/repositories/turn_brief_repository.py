...
from ...domain.repositories.turn_brief_repository import (
    ITurnBriefRepository, RepositoryException, TurnBriefNotFoundException, 
...
from ...domain.value_objects.perception_range import (
    PerceptionCapabilities, PerceptionRange, PerceptionType
)
from ...domain.value_objects.knowledge_level import (
    KnowledgeBase, KnowledgeItem, KnowledgeType, CertaintyLevel, KnowledgeSource
)
from ...domain.value_objects.awareness import (
    AwarenessState, AlertnessLevel, AttentionFocus, AwarenessModifier
)

from ..persistence.subjective_models import (
    TurnBriefORM, KnowledgeItemORM, PerceptionRecordORM, ThreatAssessmentORM
)
...
class SQLAlchemyTurnBriefRepository(ITurnBriefRepository):
    ...
    def get_by_id(self, turn_brief_id: SubjectiveId) -> Optional[TurnBrief]:
        ...
        try:
            with self.session_factory() as session:
                orm_entity = session.query(TurnBriefORM).filter(
                    TurnBriefORM.turn_brief_id == turn_brief_id.value
                ).first()
            ...
            self.logger.error(f"Database error retrieving TurnBrief {turn_brief_id}: {e}")
    ...
    def get_by_entity_id(self, entity_id: str) -> Optional[TurnBrief]:
        ...
        try:
            ...
            self.logger.error(f"Database error retrieving TurnBrief for entity {entity_id}: {e}")
    ...
    def save(self, turn_brief: TurnBrief) -> None:
        ...
        try:
            with self.session_factory() as session:
                ...
                existing_orm = session.query(TurnBriefORM).filter(
            ...
            self.logger.error(f"Database error saving TurnBrief {turn_brief.turn_brief_id}: {e}")
    ...
    def delete(self, turn_brief_id: SubjectiveId) -> bool:
        ...
        try:
            with self.session_factory() as session:
                deleted_count = session.query(TurnBriefORM).filter(
                    TurnBriefORM.turn_brief_id == turn_brief_id.value
                ).delete()
                ...
                if deleted_count > 0:
                    ...
                    self.logger.warning(f"TurnBrief {turn_brief_id} not found for deletion")
                    return False
    ...
    def find_by_world_state_version(self, world_state_version: int) -> List[TurnBrief]:
        ...
        try:
            with self.session_factory() as session:
                orm_entities = session.query(TurnBriefORM).filter(
                    TurnBriefORM.world_state_version == world_state_version
                ).all()
                
                return [self._map_orm_to_domain(orm_entity, session) for orm_entity in orm_entities]
                
        except SQLAlchemyError as e:
            self.logger.error(f"Database error finding TurnBriefs by world state version {world_state_version}: {e}")
            raise RepositoryException(f"Failed to find TurnBriefs by world state version: {e}")
    ...
    def find_by_alertness_level(self, alertness: AlertnessLevel) -> List[TurnBrief]:
        ...
        try:
            with self.session_factory() as session:
                orm_entities = session.query(TurnBriefORM).filter(
                    TurnBriefORM.current_alertness == alertness.value
                ).all()
                
                return [self._map_orm_to_domain(orm_entity, session) for orm_entity in orm_entities]
                
        except SQLAlchemyError as e:
            self.logger.error(f"Database error finding TurnBriefs by alertness level {alertness}: {e}")
            raise RepositoryException(f"Failed to find TurnBriefs by alertness level: {e}")
    ...
    def find_stale_turn_briefs(self, cutoff_time: datetime) -> List[TurnBrief]:
        ...
        try:
            with self.session_factory() as session:
                orm_entities = session.query(TurnBriefORM).filter(
                    TurnBriefORM.updated_at < cutoff_time
                ).all()
    ...
    def find_entities_with_knowledge_about(self, subject: str, 
        ...
        try:
            with self.session_factory() as session:
                ...
                results = session.query(TurnBriefORM.entity_id).join(
                    KnowledgeItemORM
                ).filter(
                    and_(
                        KnowledgeItemORM.subject == subject,
                        func.coalesce(KnowledgeItemORM.expires_at, func.now()) > func.now()
                    )
                ).distinct().all()
                ...
                for result in results:
                    ...
                    best_certainty = session.query(KnowledgeItemORM.certainty_level).filter(
                        and_(
                            KnowledgeItemORM.turn_brief_id == session.query(TurnBriefORM.turn_brief_id).filter(
                                TurnBriefORM.entity_id == entity_id
                            ).scalar(),
                            KnowledgeItemORM.subject == subject
                        )
                    ...
                    if best_certainty and best_certainty[0]:
                        try:
                            ...
                            if certainty_order.get(cert_level, 0) >= min_certainty_value:
            ...
            self.logger.error(f"Database error finding entities with knowledge about {subject}: {e}")
    ...
    def find_entities_in_perception_range_of_location(self, location_id: str, 
        ...
        try:
            ...
            self.logger.error(f"Database error finding entities that can perceive location {location_id}: {e}")
            raise RepositoryException(f"Failed to find entities in perception range: {e}")
        ...
        try:
            with self.session_factory() as session:
                ...
                for orm_entity in orm_entities:
                    ...
                    if perception_type.value in perception_capabilities.get('perception_ranges', {}):
                        entity_ids.append(orm_entity.entity_id)
                
                return entity_ids
                
        except SQLAlchemyError as e:
            self.logger.error(f"Database error finding entities with perception type {perception_type}: {e}")
            raise RepositoryException(f"Failed to find entities with perception type: {e}")
    ...
    def get_knowledge_sharing_candidates(self, entity_id: str, knowledge_type: KnowledgeType,
        ...
        try:
            with self.session_factory() as session:
                ...
                candidates = session.query(TurnBriefORM.entity_id).filter(
                    and_(
                        TurnBriefORM.entity_id != entity_id,
            ...
            self.logger.error(f"Database error finding knowledge sharing candidates for {entity_id}: {e}")
            raise RepositoryException(f"Failed to find knowledge sharing candidates: {e}")
    ...
    def count_active_turn_briefs(self, cutoff_time: Optional[datetime] = None) -> int:
        ...
        try:
            ...
            with self.session_factory() as session:
                return session.query(func.count(TurnBriefORM.id)).filter(
                    TurnBriefORM.updated_at > cutoff_time
                ).scalar()
    ...
    def get_entities_needing_updates(self, world_state_version: int) -> List[str]:
        ...
        try:
            with self.session_factory() as session:
                results = session.query(TurnBriefORM.entity_id).filter(
                    TurnBriefORM.world_state_version < world_state_version
                ).all()
    ...
    def batch_update_world_state_version(self, entity_ids: List[str], 
        ...
        try:
            with self.session_factory() as session:
                updated_count = session.query(TurnBriefORM).filter(
                    TurnBriefORM.entity_id.in_(entity_ids)
                ).update({
                ...
                self.logger.info(f"Batch updated {updated_count} TurnBriefs to world state version {new_version}")
                return updated_count
                
        except SQLAlchemyError as e:
            self.logger.error(f"Database error in batch update: {e}")
            raise RepositoryException(f"Failed to batch update world state version: {e}")
    ...
    def cleanup_expired_turn_briefs(self, expiration_time: datetime) -> int:
        ...
        try:
            with self.session_factory() as session:
                deleted_count = session.query(TurnBriefORM).filter(
                    TurnBriefORM.updated_at < expiration_time
                ).delete()
    ...
    def _map_domain_to_orm(self, turn_brief: TurnBrief) -> TurnBriefORM:
        ...
        return TurnBriefORM(
            ...
            awareness_modifiers=self._serialize_awareness_modifiers(turn_brief.awareness_state.awareness_modifiers),
            fatigue_level=turn_brief.awareness_state.fatigue_level,
            stress_level=turn_brief.awareness_state.stress_level,
            perception_capabilities=self._serialize_perception_capabilities(turn_brief.perception_capabilities),
            visible_subjects=self._serialize_visible_subjects(turn_brief.visible_subjects),
    ...
    def _update_orm_from_domain(self, orm_entity: TurnBriefORM, turn_brief: TurnBrief, session: Session) -> None:
        ...
        orm_entity.current_alertness = turn_brief.awareness_state.current_alertness.value
        ...
        orm_entity.awareness_modifiers = self._serialize_awareness_modifiers(turn_brief.awareness_state.awareness_modifiers)
        ...
        orm_entity.perception_capabilities = self._serialize_perception_capabilities(turn_brief.perception_capabilities)
        orm_entity.visible_subjects = self._serialize_visible_subjects(turn_brief.visible_subjects)
    ...
    def _map_orm_to_domain(self, orm_entity: TurnBriefORM, session: Session) -> TurnBrief:
        ...
        knowledge_items_orm = session.query(KnowledgeItemORM).filter(
            KnowledgeItemORM.turn_brief_id == orm_entity.turn_brief_id
        ).all()
        ...
        awareness_state = AwarenessState(
            ...
            awareness_modifiers=self._deserialize_awareness_modifiers(orm_entity.awareness_modifiers),
            fatigue_level=orm_entity.fatigue_level,
            stress_level=orm_entity.stress_level
        )
        
        # Create perception capabilities (simplified)
        perception_capabilities = self._deserialize_perception_capabilities(orm_entity.perception_capabilities)
        
        # Create visible subjects
        visible_subjects = self._deserialize_visible_subjects(orm_entity.visible_subjects)
    ...
    def _serialize_awareness_modifiers(self, modifiers: Dict[AwarenessModifier, float]) -> Dict[str, float]:
        """Serialize awareness modifiers for JSON storage."""
        return {modifier.value: value for modifier, value in modifiers.items()}
    
    def _deserialize_awareness_modifiers(self, data: Optional[Dict[str, Any]]) -> Dict[AwarenessModifier, float]:
    ...
    def _serialize_perception_capabilities(self, capabilities: PerceptionCapabilities) -> Dict[str, Any]:
    ...
    def _deserialize_perception_capabilities(self, data: Optional[Dict[str, Any]]) -> PerceptionCapabilities:
        ...
        return PerceptionCapabilities(
            ...
            focused_perception_multiplier=data.get("focused_perception_multiplier", 1.5)
        )
    
    def _serialize_visible_subjects(self, subjects: Dict[str, Any]) -> Dict[str, str]:
        """Serialize visible subjects for JSON storage."""
        return {subject: str(level.value) if hasattr(level, 'value') else str(level) 
                for subject, level in subjects.items()}
    
    def _deserialize_visible_subjects(self, data: Optional[Dict[str, Any]]) -> Dict[str, Any]:
