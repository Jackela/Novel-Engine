...
from ...domain.repositories.turn_brief_repository import (
    ITurnBriefRepository, TurnBriefNotFoundException, ConcurrencyException
)
...
class CreateTurnBriefCommandHandler:
    ...
    def handle(self, command: CreateTurnBriefCommand) -> SubjectiveId:
        ...
        try:
            ...
            self.logger.info(f"Created TurnBrief {turn_brief.turn_brief_id} for entity {command.entity_id}")
            return turn_brief.turn_brief_id
            
        except Exception as e:
            self.logger.error(f"Failed to create TurnBrief for entity {command.entity_id}: {e}")
...
class UpdatePerceptionCapabilitiesCommandHandler:
    ...
    def handle(self, command: UpdatePerceptionCapabilitiesCommand) -> None:
        ...
        try:
            ...
            raise InvalidCommandException(f"Invalid UpdatePerceptionCapabilitiesCommand: {e}")
        ...
        try:
            turn_brief.update_perception_capabilities(
                command.new_perception_capabilities,
                command.change_reason
            )
            
            self.repository.save(turn_brief)
            
            self.logger.info(f"Updated perception capabilities for entity {command.entity_id}")
            
        except ConcurrencyException:
            raise
        except Exception as e:
            self.logger.error(f"Failed to update perception capabilities for entity {command.entity_id}: {e}")
            raise SubjectiveCommandHandlerException(f"Perception capabilities update failed: {e}")
    ...
    def handle(self, command: UpdateAwarenessStateCommand) -> None:
        ...
        try:
            ...
            new_awareness = AwarenessState(
                ...
                current_alertness=command.new_alertness or current_awareness.current_alertness,
                attention_focus=command.new_attention_focus or current_awareness.attention_focus,
                focus_target=command.new_focus_target if command.new_focus_target is not None else current_awareness.focus_target,
                awareness_modifiers=command.awareness_modifiers if command.awareness_modifiers is not None else current_awareness.awareness_modifiers,
                fatigue_level=command.fatigue_level if command.fatigue_level is not None else current_awareness.fatigue_level,
                stress_level=command.stress_level if command.stress_level is not None else current_awareness.stress_level
            )
            ...
            self.logger.error(f"Failed to update awareness state for entity {command.entity_id}: {e}")
            raise SubjectiveCommandHandlerException(f"Awareness state update failed: {e}")


class AddPerceptionCommandHandler:
    """Handler for AddPerceptionCommand."""
    
    def __init__(self, repository: ITurnBriefRepository, fog_of_war_service: FogOfWarService):
        self.repository = repository
        ...
        try:
            ...
            if command.observer_position and command.target_position:
                visibility_results = self.fog_of_war_service.calculate_visibility_between_positions(
                    turn_brief, command.observer_position, command.target_position,
                    command.environmental_conditions
                )
                visibility_level = visibility_results.get(command.perception_type, 
                                                        turn_brief.can_perceive_at_distance(command.distance, command.perception_type))
            else:
                # Use TurnBrief's own perception calculation
                visibility_level = turn_brief.can_perceive_at_distance(command.distance, command.perception_type)
            ...
            self.logger.debug(f"Added {command.perception_type.value} perception of {command.perceived_subject} "
                            f"for entity {command.entity_id}")
            
        except ConcurrencyException:
            raise
        except Exception as e:
            self.logger.error(f"Failed to add perception for entity {command.entity_id}: {e}")
...
class RevealKnowledgeCommandHandler:
    ...
    def handle(self, command: RevealKnowledgeCommand) -> None:
        ...
        try:
            ...
            self.logger.info(f"Revealed knowledge about {command.knowledge_item.subject} "
                           f"to entity {command.entity_id}")
            
        except ConcurrencyException:
            raise
        except Exception as e:
            self.logger.error(f"Failed to reveal knowledge to entity {command.entity_id}: {e}")
...
class DetectThreatCommandHandler:
    ...
    def handle(self, command: DetectThreatCommand) -> None:
        ...
        try:
            ...
            self.logger.info(f"Entity {command.entity_id} detected {command.threat_level} "
                           f"threat: {command.threat_subject}")
            
        except ConcurrencyException:
            raise
        except Exception as e:
            self.logger.error(f"Failed to detect threat for entity {command.entity_id}: {e}")
        ...
        try:
            ...
            newly_revealed, newly_concealed, visibility_changes = \
                self.fog_of_war_service.update_visible_subjects_for_turn_brief(
                    turn_brief, command.world_positions, command.environmental_conditions
                )
            
            # Update the TurnBrief with the changes
            turn_brief.update_fog_of_war(
                newly_revealed, newly_concealed, visibility_changes, command.update_reason
            )
            
            self.repository.save(turn_brief)
            
            self.logger.debug(f"Updated fog of war for entity {command.entity_id}: "
                            f"{len(newly_revealed)} revealed, {len(newly_concealed)} concealed")
            
        except ConcurrencyException:
            raise
        except Exception as e:
            self.logger.error(f"Failed to update fog of war for entity {command.entity_id}: {e}")
...
class ShareInformationCommandHandler:
    ...
    def handle(self, command: ShareInformationCommand) -> int:
        ...
        if not source_turn_brief:
            raise EntityNotFoundException(f"Source entity {command.source_entity_id} not found")
        ...
        for target_entity_id in command.target_entity_ids:
            try:
                ...
                if not target_turn_brief:
                    self.logger.warning(f"Target entity {target_entity_id} not found, skipping")
                    continue
                ...
                propagatable_knowledge = self.fog_of_war_service.propagate_knowledge_between_entities(
                    ...
                    knowledge_types=[item.knowledge_type for item in command.knowledge_items],
                ...
                for knowledge_item in command.knowledge_items:
                    ...
                    if any(pk.subject == knowledge_item.subject for pk in propagatable_knowledge):
                ...
                self.logger.debug(f"Shared information from {command.source_entity_id} "
                                f"to {target_entity_id}")
                
            except Exception as e:
                self.logger.error(f"Failed to share information to {target_entity_id}: {e}")
                continue
        
        self.logger.info(f"Successfully shared information from {command.source_entity_id} "
...
class SubjectiveCommandHandlerRegistry:
    ...
    def __init__(self, repository: ITurnBriefRepository, fog_of_war_service: FogOfWarService):
        ...
        self.update_perception_handler = UpdatePerceptionCapabilitiesCommandHandler(repository)
        self.update_awareness_handler = UpdateAwarenessStateCommandHandler(repository)
        self.add_perception_handler = AddPerceptionCommandHandler(repository, fog_of_war_service)
        ...
        self.update_fog_of_war_handler = UpdateFogOfWarCommandHandler(repository, fog_of_war_service)
        self.share_information_handler = ShareInformationCommandHandler(repository, fog_of_war_service)
    ...
    def handle_update_perception_capabilities(self, command: UpdatePerceptionCapabilitiesCommand) -> None:
        """Handle UpdatePerceptionCapabilitiesCommand."""
        self.update_perception_handler.handle(command)
    
    def handle_update_awareness_state(self, command: UpdateAwarenessStateCommand) -> None:
