...
from ...domain.services.llm_provider import ILLMProvider, LLMRequest, LLMResponse, LLMResponseStatus
...
from ...infrastructure.policies.rate_limiting import IRateLimiter, RateLimitResult
from ...infrastructure.policies.cost_tracking import ICostTracker, CostEntry
from ...infrastructure.policies.retry_fallback import IRetryPolicy, RetryResult
...
class ProviderRouter:
    ...
    def _is_provider_healthy(self, provider_name: str) -> bool:
        ...
        return health.get('available', False) and health.get('consecutive_failures', 0) < 3
...
class ExecuteLLMService:
        ...
        try:
            ...
            if config.enable_caching and self._cache_service:
                ...
                if cached_response:
                    ...
                    if config.track_costs and self._cost_tracker:
                        await self._record_cached_usage(request, cached_response, config)
            ...
            if config.enforce_rate_limits and self._rate_limiter:
                ...
                if provider:
                    ...
                    result.rate_limit_check_time = asyncio.get_event_loop().time() - rate_limit_start
            ...
            result.provider_response_time = asyncio.get_event_loop().time() - provider_start
    ...
    async def get_execution_stats_async(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': self._execution_stats['successful_requests'] / total_requests,
            ...
            'rate_limit_rate': self._execution_stats['rate_limited_requests'] / total_requests,
            'budget_exceeded_requests': self._execution_stats['budget_exceeded_requests'],
            'budget_exceeded_rate': self._execution_stats['budget_exceeded_requests'] / total_requests
        }
    ...
    def _estimate_request_cost(self, request: LLMRequest) -> Decimal:
        ...
        input_cost = Decimal(str(estimated_input_tokens)) * model_id.cost_per_input_token
