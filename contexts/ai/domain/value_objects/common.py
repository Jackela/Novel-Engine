...
class ProviderId:
    ...
    def create_anthropic(cls, api_version: str = "1.0.0", region: str = "US") -> 'ProviderId':
        ...
        return cls(
            ...
            api_version=api_version,
            region=region,
            metadata={"official": True, "conversation_models": True, "safety_focused": True}
        )
...
class ModelId:
    ...
    def __post_init__(self):
        ...
        if self.capabilities and not all(isinstance(cap, ModelCapability) for cap in self.capabilities):
...
class TokenBudget:
    ...
    @classmethod
    def create_daily_budget(cls, budget_id: str, daily_tokens: int, cost_limit: Decimal = None) -> 'TokenBudget':
    ...
    @classmethod 
    def create_project_budget(cls, project_id: str, total_tokens: int, cost_limit: Decimal) -> 'TokenBudget':
    ...
    def __eq__(self, other) -> bool:
        ...
        if not isinstance(other, TokenBudget):
            return False
        return (
            self.budget_id == other.budget_id and
            self.allocated_tokens == other.allocated_tokens and
            self.consumed_tokens == other.consumed_tokens and
            self.reserved_tokens == other.reserved_tokens and
            self.cost_limit == other.cost_limit and
            ...
            self.rollover_enabled == other.rollover_enabled and
