...
class UpdateWorldStateResult:
    ...
    def __init__(self, success: bool, world_state: Optional[WorldState] = None,
...
class UpdateWorldStateUC:
    ...
    async def execute(self, command: ApplyWorldDelta) -> UpdateWorldStateResult:
        ...
        try:
            ...
            operations_applied += await self._apply_entity_operations(world_state, command.entity_operations)
            ...
            logger.info(f"Successfully applied {operations_applied} operations to world {command.world_state_id}")
    ...
    async def _apply_entity_operations(self, world_state: WorldState, 
                                     operations: List[EntityOperation]) -> int:
        """
    ...
    async def _apply_remove_entity(self, world_state: WorldState, operation: EntityOperation) -> None:
    ...
    async def _apply_update_entity(self, world_state: WorldState, operation: EntityOperation) -> None:
    ...
    async def _apply_environment_operation(self, world_state: WorldState,
    ...
    async def _apply_time_operation(self, world_state: WorldState,
                                   operation: TimeOperation) -> None:
        """
        ...
        world_state.advance_time(
            new_time=operation.new_time,
            reason=operation.reason
        )
    
    async def _apply_snapshot_operation(self, world_state: WorldState,
    ...
    async def _apply_reset_operation(self, world_state: WorldState,
                                    operation: ResetOperation) -> None:
        """
    ...
    async def retry_with_latest_version(self, command: ApplyWorldDelta, 
                                       max_retries: int = 3) -> UpdateWorldStateResult:
        """
        ...
        for attempt in range(max_retries + 1):
            ...
            if attempt == max_retries:
                logger.warning(f"Command failed after {max_retries} retries due to concurrency conflicts")
                return result
            
            logger.info(f"Retrying command due to concurrency conflict (attempt {attempt + 1}/{max_retries})")
