...
class PostgresWorldStateRepository(IWorldStateRepository):
    ...
    async def save(self, world_state: WorldState) -> WorldState:
        ...
        try:
            with get_db_session() as session:
                ...
                existing_model = session.query(WorldStateModel).filter(
                ...
                if existing_model:
                    ...
                    self.logger.info(f"Updated world state {world_state.id} to version {world_state.version}")
    ...
    async def get_by_id(self, world_state_id: str) -> Optional[WorldState]:
        ...
        try:
            with get_db_session() as session:
                model = session.query(WorldStateModel).filter(
                    and_(
    ...
    async def exists(self, world_state_id: str) -> bool:
        ...
        try:
            with get_db_session() as session:
                count = session.query(WorldStateModel).filter(
                    and_(
                        WorldStateModel.id == uuid.UUID(world_state_id),
                        WorldStateModel.is_deleted == False
                    )
            ...
            self.logger.error(f"Database error checking existence of world state {world_state_id}: {e}")
            raise RepositoryException(f"Failed to check world state existence: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error checking existence of world state {world_state_id}: {e}")
    ...
    async def get_all(self, offset: int = 0, limit: int = 100) -> List[WorldState]:
        ...
        try:
            with get_db_session() as session:
                models = session.query(WorldStateModel).filter(
                    WorldStateModel.is_deleted == False
                ).order_by(WorldStateModel.created_at.desc()).offset(offset).limit(limit).all()
    ...
    async def find_by_criteria(self, criteria: Dict[str, Any], 
        ...
        try:
            with get_db_session() as session:
                ...
                models = query.order_by(WorldStateModel.created_at.desc()).offset(offset).limit(limit).all()
                return [model.to_domain_aggregate() for model in models]
    ...
    async def find_entities_by_coordinates(self, world_state_id: str, 
        ...
        try:
            ...
            self.logger.error(f"Error finding entities by coordinates for world {world_state_id}: {e}")
    ...
    async def get_version(self, world_state_id: str, version: int) -> Optional[WorldState]:
        ...
        try:
            with get_db_session() as session:
                version_model = session.query(WorldStateVersionModel).filter(
                    and_(
            ...
            self.logger.error(f"Database error retrieving version {version} of world state {world_state_id}: {e}")
            raise RepositoryException(f"Failed to retrieve world state version: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error retrieving version {version} of world state {world_state_id}: {e}")
    ...
    async def get_version_history(self, world_state_id: str, 
        ...
        try:
            with get_db_session() as session:
                versions = session.query(WorldStateVersionModel).filter(
                    WorldStateVersionModel.world_state_id == uuid.UUID(world_state_id)
                ).order_by(WorldStateVersionModel.version_number.desc()).limit(limit).all()
            ...
            self.logger.error(f"Database error retrieving version history for world state {world_state_id}: {e}")
            raise RepositoryException(f"Failed to retrieve version history: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error retrieving version history for world state {world_state_id}: {e}")
    ...
    async def rollback_to_version(self, world_state_id: str, version: int) -> WorldState:
        ...
        try:
            ...
            if not target_world_state:
                raise EntityNotFoundException(f"Version {version} not found for world state {world_state_id}")

            # Get current world state
            current_world_state = await self.get_by_id_or_raise(world_state_id)

            # Create new version with rollback data
            target_world_state.version = current_world_state.version  # Will be incremented in save()
            ...
            self.logger.error(f"Error rolling back world state {world_state_id} to version {version}: {e}")
    ...
    async def create_snapshot(self, world_state_id: str, 
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                self.logger.info(f"Created snapshot '{snapshot_name}' for world state {world_state_id}")
                return str(snapshot_model.id)

        except EntityNotFoundException:
            raise
        except SQLAlchemyError as e:
            self.logger.error(f"Database error creating snapshot for world state {world_state_id}: {e}")
            raise RepositoryException(f"Failed to create snapshot: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error creating snapshot for world state {world_state_id}: {e}")
    ...
    async def restore_from_snapshot(self, world_state_id: str, 
        ...
        try:
            with get_db_session() as session:
                snapshot_model = session.query(WorldStateSnapshotModel).filter(
                    and_(
                ...
                if not snapshot_model:
                    raise EntityNotFoundException(f"Snapshot {snapshot_id} not found for world state {world_state_id}")

                # Reconstruct world state from snapshot
                snapshot_data = snapshot_model.snapshot_data['world_state']
                restored_world_state = self._reconstruct_world_state_from_version_data(snapshot_data)
    ...
    async def list_snapshots(self, world_state_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            with get_db_session() as session:
                snapshots = session.query(WorldStateSnapshotModel).filter(
                    WorldStateSnapshotModel.world_state_id == uuid.UUID(world_state_id)
                ).order_by(WorldStateSnapshotModel.created_at.desc()).all()
            ...
            self.logger.error(f"Database error listing snapshots for world state {world_state_id}: {e}")
            raise RepositoryException(f"Failed to list snapshots: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error listing snapshots for world state {world_state_id}: {e}")
    ...
    async def delete_snapshot(self, snapshot_id: str) -> bool:
        ...
        try:
            with get_db_session() as session:
                snapshot = session.query(WorldStateSnapshotModel).filter(
                    WorldStateSnapshotModel.id == uuid.UUID(snapshot_id)
                ).first()
        ...
        try:
            with get_db_session() as session:
                ...
                for world_state in world_states:
                    ...
                    validation_errors = model_to_save.validate()
                    if validation_errors:
                        raise RepositoryException(f"Validation failed for {world_state.id}: {'; '.join(validation_errors)}")
    ...
    async def optimize_storage(self, world_state_id: str) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as session:
                ...
                version_count = session.query(WorldStateVersionModel).filter(
                    WorldStateVersionModel.world_state_id == uuid.UUID(world_state_id)
                ).count()

                if version_count > 50:
                    # Delete old versions, keeping the latest 50
                    old_versions = session.query(WorldStateVersionModel).filter(
                        WorldStateVersionModel.world_state_id == uuid.UUID(world_state_id)
                    ).order_by(WorldStateVersionModel.version_number.desc()).offset(50)
                    
                    deleted_count = old_versions.count()
                    for version in old_versions:
                        session.delete(version)
                    
                    optimization_results['optimizations_performed'].append(f"Cleaned {deleted_count} old versions")
                ...
                optimization_results['processing_time_ms'] = int((end_time - start_time).total_seconds() * 1000)
            ...
            self.logger.error(f"Database error optimizing storage for world state {world_state_id}: {e}")
            raise RepositoryException(f"Failed to optimize storage: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error optimizing storage for world state {world_state_id}: {e}")
        ...
        try:
            with get_db_session() as session:
                if world_state_id:
                    ...
                    snapshot_count = session.query(WorldStateSnapshotModel).filter(
                        WorldStateSnapshotModel.world_state_id == uuid.UUID(world_state_id)
                    ).count()
                    ...
                    # Global statistics
                    total_worlds = session.query(WorldStateModel).filter(
                        WorldStateModel.is_deleted == False
                    ).count()

                    total_entities = 0
                    active_worlds = session.query(WorldStateModel).filter(
                        and_(
                            WorldStateModel.is_deleted == False,
    ...
    async def get_events_since(self, world_state_id: str, since_version: int) -> List[Dict[str, Any]]:
        ...
        try:
            with get_db_session() as session:
                ...
                ).order_by(WorldStateVersionModel.version_number.asc()).all()
            ...
            self.logger.error(f"Database error getting events since version {since_version} for world {world_state_id}: {e}")
            raise RepositoryException(f"Failed to get events since version: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error getting events since version {since_version} for world {world_state_id}: {e}")
    ...
    async def _create_version_entry(self, session: Session, world_state: WorldState, 
                                   previous_model: Optional[WorldStateModel]) -> None:
    ...
    def _reconstruct_world_state_from_version_data(self, version_data: Dict[str, Any]) -> WorldState:
        ...
        try:
            ...
            world_state = WorldState(
                ...
                world_time=datetime.fromisoformat(version_data['world_time']) if isinstance(version_data['world_time'], str) else version_data['world_time'],
                ...
                created_at=datetime.fromisoformat(version_data['created_at']) if isinstance(version_data['created_at'], str) else version_data['created_at'],
                updated_at=datetime.fromisoformat(version_data['updated_at']) if isinstance(version_data['updated_at'], str) else version_data['updated_at'],
