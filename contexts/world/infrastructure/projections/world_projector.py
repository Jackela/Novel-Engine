...
from core_platform.messaging.event_bus import DomainEvent, EventHandler, get_event_bus
from core_platform.monitoring.metrics import ProjectorMetrics
from ...domain.events.world_events import WorldStateChanged, WorldChangeType
...
class WorldProjector:
    ...
    async def handle_world_state_changed(self, event: WorldStateChanged, context: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            self.logger.debug(f"Processing world event: {event.event_type} for aggregate {event.aggregate_id}")
            ...
            self.logger.debug(f"Successfully processed event {event.event_id} in {processing_time:.3f}s")
        ...
        try:
            with get_db_session() as session:
                ...
                if read_model:
                    ...
                    self.logger.debug(f"Added entity {event.payload.get('affected_entity_id')} to read model")
        ...
        try:
            with get_db_session() as session:
                ...
                if read_model:
                    ...
                    self.logger.debug(f"Removed entity {event.payload.get('affected_entity_id')} from read model")
        ...
        try:
            with get_db_session() as session:
                ...
                if read_model:
                    ...
                    self.logger.debug(f"Moved entity {event.payload.get('affected_entity_id')} in read model")
        ...
        try:
            with get_db_session() as session:
                ...
                if read_model:
                    ...
                    self.logger.debug(f"Updated entity {event.payload.get('affected_entity_id')} in read model")
    ...
    async def _handle_state_reset(self, event: WorldStateChanged) -> None:
        ...
        try:
            with get_db_session() as session:
                ...
                deleted_count = session.query(WorldSliceReadModel).filter(
                    WorldSliceReadModel.world_state_id == UUID(world_state_id)
                ).delete()
        ...
        try:
            with get_db_session() as session:
                ...
                if read_model:
                    ...
                    read_model.world_time = datetime.fromisoformat(new_world_time) if isinstance(new_world_time, str) else new_world_time
                    read_model.last_event_timestamp = datetime.now()
                    read_model.projection_version += 1
                    
                    session.commit()
                    
                    self.logger.debug(f"Advanced time for world {world_state_id} to {new_world_time}")
    ...
    async def _get_or_create_read_model(self, session: Session, world_state_id: str) -> Optional[WorldSliceReadModel]:
        """
        ...
        try:
            ...
            self.logger.error(f"Error getting or creating read model for world {world_state_id}: {e}")
            return None
    ...
    async def _handle_failed_event(self, event: WorldStateChanged, error_message: str) -> None:
        ...
        """
        self.logger.error(f"Event processing failed - Event ID: {event.event_id}, Error: {error_message}")
