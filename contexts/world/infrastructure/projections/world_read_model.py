...
from sqlalchemy import Column, String, DateTime, Integer, Text, JSON, Index, Float, Boolean, func, ARRAY
...
class WorldSliceReadModel(BaseModel):
    ...
    entity_type_counts = Column(JSON, nullable=False, default=dict)  # {"character": 5, "object": 10}
    ...
    def get_entities_in_area(self, center_x: float, center_y: float, 
                            radius: float, entity_types: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_entities_in_bounds(self, min_x: float, max_x: float, 
    ...
    def get_world_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'spatial_bounds': {
                ...
                'min_z': self.min_z,
                'max_z': self.max_z
            } if self.min_x is not None else None,
            'environment_summary': self.environment_summary,
            'last_updated': self.last_event_timestamp.isoformat() if self.last_event_timestamp else None,
            'version': {
    ...
    def create_from_world_state(cls, world_state_data: Dict[str, Any]) -> 'WorldSliceReadModel':
        ...
        return cls(
            ...
            world_time=datetime.fromisoformat(world_state_data['world_time']) if isinstance(world_state_data.get('world_time'), str) else world_state_data.get('world_time', datetime.now()),
        ...
        if change_type == 'entity_added' and new_state:
            ...
            if entity_id in self.all_entities:
                ...
                if entity_type and entity_type in self.entities_by_type:
                    ...
                    self.entities_by_type[entity_type] = [
                        e for e in self.entities_by_type[entity_type] 
                        if e.get('id') != entity_id
                    ]
                    self.entity_type_counts[entity_type] = max(0, self.entity_type_counts[entity_type] - 1)
