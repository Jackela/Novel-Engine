...
# from contexts.world.infrastructure.projections import (
#     WorldSliceReadModel, 
#     WorldProjector, 
#     get_world_projector,
#     initialize_world_projector
# )
# from contexts.world.application.queries import (
#     GetWorldSlice,
#     GetWorldSummary,
#     GetEntitiesInArea,
#     execute_query
# )
# from contexts.world.domain.events.world_events import WorldStateChanged
...
def example_summary_queries():
    ...
    expected_summary = {
        ...
        'spatial_bounds': {
            'min_x': -100.0, 'max_x': 500.0,
            'min_y': -50.0, 'max_y': 300.0
        },
...
def example_query_patterns():
    ...
    patterns = [
        ("Spatial Range Queries", "Find entities within geographic area", "O(n) with spatial bounds pre-filtering"),
        ("Entity Type Filtering", "Get all entities of specific type", "O(1) lookup via pre-grouped data"),
        ("World State Dashboards", "Summary statistics and counts", "O(1) via pre-computed aggregations"),
        ("Real-time Game Queries", "Fast entity lookups for rendering", "Sub-50ms via denormalized JSON"),
        ("Search and Discovery", "Find worlds by name/content", "Full-text search on indexed content"),
        ("Administrative Queries", "System monitoring and analytics", "Projection health and statistics")
    ]
...
def example_architecture_benefits():
    ...
    benefits = [
        ("Read/Write Separation", "Queries never impact write performance or consistency"),
        ...
        ("Eventual Consistency", "High availability without blocking on write operations"),
        ("Performance Isolation", "Complex analytics don't affect real-time operations"), 
