...
class M9OrchestrationValidator:
    ...
    async def _validate_component_structure(self) -> List[Dict[str, Any]]:
        ...
        if saga_path.exists():
            ...
            has_compensation_methods = "plan_compensation" in content and "execute_compensation" in content
            has_rollback_logic = "rollback" in content.lower()
            
            results.append({
                'test_name': 'Saga Coordinator Structure',
                'passed': has_coordinator_class and has_compensation_methods and has_rollback_logic,
    ...
    async def _validate_documentation(self) -> List[Dict[str, Any]]:
        ...
        if arch_path.exists():
            ...
            results.append({
                ...
                'passed': has_pipeline_description and has_saga_description and content_length > 1000,
    ...
    async def _generate_validation_report(self, validation_summary: Dict[str, Any]) -> None:
        ...
        for category in validation_summary['test_categories']:
            ...
            print(f"{status} {category['category']}: {category['tests_passed']}/{category['tests_run']} passed")
                ...
                if category['tests_failed'] > 0:
                    ...
                    for result in category.get('results', []):
                        if not result.get('passed', True):
                            print(f"    • {result.get('test_name', 'Unknown')}: {result.get('details', 'No details')}")
            print()
