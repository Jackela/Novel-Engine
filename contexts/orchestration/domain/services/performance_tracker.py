...
class PerformanceTracker:
    ...
    def track_turn_start(self, turn: Turn) -> None:
        ...
        recent_starts = len(
            [
                ...
                if (
                    entry.get("event_type") == "turn_started"
                    and entry.get("timestamp", timestamp) > hour_ago
                )
            ]
        )
    ...
    def track_phase_execution(
        ...
        tracking_data = {
            ...
            "error_details": (
                phase_result.error_details if not phase_result.was_successful() else None
            ),
        }
    ...
    def track_compensation_execution(
        ...
        total_compensation_cost = sum(
            [float(action.actual_cost or 0) for action in compensation_actions]
        )
    ...
    def get_performance_summary(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
        ...
        turn_completions = [
            entry for entry in recent_metrics if entry.get("event_type") == "turn_completed"
        ]
        ...
        # Success rate
        successful_turns = len([entry for entry in turn_completions if entry.get("success", False)])
        ...
        return {
            ...
            "baseline_comparison": {
                "execution_time_vs_target": avg_execution_time
                / self.performance_baselines["target_execution_time_ms"],
                "success_rate_vs_target": success_rate
                / self.performance_baselines["target_success_rate"],
                "throughput_vs_target": throughput_per_hour
                / self.performance_baselines["target_throughput_per_hour"],
                "cost_vs_target": cost_metrics.get("avg_cost_per_turn", 0)
                / self.performance_baselines["target_cost_per_turn"],
            },
            "recommendations": self._generate_performance_recommendations(turn_completions, trends),
        }
    ...
    def get_anomaly_alerts(self) -> List[Dict[str, Any]]:
        ...
        recent_completions = [
            ...
            for entry in self.metrics_history
            if (
                entry.get("event_type") == "turn_completed"
                and entry.get("timestamp", datetime.min) > recent_time
            )
        ]
    ...
    def _calculate_turn_metrics(
        ...
        return {
            ...
            "cost_analysis": {
                ...
                "cost_per_event": float(pipeline_result.total_ai_cost)
                / max(1, pipeline_result.total_events_processed),
    ...
    def _calculate_phase_metrics(
        self, phase_result: PhaseResult, execution_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            / max(0.001, execution_time_ms / 1000),  # events per second
        }

    def _calculate_phase_performance_score(
        self, execution_time_ms: float, events_processed: int, success: bool
    ) -> float:
    ...
    def _update_real_time_metrics(
        ...
        self.real_time_metrics["average_response_time_ms"] = (
            alpha * current_time + (1 - alpha) * self.real_time_metrics["average_response_time_ms"]
        )

        # Update success rate (last 100 turns)
        recent_completions = [
            entry
            for entry in self.metrics_history[-100:]
    ...
    def _generate_optimization_recommendations(
        ...
        efficiency_score = performance_metrics.get("quality_metrics", {}).get(
            "performance_score", 1.0
        )
        if efficiency_score < 0.7:
            recommendations.append("Optimize event processing algorithms to improve throughput")
    ...
    def _analyze_phase_performance(self, recent_metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        phase_entries = [
            entry for entry in recent_metrics if entry.get("event_type") == "phase_executed"
        ]

        for phase_type in PhaseType:
            phase_data = [
                entry for entry in phase_entries if entry.get("phase_type") == phase_type.value
            ]

            if phase_data:
                execution_times = [entry["execution_time_ms"] for entry in phase_data]
                success_rate = len([e for e in phase_data if e.get("success", False)]) / len(
                    phase_data
                )
