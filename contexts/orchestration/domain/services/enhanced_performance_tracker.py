...
class EnhancedPerformanceTracker(PerformanceTracker):
    ...
    def track_turn_start(self, turn: Turn) -> None:
        ...
        participants_count = (
            len(turn.configuration.participants)
            if hasattr(turn.configuration, "participants")
            else 0
        )
        ...
        self.active_turns_tracking[turn_id] = {
            ...
            "configuration": {
                ...
                "estimated_cost": (
                    turn.configuration.get_estimated_ai_cost()
                    if hasattr(turn.configuration, "get_estimated_ai_cost")
                    else Decimal("0")
                ),
            },
        }
    ...
    def track_turn_completion(
        ...
        total_ai_cost = (
            pipeline_result.total_ai_cost
            if hasattr(pipeline_result, "total_ai_cost")
            else Decimal("0")
        )
        ...
        if hasattr(pipeline_result, "phase_results"):
            for phase_result in pipeline_result.phase_results:
                phase_name = (
                    ...
                    if hasattr(phase_result, "phase_type")
                    else "unknown"
                )
                phase_results[phase_name] = {
                    ...
                    "ai_cost": (
                        float(phase_result.get_ai_cost())
