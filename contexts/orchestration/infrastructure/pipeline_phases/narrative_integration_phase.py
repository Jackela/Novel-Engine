...
class NarrativeIntegrationPhase(BasePhaseImplementation):
    ...
    async def _execute_phase_implementation(
        ...
        try:
            ...
            story_arcs_updated = await self._update_story_arcs(context, turn_events, narrative_results)
            ...
            integration_events = await self._generate_narrative_events(
                context, len(turn_events), narrative_content_generated, story_arcs_updated
            )
            ...
            context.record_performance_metric('narrative_content_generated', float(narrative_content_generated))
            ...
            success_rate = narrative_content_generated / max(1, len(narrative_context.get('active_perspectives', ['omniscient'])))
            
            return PhaseResult(
                success=success_rate > 0.5 and failed_generations == 0,  # Success if >50% perspectives generated and no failures
    ...
    def _validate_preconditions(self, context: PhaseExecutionContext) -> None:
        ...
        if context.configuration.narrative_analysis_depth not in valid_depths:
            raise ValueError(f"Invalid narrative depth: {context.configuration.narrative_analysis_depth}")
    ...
    async def _generate_narrative_content(
        ...
        try:
            ...
            return {
                ...
                'ai_cost': ai_response.get('tokens_used', 0) * 0.002  # Higher cost for narrative generation
            }
    ...
    async def _build_narrative_prompt(
        ...
        previous_narrative = narrative_context.get('current_story_state', {}).get('recent_narrative', '')
        ...
        prompt = prompt_template.format(
            ...
            previous_narrative=previous_narrative[:500] if previous_narrative else 'Beginning of story',
    ...
    def _organize_events_for_narrative(
        ...
        for event in turn_events:
            ...
            if event_type in ['world_time_advanced', 'environment_changed', 'entity_updated']:
                event_categories['world_events'].append(event)
            elif event_type in ['agent_agreement', 'conflict_resolution', 'cooperation_success']:
    ...
    async def _build_character_context(
        ...
        return f"Character: {character_data.get('name', character_id)}, " \
               f"Personality: {character_data.get('personality', {}).get('traits', [])}, " \
               f"Current goals: {character_data.get('goals', [])[:2]}"  # Top 2 goals
    ...
    def _validate_narrative_content(
        ...
        if event_types and not any(
            event_type.replace('_', ' ') in content.lower() or
            event_type.split('_')[0] in content.lower()
