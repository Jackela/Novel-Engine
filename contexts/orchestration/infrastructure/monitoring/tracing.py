...
from opentelemetry.sdk.trace.sampling import TraceIdRatioBased, ParentBased, StaticSampler, Decision
...
class IntelligentSampler:
    ...
    def __init__(self, default_rate: float = 0.1):
        ...
        self.high_cost_sampler = TraceIdRatioBased(0.5)  # 50% for expensive operations
        self.slow_operation_sampler = TraceIdRatioBased(0.8)  # 80% for slow operations
...
class NovelEngineTracer:
    ...
    def start_turn_span(
        ...
        span = self.tracer.start_span(
            name="novel_engine.turn_execution",
        ...
        span.set_attribute("turn.narrative_depth", configuration.get('narrative_analysis_depth', 'standard'))
        span.set_attribute("turn.max_execution_time_ms", configuration.get('max_execution_time_ms', 60000))
        
        # Add user context if available (for security tracing)
        if user_context:
            span.set_attribute("user.id", user_context.get('user_id', 'anonymous'))
    ...
    def start_phase_span(
        ...
        if turn_participants:
            span.set_attribute("turn.participants_count", turn_participants)
    ...
    def record_phase_result(
        ...
        if not success:
            span.set_status(trace.Status(trace.StatusCode.ERROR, error_details or "Phase execution failed"))
            if error_details:
    ...
    def record_turn_result(
        ...
        span.set_attribute("turn.execution_time_seconds", execution_time_seconds)
        span.set_attribute("turn.total_cost", total_ai_cost)
        span.set_attribute("turn.phases_completed", ",".join(phases_completed))
        span.set_attribute("turn.completion_percentage", len(phases_completed) / 5.0)
        
        if not success:
            span.set_status(trace.Status(trace.StatusCode.ERROR, error_details or "Turn execution failed"))
            if error_details:
...
def initialize_tracing(config: Optional[NovelEngineTracingConfig] = None) -> NovelEngineTracer:
    """
