...
from ...domain.value_objects import (
    TurnId, TurnConfiguration, PhaseType, PhaseStatus, TurnState,
    CompensationAction, CompensationType
)
from ...domain.services import SagaCoordinator, PipelineOrchestrator, EnhancedPerformanceTracker
from ...infrastructure.monitoring import NovelEngineTracer, NovelEngineTracingConfig, initialize_tracing
from ...infrastructure.pipeline_phases import (
    BasePhaseImplementation, PhaseExecutionContext, PhaseResult,
    WorldUpdatePhase, SubjectiveBriefPhase, InteractionOrchestrationPhase,
    EventIntegrationPhase, NarrativeIntegrationPhase
)
...
class TurnOrchestrator:
    ...
    async def execute_turn(
        ...
        try:
            ...
            overall_success = (
                successful_phases == len(pipeline_phases) and
                len(phases_completed) == len(pipeline_phases)
            )
            ...
            if overall_success:
                ...
                logger.warning(f"Turn {turn_id} completed with failures in {execution_time_ms:.1f}ms")
        ...
        try:
            ...
            if self.tracer and phase_span:
                self.tracer.record_phase_result(
                    ...
                    ai_cost=float(phase_result.ai_usage.get('total_cost', 0)) if phase_result.ai_usage else 0.0,
                    ai_requests=phase_result.ai_usage.get('request_count', 0) if phase_result.ai_usage else 0,
                    error_details=None
                )
                phase_span.end()
    ...
    async def _execute_saga_compensation(
        ...
        logger.info(f"Saga compensation completed for turn {turn.turn_id}: "
    ...
    async def _gather_performance_metrics(
        ...
        for phase_type, result in phase_results.items():
            ...
            metrics[f'{phase_prefix}_execution_time_ms'] = result.performance_metrics.get('execution_time_ms', 0.0)
        ...
        for phase_type, result in phase_results.items():
            ...
            if phase_type == PhaseType.INTERACTION_ORCHESTRATION:
                ...
                if 'interaction_summary' in result.metadata:
                    metadata[phase_type.value]['session_results'] = result.metadata['interaction_summary']
    ...
    def _record_phase_metrics(
        ...
        if self.performance_monitoring_enabled:
            self.performance_tracker.record_phase_metrics(
                ...
                events_processed=phase_result.events_processed,
                events_generated=len(phase_result.events_generated),
                ai_cost=phase_result.ai_usage.get('total_cost', 0) if phase_result.ai_usage else 0
            )
    ...
    async def validate_turn_preconditions(
        ...
        if not participants:
            ...
            errors.append(f"Too many participants: {len(participants)} > {configuration.max_participants}")
    ...
    def get_orchestrator_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'active_turns': self.performance_tracker.get_active_turn_count() if self.performance_monitoring_enabled else 0
        }