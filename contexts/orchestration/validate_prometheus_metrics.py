...
def test_prometheus_metrics_collector():
    ...
    try:
        ...
        value_checks = {
            ...
            'phase_names': all(phase in metrics_data for phase in ['world_update', 'subjective_brief', 'interaction_orchestration'])
        }
        ...
        all_checks_passed = (
            all(core_kpis_found.values()) and 
            all(value_checks.values()) and 
            all(additional_metrics.values()) and
...
def test_enhanced_performance_tracker():
    ...
    try:
        ...
        health_checks = {
            ...
            'prometheus_integration': health_status.get('enhanced_features', {}).get('prometheus_metrics', False),
            'business_kpi_tracking': health_status.get('enhanced_features', {}).get('business_kpi_tracking', False)
        }
        
        print(f"✅ Health status: {health_checks}")
        
        all_checks_passed = (
            all(kpi_checks.values()) and
            all(prometheus_methods.values()) and
