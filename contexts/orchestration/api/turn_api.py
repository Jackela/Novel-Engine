...
from ..domain.value_objects import TurnId, TurnConfiguration, PhaseType
from ..domain.services import EnhancedPerformanceTracker
from ..infrastructure.monitoring import (
    PrometheusMetricsCollector, PrometheusMiddleware,
    OpenTelemetryMiddleware, setup_fastapi_tracing, initialize_tracing
)
...
class TurnExecutionRequest(BaseModel):
    ...
    configuration: Optional[Dict[str, Any]] = Field(
        None,
        description="Optional turn configuration parameters"
    )
    turn_id: Optional[str] = Field(
        None,
        description="Optional turn ID (generated if not provided)"
    )
    async_execution: bool = Field(
        False,
        description="Whether to execute turn asynchronously"
    )
...
app.add_middleware(
    ...
    group_paths=True,
    ignored_paths=["/health", "/metrics", "/docs", "/redoc", "/openapi.json", "/favicon.ico"],
...
setup_fastapi_tracing(
    ...
    excluded_urls=["/health", "/metrics", "/docs", "/redoc", "/openapi.json", "/favicon.ico"],
    ...
    try:
        ...
        if request.async_execution:
            ...
            background_tasks.add_task(
                ...
                turn_id, request.participants, configuration
            )
    ...
    try:
        ...
        if turn_id in active_turns:
            ...
            return TurnStatusResponse(
                ...
                execution_time_ms=detailed_status.get('execution_time_ms') if detailed_status else None,
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve turn status"
        )
...
async def list_active_turns() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to list active turns"
        )
...
async def cleanup_turn(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to cleanup turn resources"
        )
...
def _convert_to_response(result) -> TurnExecutionResponse:
    ...
    for phase_type, phase_result in result.phase_results.items():
        phase_results_dict[phase_type.value] = PhaseResultSummary(
            ...
            error_message=phase_result.error_details.get('message') if phase_result.error_details else None
        )
...
async def get_business_kpis():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Error retrieving business KPI data"
        )
