    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        response = await client.post(
            ...
            json={
                ...
                "ai_quality_specs": [
                    {"criteria": s.quality_criteria, "samples": getattr(s, "test_samples", [])}
                    for s in scenarios
                    if s.test_type == TestType.AI_QUALITY
                ],
                "strategy": "ADAPTIVE",
        ...
        if response.status_code == 200:
            ...
            for phase in result.get("phase_results", []):
                ...
                print(
                    f"   {status_icon} {phase.get('phase')}: {phase.get('status')} (Score: {phase.get('score', 0):.2f})"
                )
