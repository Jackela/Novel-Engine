...
async def comprehensive_framework_test():
    ...
    async with httpx.AsyncClient(timeout=60.0):
        ...
        print(
            f"    {'✅' if perf_test_passed else '❌'} 性能测试: {'达标' if perf_test_passed else '未达标'}"
        )
        ...
        print(
            f"    {'✅' if workflow_passed else '❌'} 工作流执行: {'完成' if workflow_passed else '失败'}"
        )
    ...
    categories_passed = sum(
        1 for cat in test_results["test_categories"] if cat["passed"]
    )
    ...
    capabilities = {
        "功能测试能力": (
            100
            if any(
                c["category"] == "功能测试" and c["passed"]
                for c in test_results["test_categories"]
            )
            else 0
        ),
        "AI质量评估能力": (
            100
            if any(
                c["category"] == "AI质量评估" and c["passed"]
                for c in test_results["test_categories"]
            )
            else 0
        ),
