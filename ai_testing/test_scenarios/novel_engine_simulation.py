...
async def comprehensive_framework_test():
    ...
    async with httpx.AsyncClient(timeout=60.0):
        ...
        functional_test_passed = (
            len(dialogue_result["dialogue"]) > 0
            and dialogue_result["quality_score"] > 0.7
            and dialogue_result["generation_time_ms"] < 1000
        )

        print(
            f"    {'✅' if functional_test_passed else '❌'} 对话生成: {'成功' if functional_test_passed else '失败'}"
        )
        ...
        quality_test_passed = quality_result["overall_score"] > 0.6 and all(
            0 <= score <= 1 for score in quality_result.values()
        )

        print(
            f"    {'✅' if quality_test_passed else '❌'} 质量评估: {'有效' if quality_test_passed else '无效'}"
        )
        ...
        test_results["test_categories"].append(
            {
                ...
                "tests": [
                    {"name": "内容质量评估", "passed": quality_test_passed, "metrics": quality_result}
                ],
            }
        )
        ...
        # 验证性能测试
        perf_test_passed = (
            perf_result["error_rate"] < 0.05
            and perf_result["avg_response_time_ms"] < 500
            and perf_result["requests_per_second"] > 10
        )
        ...
        test_results["test_categories"].append(
            {
                ...
                "tests": [{"name": "负载测试", "passed": perf_test_passed, "metrics": perf_result}],
            }
        )
        ...
        error_handling_passed = all(
            s["detected"] and s["handled_correctly"] for s in error_scenarios
        )

        print(
            f"    {'✅' if error_handling_passed else '❌'} 错误处理: {'完善' if error_handling_passed else '不完善'}"
        )
    ...
    capabilities = {
        "功能测试能力": 100
        if any(c["category"] == "功能测试" and c["passed"] for c in test_results["test_categories"])
        else 0,
        "AI质量评估能力": 100
        if any(c["category"] == "AI质量评估" and c["passed"] for c in test_results["test_categories"])
        else 0,
        "性能测试能力": 100
        if any(c["category"] == "性能测试" and c["passed"] for c in test_results["test_categories"])
        else 0,
        "集成测试能力": 100
        if any(c["category"] == "集成测试" and c["passed"] for c in test_results["test_categories"])
        else 0,
        "错误处理检测": 100
        if any(c["category"] == "错误处理" and c["passed"] for c in test_results["test_categories"])
        else 0,
    }
