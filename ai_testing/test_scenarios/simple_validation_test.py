...
async def test_framework_effectiveness():
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   {'✅' if test_passed else '❌'} Orchestrator状态: {data.get('status')}"
                )
            ...
            print(f"   ❌ 错误: {str(e)}")
            results["tests"].append(
                {"name": "框架健康检查", "passed": False, "error": str(e)}
            )
        ...
        try:
            ...
            print(f"   ❌ 错误: {str(e)}")
            results["tests"].append(
                {"name": "API测试能力", "passed": False, "error": str(e)}
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   {'✅' if service_count > 0 else '❌'} 发现 {service_count} 个服务"
                )
            ...
            print(f"   ❌ 错误: {str(e)}")
            results["tests"].append(
                {"name": "服务发现", "passed": False, "error": str(e)}
            )
        ...
        try:
            ...
            print(f"   ❌ 错误: {str(e)}")
            results["tests"].append(
                {"name": "综合测试能力", "passed": False, "error": str(e)}
            )
    ...
    # 保存结果
    report_path = Path(
        "ai_testing/validation_reports/framework_effectiveness_test.json"
    )
