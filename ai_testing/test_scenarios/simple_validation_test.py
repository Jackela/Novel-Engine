...
async def test_framework_effectiveness():
    ...
    results = {"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "tests": [], "summary": {}}
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                results["tests"].append(
                    {"name": "框架健康检查", "passed": False, "error": f"HTTP {response.status_code}"}
                )
        except Exception as e:
            print(f"   ❌ 错误: {str(e)}")
            results["tests"].append({"name": "框架健康检查", "passed": False, "error": str(e)})
        ...
        try:
            ...
            print(f"   ❌ 错误: {str(e)}")
            results["tests"].append({"name": "API测试能力", "passed": False, "error": str(e)})
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                all_healthy = all(
                    s.get("status") in ["healthy", "ready", "degraded"] for s in services.values()
                )
                ...
                results["tests"].append(
                    {"name": "服务发现", "passed": False, "error": f"HTTP {response.status_code}"}
                )
        except Exception as e:
            print(f"   ❌ 错误: {str(e)}")
            results["tests"].append({"name": "服务发现", "passed": False, "error": str(e)})
        ...
        try:
            ...
            response = await client.post(
                "http://localhost:8000/test/comprehensive", json=test_payload
            )

            if response.status_code == 200:
                result = response.json()
                has_results = all(
                    key in result for key in ["overall_passed", "overall_score", "phase_results"]
                )
                ...
                results["tests"].append(
                    {"name": "综合测试能力", "passed": False, "error": f"HTTP {response.status_code}"}
                )
        except Exception as e:
            print(f"   ❌ 错误: {str(e)}")
            results["tests"].append({"name": "综合测试能力", "passed": False, "error": str(e)})
        ...
        try:
            ...
            if test_response.status_code == 200:
                ...
                print(
                    f"   {'✅' if error_detected else '❌'} 错误检测: {'正确检测到故障' if error_detected else '未能检测故障'}"
                )
                results["tests"].append(
                    {
                        "name": "错误检测能力",
            ...
            results["tests"].append(
                {"name": "错误检测能力", "passed": True, "details": {"exception_caught": True}}
            )
