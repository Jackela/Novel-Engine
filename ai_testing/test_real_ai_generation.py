...
def test_real_ai_generation():
    ...
    try:
        ...
        has_template_patterns = any(
            any(indicator in d["dialogue"] for indicator in template_indicators) for d in dialogues
        )
        ...
        success_criteria = [
            ...
            all(len(d["dialogue"]) > 5 for d in dialogues),  # All dialogues have content
        ]
        ...
        print(f"Content quality: {'✅' if all(len(d['dialogue']) > 5 for d in dialogues) else '❌'}")
        ...
        report_file = (
            f"reports/real_ai_generation_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
