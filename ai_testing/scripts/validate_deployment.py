...
class DeploymentValidator:
    ...
    def __init__(self, base_url: str = "http://localhost"):
        ...
        self.services = [
            ServiceConfig(
                "Master Orchestrator", f"{base_url}:8000", 8000, required=True
            ),
            ServiceConfig(
            ...
            ServiceConfig(
                "Notification Service", f"{base_url}:8005", 8005, required=True
            ),
        ]
    ...
    async def _test_api_testing_service(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            response = await client.post(
                f"{self.base_url}:8002/test/single", params=test_params
            )
            duration_ms = int((time.time() - start_time) * 1000)
    ...
    async def _test_browser_automation_service(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.validation_results.append(
                    ValidationResult(
                        ...
                        error_message=(
                            ...
                            else f"Service status: {browser_status}"
                        ),
                    )
                )
                ...
                if browser_status in ["healthy", "ready"]:
                    console.print(
                        f"    ✅ Browser Automation: [green]Service ready[/green] ({duration_ms}ms)"
                    )
                else:
                    console.print(
                        f"    ⚠️  Browser Automation: [yellow]Service not ready[/yellow] ({duration_ms}ms)"
                    )
            else:
                self.validation_results.append(
                    ValidationResult(
                ...
                console.print(
                    f"    ❌ Browser Automation: [red]Failed with HTTP {response.status_code}[/red]"
                )
    ...
    async def _test_notification_service(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.validation_results.append(
                    ValidationResult(
                        ...
                        error_message=(
                            ...
                            else f"Service status: {notification_status}"
                        ),
                    )
                )
                ...
                if notification_status in ["healthy", "ready"]:
                    console.print(
                        f"    ✅ Notification Service: [green]Service ready[/green] ({duration_ms}ms)"
                    )
                else:
                    console.print(
                        f"    ⚠️  Notification Service: [yellow]Service not ready[/yellow] ({duration_ms}ms)"
                    )
            else:
                self.validation_results.append(
                    ValidationResult(
                ...
                console.print(
                    f"    ❌ Notification Service: [red]Failed with HTTP {response.status_code}[/red]"
                )
    ...
    async def _test_minimal_comprehensive_workflow(self, client: httpx.AsyncClient):
        ...
        try:
                ...
                "api_test_specs": [
                    {
                        ...
                        "headers": {
                            "User-Agent": "AI-Testing-Framework-Validation/1.0"
                        },
            ...
            response = await client.post(
                f"{self.base_url}:8000/test/comprehensive", json=test_request
            )
            duration_ms = int((time.time() - start_time) * 1000)
            ...
            if response.status_code == 200:
                ...
                if overall_passed:
                    console.print(
                        f"    ✅ E2E Workflow: [green]Success (Score: {overall_score:.3f})[/green] ({duration_ms}ms)"
                    )
                else:
                    console.print(
                        f"    ⚠️  E2E Workflow: [yellow]Partial success (Score: {overall_score:.3f})[/yellow] ({duration_ms}ms)"
                    )
            else:
                self.validation_results.append(
                    ValidationResult(
                ...
                console.print(
                    f"    ❌ E2E Workflow: [red]Failed with HTTP {response.status_code}[/red]"
                )
        ...
        for service in self.services:
            ...
            try:
                ...
                if duration_ms < 1000:  # < 1 second is good
                    console.print(
                        f"    ✅ {service.name}: [green]{duration_ms}ms[/green]"
                    )
                elif duration_ms < 3000:  # < 3 seconds is acceptable
                    console.print(
                        f"    ⚠️  {service.name}: [yellow]{duration_ms}ms[/yellow]"
                    )
                else:  # > 3 seconds is slow
                    console.print(
                        f"    ❌ {service.name}: [red]{duration_ms}ms (slow)[/red]"
                    )
            ...
            self.validation_results.append(
                ValidationResult(
                    ...
                    error_message=(
                        ...
                        else f"Average response time too slow: {avg_response_time:.1f}ms"
                    ),
                )
            )
    ...
    async def _test_concurrent_requests(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            self.validation_results.append(
                ValidationResult(
                    ...
                    error_message=(
                        None
                        if success_rate >= 0.8
                        else f"Low success rate: {success_rate:.1%}"
                    ),
                )
            )
            ...
            if success_rate >= 0.8:
                console.print(
                    f"    ✅ Concurrent handling: [green]{success_rate:.1%} success rate[/green] ({duration_ms}ms)"
                )
            else:
                console.print(
                    f"    ⚠️  Concurrent handling: [yellow]{success_rate:.1%} success rate[/yellow] ({duration_ms}ms)"
                )
    ...
    async def _test_information_exposure(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.validation_results.append(
                    ValidationResult(
                        ...
                        error_message=(
                            ...
                            else f"Sensitive information exposed: {', '.join(exposed_sensitive)}"
                        ),
                    )
                )
                ...
                if len(exposed_sensitive) == 0:
                    console.print(
                        "    ✅ Information exposure: [green]No sensitive data exposed[/green]"
                    )
                else:
                    console.print(
                        f"    ⚠️  Information exposure: [yellow]Sensitive fields found: {', '.join(exposed_sensitive)}[/yellow]"
                    )
            else:
                self.validation_results.append(
                    ValidationResult(
                ...
                console.print(
                    "    ❌ Information exposure: [red]Health endpoint error[/red]"
                )
    ...
    async def _test_cors_configuration(self, client: httpx.AsyncClient):
        ...
        try:
            ...
            self.validation_results.append(
                ValidationResult(
                    ...
                    error_message=(
                        ...
                        else f"CORS issues: {', '.join(cors_issues)}"
                    ),
                )
            )
            ...
            if cors_properly_configured and len(cors_issues) == 0:
                console.print(
                    "    ✅ CORS configuration: [green]Properly configured[/green]"
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="AI Testing Framework Deployment Validation"
    )
    parser.add_argument(
        "--base-url",
