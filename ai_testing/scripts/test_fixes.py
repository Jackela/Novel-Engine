...
async def validate_deployment_fixes():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if status in ["healthy", "degraded"]:
                    print(f"   ✅ Orchestrator health: {status} (HTTP {response.status_code})")
                    results.append(("Orchestrator Health", True, f"Status: {status}"))
                else:
                    print(f"   ❌ Orchestrator health: {status} (unexpected status)")
                    results.append(("Orchestrator Health", False, f"Unexpected status: {status}"))
            else:
                print(f"   ❌ Orchestrator health: HTTP {response.status_code}")
                results.append(("Orchestrator Health", False, f"HTTP {response.status_code}"))
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if status in ["healthy", "degraded"]:
                    ...
                    results.append(
                        ("Notification Service", True, f"Status: {status}, Channels: {channels}")
                    )
                else:
                ...
                results.append(("Notification Service", False, f"HTTP {response.status_code}"))
        except Exception as e:
        ...
        try:
            ...
            response = await client.post("http://localhost:8002/test/single", params=test_params)
            ...
            if response.status_code == 200:
                ...
                if test_passed:
                    ...
                    results.append(
                        ("API Testing Functionality", True, "Service working, test failed")
                    )
            else:
                print(f"   ❌ API Testing service: HTTP {response.status_code}")
                results.append(("API Testing Functionality", False, f"HTTP {response.status_code}"))
        except Exception as e:
        ...
        try:
            ...
            if success_rate >= 0.8:
                print(f"   ✅ Concurrent handling: {success_rate:.0%} success ({successful}/5)")
                results.append(("Concurrent Handling", True, f"{success_rate:.0%} success"))
            else:
                print(f"   ❌ Concurrent handling: {success_rate:.0%} success ({successful}/5)")
                results.append(("Concurrent Handling", False, f"{success_rate:.0%} success"))
        except Exception as e:
        ...
        try:
            ...
            response = await client.post(
                "http://localhost:8000/test/comprehensive", json=test_request, timeout=60.0
            )
            ...
            if response.status_code == 200:
                ...
                if overall_passed or overall_score > 0:
                    ...
                    results.append(("E2E Workflow", True, f"Score: {overall_score:.2f}"))
                else:
                    print(f"   ⚠️  E2E Workflow: Partial success (Score: {overall_score:.2f})")
                    results.append(("E2E Workflow", False, f"Score: {overall_score:.2f}"))
            else:
