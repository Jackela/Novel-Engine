...
class TestServiceHealthChecks:
    ...
    async def test_all_services_health(self, http_client, wait_for_services):
        ...
        response = await http_client.get(
            f"{SERVICE_URLS['orchestrator']}/services/health"
        )
        assert response.status_code == 200
        ...
        for service_name in [
            ...
            assert service_health["status"] in [
                "healthy",
                "ready",
                "degraded",
                "starting",
...
class TestComprehensiveOrchestration:
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_test_status_tracking(
...
class TestResultsAggregation:
    ...
    async def test_health_aggregation(self, http_client, wait_for_services):
        ...
        response = await http_client.get(
            f"{SERVICE_URLS['results-aggregation']}/health"
        )
        assert response.status_code == 200
...
async def test_full_system_integration():
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        assert "phase_results" in result
        assert (
            len(result["phase_results"]) >= 2
        )  # At least API testing and Integration phases
