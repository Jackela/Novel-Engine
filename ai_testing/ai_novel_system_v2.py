...
class EnhancedNovelGenerator:
    ...
    def create_characters(self) -> List[Character]:
        ...
        for char in characters:
            self.dialogue_engine.register_character(char.name, char.personality, char.speech_style)
    ...
    def generate_story_blueprint(self, target_length: int = 60) -> StoryBlueprint:
        ...
        blueprint = self.story_architect.design_story_structure(self.characters, target_length)
            ...
            for char in self.characters:
                if char.name == arc.character_name:
                    char.arc_description = (
                        f"{arc.initial_state} → {arc.transformation} → {arc.final_state}"
                    )
        ...
        self.event_orchestrator = DynamicEventOrchestrator(blueprint, self.dialogue_engine)
    ...
    def generate_story_events(self) -> List[StoryEvent]:
        ...
        for i in range(self.story_blueprint.target_length):
            ...
            orchestrated_event = self.event_orchestrator.generate_next_event(
                self.characters, current_location
            )
    ...
    def _generate_thought(self, context_event: StoryEvent) -> Optional[StoryEvent]:
        ...
        if context_event.tension_level > 0.7:
            thoughts = ["这比预想的更加危险...", "时间不多了，必须做出选择", "我能感受到命运的重量"]
        # Medium tension thoughts
        elif context_event.tension_level > 0.4:
            thoughts = ["线索开始汇聚成完整的图景", "也许答案一直在我们面前", "这种感觉...似曾相识"]
        # Low tension thoughts
        else:
            thoughts = ["在宁静中，真理更加清晰", "每一步都在改变未来", "我们的相遇绝非偶然"]
...
class ImprovedLiteraryWriter:
    ...
    def transform_to_novel(
        self, events: List[StoryEvent], characters: List[Character], blueprint: StoryBlueprint
    ) -> str:
        ...
        for stage, stage_event_list in stage_events.items():
            if stage_event_list:
                chapter = self._write_structured_chapter(
                    chapter_num, stage, stage_event_list, characters, blueprint
                )
...
async def generate_enhanced_novel():
    ...
    data = {
        ...
        "blueprint": {
            ...
            "plot_points": [
                {"stage": p.stage.value, "description": p.description, "tension": p.tension_level}
