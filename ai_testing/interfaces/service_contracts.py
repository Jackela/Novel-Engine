...
class UITestSpec(BaseModel):
    ...
    # Performance
    performance_metrics: List[str] = Field(default_factory=lambda: ["FCP", "LCP", "CLS"])
...
class ITestOrchestrator(ABC):
    ...
    @abstractmethod
    async def create_test_plan(self, scenarios: List[TestScenario], context: TestContext) -> str:
...
class IAPITesting(ABC):
    ...
    @abstractmethod
    async def execute_api_test(self, test_spec: APITestSpec, context: TestContext) -> TestResult:
...
class IAIQualityAssessment(ABC):
    ...
    @abstractmethod
    async def assess_quality(self, test_spec: AIQualitySpec, context: TestContext) -> TestResult:
    ...
    @abstractmethod
    async def generate_quality_report(self, results: List[TestResult]) -> Dict[str, Any]:
...
class QualityAssessmentEvent(BaseModel):
    ...
    event_type: Literal["assessment_completed", "threshold_breached", "pattern_detected"]
    assessment_id: str
...
def create_test_context(
    session_id: str, user_id: Optional[str] = None, environment: str = "test", **metadata
) -> TestContext:
    ...
    return TestContext(
        ...
        metadata={"created_by": "ai_testing_framework", "framework_version": "1.0.0", **metadata},
    )
