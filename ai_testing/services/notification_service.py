    ...
    async def send(self, notification: Notification) -> bool:
        ...
        try:
                ...
                "attachments": [
                    {
                        ...
                        "fields": [
                            {"title": "Message", "value": notification.content, "short": False}
                        ],
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(self.webhook_url, json=slack_payload, timeout=30.0)
...
class FileChannelHandler(ChannelHandler):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.output_directory = Path(config.get("output_directory", "ai_testing/notifications"))
        self.filename_pattern = config.get("filename_pattern", "notifications_{date}.log")
    ...
    async def send(self, notification: Notification) -> bool:
        ...
        try:
            ...
            priority = notification.priority.value
            log_entry = (
                f"[{timestamp}] [{priority}] {notification.subject}: {notification.content}\n"
            )
...
class AlertDetector:
    ...
    async def _evaluate_rule(self, rule: NotificationRule, test_result: TestResult) -> bool:
        """Evaluate if rule should trigger for test result"""

        # Check service filter
        if rule.service_names:
            service_name = (
                test_result.execution_id.split("_")[0] if "_" in test_result.execution_id else ""
            )
        ...
        if AlertType.PERFORMANCE_DEGRADATION in rule.alert_types:
            if rule.max_response_time_ms and test_result.duration_ms > rule.max_response_time_ms:
                return True
    ...
    async def _create_alert_from_result(
        ...
        alert = Alert(
            ...
            source_service=test_result.execution_id.split("_")[0]
            ...
            current_values={"score": test_result.score, "duration_ms": test_result.duration_ms},
            threshold_values={
...
class NotificationService(INotificationService):
    ...
    def _initialize_channel_handlers(self):
        ...
        if email_config.get("enabled", False):
            self.channel_handlers[NotificationChannel.EMAIL] = EmailHandler(email_config)

        # Slack handler
        slack_config = self.config.get("slack", {})
        if slack_config.get("enabled", False):
            self.channel_handlers[NotificationChannel.SLACK] = SlackHandler(slack_config)

        # Webhook handler
        webhook_config = self.config.get("webhook", {})
        if webhook_config.get("enabled", False):
            self.channel_handlers[NotificationChannel.WEBHOOK] = WebhookHandler(webhook_config)
    ...
    async def send_alert(
        self, alert_type: str, message: str, priority: str = "medium", recipients: List[str] = None
    ) -> str:
        """Send custom alert"""
    ...
    async def send_system_notification(
        self, title: str, message: str, recipients: List[str], channels: List[str] = None
    ) -> List[str]:
    ...
    async def _create_notification(
        self, alert: Alert, rule: NotificationRule, channel: NotificationChannel, recipient: str
    ) -> Notification:
        ...
        # Generate content
        subject, content, formatted_content = await self._generate_content(alert, template)
...
This is an automated notification from Novel-Engine AI Testing System.
    ...
    async def _generate_content(
        ...
        if not template:
            template = self._get_default_template(alert.alert_type, NotificationChannel.EMAIL)
        ...
        while True:
            try:
                if self.pending_notifications:
                    ...
                    delivery_tasks = [
                        self._deliver_notification(notification) for notification in batch
                    ]
    ...
    async def _deliver_notification(self, notification: Notification):
        ...
        try:
            ...
            if not handler:
                ...
                notification.error_message = f"No handler for channel {notification.channel.value}"
                return
            ...
            if success:
                ...
                if notification.retry_count < notification.max_retries:
                    ...
                    await asyncio.sleep(30 * notification.retry_count)  # Exponential backoff
                    self.pending_notifications.append(notification)
    ...
    async def send_quality_alert(self, alert_type: str, details: Dict[str, Any]) -> bool:
        """Send quality threshold alert"""
        try:
            # Determine priority based on alert severity
            severity = details.get("severity", "medium")
            priority = (
                NotificationPriority.HIGH if severity == "high" else NotificationPriority.MEDIUM
            )
    ...
    async def send_performance_alert(
        ...
        try:
            ...
            priority = NotificationPriority.HIGH if ratio > 2.0 else NotificationPriority.MEDIUM
...
async def send_custom_alert(
    alert_type: str, message: str, priority: str = "medium", recipients: Optional[List[str]] = None
):
    """Send custom alert"""
    service: NotificationService = app.state.notification_service
...
async def send_notification(
    title: str, message: str, recipients: List[str], channels: Optional[List[str]] = None
):
    """Send system notification"""
    service: NotificationService = app.state.notification_service

    notification_ids = await service.send_system_notification(title, message, recipients, channels)
    return notification_ids
