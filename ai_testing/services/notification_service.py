...
class ConsoleChannelHandler(ChannelHandler):
    ...
    async def send(self, notification: Notification) -> bool:
        ...
        priority = notification.priority.value
        print(f"[NOTIFICATION] [{timestamp}] [{priority}] {notification.subject}: {notification.content}")
...
class AlertDetector:
    ...
    async def analyze_aggregated_results(
        ...
        if performance_metrics.get("avg_response_time_ms", 0) > 5000:
            alerts.append(Alert(
                ...
                current_values={"avg_response_time_ms": performance_metrics["avg_response_time_ms"]},
    ...
    async def _evaluate_rule(self, rule: NotificationRule, test_result: TestResult) -> bool:
        ...
        if rule.service_names:
            service_name = test_result.execution_id.split('_')[0] if '_' in test_result.execution_id else ""
    ...
    async def _create_alert_from_result(self, rule: NotificationRule, test_result: TestResult) -> Alert:
        ...
        alert = Alert(
            ...
            source_service=test_result.execution_id.split('_')[0] if '_' in test_result.execution_id else "",
...
class NotificationService(INotificationService):
    ...
    async def _process_alert(
        ...
        # Find matching rules
        matching_rules = [
            rule for rule in self.alert_detector.rules
        ...
        logger.info(f"Generated {len(self.pending_notifications)} notifications for alert {alert.alert_id}")
...
async def get_notification_status():
    ...
    return {
        ...
        "sent_notifications_today": len([
            n for n in service.sent_notifications
