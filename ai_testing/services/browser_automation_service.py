...
class UIAction(BaseModel):
    ...
    action_type: str = Field(
        ..., description="Type of action: click, type, select, etc."
    )
...
class UIAssertion(BaseModel):
    ...
    assertion_type: str = Field(
        ..., description="Type of assertion: visible, text, value, etc."
    )
...
class BrowserAutomationService(IBrowserAutomation):
    ...
    async def execute_ui_test(
        ...
        try:
            ...
            try:
                # Execute test steps
                test_results = await self._execute_ui_test_steps(
                    page, test_spec, context
                )

                # Capture final screenshot
                screenshot_path = await self._capture_screenshot(
                    page, f"{test_id}_final"
                )
    ...
    async def _execute_ui_test_steps(
        ...
        try:
            ...
            actions_success_rate = results["actions_completed"] / max(
    ...
    async def _execute_ui_action(self, page: Page, action: UIAction):
        ...
        if action.wait_for:
            await page.wait_for_selector(
                action.wait_for, timeout=action.timeout_ms or 30000
            )
        ...
        if action.action_type == "click":
            ...
            await page.fill(
                action.selector, action.value or "", timeout=action.timeout_ms
            )
    ...
    async def _run_visual_regression_test(
        ...
        size_diff = (
            abs(current_size - baseline_size) / baseline_size
            if baseline_size > 0
            else 1.0
        )
    ...
    async def _capture_screenshot(self, page: Page, name: str) -> str:
        ...
        screenshot_path = (
            self.browser_manager.screenshots_dir / f"{name}_{timestamp}.png"
        )
    ...
    def _generate_ui_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        assertions_rate = results["assertions_passed"] / max(
            results["total_assertions"], 1
        )
        if assertions_rate < 0.8:
            recommendations.append(
                "Some UI assertions failed - verify expected UI state"
            )
        ...
        if results.get("performance_metrics"):
            ...
            if load_time > self.performance_thresholds["load_time_ms"]:
                recommendations.append(
                    f"Page load time ({load_time:.0f}ms) exceeds threshold"
                )

            failed_requests = perf.get("failed_requests", 0)
            if failed_requests > 0:
                recommendations.append(
                    f"{failed_requests} failed resource requests detected"
                )
        ...
        if results.get("visual_results"):
            ...
            if not visual.get("visual_match", True):
                recommendations.append(
                    "Visual differences detected - review UI changes"
                )
...
async def capture_page_screenshot(
    page_url: str, viewport: Optional[Dict[str, int]] = None
):
    """Capture page screenshot"""
    service: BrowserAutomationService = app.state.browser_service
