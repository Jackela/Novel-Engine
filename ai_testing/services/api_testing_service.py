...
class APITestRequest(BaseModel):
    ...
    endpoints: List[Dict[str, Any]] = Field(..., description="List of endpoints to test")
...
class APITestExecutor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.default_max_response_time = config.get("default_max_response_time_ms", 2000)
    ...
    async def execute_api_test_suite(
        ...
        try:
            ...
            test_type_str = request.test_type.value if hasattr(request.test_type, 'value') else str(request.test_type)
            ...
            if request.test_type == APITestType.LOAD:
                ...
                results = await self._execute_functional_tests(request, endpoints, context)
            
            # Calculate summary statistics
            response_times = [r.response_time_ms for r in results]
            successful_results = [r for r in results if r.status_code < 400 and not r.errors]
            ...
            performance_analysis = self._generate_performance_analysis(response_times, request)
            ...
            suite_result = APITestSuiteResult(
                ...
                p95_response_time_ms=self._calculate_percentile(response_times, 95) if response_times else 0.0,
            ...
            logger.info(f"API test suite completed: {request.test_id} (Score: {overall_score:.3f})")
            return suite_result
    ...
    async def _execute_load_tests(
        ...
        logger.info(f"Starting load test: {request.concurrent_users} users for {request.test_duration_seconds}s")
        ...
        for user_id in range(request.concurrent_users):
            for endpoint in endpoints:
                for req_num in range(requests_per_user):
                    delay = (req_num * len(endpoints) + endpoints.index(endpoint)) / request.concurrent_users
        ...
        successful_results = [r for r in load_results if isinstance(r, APITestResult) and not r.errors]
        failed_results = [r for r in load_results if not isinstance(r, APITestResult) or r.errors]
        ...
        for endpoint_path, endpoint_results in endpoint_aggregates.items():
            if endpoint_results:
                ...
                success_rate = len([r for r in endpoint_results if r.status_validation]) / len(endpoint_results)
        ...
        logger.info(f"Load test completed: {len(successful_results)} successful, {len(failed_results)} failed")
    ...
    async def _test_single_endpoint(
        ...
        try:
            response = await self.http_client.request(
                ...
                json=request_data if endpoint.content_type == "application/json" else None,
                data=request_data if endpoint.content_type != "application/json" else None,
    ...
    async def _load_test_single_request(
        ...
        return await self._test_single_endpoint(base_url, endpoint, auth_headers, context)
    ...
    def _validate_response(
        ...
        if response.status_code >= 400:
            ...
            validation_result["errors"].append(f"HTTP error response: {response.status_code}")
    ...
    def _generate_performance_analysis(
        ...
        max_threshold = request.performance_requirements.get("max_response_time_ms", self.default_max_response_time)
    ...
    def _generate_security_analysis(
        ...
        for result in results:
            ...
            response_headers = {k.lower(): v for k, v in result.response_headers.items()}
            ...
            if missing_headers:
                security_issues.append(f"{result.endpoint_path}: Missing security headers: {', '.join(missing_headers)}")
            
            # Check for information disclosure
            if result.status_code >= 500:
                security_issues.append(f"{result.endpoint_path}: Server error may expose internal information")
    ...
    def _generate_recommendations(
        ...
        if response_times:
            ...
            if slow_endpoints:
                ...
                recommendations.append(f"Focus on optimizing slow endpoints: {', '.join(slow_paths)}")
        ...
        if not recommendations:
            recommendations.append("API testing passed all checks - maintain current quality")
...
class APITestingService(IAPITesting):
    ...
    async def execute_api_test(
        ...
        try:
            ...
            if parsed.scheme:  # Full URL provided
                ...
                base_url = context.metadata.get('base_url', 'http://localhost:8000') if context.metadata else 'http://localhost:8000'
            ...
            # Execute test
            suite_result = await self.test_executor.execute_api_test_suite(test_request, context)
            
            # Extract single endpoint result
            endpoint_result = suite_result.endpoint_results[0] if suite_result.endpoint_results else None
            ...
            return TestResult(
                ...
                api_results=endpoint_result.dict() if endpoint_result else {},
                performance_metrics={
                    "response_time_ms": endpoint_result.response_time_ms if endpoint_result else 0.0,
                    "response_size_bytes": endpoint_result.response_size_bytes if endpoint_result else 0
                },
            ...
            return TestResult(
                ...
                error_message=str(e),
                recommendations=["Check API endpoint availability", "Verify request configuration"]
            )
    ...
    async def test_api_performance(
        ...
        return {
            ...
            "requests_per_second": suite_result.total_endpoints / test_duration_seconds if test_duration_seconds > 0 else 0.0
        }
    ...
    async def run_load_test(
        ...
        try:
                    ...
                    while (time.time() - session_start) < duration_seconds:
                        ...
                        try:
                            ...
                            session_results.append({
                                "success": response.status_code == test_spec.expected_status,
            ...
            if all_requests:
                ...
                results["successful_requests"] = sum(1 for r in all_requests if r.get("success", False))
                ...
                results["requests_per_second"] = results["total_requests"] / total_duration if total_duration > 0 else 0
                
                # Collect error details
                errors = [r.get("error") for r in all_requests if r.get("error")]
                results["errors"] = list(set(errors))[:10]  # Limit to first 10 unique errors
...
async def health_check():
    ...
    executor_status = "connected" if service.test_executor.http_client else "disconnected"
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            result = TestResult(
                ...
                errors=[] if passed else [f"Status code mismatch: expected {expected_status}, got {response.status_code}"],
                warnings=[]
            )
