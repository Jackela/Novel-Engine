...
class ResultsAnalyzer:
    ...
    def analyze_trends(
        self, results: List[TestResult], time_window_days: int = 7
    ) -> List[TestTrend]:
        ...
        for metric_key, metric_name in metric_trends:
            trend = self._analyze_metric_trend(
                time_series_data, metric_key, metric_name
            )
            if trend:
                trends.append(trend)
    ...
    def detect_quality_insights(
        ...
        if historical_results:
            comparative_insights = self._compare_with_historical(
    ...
    def _create_time_series(
        self, results: List[TestResult], time_window_days: int
        ...
        for result in recent_results:
            ...
            day_key = result.timestamp.replace(
    ...
    def _analyze_metric_trend(
        ...
        insights = self._generate_trend_insights(
            metric_name, trend_direction, slope, values
        )
    ...
    def _analyze_quality_metrics_trends(
        ...
        for metric, data_points in quality_data.items():
            ...
            insights = self._generate_quality_trend_insights(
                metric, trend_direction, values
            )
                ...
                insights.append(
                    QualityInsight(
                        ...
                        evidence=[
                            ...
                            f"Previous average: {older_avg_score:.3f}",
                        ],
                        recommendations=[
                            "Continue current testing practices",
                            "Investigate successful patterns",
                        ],
                        priority="medium",
                    )
                )
                ...
                insights.append(
                    QualityInsight(
                        ...
                        evidence=[
                            ...
                            f"Standard deviation: {std_dev:.3f}",
                        ],
            ...
            # High variability
                ...
                insights.append(
                    QualityInsight(
                        ...
                        recommendations=[
                            "Investigate quality inconsistency",
                ...
                insights.append(
                    QualityInsight(
                        ...
                        recommendations=[
                            ...
                            "Maintain improvement trajectory",
                        ],
                        priority="medium",
                    )
                )
    ...
    def _generate_trend_recommendations(
        self, metric_name: str, trend_direction: TrendDirection
    ) -> List[str]:
    ...
    def _generate_quality_recommendations(
        self, metric: QualityMetric, trend_direction: TrendDirection
    ) -> List[str]:
...
class ResultsAggregationService(IResultsAggregation):
    ...
    async def collect_test_results(
        self, time_period: timedelta, service_names: List[str] = None
    ) -> List[TestResult]:
        ...
        for service_name in target_services:
            try:
                service_results = await self._collect_from_service(
    ...
    async def generate_aggregated_report(
        ...
        try:
            ...
            results = await self.collect_test_results(
                time_period, request.service_names
            )

            # Filter results by time and other criteria
            filtered_results = self._filter_results(
                results, request, start_time, end_time
            )
            ...
            if request.include_quality_insights:
                quality_insights = self.analyzer.detect_quality_insights(
                    filtered_results
                )
    ...
    async def export_report(
        self, report_id: str, format: ReportFormat = ReportFormat.JSON
    ) -> str:
    ...
    async def _collect_from_service(
        self, service_name: str, time_period: timedelta
    ) -> List[TestResult]:
    ...
    def _generate_overall_summary(self, results: List[TestResult]) -> TestSummary:
        ...
        for result in results:
            if (
                ...
                and "avg_response_time_ms" in result.performance_metrics
            ):
    ...
    def _generate_test_type_summaries(
        self, results: List[TestResult]
    ) -> Dict[TestType, TestSummary]:
    ...
    def _generate_service_summaries(
        self, results: List[TestResult]
        ...
        for result in results:
            ...
            service_name = (
                result.execution_id.split("_")[0]
                if "_" in result.execution_id
                else "unknown"
            )
        ...
        for service_name, service_results_list in service_results.items():
            summaries[service_name] = self._generate_overall_summary(
    ...
    def _generate_recommendations(
        ...
        if results:
            ...
            if avg_score < 0.7:
                recommendations.append(
                    "Overall quality below target - review testing standards"
                )
            elif avg_score > 0.9:
                recommendations.append(
                    "Excellent overall quality - maintain current practices"
                )
        ...
        # Based on trends
        declining_trends = [
            t for t in trends if t.trend_direction == TrendDirection.DECLINING
        ]
        if declining_trends:
        ...
        for insight in quality_insights[:3]:  # Top 3 insights
            ...
            )  # First recommendation from each
        ...
        for scenario_data in failure_counts.values():
            ...
            if scenario_failures:
                scenario_data["avg_score"] = statistics.mean(
                    [f.score for f in scenario_failures]
                )
    ...
    def _calculate_data_completeness(
        self, results: List[TestResult], time_period: timedelta
    ) -> float:
        ...
        if dashboard_type == "overview":
            return {
                ...
                "summary": {
                    ...
                    "pass_rate": (
                        len([r for r in recent_results if r.passed])
                        / len(recent_results)
                        if recent_results
                        else 0.0
                    ),
                    "average_score": (
                        statistics.mean([r.score for r in recent_results])
                        if recent_results
                        else 0.0
                    ),
                    "average_duration": (
            ...
            return {
                ...
                "quality_metrics": {
                    ...
                    "ui_quality": (
                        statistics.mean(
                            [r.score for r in recent_results if r.ui_results]
                        )
                        if recent_results
                        else 0.0
                    ),
                    "api_quality": (
                        statistics.mean(
                            [r.score for r in recent_results if r.api_results]
                        )
                        if recent_results
                        else 0.0
                    ),
...
async def collect_results(
    time_period_hours: int = 24, service_names: Optional[List[str]] = None
):
    """Collect test results from all services"""
    service: ResultsAggregationService = app.state.aggregation_service
...
async def get_trends(time_window_days: int = 7):
