...
class ResultsAnalyzer:
    ...
    def _analyze_metric_trend(
        self, time_series_data: Dict[datetime, Dict[str, float]], metric_key: str, metric_name: str
    ) -> Optional[TestTrend]:
    ...
    def _generate_trend_insights(
        self, metric_name: str, trend_direction: TrendDirection, slope: float, values: List[float]
    ) -> List[str]:
    ...
    def _generate_quality_trend_insights(
        self, metric: QualityMetric, trend_direction: TrendDirection, values: List[float]
    ) -> List[str]:
...
class ResultsAggregationService(IResultsAggregation):
    ...
    async def generate_aggregated_report(self, request: ReportRequest) -> AggregatedResults:
    ...
    async def export_report(self, report_id: str, format: ReportFormat = ReportFormat.JSON) -> str:
    ...
    def _generate_service_summaries(self, results: List[TestResult]) -> Dict[str, TestSummary]:
        ...
        for result in results:
            ...
            service_name = (
                result.execution_id.split("_")[0] if "_" in result.execution_id else "unknown"
            )
    ...
    def _identify_top_performers(self, results: List[TestResult]) -> List[Dict[str, Any]]:
        ...
        top_performers = sorted(
            performance_data.values(), key=lambda x: x["performance_index"], reverse=True
        )[:5]
    ...
    async def get_test_results(self, filters: Dict[str, Any], limit: int = 100) -> List[TestResult]:
        ...
        if dashboard_type == "overview":
            return {
                ...
                "summary": {
                    ...
                    "pass_rate": len([r for r in recent_results if r.passed]) / len(recent_results)
                    if recent_results
                    else 0.0,
                    "average_score": statistics.mean([r.score for r in recent_results])
                    if recent_results
                    else 0.0,
                    "average_duration": statistics.mean([r.duration_ms for r in recent_results])
                    if recent_results
                    else 0.0,
                },
                "status_distribution": {
            ...
            return {
                ...
                "last_updated": datetime.now().isoformat(),
                "quality_metrics": {
                    "overall_score": statistics.mean([r.score for r in recent_results])
                    if recent_results
                    else 0.0,
                    "ui_quality": statistics.mean([r.score for r in recent_results if r.ui_results])
                    if recent_results
                    else 0.0,
                    "api_quality": statistics.mean(
                        [r.score for r in recent_results if r.api_results]
                    )
                    if recent_results
                    else 0.0,
                    "ai_quality": statistics.mean(
...
async def collect_results(time_period_hours: int = 24, service_names: Optional[List[str]] = None):
    """Collect test results from all services"""
    service: ResultsAggregationService = app.state.aggregation_service
