...
class AITestOrchestrator(ITestOrchestrator):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.services = {
            "browser-automation": ServiceEndpoint(
                name="browser-automation", base_url=self.service_endpoints["browser_automation"]
            ),
            ...
            "results-aggregation": ServiceEndpoint(
                name="results-aggregation", base_url=self.service_endpoints["results_aggregation"]
            ),
    ...
    async def _call_gemini_for_planning(self, prompt: str) -> Dict[str, Any]:
        ...
        return {
            "execution_order": [
                s.id for s in self.active_plans[list(self.active_plans.keys())[0]].scenarios
            ],
            "parallel_groups": [],
            "risk_assessment": {},
    ...
    async def _optimize_plan_heuristic(self, plan: TestPlan) -> TestPlan:
        ...
        plan.risk_assessment = {
            s.id: 0.3 if s.test_type == TestType.AI_QUALITY else 0.1 for s in plan.scenarios
        }
    ...
    async def _route_test_execution(
        self, scenario: TestScenario, execution: TestExecution
    ) -> TestResult:
    ...
    async def _store_plan_in_redis(self, plan: TestPlan):
        ...
        if self.redis_client:
            await self.redis_client.setex(
                f"plan:{plan.id}", 3600, plan.model_dump_json()  # 1 hour TTL
            )
    ...
    async def _update_execution_in_redis(self, execution: TestExecution):
        ...
        if self.redis_client:
            await self.redis_client.setex(
                f"execution:{execution.id}", 3600, execution.model_dump_json()  # 1 hour TTL
            )
