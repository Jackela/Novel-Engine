...
class AITestOrchestrator(ITestOrchestrator):
        ...
        try:
            ...
            for scenario in scenarios:
                ...
                validation_errors.extend(
                    [f"Scenario {scenario.name}: {error}" for error in errors]
                )
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to create test plan: {str(e)}"
            )
    ...
    async def execute_test_plan(self, plan_id: str) -> List[TestExecution]:
        ...
        try:
            ...
            if not plan:
                raise HTTPException(
                    status_code=404, detail=f"Test plan {plan_id} not found"
                )
    ...
    async def _optimize_plan_heuristic(self, plan: TestPlan) -> TestPlan:
        ...
        sorted_scenarios = sorted(
            plan.scenarios, key=lambda s: s.priority, reverse=True
        )
    ...
    async def _orchestrate_execution(
        self, plan: TestPlan, executions: List[TestExecution]
    ):
    ...
    async def _load_execution_from_redis(
...
async def health_check():
    ...
    healthy_services = sum(
        1 for healthy in orchestrator.service_health.values() if healthy
    )
