...
class EventCandidate:
    ...
    def score(self, current_tension: float, target_tension: float, orchestrator=None) -> float:
...
class DynamicEventOrchestrator:
    ...
    def __init__(self, blueprint: StoryBlueprint, dialogue_engine: ContextAwareDialogueEngine):
    ...
    def generate_next_event(self, characters: List, current_location: str) -> Dict:
        ...
        candidates = self._generate_event_candidates(characters, current_location, plot_guidance)
        ...
        if event.get("characters"):
            for char_name in event.get("characters", []):
                self.character_usage[char_name] = self.character_usage.get(char_name, 0) + 1
    ...
    def _get_plot_guidance(self, progress: float) -> Dict:
        ...
        stage_index = min(
            int(progress * len(self.blueprint.plot_points)), len(self.blueprint.plot_points) - 1
        )
    ...
    def _generate_event_candidates(
        ...
        if "dialogue" in suggested_types or tension < 0.7:
            ...
            if not self.character_turn_queue:
                ...
                sorted_chars = sorted(characters, key=lambda c: self.character_usage.get(c.name, 0))
    ...
    def _realize_event(self, candidate: EventCandidate, guidance: Dict) -> Dict:
        ...
        if candidate.event_type == "dialogue":
            ...
            event["content"] = {"speaker": character, "dialogue": dialogue, "emotion": emotion}
    ...
    def _generate_conflict_content(self, characters: List[str], guidance: Dict) -> Dict:
        ...
        if len(characters) < 2:
            ...
            conflicts = [
                {"description": f"{char}内心充满矛盾", "details": "责任与欲望在撕扯着灵魂"},
                {"description": f"{char}面临艰难抉择", "details": "每个选择都意味着牺牲"},
                {"description": f"{char}质疑自己的信念", "details": "曾经的确定如今动摇"},
            ]
        else:
        ...
        available = [c for c in conflicts if c.get("description", "") not in self.used_conflicts]
    ...
    def _generate_discovery_content(self, character: str, guidance: Dict) -> Dict:
        ...
        if stage in [PlotStage.SETUP, PlotStage.INCITING_INCIDENT]:
            discoveries = [
                {"description": f"{character}发现了神秘的能量痕迹", "details": "这些痕迹指向一个被遗忘的真相"},
                {"description": f"{character}察觉到了时空的异常", "details": "现实的结构出现了细微的裂痕"},
                {"description": f"{character}找到了古老的记录", "details": "记录中包含着预言的片段"},
                {"description": f"{character}感应到了其他维度的存在", "details": "有什么在呼唤着他们"},
                {"description": f"{character}解读出了隐藏的信息", "details": "信息指向一个关键的坐标"},
            ]
        elif stage in [PlotStage.RISING_ACTION, PlotStage.MIDPOINT]:
            discoveries = [
                {"description": f"{character}理解了能量的真正本质", "details": "原来一切都是相互连接的"},
                {"description": f"{character}发现了控制的方法", "details": "关键在于频率的同步"},
                {"description": f"{character}找到了失落的碎片", "details": "碎片完成了拼图的关键部分"},
                {"description": f"{character}破解了防护机制", "details": "通道终于打开了"},
                {"description": f"{character}觉醒了潜藏的能力", "details": "新的力量在体内苏醒"},
            ]
        elif stage in [PlotStage.CRISIS, PlotStage.CLIMAX]:
            discoveries = [
                {"description": f"{character}发现了敌人的弱点", "details": "胜利的希望重新点燃"},
                {"description": f"{character}理解了牺牲的意义", "details": "有些东西比生命更重要"},
                {"description": f"{character}找到了最后的钥匙", "details": "所有的准备都是为了这一刻"},
                {"description": f"{character}看透了命运的真相", "details": "原来一切都是必然"},
                {"description": f"{character}发现了逆转的方法", "details": "还有一线生机"},
            ]
        else:  # RESOLUTION, DENOUEMENT
            discoveries = [
                {"description": f"{character}理解了整个事件的意义", "details": "每个牺牲都有其价值"},
                {"description": f"{character}发现了新的可能性", "details": "结束也是开始"},
                {"description": f"{character}找到了内心的平静", "details": "一切都回归了本源"},
                {"description": f"{character}获得了终极的智慧", "details": "真理原来如此简单"},
            ]

        # Filter unused discoveries
        available = [d for d in discoveries if d["description"] not in self.used_discoveries]
    ...
    def _generate_action_content(self, characters: List[str], guidance: Dict) -> Dict:
        ...
        if stage in [PlotStage.SETUP, PlotStage.INCITING_INCIDENT]:
            actions = [
                {"description": "探索着神秘的能量源", "details": "波动的频率似乎在回应他们的存在"},
                {"description": "解析着古老的符文序列", "details": "每个符号都蕴含着深层的意义"},
                {"description": "建立起量子共振连接", "details": "意识开始在更高维度交汇"},
                {"description": "激活了休眠的观测装置", "details": "尘封的真相逐渐显现"},
                {"description": "感知到了异常的时空波动", "details": "现实的边界开始模糊"},
            ]
        elif stage in [PlotStage.RISING_ACTION, PlotStage.MIDPOINT]:
            actions = [
                ...
                {"description": "同步了彼此的量子频率", "details": "三人的意识达到完美和谐"},
                {"description": "开启了隐藏的通道", "details": "通往未知领域的路径展现"},
            ]
        elif stage in [PlotStage.CRISIS, PlotStage.CLIMAX]:
            actions = [
                {"description": "集结所有力量进行最后冲击", "details": "能量达到了临界点"},
                {"description": "牺牲了部分能量稳定局势", "details": "平衡在痛苦中重建"},
                {"description": "创造了新的现实锚点", "details": "多元宇宙的根基得以加固"},
                {"description": "引导着失控的能量回归", "details": "混沌逐渐转为秩序"},
                {"description": "完成了命运的最终编织", "details": "所有线索汇聚于此"},
            ]
        else:  # RESOLUTION, DENOUEMENT
            actions = [
                {"description": "恢复了宇宙的平衡", "details": "和谐的振动遍布各个维度"},
        ...
        available_actions = [a for a in actions if a["description"] not in self.used_actions]
        ...
        if not available_actions:
            ...
            return {"description": f"{char_name}在{location}采取了关键行动", "details": "这一刻将被历史铭记"}
    ...
    def _generate_environment_content(self, guidance: Dict) -> Dict:
        ...
        location_environments = {
            "虚空观察站的控制室": [
                {"description": "控制室的全息投影突然闪烁", "details": "警告信号在空中形成血红的符文"},
                {"description": "观测仪器检测到异常波动", "details": "数据流如瀑布般在屏幕上流淌"},
                {"description": "空间站的人工智能苏醒", "details": "古老的守护者开始低语"},
            ],
            "量子花园的中心": [
                {"description": "花园中的植物开始同步摇摆", "details": "它们似乎在传递某种信息"},
                {"description": "量子花朵绽放出七彩光芒", "details": "每片花瓣都映射着不同的现实"},
                {"description": "时间在花园中流速异常", "details": "一秒如千年，千年如一瞬"},
            ],
            "维度交汇点": [
                {"description": "不同现实的边界开始模糊", "details": "过去、现在和未来的影像交织在一起"},
                {"description": "维度涟漪如水波扩散", "details": "每个涟漪都是一个可能的世界"},
                {"description": "交汇点的核心开始脉动", "details": "宇宙的心跳在此处回响"},
            ],
        }
        ...
        stage_environments = {
            PlotStage.SETUP: [
                {"description": "周围充满了神秘的预感", "details": "空气中弥漫着即将到来的变化"},
                {"description": "环境透露出隐藏的线索", "details": "细心观察能发现真相的蛛丝马迹"},
                {"description": "平静的表象下暗流涌动", "details": "风暴前的宁静让人不安"},
            ],
            PlotStage.RISING_ACTION: [
                {"description": "能量场开始剧烈波动", "details": "空间结构出现微小的扭曲"},
                {"description": "环境响应着角色的情绪", "details": "现实似乎具有了意识"},
            ...
            PlotStage.RESOLUTION: [
                ...
                {"description": "新的秩序正在建立", "details": "破碎的已被修复，失去的已被找回"},
                {"description": "黎明的曙光照亮前路", "details": "希望在每个角落生根发芽"},
            ],
        }
        ...
        if location in location_environments:
            ...
            candidates = stage_environments.get(
                stage,
                [
                    {"description": "周围的空间发生了微妙的变化", "details": "能量场的频率正在改变"},
                    {"description": "环境散发着异样的气息", "details": "某种变化正在酝酿"},
                    {"description": "空间的质感变得不同", "details": "现实的边界开始模糊"},
                ],
            )

        # Filter unused environments
        available = [e for e in candidates if e["description"] not in self.used_environments]
    ...
    def _generate_crisis_content(self, characters: List[str], guidance: Dict) -> Dict:
        ...
        if stage in [PlotStage.MIDPOINT, PlotStage.CRISIS]:
            crises = [
                {"description": "时空裂缝即将吞噬一切", "choice": "牺牲一个维度来拯救其他所有维度", "stakes": "数十亿生命的存亡"},
                {"description": "量子病毒开始感染主系统", "choice": "切断与其他维度的连接或冒险寻找解药", "stakes": "多元宇宙的连接性"},
                {
                    "description": "只有一个人能进入核心修复系统",
                    "choice": "谁将承担这个可能有去无回的任务",
                    "stakes": "个人牺牲与集体生存",
                },
                {"description": "能量核心即将失控爆炸", "choice": "分散能量到多个维度或集中控制", "stakes": "整个星系的稳定性"},
                {"description": "意识融合装置出现故障", "choice": "强行分离或永久融合", "stakes": "个体性与集体智慧的平衡"},
                {"description": "时间循环即将崩溃", "choice": "重置时间线或接受新的因果律", "stakes": "过去、现在和未来的连续性"},
                {"description": "维度屏障开始瓦解", "choice": "加固屏障或允许维度融合", "stakes": "多元现实的独立性"},
                {"description": "守护者系统要求最高权限", "choice": "授予权限或摧毁系统", "stakes": "自由意志与安全保障"},
            ]
        elif stage == PlotStage.CLIMAX:
            crises = [
                {"description": "最终选择的时刻到来", "choice": "拯救一个世界或拯救所有世界", "stakes": "存在的意义本身"},
                {"description": "创造者留下的终极考验", "choice": "继承力量或摧毁力量", "stakes": "未来的进化方向"},
                {"description": "命运之轮停止转动", "choice": "重新编织命运或接受既定结局", "stakes": "自由意志的终极证明"},
                {"description": "所有维度同时崩塌", "choice": "创造新宇宙或修复旧宇宙", "stakes": "存在与虚无的界限"},
            ]
        else:
            # Early or late stage crises
            crises = [
                {"description": "意外的真相揭示", "choice": "接受真相或维持幻象", "stakes": "信念体系的根基"},
                {"description": "盟友背叛的可能", "choice": "信任直觉或寻求证据", "stakes": "团队的凝聚力"},
                {"description": "道德困境的考验", "choice": "坚持原则或灵活变通", "stakes": "灵魂的纯洁性"},
            ]
