...
class StoryArchitect:
    ...
    def __init__(self):
        self.themes = ["统一与分裂的平衡", "个体意识与集体智慧", "选择的重量与命运的必然", "时间的循环与线性", "现实的多重性与唯一性"]
    ...
    def design_story_structure(self, characters: List, target_length: int = 50) -> StoryBlueprint:
    ...
    def _design_character_arcs(self, characters: List) -> List[CharacterArc]:
        ...
        arc_templates = [
            {"initial": "孤独的探索者", "transformation": "学会信任与合作", "final": "连接的守护者"},
            {"initial": "理性的分析者", "transformation": "接受直觉与情感", "final": "平衡的智者"},
            {"initial": "充满疑虑的旁观者", "transformation": "承担责任与使命", "final": "坚定的行动者"},
        ]
        ...
        for i, character in enumerate(characters[:3]):
            ...
            arc = CharacterArc(
                ...
                key_moments=[PlotStage.INCITING_INCIDENT, PlotStage.MIDPOINT, PlotStage.CLIMAX],
            )
            arcs.append(arc)
