...
class MultiModalAssessmentResult(BaseModel):
    ...
    modality_results: Dict[ModalityType, ModalityAssessmentResult] = Field(
...
class VisualQualityAssessor:
    ...
    async def _assess_color_harmony(self, image: Image.Image) -> float:
        ...
        try:
            ...
            sampled_pixels = pixels[
                np.random.choice(len(pixels), sample_size, replace=False)
            ]
            ...
            saturation_balance = (
                1.0 - abs(saturation_mean - 0.5) * 2
            )  # Prefer moderate saturation
    ...
    async def _assess_aesthetic_appeal(self, image: Image.Image) -> float:
        ...
        try:
            ...
            aspect_ratio = width / height
            golden_ratio = 1.618
            golden_ratio_score = 1.0 - min(
                abs(aspect_ratio - golden_ratio) / golden_ratio, 1.0
            )
    ...
    def _generate_visual_analysis(
        ...
        if all_scores:
            ...
            if avg_score >= 0.8:
                analysis_parts.append(
                    "Overall: High visual quality with strong aesthetic appeal"
                )
            elif avg_score >= 0.6:
                analysis_parts.append(
                    "Overall: Good visual quality with room for improvement"
                )
            else:
                analysis_parts.append(
                    "Overall: Visual quality needs significant improvement"
                )
    ...
    def _generate_visual_recommendations(
        ...
        if all_scores:
            ...
            if avg_score < 0.5:
                recommendations.append(
                    "Consider regenerating image with improved prompts"
                )
            elif avg_score >= 0.8:
                recommendations.append(
                    "Excellent visual quality - maintain current approach"
                )
...
class CrossModalAssessmentEngine:
    ...
    async def _assess_narrative_visual_consistency(
        ...
        try:
            ...
            tone_mood_alignment = self._calculate_tone_mood_alignment(
                narrative_tone, visual_mood
            )
    ...
    async def _assess_multimodal_coherence(
        ...
        try:
            ...
            if text_content:
                contextual_coherence = self._assess_contextual_coherence(
    ...
    async def _assess_accessibility_compliance(
        ...
        try:
            ...
            if image_content:
                visual_accessibility = await self._assess_visual_accessibility(
                    image_content
                )
    ...
    def _calculate_tone_mood_alignment(
        self, narrative_tone: str, visual_mood: str
    ) -> float:
    ...
    async def assess_multimodal_content(
        ...
        try:
            ...
            if request.cross_modal_assessment and len(modality_results) > 1:
                cross_modal_scores = (
                    await self.cross_modal_engine.assess_cross_modal_quality(
            ...
            result = MultiModalAssessmentResult(
                ...
                cross_modal_analysis=self._generate_cross_modal_analysis(
                    cross_modal_scores
                ),
    ...
    def _generate_comprehensive_analysis(
        ...
        if cross_modal_scores:
            ...
            if avg_cross_modal < 0.6:
                recommendations.append(
                    "Improve alignment between different content modalities"
                )
