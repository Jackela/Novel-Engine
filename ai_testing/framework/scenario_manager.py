...
class AIScenarioGenerator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.creativity_temperature = config.get("scenario_generation", {}).get("creativity", 0.7)
    ...
    async def generate_scenarios_for_api(
        self, api_spec: Dict[str, Any], scenario_count: int = 5, focus_areas: List[str] = None
    ) -> List[TestScenario]:
        """Generate test scenarios for API endpoints"""

        if not self.gemini_api_key:
            logger.warning("Gemini API key not configured, using template-based generation")
            return self._generate_template_scenarios(api_spec, scenario_count)

        prompt = self._create_api_scenario_prompt(api_spec, scenario_count, focus_areas or [])
        ...
        try:
            ...
            for scenario_data in scenarios_data:
                scenario = TestScenario(
                    ...
                    expected_outcomes=scenario_data.get("config", {}).get("expected_outcomes", []),
                    quality_thresholds={
                        QualityMetric(k): v
    ...
    def _generate_template_scenarios(
        ...
        scenarios.append(
            TestScenario(
                ...
                priority=8,
                config={
                    "api_spec": {"endpoint": "/characters", "method": "GET", "expected_status": 200}
                },
            ...
            TestScenario(
                ...
                config={
                    "api_spec": {
                        ...
                        "method": "POST",
                        "request_body": {"character_names": ["krieg", "ork"], "turns": 3},
                        "expected_status": 200,
                    }
                },
                expected_outcomes=[
    ...
    def _generate_default_user_journey(self, journey_description: str) -> List[TestScenario]:
        ...
        return [
            TestScenario(
                ...
                expected_outcomes=["Journey completes successfully", "User experience is smooth"],
            )
        ]

    def _generate_default_ai_quality_scenarios(self, ai_features: List[str]) -> List[TestScenario]:
...
class ScenarioManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.scenarios_directory = Path(config.get("scenarios_directory", "ai_testing/scenarios"))
        self.templates_directory = Path(config.get("templates_directory", "ai_testing/templates"))
    ...
    async def create_scenario(
        self, name: str, description: str, test_type: TestType, config: Dict[str, Any], **kwargs
    ) -> TestScenario:
        """Create a new test scenario"""

        scenario = TestScenario(
            name=name, description=description, test_type=test_type, config=config, **kwargs
        )
    ...
    def list_scenarios(
        ...
        if category:
            scenarios = [s for s in scenarios if s.config.get("category") == category.value]

        if tags:
            scenarios = [
                s for s in scenarios if any(tag in s.config.get("tags", []) for tag in tags)
            ]

        return sorted(scenarios, key=lambda s: s.priority, reverse=True)

    async def update_scenario(self, scenario_id: str, updates: Dict[str, Any]) -> TestScenario:
    ...
    async def create_collection(
        ...
        # Create collection
        collection = ScenarioCollection(name=name, description=description, scenarios=scenarios)

        # Calculate collection metrics
        collection.total_estimated_minutes = sum(s.timeout_seconds // 60 for s in scenarios)
    ...
    async def generate_scenarios_for_novel_engine(
        ...
        for journey in user_journeys:
            journey_scenarios = await self.ai_generator.generate_user_journey_scenarios(journey)
        ...
        ai_scenarios = await self.ai_generator.generate_ai_quality_scenarios(ai_features)
    ...
    async def _get_novel_engine_api_spec(self) -> Dict[str, Any]:
        ...
        return {
            "endpoints": [
                ...
                {"path": "/characters", "method": "GET", "description": "List characters"},
                {"path": "/characters/{id}", "method": "GET", "description": "Get character"},
                {"path": "/simulations", "method": "POST", "description": "Run simulation"},
