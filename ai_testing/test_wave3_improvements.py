...
def test_content_variety():
    ...
    print(f"✅ Dialogue variety: {dialogue_variety:.1%} ({unique_dialogues}/{len(dialogues)} unique)")
    ...
    print(f"  Dialogue Variety: {evaluation['enhanced_metrics']['dialogue_variety']:.1%}")
    print(f"  Repetition Ratio: {evaluation['enhanced_metrics']['repetition_ratio']:.1%}")
    ...
    success_criteria = [
        ...
        ("Good character balance (<3:1)", imbalance_ratio < 3 if character_counter else False),
        ("Low repetition (<20%)", evaluation['enhanced_metrics']['repetition_ratio'] < 0.2),
        ("Multiple event types (>=4)", evaluation['enhanced_metrics']['unique_event_types'] >= 4)
    ]
