...
class AIBrowserController:
    ...
    async def make_decision(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        decision = {"timestamp": time.time(), "context": context, "action": None, "reasoning": ""}
        ...
        if "button" in str(context.get("elements", {})):
            ...
            if buttons:
                ...
                if "Create" in " ".join(button_texts):
                    ...
                    decision["reasoning"] = "Found 'Create' button - will test character creation"
                elif "Generate" in " ".join(button_texts):
                    decision["action"] = "click_generate"
                    decision["reasoning"] = "Found 'Generate' button - will test story generation"
                elif "Start" in " ".join(button_texts):
                    decision["action"] = "click_start"
                    decision["reasoning"] = "Found 'Start' button - will begin interaction"
                else:
                    decision["action"] = "click_first_button"
                    decision["reasoning"] = f"Found buttons: {button_texts[:3]} - will explore"
    ...
    async def execute_action(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if decision["action"] == "click_create":
                ...
                for i, input_elem in enumerate(inputs[:3]):  # Fill first 3 inputs
                    ...
                    if "name" in placeholder.lower() or "name" in name.lower():
                        ...
                        result["details"]["filled_description"] = character_data["description"]
                    elif "trait" in placeholder.lower() or "personality" in name.lower():
            ...
            result["error"] = str(e)
            self.test_results["errors"].append({"action": decision["action"], "error": str(e)})
    ...
    def generate_character_data(self) -> Dict[str, str]:
        ...
        characters = [
            {"name": "量子幽灵", "description": "一个能在多个维度间穿梭的神秘存在", "traits": "智慧、神秘、超然"},
            {"name": "赛博武士", "description": "融合了古代武士道精神和未来科技的战士", "traits": "忠诚、勇敢、高科技"},
            {"name": "梦境编织者", "description": "能够操纵和创造梦境世界的魔法师", "traits": "创造力、想象力、神秘力量"},
        ]
    ...
    async def evaluate_content_quality(self, content: str) -> Dict[str, float]:
        ...
        self.test_results["quality_assessments"].append(
            {"timestamp": time.time(), "content_preview": content[:200], "metrics": quality_metrics}
        )
    ...
    async def run_interactive_test(self, duration_seconds: int = 30):
        ...
        while time.time() - start_time < duration_seconds:
            ...
            if len(page_text) > 500 and "故事" in page_text or "story" in page_text.lower():
                await self.evaluate_content_quality(page_text)

            # Wait before next action
            await self.page.wait_for_timeout(2000)

        logger.info(f"✅ Test completed: {interaction_count} interactions in {duration_seconds}s")
