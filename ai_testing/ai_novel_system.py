...
class StoryEvent:
    ...
    def to_dict(self):
        ...
        if self.plot_stage:
            data["plot_stage"] = (
                self.plot_stage.value
                if hasattr(self.plot_stage, "value")
                else str(self.plot_stage)
            )
        return data
...
class EnhancedDialogueGenerator:
    ...
    def _generate_scene_transition(
        ...
        available_dialogues = [
            d for d in transition_dialogues if d not in self.used_phrases
        ]
...
class NovelQualityEvaluator:
    ...
    def evaluate(self, novel_text: str, events: List[StoryEvent]) -> Dict[str, Any]:
        ...
        evaluation = {
            ...
            "action_count": len(
                [e for e in events if e.event_type == ActionType.ACTION]
            ),
            "scene_count": len(
...
async def generate_complete_novel():
    ...
    try:
        ...
        for char in characters:
            dialogue_engine.register_character(
                char.name, char.personality, char.speech_style
            )
        ...
        for i in range(blueprint.target_length):
            ...
            event = StoryEvent(
                ...
                content=content,
                location=current_location,
                tension_level=blueprint.get_current_tension(
                    i / blueprint.target_length
                ),
            )
            events.append(event)
