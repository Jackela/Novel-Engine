...
class StoryEvent:
    ...
    def to_dict(self):
        ...
        if self.plot_stage:
            data["plot_stage"] = (
                self.plot_stage.value if hasattr(self.plot_stage, "value") else str(self.plot_stage)
            )
        return data
...
class EnhancedDialogueGenerator:
    ...
    def generate_dialogue_sequence(
        self, characters: List[Character], turns: int = 30
    ) -> List[StoryEvent]:
        ...
        for turn in range(turns):
            ...
            if event_type == ActionType.DIALOGUE:
                events.append(
                    self._generate_unique_dialogue(
                        speaker, characters, current_location, tension_level
                    )
                )
    ...
    def _generate_crisis(
        self, characters: List[Character], location: str, tension: float
    ) -> StoryEvent:
        """Generate crisis event"""
        crises = ["时空裂缝突然扩大，威胁着整个维度的稳定！", "量子病毒开始侵蚀现实的基础结构！", "多个时间线开始崩塌融合！", "维度屏障出现了不可逆的裂痕！"]
    ...
    def _generate_discovery(
        self, character: Character, location: str, tension: float
    ) -> StoryEvent:
    ...
    def _generate_dialogue(
        self, speaker: Character, others: List[Character], location: str
    ) -> StoryEvent:
    ...
    def _generate_scene_transition(
        self, characters: List[Character], old_location: str
    ) -> List[StoryEvent]:
        """Generate unique scene transition events without repetition"""
        events = []

        transitions = ["一道光门突然出现", "空间开始扭曲", "传送阵被激活", "时空裂缝打开", "量子通道形成", "维度边界开始模糊", "时间涟漪扩散开来"]
...
class EnhancedLiteraryWriter:
    ...
    def _group_by_dramatic_structure(
        self, events: List[StoryEvent], blueprint: "StoryBlueprint"
    ...
    def _write_chapter(
        ...
        chapter_titles = {1: "第一章：觉醒的征兆", 2: "第二章：交织的命运", 3: "第三章：终极的选择"}
    ...
    def _generate_ending(
        ...
        if blueprint and hasattr(blueprint, "theme"):
            ending += (
                f"关于{blueprint.theme if hasattr(blueprint, 'theme') else self.theme}的真相终于显现。\n\n"
            )
        else:
...
class NovelQualityEvaluator:
    ...
    def _evaluate_pacing(self, events: List[StoryEvent]) -> float:
        ...
        dialogue_ratio = len([e for e in events if e.event_type == ActionType.DIALOGUE]) / len(
            events
        )
...
async def generate_complete_novel():
    ...
    try:
        ...
        for i in range(blueprint.target_length):
            orchestrated_event = event_orchestrator.generate_next_event(
            ...
            event_type = event_type_map.get(
                orchestrated_event.get("type", "dialogue"), ActionType.DIALOGUE
            )
            ...
            if event_type == ActionType.DIALOGUE:
                ...
                character = (
                    orchestrated_event.get("characters", ["Narrator"])[0]
                    if orchestrated_event.get("characters")
                    else "Narrator"
                )
        ...
        print(
            f"  Zero repetition achieved: {len(dialogue_engine.memory.said_phrases)} unique dialogues"
        )
    ...
    novel = writer.transform_to_novel(
        events, characters, blueprint if "blueprint" in locals() else None
    )
