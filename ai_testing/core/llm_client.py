...
class LLMClient:
    ...
    def generate_narrative(
        ...
        try:
            if self.active_provider:
                return self.active_provider.generate_narrative(
                    events, style, temperature
                )
            else:
                raise RuntimeError("No active LLM provider available")
    ...
    def test_generation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_results["dialogue_test"] = (
                len(dialogue) > 0 and "生成失败" not in dialogue
            )
            ...
            test_results["narrative_test"] = (
                len(narrative) > 0 and "生成失败" not in narrative
            )
