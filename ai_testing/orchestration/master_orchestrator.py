...
class ServiceHealthMonitor:
        ...
        try:
            async with httpx.AsyncClient(timeout=service.timeout_seconds) as client:
                ...
                if response.status_code == 200:
                    ...
                    error_result = {"status": "unhealthy", "error": f"HTTP {response.status_code}"}
    ...
    async def check_all_services_health(self) -> Dict[str, Dict[str, Any]]:
        ...
        filtered_services = [name for name in self.services.keys() if name != "orchestrator"]
    ...
    async def wait_for_services_ready(self, required_services: List[str], timeout_seconds: int = 120) -> bool:
...
class TestPhaseExecutor:
    ...
    async def execute_phase(
        ...
        try:
            ...
            if phase == TestingPhase.API_TESTING:
                results = await self._execute_api_testing_phase(test_specs, context, config or {})
            elif phase == TestingPhase.UI_TESTING:
                results = await self._execute_ui_testing_phase(test_specs, context, config or {})
            elif phase == TestingPhase.AI_QUALITY_ASSESSMENT:
                results = await self._execute_ai_quality_phase(test_specs, context, config or {})
            elif phase == TestingPhase.INTEGRATION_VALIDATION:
                results = await self._execute_integration_phase(test_specs, context, config or {})
            else:
                results = []
            ...
            return TestPhaseResult(
                ...
                recommendations=["Check service availability", "Review test specifications"]
            )
    ...
    async def _execute_api_testing_phase(
        ...
        if not service:
            logger.warning("API Testing service not registered, returning empty results")
            return []
        ...
        for spec_data in test_specs:
            try:
                ...
                response = await self.http_client.post(
                    f"{service.base_url}/test/single",
                    params=test_params
                )
    ...
    async def _execute_integration_phase(
        ...
        if not health_results:
            for service_name in self.health_monitor.services:
                if service_name != "orchestrator":
                    health = await self.health_monitor.check_service_health(service_name)
        ...
        all_services_healthy = (healthy_services / total_services) >= 0.6 if total_services > 0 else True
...
class MasterOrchestrator:
    ...
    async def initialize(self):
        ...
        services_ready = await self.health_monitor.wait_for_services_ready(required_services, timeout_seconds=60)
        
        if not services_ready:
            logger.warning("Some services not ready, continuing with limited functionality")
    ...
    async def execute_comprehensive_test(
        ...
        logger.info(f"Starting comprehensive test: {request.test_name} (Session: {request.test_session_id})")
        ...
        try:
            ...
            phase_results = await self._execute_phases(execution_phases, request, context)
            
            # Aggregate final results
            result = await self._aggregate_final_results(request, phase_results, started_at, start_time)
            
            # Send notifications if configured
            if request.notification_config:
                await self._send_completion_notification(result, request.notification_config)
            
            # Store completed session
            self.completed_sessions[request.test_session_id] = result
            
            logger.info(f"Comprehensive test completed: {request.test_name} (Score: {result.overall_score:.3f})")
            return result
            ...
            error_result = ComprehensiveTestResult(
                ...
                recommendations=["Check service availability", "Review test configuration"]
            )
    ...
    def _plan_execution_phases(self, request: ComprehensiveTestRequest) -> List[Tuple[TestingPhase, List[Dict[str, Any]]]]:
        ...
        if request.ai_quality_specs:
            phases.append((TestingPhase.AI_QUALITY_ASSESSMENT, request.ai_quality_specs))
    ...
    async def _execute_phases(
        ...
        if request.strategy == OrchestrationStrategy.PARALLEL and request.parallel_execution:
            # Execute compatible phases in parallel
            phase_tasks = []
            
            for phase, test_specs in execution_phases:
                if phase != TestingPhase.INTEGRATION_VALIDATION:  # Integration runs last
            ...
            integration_phases = [(p, s) for p, s in execution_phases if p == TestingPhase.INTEGRATION_VALIDATION]
            for phase, test_specs in integration_phases:
                result = await self.phase_executor.execute_phase(phase, test_specs, context)
                phase_results.append(result)
    ...
    async def _aggregate_final_results(
        ...
        overall_score = sum(r.score for r in phase_results) / len(phase_results) if phase_results else 1.0
        ...
        return ComprehensiveTestResult(
            test_session_id=request.test_session_id,
            test_name=request.test_name,
            overall_status=TestStatus.COMPLETED if overall_passed else TestStatus.FAILED,
    ...
    try:
        ...
        if not filtered_health:
            ...
            for service_name in orchestrator.health_monitor.services:
                if service_name != "orchestrator":
                    health = await orchestrator.health_monitor.check_service_health(service_name)
                    filtered_health[service_name] = health
        
        # Count healthy services (including degraded as acceptable)
        healthy_count = sum(
            1 for health in filtered_health.values()
        ...
        return ServiceHealthResponse(
            ...
            external_dependencies={k: v.get("status", "unknown") for k, v in filtered_health.items()},
