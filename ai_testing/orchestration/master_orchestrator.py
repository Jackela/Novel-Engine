        ...
        try:
            async with httpx.AsyncClient(timeout=service.timeout_seconds) as client:
                ...
                if response.status_code == 200:
                    ...
                    # Cache result
                    self.health_cache[cache_key] = {"data": health_data, "timestamp": time.time()}
...
class TestPhaseExecutor:
    ...
    async def _execute_api_testing_phase(
        self, test_specs: List[Dict[str, Any]], context: TestContext, config: Dict[str, Any]
    ) -> List[TestResult]:
    ...
    async def _execute_ai_quality_phase(
        ...
        for spec_data in test_specs:
            try:
                ...
                response = await self.http_client.post(f"{service.base_url}/assess", json=spec_data)
...
class MasterOrchestrator:
    ...
    async def _execute_phases(
        ...
        if request.strategy == OrchestrationStrategy.PARALLEL and request.parallel_execution:
            ...
            integration_phases = [
                (p, s) for p, s in execution_phases if p == TestingPhase.INTEGRATION_VALIDATION
            ]
    ...
    async def _aggregate_final_results(
        ...
        overall_score = (
            sum(r.score for r in phase_results) / len(phase_results) if phase_results else 1.0
        )
...
async def health_check():
    ...
    try:
        ...
        filtered_health = {
            name: health for name, health in service_health.items() if name != "orchestrator"
        }
