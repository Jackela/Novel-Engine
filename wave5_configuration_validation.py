        ...
        for yaml_file in yaml_files:
            ...
            try:
                if yaml_path.exists():
                    ...
                    results["yaml_files_parsed"][yaml_file] = {"success": False, "error": "File not found"}
                    results["all_configs_valid"] = False
            except Exception as e:
                results["yaml_files_parsed"][yaml_file] = {"success": False, "error": str(e)}
                results["all_configs_valid"] = False
        ...
        try:
            ...
            for env in environments:
                try:
                    ...
                    results["environments"][env] = {
                        ...
                        "has_database_config": "database" in config if config else False,
            ...
            try:
                ...
                if base_config and dev_config:
                    ...
                    results["inheritance_working"] = len(base_keys.intersection(dev_keys)) > 0
    ...
    def validate_python_imports(self) -> Dict[str, Any]:
        ...
        for module_name, import_path in import_tests:
            try:
                ...
                results["imports"][module_name] = {"success": False, "error": f"Unexpected error: {str(e)}"}
                results["all_imports_successful"] = False
    ...
    def validate_path_resolution(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for config_name, config_path in config_paths:
                ...
                if not full_path.exists() or not (full_path.is_file() and os.access(full_path, os.R_OK)):
                    results["all_paths_resolved"] = False
    ...
    def validate_config_loader(self) -> Dict[str, Any]:
        ...
        try:
            ...
            loader_tests = [
                ...
                ("load_environment_config", lambda: loader.load_environment_config("development")),
                ("get_config_path", lambda: loader.get_config_path("environments", "development.yaml")),
    ...
    def validate_cross_environment(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(configs) > 1:
                ...
                if loaded_configs:
                    key_sets = [set(config.keys()) for config in loaded_configs.values()]
                    ...
                    results["consistency_check"] = {
                        ...
                        "consistency_ratio": len(common_keys) / len(all_keys) if all_keys else 0
                    }
...
def main():
    ...
    # Save results
    report_file = project_root / f"wave5_configuration_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
