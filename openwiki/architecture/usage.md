...
Each completed proposal generation (synchronous and streamed) records its job and usage event atomically through `recordCompletedJobWithUsage`; a retried proposal uses `markJobOutcomeWithUsage`. Both carry provider, model, prompt/completion tokens, and request evidence JSON (`server/src/contexts/studio/application/proposal_landing.ts`, `server/src/contexts/studio/application/job_retry_executor.ts`, `server/src/contexts/studio/infrastructure/job_store_part.ts`). When the provider reports no token counts, the deterministic fallback `resolvedTokenCount` derives them from the instruction and proposal text (`server/src/contexts/studio/application/proposal_landing.ts`). Each completed lorebook-wizard extraction segment records its job and usage event through the same shared `recordCompletedJobWithUsage` landing, and a retried extraction uses `markJobOutcomeWithUsage` (`server/src/contexts/studio/application/lore_extract_service.ts`). Reviews and exports do not write usage events; the export job's provider is the honest `studio` renderer label, not an AI model.
